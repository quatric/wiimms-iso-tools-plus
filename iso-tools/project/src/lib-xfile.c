...
)
{
	...
	if (data_size >= 0x208 && !memcmp (d + 0x200, "NKIT v01", 8))
		return data_size >= WII_MAGIC_OFF + WII_MAGIC_LEN && be32 (d + WII_MAGIC_OFF) == WII_MAGIC
			? XF_NKIT_WII
			: XF_NKIT_GC;
...
enumError XExtract (ccp source, ccp dest)
{
	...
	if (fform == XF_UNKNOWN)
		return ERROR0 (ERR_WRONG_FILE_TYPE, "Not a supported container: %s\n", source);
	...
	if (!xformat_info[fform].can_extract)
		return ERROR0 (ERR_NOT_IMPLEMENTED,
			...
			xformat_info[fform].can_convert ? " (XCONVERT works)" : "", source);

	switch (fform)
	{
		case XF_WUD:
		case XF_WUX:
			return XExtractWiiU (source, fform, dest);
		case XF_NDS:
		...
		case XF_CIA:
			return XExtractCIA (source, dest);
		case XF_XCI:
...
enumError XCreate (ccp source, ccp dest, xformat_t format)
{
	...
	if (!xformat_info[format].can_create)
		return ERROR0 (ERR_NOT_IMPLEMENTED, "Creating %s images is not implemented yet.\n",
...
enumError XConvert (ccp source, ccp dest, xformat_t format)
{
	...
	if (!xformat_info[src_format].can_convert || !xformat_info[format].can_convert)
		return ERROR0 (ERR_NOT_IMPLEMENTED, "Can't convert %s to %s.\n",
	...
	if (src_wiiu != dst_wiiu)
		return ERROR0 (ERR_SYNTAX, "%s and %s are unrelated formats.\n",
			xformat_info[src_format].name, xformat_info[format].name);
