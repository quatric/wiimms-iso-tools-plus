...
static void version_exit ()
{
	if (brief_count > 1)
		...
		printf ("%s%s\n%.*s", ProgInfo.progname, TITLE, (int)strlen (default_settings) - 1,
			default_settings);
	exit (ERR_OK);
}
...
static enumError OpenOutput (SuperFile_t *fo, SuperFile_t *fi, ccp fname, ccp testmode_msg)
{
...
enumError CloseInOut (SuperFile_t *fi, SuperFile_t *fo, enumError prev_err, bool remove_source)
{
...
enumError CatRaw (
	SuperFile_t *fi, SuperFile_t *fo, ccp out_fname, bool ignore_raw, bool remove_source)
{
	TRACE ("CatRaw()\n");
	DASSERT (fi);
...
enumError CatCISO (SuperFile_t *fi, CISO_Head_t *ch, SuperFile_t *fo, ccp out_fname,
...
enumError CatWDF (ccp fname, SuperFile_t *fo, ccp out_fname, bool ignore_raw, bool remove_source)
{
	TRACE ("CatWDF()\n");
	DASSERT (fname);
	DASSERT (fo || out_fname);
	...
	if (err == ERR_NO_WDF)
	{
		...
		if (!err && !memcmp (&ch.magic, "CISO", 4) && !*(ccp)&ch.block_size && *ch.map == 1)
		{
...
static int print_range (FILE *f, ccp text, u64 end, u64 len)
{
	...
	if (err)
		fprintf (f, "    %-18s: %10llx .. %10llx [%10llx!=%10llx] INVALID!\n", text, prev_val,
			end - opt_minus1, len, end - prev_val);
	else
		fprintf (
			f, "    %-18s: %10llx .. %10llx [%10llx]\n", text, prev_val, end - opt_minus1, len);
...
enumError ciso_dump (FILE *f, CISO_Head_t *ch, WFile_t *df, ccp fname)
{
	...
	const u64 iso_file_size = iso_file_usage > min_file_size ? iso_file_usage : min_file_size;

	//----- print header

	u8 *m = (u8 *)ch->magic;
	fprintf (f, "  %-18s:         \"%s\" = %02x-%02x-%02x-%02x\n", "Magic", wd_print_id (m, 4, 0),
		m[0], m[1], m[2], m[3]);
	...
	fprintf (f, "  %-18s: %10llx/hex =%11lld\n", "Real file size", (u64)df->st.st_size,
		(u64)df->st.st_size);
		...
		for (bptr = ch->map;;)
		{
			...
			if (b1 == b2 - 1)
				fprintf (f, "%11x      : %9llx .. %9llx : %9llx .. %9llx : %9llx\n", b1, cfa,
					cfa + siz - opt_minus1, via, via + siz - opt_minus1, siz);
			else
				fprintf (f, "%8x .. %4x : %9llx .. %9llx : %9llx .. %9llx : %9llx\n", b1, b2 - 1,
					cfa, cfa + siz - opt_minus1, via, via + siz - opt_minus1, siz);
...
enumError wia_dump (FILE *f, WFile_t *df, ccp fname)
{
	...
	fprintf (f, "    %-23s: \"%s\"  %02x %02x %02x %02x\n", "Magic", wd_print_id (m, 4, 0), m[0],
		m[1], m[2], m[3]);
	fprintf (f, "    %-23s: 0x%08x => %s\n", "Version", wia->fhead.version,
		PrintVersionWIA (0, 0, wia->fhead.version));
	fprintf (f, "    %-23s: 0x%08x => %s\n", "Version/compatible", wia->fhead.version_compatible,
	...
	if (wia->fhead.disc_size == sizeof (wia_disc_t))
		...
		fprintf (f, "    %-23s: %10u = %9x [current: %zu = %zx/hex]\n", "Size of disc section",
			wia->fhead.disc_size, wia->fhead.disc_size, sizeof (wia_disc_t), sizeof (wia_disc_t));
	...
	if (wia->fhead.iso_file_size)
	{
		...
		fprintf (f, "    %-23s: %10llu =%10llx/hex = %s  %4.*f%%\n", "Total file size",
			...
			wd_print_size_1024 (0, 0, wia->fhead.wia_file_size, true), percent <= 9.9 ? 2 : 1,
			percent);
	}
	else
		fprintf (f, "    %-23s: %10llu =%10llx/hex = %s\n", "Total file size",
	...
	if (disc->disc_type)
		fprintf (f, "    %-23s: %s, %.64s\n", "ID & title", wd_print_id (disc->dhead, 6, 0),
			disc->dhead + WII_TITLE_OFF);
	fprintf (f, "    %-23s: %10u = %s\n", "Disc type", disc->disc_type,
		wd_get_disc_type_name (disc->disc_type, "?"));
	fprintf (f, "    %-23s: %10u = %s\n", "Compression method", disc->compression,
		wd_get_compression_name (disc->compression, "?"));
	fprintf (f, "    %-23s: %10u\n", "Compression level", disc->compr_level);
	fprintf (f, "    %-23s: %10u =%10x/hex = %s\n", "Chunk size", disc->chunk_size,
		disc->chunk_size, wd_print_size_1024 (0, 0, disc->chunk_size, true));

	fprintf (f, "    %-23s: %10u\n", " Number of partitions", disc->n_part);
	fprintf (f, "    %-23s: %10llu = %9llx/hex = %s\n", " Offset of Part.header", disc->part_off,
		disc->part_off, wd_print_size_1024 (0, 0, disc->part_off, true));
	fprintf (f, "    %-23s: %2d * %5u = %9u\n", " Size of Part.header", disc->n_part,
		disc->part_t_size, disc->n_part * disc->part_t_size);
	...
	fprintf (f, "    %-23s: %10u = %9x/hex = %s\n", "Size of raw data tab.", disc->raw_data_size,
		disc->raw_data_size, wd_print_size_1024 (0, 0, disc->raw_data_size, true));

	fprintf (f, "    %-23s: %10u\n", " Num. of group elem.", disc->n_groups);
	fprintf (f, "    %-23s: %10llu = %9llx/hex = %s\n", " Offset of group tab.", disc->group_off,
		disc->group_off, wd_print_size_1024 (0, 0, disc->group_off, true));
	fprintf (f, "    %-23s: %10u = %9x/hex = %s\n", " Size of group tab.", disc->group_size,
		disc->group_size, wd_print_size_1024 (0, 0, disc->group_size, true));

	fprintf (f, "    %-23s: %10u = %9x/hex = %s\n", "Compressor data length", disc->compr_data_len,
		disc->compr_data_len, wd_print_size_1024 (0, 0, disc->compr_data_len, true));
...
enumError gcz_dump (FILE *f, GCZ_Head_t *gh1, WFile_t *df, ccp fname)
{
	...
	fprintf (f,
		...
		gh.magic, gh.sub_type, gh.compr_size, gh.compr_size, gh.image_size, gh.image_size,
		gh.block_size, gh.block_size, gh.num_blocks, gh.num_blocks);
	...
	for (blk = 0; blk < gh.num_blocks; blk++)
	{
		...
		u32 blk_size
			= (blk == gh.num_blocks - 1 ? gh.compr_size : le64 (offset + blk + 1)) - blk_off;
...
static u64 PrintAlignWC (FILE *f, ccp title, u64 num)
{
	...
	fprintf (f, "    %-17s : %s\n", title, m ? wd_print_size_1024 (0, 0, m, true) : "   0");
	return m;
}
...
enumError wdf_dump (FILE *f, ccp fname)
{
	...
	if (!memcmp (&wh, "CISO", CISO_MAGIC_SIZE))
	{
		...
		err = ReadAtF (
			&df, CISO_MAGIC_SIZE, (char *)&ch + CISO_MAGIC_SIZE, sizeof (ch) - CISO_MAGIC_SIZE);
		if (err)
		{
			ResetWFile (&df, false);
			return err;
	...
	err = ReadAtF (
		&df, CISO_MAGIC_SIZE, (char *)&wh + CISO_MAGIC_SIZE, sizeof (wh) - CISO_MAGIC_SIZE);
	if (err)
	{
		ResetWFile (&df, false);
		return err;
	...
	fprintf (f, "    %-18s: \"%s\"  %02x %02x %02x %02x  %02x %02x %02x %02x\n", "Magic",
		wd_print_id (m, 8, 0), m[0], m[1], m[2], m[3], m[4], m[5], m[6], m[7]);
...
#define PRINT32F(a, t)                                                                             \
	...
	fprintf (f, "    %-18s: %10llx/hex =%11lld  %4.2f%%\n", "  WDF file size ", (u64)df.st.st_size,
		(u64)df.st.st_size, 100.0 * df.st.st_size / wh.file_size);
	...
	if (df.split_used > 1)
		snprintf (mi->info, sizeof (mi->info), "-- end of %u joined files --", df.split_used);
	else
	...
	if (opt_chunk)
	{
		...
		for (idx = 0, w = wc; idx < wh.chunk_n; idx++, w++)
		{
			fprintf (f, "%6d. %10llx..%10llx %9llx %10llx..%10llx  %9llx\n", idx, w->data_off,
				w->data_off + w->data_size - opt_minus1, w->data_size, w->file_pos,
				w->file_pos + w->data_size - opt_minus1,
...
enumError CheckCommand (int argc, char **argv)
{
	...
	switch (the_cmd)
	{
		case CMD_VERSION:
			version_exit ();
		case CMD_HELP:
			PrintHelp (&InfoUI_wdf, stdout, 0, "+HELP", default_settings, URI_HOME,
				first_param ? first_param->arg : 0);
			break;
...
int main (int argc, char **argv)
{
	...
	snprintf (default_settings, sizeof (default_settings), "Default settings for '%s': %s --%s\n\n",
	...
	if (argc < 2)
	{
		printf ("\n%s\n%s%s\nVisit %s%s for more info.\n\n", text_logo, ProgInfo.progname, TITLE,
			URI_HOME, NAME);
		hint_exit (ERR_OK);
	}
