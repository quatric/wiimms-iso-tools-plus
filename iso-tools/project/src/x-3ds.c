...
static enumError extract_range (ccp fname, const u8 *image, u64 image_size, u64 off, u64 size)
{
	if (!size)
		return ERR_OK;
	if (off + size > image_size)
		return ERROR0 (
			ERR_INVALID_FILE, "Range 0x%llx+0x%llx is outside the image: %s\n", off, size, fname);
...
static void aes_ctr_crypt (
	const aes_key_t *akey, const u8 ctr_start[AES_BLOCK_SIZE], const u8 *in, u8 *out, u64 len)
{
...
static void u128_rol (u8 *v, uint bits)
{
	...
	for (int i = 0; i < 16; i++)
	{
		...
		tmp[i] = bit_shift ? v[src] << bit_shift | v[src2] >> (8 - bit_shift) : v[src];
	}
	memcpy (v, tmp, 16);
}
...
{
	static const u8 C[16] = { 0xFF, 0x34, 0x07, 0xCC, 0x3D, 0x68, 0x54, 0xF0, 0xA2, 0x4D, 0x5D,
		0x68, 0xE4, 0xA6, 0x7C, 0x4C };
	u8 x[16];
	memcpy (x, keyx, 16);
	u128_rol (x, 2);
	for (int i = 0; i < 16; i++)
...
static u8 *ncch_decrypt_region (const u8 *ncch, // NCCH start
...
static enumError dump_ncch (ccp dest, // directory to write into (created)
...
)
{
	...
	const u8 *keyx = 0;
	if (keys->keyx_2c.set || keys->keyx_25.set || keys->keyx_18.set || keys->keyx_1b.set)
	{
		switch (crypto_method)
		{
			case 0x00:
				keyx = keys->keyx_2c.set ? keys->keyx_2c.key : 0;
				break;
			case 0x01:
				keyx = keys->keyx_25.set ? keys->keyx_25.key : 0;
				break;
			case 0x0a:
				keyx = keys->keyx_18.set ? keys->keyx_18.key : 0;
				break;
			case 0x0b:
				keyx = keys->keyx_1b.set ? keys->keyx_1b.key : 0;
				break;
		}
	}
	...
	{
		u64 got = 0;
		u8 *data = ncch_decrypt_region (ncch, exefs_off, exefs_size, no_crypto, use_key, 2, &got);
		if (data)
		{
	...
	if (romfs_size && romfs_off + romfs_size <= ncch_avail && !(flags7 & NCCH_BIT_NO_ROMFS))
	{
		u64 got = 0;
		u8 *data = ncch_decrypt_region (ncch, romfs_off, romfs_size, no_crypto, use_key, 3, &got);
		if (data)
		{
...
static void print_ncch_line (uint idx, const u8 *ncch)
{
	...
	printf ("      partition %u: %-10s  %s%s%s\n", idx, product,
		...
		flags7 & NCCH_BIT_FIXED_KEY ? " fixed-key" : "", flags7 & NCCH_BIT_SEED ? " seed" : "");
}
...
enumError XExtractCIA (ccp source, ccp dest)
{
	...
	u64 pos = hsize;
	const u64 cert_off = pos;
	pos += cert_size;
	const u64 tik_off = pos;
	pos += tik_size;
	const u64 tmd_off = pos;
	pos += tmd_size;
	const u64 content_off = pos;
	pos += (content_size + 0x3f) & ~(u64)0x3f;
