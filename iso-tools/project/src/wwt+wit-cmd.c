...
void cmd_version_section (bool sect_header, ccp name_short, ccp name_long)
{
	...
	printf ("prog=%s\n"
			...
			"posix_c_source=" CONVERT_TO_STRING (_POSIX_C_SOURCE) "\n"
																  ...
																  "url=" URI_HOME "%s\n"
																  "\n",
		name_short, name_long, BETA_VERSION, e[0], e[1], e[2], e[3],
		endian == 0x01020304	   ? "little"
			: endian == 0x04030201 ? "big"
								   : "mixed",
		HAVE_STATTIME_NSEC, name_short);
}
...
void print_default_compr (ccp mode)
{
	...
	printf ("mode-%s=%s\n", mode, wd_print_compression (0, 0, compr, level, csize, 2));
};
	...
	if (print_sections)
	{
		...
		for (param = first_param; param; param = param->next, index++)
		{
			...
			printf ("\n[compression-mode-%u]\n"
					...
					"num=%d\n"
					"name=%s\n",
				index, param->arg, compr, wd_get_compression_name (compr, "-"));
		...
		for (param = first_param; param; param = param->next)
		{
			...
			else
			{
				...
				printf (" %-11s %s  %s   %.30s\n",
					...
					wd_print_size_1024 (0, 0, write_size, true), param->arg);
			}
		}
...
static const KeywordTab_t feature_tab[] = { //--- image formats

	{ FEAT_ISO, "ISO", 0, 0 }, { FEAT_WDF, "WDF", 0, 0 }, { FEAT_WDF1, "WDF1", "WDFV1", 0 },
	{ FEAT_WDF2, "WDF2", "WDFV2", 0 }, { FEAT_CISO, "CISO", "WBI", 0 }, { FEAT_WBFS, "WBFS", 0, 0 },
	{ FEAT_WIA, "WIA", 0, 0 }, { FEAT_GCZ, "GCZ", 0, 0 },
...
static bool check_feature (uint feat)
{
	...
	switch (feat)
	{
		case FEAT_ISO:
			stat = 1;
			msg = "Image format ISO";
			break;
		case FEAT_WDF:
			stat = 1;
			msg = "Image format WDF";
			break;
		case FEAT_WDF1:
			stat = 1;
			msg = "Image format WDF v1";
			break;
		case FEAT_WDF2:
			stat = 1;
			msg = "Image format WDF v2";
			break;
		case FEAT_CISO:
			stat = 1;
			msg = "Image format CISO";
			break;
		case FEAT_WBFS:
			stat = 1;
			msg = "Image format WBFS";
			break;
		case FEAT_WIA:
			stat = 1;
			msg = "Image format WIA";
			break;

#if HAVE_ZLIB
		case FEAT_GCZ:
			stat = 1;
			msg = "Image format GCZ";
			break;
#else
		case FEAT_GCZ:
			stat = 0;
...
enumError cmd_features ()
{
	...
	if (verbose >= -1)
		printf (
			"> Exit status 1: %u of %u requested features are supported.\n", n_supported, n_total);
	return 1;
}
...
enumError cmd_test_options ()
{
	...
	if (opt_wdf_version || opt_wdf_align || opt_wdf_min_holesize)
	{
		...
		if (use_wdf_align != opt_wdf_align)
			snprintf (iobuf, sizeof (iobuf), " => use %x (%s)", use_wdf_align,
				wd_print_size_1024 (0, 0, use_wdf_align, false));
		else
			*iobuf = 0;
...
#ifdef TEST
	{
		...
		for (;;)
		{
			...
			printf ("   CISO block size %12llx = %12lld -> %5u * %8x/hex == %12llx/hex\n", *fs_ptr,
				*fs_ptr, n_blocks, block_size, (u64)block_size * n_blocks);
			if (!*++fs_ptr)
				break;
		}
	}
	...
	printf ("  set-time:    %16llx = %12lld = %s\n", (u64)opt_set_time, (u64)opt_set_time,
		buf_set_time);
#endif
...
static void info_image_formats ()
{
	if (print_sections)
	{
		...
		for (oft = 1; oft < OFT__N; oft++)
		{
			...
			printf ("\n"
				...
				info->name, info->name, info->info, info->option, info->ext1 ? info->ext1 : "",
				info->ext2 ? info->ext2 : "", info->attrib & OFT_A_READ ? "read " : "",
				info->attrib & OFT_A_CREATE ? "create " : "",
				info->attrib & OFT_A_MODIFY ? "modify " : "",
				info->attrib & OFT_A_EXTEND ? "extend " : "",
				info->attrib & OFT_A_FST ? "fst " : "", info->attrib & OFT_A_COMPR ? "compr " : "",
	...
	printf ("\n"
			...
			" %.*s\n",
		info_fw, "Description", info_fw + 64, wd_sep_200);

	for (oft = 1; oft < OFT__N; oft++)
	{
		const OFT_info_t *i = oft_info + oft;
		printf ("  %-4s  %-*s  %-7s %-5s %-5s %s %s %s %s %s\n", i->name, info_fw, i->info,
			i->option ? i->option : "  -", i->ext1 && *i->ext1 ? i->ext1 : " -",
			i->ext2 && *i->ext2 ? i->ext2 : " -", i->attrib & OFT_A_READ ? "read" : "-   ",
...
enumError cmd_info ()
{
	static const KeywordTab_t cmdtab[]
		= { { INFO__ALL, "ALL", 0, 0 }, { INFO_IMAGE_FORMAT, "IMAGE-FORMATS", "IMAGEFORMATS", 0 },
