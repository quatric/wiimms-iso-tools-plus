...
{
	return compr_level < 1 ? 5 : compr_level < 7 ? compr_level : 7;
}

///////////////////////////////////////////////////////////////////////////////

u32 CalcMemoryUsageLZMA (int compr_level, // valid are 1..9 / 0: use default value
...
u32 CalcMemoryUsageLZMA2 (int compr_level, // valid are 1..9 / 0: use default value
	bool is_writing // false: reading mode, true: writing mode
)
{
	static u32 read_size[] = {
...
static void *AllocLZMA (void *p, size_t size)
{
	...
	alloc_size += size;
#if LOG_ALLOC > 1
	PRINT ("$$$ LZMA ALLOC [%p]: %zu/%u [%u-%u=%u]\n", ptr, size, alloc_size, alloc_count,
...
static void FreeLZMA (void *p, void *ptr)
{
	...
	if (ptr)
	{
		...
		if (LOG_ALLOC > 1 || free_count == 1)
			PRINT ("$$$ LZMA FREE [%p]: %u [%u-%u=%u]\n", ptr, alloc_size, alloc_count, free_count,
...
SRes sz_progress (void *pp, UInt64 in_size, UInt64 out_size)
{
	...
	if (in_size != ~(UInt64)0)
	{
		...
		noPRINT ("$$$ sz_progress(%10llu,%10llu) => %10llu +%10llu =%10llu  /%10llu\n", in_size,
			out_size, f->bytes_read, f->bytes_written, f->bytes_read + f->bytes_written,
...
enumError EncLZMA_Open (EncLZMA_t *lzma, // object, will be initialized
...
)
{
	...
	if (!lzma->handle)
		return ERROR0 (ERR_LZMA, "Error while opening LZMA stream: %s\n-> LZMA error: %s\n",
	...
	{
		EncLZMA_Close (lzma);
		return ERROR0 (ERR_LZMA, "Error while setup LZMA properties: %s\n-> LZMA error: %s\n",
	...
	{
		EncLZMA_Close (lzma);
		return ERROR0 (ERR_LZMA, "Error while writing LZMA properties: %s\n-> LZMA error: %s\n",
			lzma->error_object, GetMessageLZMA (res, "?"));
	}
...
enumError EncLZMA_WriteDataToFile (EncLZMA_t *lzma, // valid pointer, opened with EncLZMA_Open()
...
enumError EncLZMA_WriteList2File (EncLZMA_t *lzma, // valid pointer, opened with EncLZMA_Open()
	SuperFile_t *file, // destination file and progress support
					   //  -> write to 'file->f' at current offset
	bool write_props, // true: write encoding properties
...
)
{
	...
	SRes res = LzmaEnc_Encode (lzma->handle, (ISeqOutStream *)&outfile, (ISeqInStream *)&inbuf,
		(ICompressProgress *)&progress, &lzma_alloc, &lzma_alloc);
	if (res != SZ_OK)
	{
		EncLZMA_Close (lzma);
		return ERROR0 (ERR_LZMA, "Error while writing LZMA stream: %s\n-> LZMA error: %s\n",
			lzma->error_object, GetMessageLZMA (res, "?"));
	}
...
enumError EncLZMA_Close (EncLZMA_t *lzma // valid pointer
)
{
	DASSERT (lzma);
	if (lzma->handle)
	...
	)
{
	...
	return EncLZMA_List2File (
		lzma, file, compr_level, write_props, write_endmark, &list, bytes_written);
}
	...
	)
{
	...
	for (;;)
	{
		...
		noPRINT ("DECODED, res=%s, stat=%d, in=%zu/%zu out=%zu/%zu\n", GetMessageLZMA (res, "?"),
			status, in_len, in_buf_len, out_len, buf_size);

		if (res == SZ_OK && !in_len && !out_len && status != LZMA_STATUS_FINISHED_WITH_MARK)
			res = SZ_ERROR_DATA;

		if (res != SZ_OK)
			return ERROR0 (ERR_LZMA, "Error while reading LZMA stream: %s\n-> LZMA error: %s\n",
				f->fname, GetMessageLZMA (res, "?"));
...
enumError EncLZMA2_Open (EncLZMA_t *lzma, // object, will be initialized
	ccp error_object, // object name for error messages
	int compr_level, // valid are 1..9 / 0: use default value
	bool write_endmark // true: write end marker at end of stream
)
{
	...
	if (!lzma->handle)
		return ERROR0 (ERR_LZMA, "Error while opening LZMA2 stream: %s\n-> LZMA2 error: %s\n",
			lzma->error_object, GetMessageLZMA (SZ_ERROR_MEM, "?"));
	...
	{
		EncLZMA2_Close (lzma);
		return ERROR0 (ERR_LZMA, "Error while setup LZMA2 properties: %s\n-> LZMA2 error: %s\n",
			lzma->error_object, GetMessageLZMA (res, "?"));
	}
...
enumError EncLZMA2_WriteDataToFile (EncLZMA_t *lzma, // valid pointer, opened with EncLZMA2_Open()
	SuperFile_t *file, // destination file and progress support
					   //  -> write to 'file->f' at current offset
	bool write_props, // true: write encoding properties
	const void *data, // data to write
...
enumError EncLZMA2_WriteList2File (EncLZMA_t *lzma, // valid pointer, opened with EncLZMA2_Open()
	SuperFile_t *file, // destination file and progress support
					   //  -> write to 'file->f' at current offset
	bool write_props, // true: write encoding properties
	DataList_t *data_list, // NULL or data list (modified)
...
)
{
	...
	SRes res = Lzma2Enc_Encode (lzma->handle, (ISeqOutStream *)&outfile, (ISeqInStream *)&inbuf,
		(ICompressProgress *)&progress);
	if (res != SZ_OK)
	{
		EncLZMA2_Close (lzma);
		return ERROR0 (ERR_LZMA, "Error while writing LZMA2 stream: %s\n-> LZMA2 error: %s\n",
			lzma->error_object, GetMessageLZMA (res, "?"));
	}
...
enumError EncLZMA2_Close (EncLZMA_t *lzma // valid pointer
)
{
	DASSERT (lzma);
	if (lzma->handle)
