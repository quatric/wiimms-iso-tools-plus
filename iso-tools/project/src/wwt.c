...
#define TITLE WWT_SHORT ": " WWT_LONG " v" VERSION " r" REVISION " " SYSTEM2 " - " AUTHOR " - " DATE
...
enumError cmd_test ()
{
	...
	SubstString_t tab[] = { { '1', 0, 0, "1234567890" }, { 'a', 'A', 0, "AbCdEfGhIjKlMn" },
		{ 'x', 'X', 0, "xyz" }, { 0, 0, 0, 0 } };
...
enumError cmd_find ()
{
	...
	if (print_sections)
	{
		...
		printf ("[FIND]\n"
				...
				"n-wbfs=%u\n"
				"n-print=%u\n",
			n_total, n_wbfs, long_count ? n_total : n_wbfs);

		int count = 0;
		for (info = first_partition_info; info; info = info->next)
		{
			if (info->part_mode >= PM_WBFS || info->part_mode > PM_IGNORE && long_count)
			{
				...
				printf ("\n"
						...
						"size=%llu\n"
						"disc-usage=%llu\n",
					count++, info->path, info->real_path,
					GetFileModeText (info->filemode, false, "-"), is_wbfs, info->hss,
					info->file_size, info->disk_usage ? info->disk_usage : info->file_size);

				if (is_wbfs)
					printf ("wbfs-hss=%u\n"
							"wbfs-wss=%u\n"
							"wbfs-size=%llu\n",
						info->wbfs_hss, info->wbfs_wss, info->wbfs_size);
			}
		}
		putchar ('\n');
	}
				...
				for (info = first_partition_info; info; info = info->next)
					if (info->part_mode > PM_IGNORE)
						printf ("%-5s %s %7lld %7lld  %s\n",
							...
							(info->disk_usage + MiB / 2) / MiB, (info->file_size + MiB / 2) / MiB,
							info->path);
				if (print_header)
					printf ("\n");
				break;
				...
				for (info = first_partition_info; info; info = info->next)
					if (info->part_mode > PM_IGNORE)
						printf ("%-5s %s %13lld %13lld  %s\n",
							GetFileModeText (info->filemode, false, "-"),
							info->part_mode >= PM_WBFS ? "WBFS" : " -- ", info->disk_usage,
							info->file_size, info->real_path);
				if (print_header)
					printf ("\n");
				break;
		}
...
enumError cmd_space ()
{
	...
	if (print_header)
		printf ("\n%s   size    used used%%   free   discs    file   (sizes in MiB)%s\n"
				"%s--------------------------------------------------------------%s\n",
			colout->heading, colout->reset, colout->heading, colout->reset);

	WBFS_t wbfs;
	InitializeWBFS (&wbfs);
	PartitionInfo_t *info;
	for (err = GetFirstWBFS (&wbfs, &info, false); !err && !SIGINT_level;
		err = GetNextWBFS (&wbfs, &info, false))
	{
		printf ("%7d %7d %3d%% %7d %4d/%-4d  %s\n", wbfs.total_mib, wbfs.used_mib,
			100 * wbfs.used_mib / (wbfs.total_mib), wbfs.free_mib, wbfs.used_discs,
			wbfs.total_discs, long_count ? info->real_path : info->path);
	}
	if (print_header)
		printf ("\n");
...
enumError cmd_dump ()
{
	...
	const int invalid = long_count > 4 ? 2 : OptionUsed[OPT_INODE] || long_count > 3;
...
static ccp print_fragment (fragment_summary_t *sum, // summary record
...
static void print_fragment_summary (fragment_summary_t *sum // summary record
)
{
	if (!print_sections && sum->n_images > 0)
	{
		if (sum->n_frag_images > 0)
		{
			...
			printf ("       %u of %u disc%s %s fragmented, weighted ratio = %4.2f.\n",
				sum->n_frag_images, sum->n_images, sum->n_images > 1 ? "s" : "",
				sum->n_frag_images > 1 ? "are" : "is", ratio);
		}
		else
			printf (
				"       %u unfragmented disc%s.\n", sum->n_images, sum->n_images > 1 ? "s" : "");
	}
}
...
static int print_list_mixed ()
{
	...
	if (print_sections)
	{
		...
		for (i = 0, witem = pi_wlist.first_disc; i < pi_wlist.used; i++, witem++)
		{
			...
			PrintSectWDiscListItem (
				stdout, witem, pi > 0 && pi <= pi_count ? pi_list[pi]->path : 0);
		}
		return ERR_OK;
	}
	...
	if (print_header)
	{
		...
		footer_len = snprintf (footer, sizeof (footer),
			"Total: %u discs, %u MiB ~ %u GiB used, %u MiB ~ %u GiB free.", pi_wlist.used,
			pi_wlist.total_size_mib, (pi_wlist.total_size_mib + 512) / 1024, pi_free_mib,
			(pi_free_mib + 512) / 1024);
	}
	...
	if (long_count > 1)
	{
		if (print_header)
		{
			...
			printf ("\nID6     MiB %s  WI  %n%d discs (%d GiB)%n\n",
				print_fragments ? "fragments" : "Reg.", &n1, pi_wlist.used,
				(pi_wlist.total_size_mib + 512) / 1024, &n2);
			max_name_wd += n1;
			if (max_name_wd < n2)
				max_name_wd = n2;
		...
		for (i = pi_wlist.used, witem = pi_wlist.first_disc; i-- > 0; witem++)
			printf ("%s %4d %s %3d  %s\n", witem->id6, witem->size_mib,
				print_fragments ? print_fragment (&fsum, witem, "")
								: GetRegionInfo (witem->id6[3])->name4,
				witem->part_index, witem->title ? witem->title : witem->name64);
	}
	else if (long_count)
	{
		if (print_header)
...
enumError cmd_list (int long_level)
{
		...
		err = GetNextWBFS (&wbfs, &info, false))
	{
		...
		if (wlist)
		{
			...
			if (print_header)
			{
				...
				footer_len = snprintf (footer, sizeof (footer),
					"Total: %d/%d discs, %d MiB ~ %d GiB used, %d MiB ~ %d GiB free.", wlist->used,
					wbfs.total_discs, wlist->total_size_mib, (wlist->total_size_mib + 512) / 1024,
			...
			else if (long_count)
			{
				if (print_header)
				{
					...
					printf ("ID6   %s  MiB %s %n%d/%d discs (%d GiB)%n\n", pt.head,
						print_fragments ? "fragments " : "Reg.", &n1, wlist->used, wbfs.total_discs,
			...
			else
			{
				if (print_header)
				{
					...
					printf ("ID6   %s  %n%d/%d discs (%d GiB)%n\n", pt.head, &n1, wlist->used,
						wbfs.total_discs, (wlist->total_size_mib + 512) / 1024, &n2);
					noTRACE ("N1=%d N2=%d max_name_wd=%d\n", n1, n2, max_name_wd);
					max_name_wd += n1;
					if (max_name_wd < n2)
						max_name_wd = n2;
	...
	if (print_header && gt_wbfs > 1)
		printf ("Grand total: %d WBFS, %d/%d discs, %d GiB used, %d GiB free.\n\n", gt_wbfs,
			gt_disc, gt_max_disc, (gt_used_mib + 512) / 1024, (gt_free_mib + 512) / 1024);
...
enumError cmd_format ()
{
	...
	for (param = first_param; param; param = param->next)
	{
		...
		if (StatFile (&st, param->arg, 0))
		{
			...
			if (!opt_size)
			{
				error_count++;
				ERROR0 (
					ERR_SEMANTIC, "Option --size needed to create a new WBFS file: %s", param->arg);
				continue;
			}
		...
		else
		{
			...
			{
				ERROR0 (ERR_WRONG_FILE_TYPE, "%s: Neither regular file nor block device: %s\n",
		...
		else
		{
			...
			par.setup_iinfo = !recover && (filemode > FM_PLAIN || OptionUsed[OPT_INODE]);
	...
	if (verbose >= 0)
	{
		printf ("** ");
		if (create_count > 0)
			printf ("%d file%s created, ", create_count, create_count == 1 ? "" : "s");
...
enumError cmd_recover ()
{
	...
	for (err = GetFirstWBFS (&wbfs, &info, true); !err && !SIGINT_level;
...
enumError cmd_check ()
{
	...
	PartitionInfo_t *info;
	int err_count = 0;
	for (err = GetFirstWBFS (&wbfs, &info, repair_mode); !err && !SIGINT_level;
...
enumError cmd_edit ()
{
	...
	printf ("\n* MODIFY WBFS partition %s:\n"
			...
			"  > Number of slots:  %3u\n"
			"\n",
		info->path, w->wbfs_sec_sz, w->wbfs_sec_sz, (double)w->wbfs_sec_sz / MiB, w->n_wbfs_sec - 1,
		w->n_wbfs_sec_per_disc, wbfs.used_discs, w->max_disc);
		...
		if (cptr->id == DO_RM || cptr->id == DO_ACT || cptr->id == DO_INV)
		{
			...
			switch (cptr->id)
			{
				case DO_RM:
					...
					break;
				case DO_ACT:
					mode = WBFS_SLOT_VALID;
					info = "activate";
					break;
				default:
					mode = WBFS_SLOT_INVALID;
					info = "invalidate";
					break;
			}
					...
					if (n1 > 0 && n1 <= n2)
					{
						...
						if (n1 == n2)
							printf ("  - Disc %s: %sset block %u to WBFS block %u.\n", cptr->name1,
								testmode ? "WOULD " : "", n1, val);
						else
							printf ("  - Disc %s: %sset blocks %u..%u to WBFS blocks %u..%u.\n",
								cptr->name1, testmode ? "WOULD " : "", n1, n2, val,
								(u16)(val + count * add));
						if (!testmode)
							while (n1 <= n2)
							{
...
enumError cmd_phantom ()
{
		...
		err = GetNextWBFS (&wbfs, &info, true))
	{
		...
		bool abort = false;
		ParamList_t *param;
		for (param = first_param; param && !SIGINT_level && !abort; param = param->next)
		{
			char *arg = param->arg;
			if (!arg || !*arg)
				continue;
	...
	if (verbose >= 0)
	{
		if (wbfs_count > 1)
		{
			printf ("** %d phantom%s added, %d of %d WBFS used.\n", total_add_count,
				total_add_count == 1 ? "" : "s", wbfs_mod_count, wbfs_count);
		}
		if (verbose >= 1)
			printf ("\n");
	}
...
static void count_jobs (WBFS_t *w, Iterator_t *it, bool count_it)
{
	...
	while (ptr < end)
	{
		...
		if (!wfound || it->overwrite || it->newer && (!item->mtime || item->mtime > wfound->mtime))
		{
			item->flag = 1;
			if (count_it)
				it->job_total++;
...
enumError exec_add (SuperFile_t *sf, Iterator_t *it)
{
	...
	if (it->newer && sf->f.fatt.mtime.tv_sec && OpenWDiscID6 (it->wbfs, sf->f.id6_dest) == ERR_OK)
	{
		exists = 1;
		ASSERT (it->wbfs->disc);
			...
			if (testmode || verbose >= 0)
			{
				if (print_sections)
					printf ("[REMOVE]\n"
							...
							"title=%s\n"
							"\n",
						testmode, sf->f.id6_dest, title);
				else
					printf (" - %s%-*s [%s] %s\n", testmode ? "WOULD " : "", 2 * fw_counter + 5,
						"REMOVE", sf->f.id6_dest, title);
			}
	...
	if (testmode || verbose >= 0 || progress > 0)
	{
		...
		if (print_sections)
			printf ("[ADD]\n"
					...
					"source-n-split=%d\n"
					"\n",
				testmode, it->job_count, it->job_total, sf->f.id6_dest, title, sf->f.fname,
				it->real_path, oft_info[sf->iod.oft].name, sf->f.split_used);
		else
			printf (" - %sADD %*u/%u [%s] %s:%s\n", testmode ? "WOULD " : "", fw_counter,
				it->job_count, it->job_total, sf->f.id6_dest, oft_info[sf->iod.oft].name,
				sf->f.fname);
		fflush (stdout);
	}
					...
					if (testmode || verbose >= 0)
					{
						if (print_sections)
							printf ("[REMOVE]\n"
									...
									"title=%s\n"
									"\n",
								testmode, ptr->id6, GetTitle (ptr->id6, ptr->name64));
						else
							printf (" - %s%-*s [%s] %s\n", testmode ? "WOULD " : "",
								2 * fw_counter + 5, "REMOVE", ptr->id6,
			...
			if (testmode || verbose >= 0)
			{
				if (print_sections)
				{
					...
					printf ("n-removed=%u\n"
							"n-added=%u\n"
							"\n",
						it.rm_count, it.done_count);
				}
				else
				{
					*iobuf = 0;
					if (it.rm_count)
						snprintf (iobuf, sizeof (iobuf), " %d disc%s removed,", it.rm_count,
							it.rm_count == 1 ? "" : "s");
					printf ("* WBFS #%d:%s %d disc%s added.\n", wbfs_count, iobuf, it.done_count,
						it.done_count == 1 ? "" : "s");
				}
			}
		}
...
enumError cmd_dup ()
{
	...
	if (n_wbfs > 1 && !dest_is_dir)
		return ERROR0 (
			ERR_SEMANTIC, "There are %u (>1) sources => destination must be a directory.", n_wbfs);
	...
	InitializeWBFS (&wbfs);
	PartitionInfo_t *info;
	for (err = GetFirstWBFS (&wbfs, &info, false); !err && !SIGINT_level && wbfs_count < job_limit;
		err = GetNextWBFS (&wbfs, &info, false))
	{
		wbfs_count++;
		...
		if (testmode || verbose >= 0 || progress > 0)
		{
			...
			if (print_sections)
				printf ("[DUP]\n"
						...
						"dest-path=%s\n"
						"\n",
					testmode, wbfs_count, n_wbfs, info->path, dest);
			else
				printf (" - %sDUP %u/%u %s -> %s\n", testmode ? "WOULD " : "", wbfs_count, n_wbfs,
					info->path, dest);
			fflush (stdout);
...
enumError cmd_extract ()
{
	...
	for (err = GetFirstWBFS (&wbfs, &info, false);
		!err && !SIGINT_level && extract_count < job_limit; err = GetNextWBFS (&wbfs, &info, false))
	{
		wbfs_count++;
		...
		int slot;
		for (slot = 0;
			slot < wbfs.wbfs->max_disc && extract_count < job_limit && wbfs_go && !SIGINT_level;
			slot++)
		{
			ccp id6, title;
			...
			if (item)
			{
				...
				if (dhead)
				{
					...
					int conv_count = SubstFileName (fbuf, sizeof (fbuf), id6,
						(ccp)dhead->disc_title, info->path, 0, dpath, oft);
					noTRACE ("|%s|%s|\n", dpath, fbuf);
					...
					if (print_sections)
					{
						printf ("[EXTRACT]\n"
							...
							testmode > 0,
							oft == OFT_FST				   ? "extract"
								: part_selector.whole_disc ? "copy/raw"
														   : "copy/scrubbed",
							extract_count + 1, slot, id6, title, fo.f.fname, oft_info[oft].name);
						fflush (stdout);
					}
			...
			if (verbose >= 0)
			{
				if (print_sections)
				{
					...
					printf ("n-removed=%u\n"
							"n-extracted=%u\n"
							"\n",
						wbfs_rm_count, wbfs_extract_count);
				}
				else
				{
					char buf[50];
					if (wbfs_rm_count)
						snprintf (buf, sizeof (buf), ", %d disc%s removed", wbfs_rm_count,
							wbfs_rm_count == 1 ? "" : "s");
					else
						*buf = 0;
	...
	if (verbose >= 0)
	{
		if (wbfs_count > 1 && !print_sections)
		{
			printf ("** %d disc%s extracted, %d of %d WBFS used.\n", extract_count,
				extract_count == 1 ? "" : "s", wbfs_used_count, wbfs_count);
			if (rm_count)
				printf ("** %d disc%s removed, %d of %d WBFS modified.\n", rm_count,
					rm_count == 1 ? "" : "s", wbfs_mod_count, wbfs_count);
		}
		if (verbose >= 1)
			printf ("\n");
	}
...
enumError cmd_scrub ()
{
	...
	InitializeWBFS (&wbfs);
	PartitionInfo_t *info;
	for (err = GetFirstWBFS (&wbfs, &info, true); !err && !SIGINT_level && scrub_count < job_limit;
		err = GetNextWBFS (&wbfs, &info, true))
	{
		wbfs_count++;
		...
		int slot;
		for (slot = 0;
			slot < wbfs.wbfs->max_disc && scrub_count < job_limit && wbfs_go && !SIGINT_level;
			slot++)
		{
			ccp id6, title;
			const IdItem_t *item = CheckParamSlot (&wbfs, slot, true, &id6, &title);
				...
				if (dhead)
				{
					if (print_sections)
					{
						printf ("[SCRUB]\n"
								...
								"title=%s\n"
								"\n",
							testmode > 0, scrub_count + 1, slot, id6, title);
						fflush (stdout);
					}
					...
					else
					{
						...
						const u32 wii_sec_per_wbfs_sect = 1 << (w->wbfs_sec_sz_s - w->wii_sec_sz_s);
			...
			if (verbose >= 0)
			{
				...
				else
				{
					printf ("* WBFS #%d: %d disc%s scrubbed/patched.\n", wbfs_count,
						wbfs_scrub_count, wbfs_scrub_count == 1 ? "" : "s");
				}
			}
		}
	}
	...
	if (verbose >= 0)
	{
		if (wbfs_count > 1 && !print_sections)
		{
			printf ("** %d disc%s scrubbed, %d of %d WBFS modified.\n", scrub_count,
				scrub_count == 1 ? "" : "s", wbfs_modified_count, wbfs_count);
		}
		if (verbose >= 1)
			printf ("\n");
	}
...
enumError cmd_remove ()
{
		...
		err = GetNextWBFS (&wbfs, &info, true))
	{
		...
		int slot;
		for (slot = 0; slot < wbfs.wbfs->max_disc && wbfs_go && !SIGINT_level; slot++)
		{
			ccp id6, title;
				...
				if (testmode || verbose > 0)
				{
					...
					if (print_sections)
					{
						printf ("[REMOVE]\n"
								...
								"title=%s\n"
								"\n",
							testmode > 0, wbfs_rm_count + rm_count + 1, slot, id6, title);
					}
					else
						printf (" - %s%s [%s] %s\n", testmode ? "WOULD " : "",
		...
		if (wbfs_rm_count)
		{
			...
			if (verbose >= 0)
			{
				if (print_sections)
				{
					...
					printf ("* WBFS #%d: %d disc%s %s.\n", wbfs_count, wbfs_rm_count,
						wbfs_rm_count == 1 ? "" : "s", free_slot_only ? "droped" : "removed");
			}
		}
	}
	ResetWBFS (&wbfs);
	...
	if (verbose >= 0 && !print_sections)
	{
		if (wbfs_count > 1)
			printf ("** %d disc%s %s, %d of %d WBFS modified.\n", rm_count,
				rm_count == 1 ? "" : "s", free_slot_only ? "droped" : "removed", wbfs_mod_count,
				wbfs_count);
...
enumError cmd_rename (bool rename_id)
{
		...
		err = GetNextWBFS (&wbfs, &info, true))
	{
		...
		bool wbfs_go = true;
		ParamList_t *param;
		for (param = first_param; param && wbfs_go && !SIGINT_level; param = param->next)
		{
			...
			if (*sel == '+')
			{
				...
				for (idx = 0; idx < wbfs.used_discs; idx++)
				{
					...
					if (err)
						return err;
					RenameWDisc (
						&wbfs, param->id6, param->arg, change_wbfs, change_iso, verbose, testmode);
					wbfs_mv_count++;
				}
				param->count++;
				continue;
	...
	if (verbose >= 0)
	{
		if (wbfs_count > 1)
			printf ("** %d disc%s changed, %d of %d WBFS modified.\n", mv_count,
				mv_count == 1 ? "" : "s", wbfs_mod_count, wbfs_count);
...
enumError cmd_touch ()
{
	...
	const u64 set_time = !opt_set_time || opt_set_time == (time_t)-1 ? time (0) : opt_set_time;
		...
		err = GetNextWBFS (&wbfs, &info, true))
	{
		...
		if (wbfs_touch_count)
		{
			...
			if (verbose >= 0)
				printf ("* WBFS #%d: %d disc%s touched.\n", wbfs_count, wbfs_touch_count,
					wbfs_touch_count == 1 ? "" : "s");
		}
	}
	ResetWBFS (&wbfs);
	...
	if (verbose >= 0)
	{
		if (wbfs_count > 1)
			printf ("** %d disc%s touched, %d of %d WBFS modified.\n", touch_count,
				touch_count == 1 ? "" : "s", wbfs_mod_count, wbfs_count);
...
enumError cmd_verify ()
{
	...
	InitializeWBFS (&wbfs);
	PartitionInfo_t *info;
	for (err = GetFirstWBFS (&wbfs, &info, remove); !err && !SIGINT_level;
		err = GetNextWBFS (&wbfs, &info, remove))
	{
		...
		int slot;
		for (slot = 0; slot < wbfs.wbfs->max_disc && !SIGINT_level; slot++)
		{
		...
		if (wbfs_verify_count)
		{
			...
			if (verbose >= 0)
			{
				if (wbfs_fail_count)
				{
					...
					snprintf (fail_buf, sizeof (fail_buf), ", %d bad disc%s %s", wbfs_fail_count,
						wbfs_fail_count == 1 ? "" : "s", fail_verb);
				}
				else
					*fail_buf = 0;
				printf ("* WBFS #%d: %d disc%s verified%s.\n", wbfs_count, wbfs_verify_count,
					wbfs_verify_count == 1 ? "" : "s", fail_buf);
			}
		}
	}
	ResetWBFS (&wbfs);
	...
	if (verbose >= 0)
	{
		...
		{
			if (fail_count)
				snprintf (fail_buf, sizeof (fail_buf), ", %d bad disc%s %s", fail_count,
					fail_count == 1 ? "" : "s", fail_verb);
			else
				*fail_buf = 0;
			printf ("** Total: %d disc%s of %d WBFS verified%s.\n", verify_count,
				verify_count == 1 ? "" : "s", wbfs_count, fail_buf);
		}
	...
	for (;;)
	{
		...
		switch ((enumGetOpt)opt_stat)
		{
			...
			case GO_ALL:
				...
				break;
			case GO_PART:
				AtFileHelper (optarg, 0, 0, AddPartition);
				break;
			case GO_WBFS_ALLOC:
				err += ScanOptWbfsAlloc (optarg);
				break;
			case GO_SOURCE:
			...
			case GO_SIZE:
				if (ScanSizeOptU64 (
						&opt_size, optarg, GiB, 0, "size", MIN_WBFS_SIZE, 0, 0, 0, true))
					err++;
				break;

			case GO_HSS:
				if (ScanSizeOptU32 (
						&opt_hss, optarg, 1, 0, "hss", 512, WII_SECTOR_SIZE, 0, 1, true))
					err++;
				break;

			case GO_WSS:
				if (ScanSizeOptU32 (&opt_wss, optarg, 1, 0, "wss", 1024, 0, 0, 1, true))
					err++;
				break;
...
enumError CheckCommand (int argc, char **argv)
{
	...
	switch ((enumCommands)cmd_ct->id)
	{
		...
		case CMD_HELP:
			PrintHelp (
				&InfoUI_wwt, stdout, 0, "HELP", 0, URI_HOME, first_param ? first_param->arg : 0);
			break;
		case CMD_CONFIG:
			err = cmd_config ();
			break;
