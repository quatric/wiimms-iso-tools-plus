...
static void test_create_file ()
{
	...
	printf ("*** created -> press ENTER: ");
	fflush (stdout);
	getchar ();

	CloseWFile (&f, 1);
	printf ("*** closed -> press ENTER: ");
	fflush (stdout);
	getchar ();
}
...
enumError CreateWBFSFile (SuperFile_t *sf, // valid pointer
	ccp fname, int overwrite,
...
int test_print_size (int argc, char **argv)
{
	...
	u64 i = 0;
	printf ("%21llu |%s|%s|  |%s|\t|%s|\n", i, wd_print_size_1000 (0, 0, i, true),
		wd_print_size_1024 (0, 0, i, true), wd_print_size_1000 (0, 0, i, false),
...
static void dump_wbfs (WBFS_t *w, enumError err, ccp title)
{
	...
	if (sf)
	{
		printf ("sf: id=%s,%s, oft=%u=%s, wbfs=%p, fname=%s\n", sf->f.id6_src, sf->f.id6_dest,
			sf->iod.oft, oft_info[sf->iod.oft].name, sf->wbfs, sf->f.fname);
...
static void test_open_disc (int argc, char **argv)
{
	...
	for (i = 1; i < argc; i++)
	{
		...
		if (!strcmp (argv[i], "-f"))
			pfst |= WD_PFST__ALL;
		if (!strcmp (argv[i], "-u"))
			pfst |= WD_PFST_UNUSED | WD_PFST_HEADER;
		if (!strcmp (argv[i], "-o"))
			pfst |= WD_PFST_OFFSET | WD_PFST_HEADER;
		if (!strcmp (argv[i], "-h"))
			pfst |= WD_PFST_SIZE_HEX | WD_PFST_HEADER;
		if (!strcmp (argv[i], "-d"))
			pfst |= WD_PFST_SIZE_DEC | WD_PFST_HEADER;
		if (!strcmp (argv[i], "-L"))
			logging++;
		...
		if (!OpenSF (&sf, argv[i], false, false))
		{
			printf ("*** %s\n", sf.f.fname);
			enumError err;
			wd_disc_t *disc = wd_open_disc (
				WrapperReadDirectSF, &sf, sf.file_size, sf.f.fname, opt_force, &err);
			if (disc)
			{
				putchar ('\n');
...
wd_part_info_t *wd_get_part_info (wd_part_info_t *pinfo, // store info here.
...
void wd_print_part_info_section (FILE *f, // valid output file
...
)
{
	...
	printf ("part-index=%u\n"
			...
			"part-type=%x\n"
			"\n",
		pinfo->index, pinfo->ptab_index, pinfo->ptab_part_index, pinfo->part_type);
}

///////////////////////////////////////////////////////////////////////////////

wd_disc_info_t *wd_get_disc_info (wd_disc_info_t *dinfo, // store info here.
														 // if NULL: allocate mem -> call FREE()
...
void wd_print_disc_info_section (FILE *f, // valid output file
...
)
{
	...
	printf ("disc-id=%s\n"
			...
			"db-title=%s\n"
			"\n",
		wd_print_id (dinfo->id6, 6, 0), dinfo->title, GetTitle (dinfo->id6, ""));
...
int patch_server (struct wd_iterator_t *it // iterator struct with all infos
)
{
	...
	wd_memmap_item_t *item
		= wd_insert_memmap_alloc (&it->disc->patch, WD_PAT_DATA, it->off4 << 2, it->size);
...
static const KeywordTab_t CommandTab[] = { { CMD_TEST, "TEST", "T", 0 },

	{ CMD_FILENAME, "FILENAME", "FN", 0 }, { CMD_MATCH_PATTERN, "MATCH", 0, 0 },
	{ CMD_OPEN_DISC, "OPENDISC", "ODISC", 0 }, { CMD_HEXDUMP, "HEXDUMP", 0, 0 },
	...
	{ CMD_DEVELOP, "DEVELOP", "D", 0 }, { CMD_HELP, "HELP", "?", 0 },
...
int main (int argc, char **argv)
{
	...
	if (0)
	{
		...
		PRINT ("sizeof(id6_t)=%zd, %p,%p,%p -> %zu,%zu,%zu\n", sizeof (id6_t), id6, id6 + 1,
			id6 + 2, (ccp)id6 - iobuf, (ccp)(id6 + 1) - iobuf, (ccp)(id6 + 2) - iobuf);
	}
#endif
	...
	switch (cmd_ct->id)
	{
		...
		case CMD_MATCH_PATTERN:
			...
			break;
		case CMD_OPEN_DISC:
			test_open_disc (argc, argv);
			break;
		case CMD_HEXDUMP:
			test_hexdump (argc, argv);
			break;
		case CMD_PATCH_HOST:
			test_patch_host (argc, argv);
			break;
