...
static int valid_offset (int fd, off_t off)
{
	...
	char ch;
	if (!off || off > ~(off_t)0 / 2 || lseek (fd, off, SEEK_SET) == (off_t)-1)
		return 0;
...
enumError StatFile (struct stat *st, ccp fname, int fd)
{
	...
	TRACE (" - st_mode:    %llx [REG=%u,DIR=%d,CHR=%d,BLK=%d,FIFO=%d,LNK=%d,SOCK=%d]\n",
		(u64)st->st_mode, S_ISREG (st->st_mode), S_ISDIR (st->st_mode), S_ISCHR (st->st_mode),
		S_ISBLK (st->st_mode), S_ISFIFO (st->st_mode), S_ISLNK (st->st_mode),
		S_ISSOCK (st->st_mode));
	TRACE (" - st_nlink:   %llu\n", (u64)st->st_nlink);
	TRACE (" - st_uid:     %llu\n", (u64)st->st_uid);
	TRACE (" - st_gid:     %llu\n", (u64)st->st_gid);
...
enumError XCloseWFile (XPARM WFile_t *f, bool remove_file)
{
	...
	if (close_err)
	{
		...
		if (!f->disable_errors)
			PrintError (XERROR1, err, "Close file failed: %s\n", f->fname);
	}
	...
	XPARM WFile_t *f, enumIOMode iomode, int default_flags, int force_flags)
{
	...
	TRACE ("#F# OpenWFileHelper(%p,%x,%x,%x) dis=%d, mkdir=%d, fname=%s\n", f, iomode,
		default_flags, force_flags, f->disable_errors, f->create_directory, f->fname);
	...
	f->active_open_flags = (f->open_flags ? f->open_flags : default_flags) | force_flags;
	...
	if (f->fd == -1)
	{
		if (f->active_open_flags & O_CREAT)
		{
			...
			if (!f->disable_errors)
				PrintError (XERROR1, f->last_error, "Can't create file: %s\n", f->fname);
		}
		else
		{
			f->max_error = f->last_error = ERR_CANT_OPEN;
			if (!f->disable_errors)
				PrintError (XERROR1, f->last_error, "Can't open file: %s\n", f->fname);
		}
	}
	...
	else
	{
		...
		f->seek_allowed
			= GetFileMode (f->st.st_mode) != FM_OTHER && lseek (f->fd, 0, SEEK_SET) != (off_t)-1;
	...
	TRACE ("#F# OpenWFileHelper(%p) returns %d, fd=%d, fp=%p, seek-allowed=%d, rw=%d,%d\n", f,
		f->last_error, GetFD (f), GetFP (f), f->seek_allowed, f->is_reading, f->is_writing);
...
enumError XCheckCreated (XPARM ccp fname, bool disable_errors, enumError err_code)
{
	...
	{
		if (!disable_errors)
			PrintError (XERROR0, err_code, "File already created: %s\n", fname);
		return err_code;
	}
	return ERR_OK;
}
...
enumError XCreateWFile (XPARM WFile_t *f, ccp fname, enumIOMode iomode, int overwrite)
{
	...
	if (!stat (fname, &f->st))
	{
		...
		if (S_ISBLK (f->st.st_mode) || S_ISCHR (f->st.st_mode))
		{
			if (!overwrite)
				return PrintError (
					XERROR0, ERR_ALREADY_EXISTS, "Can't overwrite block device: %s\n", fname);
		...
		else
		{
			if (!S_ISREG (f->st.st_mode))
				return PrintError (XERROR0, ERR_WRONG_FILE_TYPE, "Not a plain file: %s\n", fname);

			if (!overwrite)
				return PrintError (XERROR0, ERR_ALREADY_EXISTS, "File already exists: %s\n", fname);
		}
	}
	...
	if (!disable_errors)
		PrintError (XERROR0, ERR_CANT_CREATE, "Can't create temp file: %s\n", fname);
...
uint ReduceToPathAndType (char *buf, // valid return buffer
...
void GenWFileName (WFile_t *f, ccp path, ccp name, ccp title, ccp id6, ccp ext)
{
	...
	else
	{
		...
		{
			dest += snprintf (
				dest, end - dest, "%s[%s]", dest > fbuf && dest[-1] == '/' ? "" : " ", id6);
			TRACE (" >ID6:  |%.*s|\n", (int)(dest - fbuf), fbuf);
		}
	}
	...
	if (ext)
	{
		...
		if (elen > 0 && (dest - elen <= fbuf || memcmp (dest - elen, ext, elen))
			&& dest + elen < end)
		{
			TRACE ("add ext %s\n", ext);
...
const OFT_info_t oft_info[OFT__N + 1] = {
	...
	{ OFT_UNKNOWN, 0, IOM__IS_DEFAULT, "?", 0, "\0", 0, "Unkown file format" },

	{ OFT_PLAIN, OFT_A_READ | OFT_A_CREATE | OFT_A_EXTEND | OFT_A_MODIFY | OFT_A_LOADER,
		IOM_IS_IMAGE, "ISO", "--iso", ".iso", 0, "Plain file" },

	{ OFT_WDF1, OFT_A_READ | OFT_A_CREATE | OFT_A_EXTEND | OFT_A_MODIFY | OFT_A_WDF, IOM_IS_IMAGE,
		"WDF1", "--wdf1", ".wdf", ".wdf1", "Wii Disc Format v1" },

	{ OFT_WDF2, OFT_A_READ | OFT_A_CREATE | OFT_A_EXTEND | OFT_A_MODIFY | OFT_A_WDF, IOM_IS_IMAGE,
		"WDF2", "--wdf2", ".wdf", ".wdf2", "Wii Disc Format v2" },

	{ OFT_CISO, OFT_A_READ | OFT_A_CREATE | OFT_A_MODIFY | OFT_A_NOSIZE | OFT_A_LOADER,
		IOM_IS_IMAGE, "CISO", "--ciso", ".ciso", ".wbi", "Compact ISO" },

	{ OFT_WBFS,
		OFT_A_READ | OFT_A_CREATE | OFT_A_EXTEND | OFT_A_MODIFY | OFT_A_NOSIZE | OFT_A_LOADER,
		IOM_IS_IMAGE, "WBFS", "--wbfs", ".wbfs", 0, "Wii Backup File System" },

	{ OFT_WIA, OFT_A_READ | OFT_A_CREATE | OFT_A_COMPR, IOM_IS_COMPRESSED, "WIA", "--wia", ".wia",
		".rvz", "Compressed Wii ISO Archive" },

	{ OFT_GCZ, OFT_A_READ | OFT_A_CREATE | OFT_A_COMPR, IOM_IS_COMPRESSED, "GCZ", "--gcz", ".gcz",
		0, "Dolphins GameCube Zip" },

	{ OFT_FST, OFT_A_READ | OFT_A_CREATE | OFT_A_FST | OFT_A_DEST_EDIT, IOM__IS_DEFAULT, "FST",
		"--fst", "\0", 0, "Extracted File System" },

	{ OFT__N, 0, IOM__IS_DEFAULT, 0, 0, 0, 0, 0 },
};

///////////////////////////////////////////////////////////////////////////////

const KeywordTab_t ImageTypeTab[]
	= { { OFT_PLAIN, "ISO", "PLAIN", 0 }, { OFT__WDF_DEF, "WDF", 0, 0 },
		  { OFT_WDF1, "WDF1", "WDFV1", 0 }, { OFT_WDF2, "WDF2", "WDFV2", 0 },
		  { OFT_CISO, "CISO", 0, 0 }, { OFT_WBFS, "WBFS", 0, 0 }, { OFT_WIA, "WIA", 0, 0 },
		  { OFT_GCZ, "GCZ", "DOLPHIN", 0 }, { OFT_FST, "FST", 0, 1 }, { 0, 0, 0, 0 } };
...
static void ExtractSplitMap (WFile_t *f, // main file
	...
	if (!f2->prealloc_done && f->prealloc_map.used)
	{
		...
		for (ptr = f->prealloc_map.field; ptr < end_field; ptr++)
		{
			...
			if (beg < split_end && end > f2->split_off)
			{
				...
				DASSERT (item);
				snprintf (item->info, sizeof (item->info), "v-off=%llx, %s\n", (u64)beg,
					wd_print_size (0, 0, end - beg, false, WD_SIZE_AUTO));
			}
		}
	}
}
...
enumError XSetupSplitWFile (XPARM WFile_t *f, enumOFT oft, off_t split_size)
{
	...
	{
		if (!f->disable_errors)
			ERROR0 (ERR_WARNING, "Split file support not possible: %s\n", f->fname);
		return ERR_WARNING;
	}
	...
	*list = first = MALLOC (sizeof (WFile_t));
	...
	if (f->is_reading)
	{
		...
		for (idx = 1; idx < MAX_SPLIT_FILES; idx++)
		{
			...
			enumError err
				= XOpenWFileHelper (XCALL fi, have_stream ? IOM_FORCE_STREAM : IOM_NO_STREAM,
			...
			fi->split_off = f->split_f[idx - 1]->split_off + f->split_f[idx - 1]->split_filesize;
			fi->split_filesize = fi->st.st_size;

			PRINT ("#S#%u# Open %s, soff=%llx, ssize=%llx\n", idx, fname, (u64)fi->split_off,
				(u64)fi->split_filesize);
	...
	{
		TRACE ("#S#%u# new ssize: %llx -> %llx\n", f->split_used - 1, (u64)fi->split_filesize,
			(u64)f->split_filesize);
...
enumError XCreateSplitFile (XPARM WFile_t *f, uint split_idx)
{
	...
	while (f->split_used <= split_idx)
	{
		...
		*ptr = f2;
		...
		enumError err = XOpenWFileHelper (
			XCALL f2, prev && prev->fp ? IOM_FORCE_STREAM : IOM_NO_STREAM, flags, flags);
		if (err)
			return err;
		if (prev)
...
void DefineCachedArea (WFile_t *f, off_t off, size_t count)
{
	...
	TRACE ("DefineCachedArea(,%llx,%zx) fd=%d rw=%d,%d is_caching=%d, filesize=%llx\n", (u64)off,
		count, f->fd, f->is_reading, f->is_writing, f->is_caching, (u64)f->st.st_size);
	if (!f->is_caching)
		return;
...
enumError XAnalyzeWH (XPARM WFile_t *f, wdf_header_t *wh0, bool print_err)
{
	...
	if (!f->seek_allowed)
	{
		...
		return print_err
			? PrintError (XERROR0, ERR_WRONG_FILE_TYPE, "Wrong file type: %s\n", f->fname)
			: ERR_WRONG_FILE_TYPE;
	}
	...
	if (!wh.wdf_version || wh.wdf_version > WDF_MAX_VERSION && wh.wdf_compatible > WDF_MAX_VERSION)
	{
		PRINT (" - wrong WDF version: %u,%u\n", wh.wdf_version, wh.wdf_compatible);
		return print_err ? PrintError (XERROR0, ERR_WDF_VERSION,
...
static FileCache_t *XCacheHelper (XPARM WFile_t *f, off_t off, size_t count)
{
	...
	while (cptr)
	{
		TRACE (" - SEARCH=%llx..%llx  CACHE=%llx..%llx\n", (u64)off, (u64)off + count,
			(u64)cptr->off, (u64)cptr->off + cptr->count);
		...
		if (!cptr->data)
		{
			...
			cptr->data = data;
			TRACE (TRACE_RDWR_FORMAT, "#F# FILL CACHE", GetFD (f), GetFP (f), (u64)cptr->off,
	...
	if (cptr)
	{
		...
		TRACE (TRACE_RDWR_FORMAT, "#F# CACHE FOUND", GetFD (f), GetFP (f), (u64)cptr->off,
			(u64)cptr->off + cptr->count, cptr->count, "");
	}
		...
		else
		{
			...
			for (;;)
			{
				...
				noPRINT ("CALL fallocate(%d,0,%9llx,%9llx [%s])\n", f->fd, (u64)found->off,
					(u64)found->size, wd_print_size_1024 (0, 0, found->size, true));
...
enumError XTellF (XPARM WFile_t *f)
{
	...
	f->file_off = f->fp ? ftello (f->fp)
		: f->fd != -1	? lseek (f->fd, (off_t)0, SEEK_CUR)
						: (off_t)-1;
...
enumError XSeekF (XPARM WFile_t *f, off_t off)
{
	...
	if (!f->seek_allowed)
	{
		if (!f->is_reading && off >= f->file_off)
		{
			...
			TRACE (TRACE_RDWR_FORMAT, "#F# SEEK SIMULATION", GetFD (f), GetFP (f), (u64)f->file_off,
			...
			while (skip_size)
			{
				...
				const size_t max_write
					= skip_size < sizeof (zerobuf) ? (size_t)skip_size : sizeof (zerobuf);
		...
		if (!f->is_writing && off >= f->file_off)
		{
			...
			while (skip_size)
			{
				...
				const size_t max_read
					= skip_size < sizeof (iobuf) ? (size_t)skip_size : sizeof (iobuf);
		...
		{
			char buf[100];
			snprintf (buf, sizeof (buf), "ID=%s, OFF=%llx, SIZE=%x", f->id6_src,
				(u64)f->cache_info_off, (u32)f->cache_info_size);
			fprintf (stderr,
				"\n"
				"************************************************************************\n"
	...
	TRACE (TRACE_SEEK_FORMAT, "#F# SeekF()", GetFD (f), GetFP (f), (u64)off,
		off < f->max_off ? " <" : "");
	...
	if (err)
	{
		...
		if (f->max_error < f->last_error)
			f->max_error = f->last_error;
		if (!f->disable_errors)
			PrintError (XERROR1, f->last_error, "Seek failed [%c=%d,%llu]: %s\n", GetFT (f),
				GetFD (f), (u64)off, f->fname);
		f->file_off = (off_t)-1;
	}
	else
	{
...
{
	ASSERT (f);
	TRACE (TRACE_SEEK_FORMAT, "#F# SetSizeF()", GetFD (f), GetFP (f), (u64)size,
		size < f->max_off		? " <"
			: size > f->max_off ? " >"
								: "");
	...
	else if (ftruncate (f->fd, size))
	{
		...
		if (f->max_error < f->last_error)
			f->max_error = f->last_error;
		if (!f->disable_errors)
			PrintError (XERROR1, f->last_error, "Set file size failed [%c=%d,%llu]: %s\n",
...
enumError XPreallocateF (XPARM // debugging and tracing infos
...
enumError XReadF (XPARM WFile_t *f, void *iobuf, size_t count)
{
	...
	{
		TRACE ("#F# --- cache\n");
		TRACE (TRACE_RDWR_FORMAT, "#F# ReadF()+C", GetFD (f), GetFP (f), (u64)f->cur_off,
			(u64)f->cur_off + count, count, f->cur_off < f->max_off ? " <" : "");
		...
		while (count)
		{
			...
			if (count > 0)
			{
				...
				if (size > count)
					size = count;
				TRACE (TRACE_RDWR_FORMAT, "#F# COPY FROM CACHE", GetFD (f), GetFP (f), (u64)my_off,
					(u64)my_off + size, size, "");
	...
	{
		TRACE ("#S# ---\n");
		TRACE (TRACE_RDWR_FORMAT, "#S#   ReadF()", GetFD (f), GetFP (f), (u64)f->cur_off,
			(u64)f->cur_off + count, count, f->cur_off < f->max_off ? " <" : "");
		...
		while (count > 0)
		{
			...
			TRACE ("#S#%zd# off=%llx cur_of=%llx count=%zx fsize=%llx\n", ptr - f->split_f,
				(u64)off, (u64)f->cur_off, count, (u64)cur->split_filesize);
	...
	TRACE (TRACE_RDWR_FORMAT, "#F# ReadF()", GetFD (f), GetFP (f), (u64)f->cur_off,
		(u64)f->cur_off + count, count, f->cur_off < f->max_off ? " <" : "");

	if (f->read_behind_eof && (f->st.st_size > 0 || f->is_writing))
	{
		const off_t max_read = f->st.st_size > f->file_off ? f->st.st_size - f->file_off : 0;

		TRACE ("read_behind_eof=%d, st_size=%llx, count=%zx, max_read=%llx\n", f->read_behind_eof,
			(u64)f->st.st_size, count, (u64)max_read);
			...
			if (f->read_behind_eof == 1)
			{
				...
				if (!f->disable_errors)
					PrintError (XERROR0, ERR_WARNING,
						...
						GetFD (f), (u64)f->file_off, count, f->fname);
			}
	...
	if (err || read_count < count && !f->read_behind_eof)
	{
		if (!f->disable_errors && f->last_error != ERR_READ_FAILED)
			PrintError (XERROR1, ERR_READ_FAILED, "Read failed [%c=%d,%llu+%zu]: %s\n", GetFT (f),
				GetFD (f), (u64)f->file_off, count, f->fname);
...
enumError XWriteF (XPARM WFile_t *f, const void *iobuf, size_t count)
{
	...
	{
		TRACE ("#S# ---\n");
		TRACE (TRACE_RDWR_FORMAT, "#S#   WriteF()", GetFD (f), GetFP (f), (u64)f->cur_off,
			(u64)f->cur_off + count, count, f->cur_off < f->max_off ? " <" : "");
	...
	TRACE (TRACE_RDWR_FORMAT, "#F# WriteF()", GetFD (f), GetFP (f), (u64)f->file_off,
		(u64)f->file_off + count, count, f->file_off < f->max_off ? " <" : "");
	...
	if (err)
	{
		if (!f->disable_errors && f->last_error != ERR_WRITE_FAILED)
			PrintError (XERROR1, ERR_WRITE_FAILED, "Write failed [%c=%d,%llu+%zu%s]: %s\n",
				GetFT (f), GetFD (f), (u64)f->file_off, count,
				f->active_open_flags & O_DSYNC ? ",DSYNC" : "", f->fname);
...
{
	TRACE (TRACE_RDWR_FORMAT, "#F# WriteZeroAtF()", GetFD (f), GetFP (f), (u64)off,
		(u64)off + count, count, off < f->max_off ? " <" : "");
...
{
	ASSERT (f);
	TRACE (TRACE_RDWR_FORMAT, "#F# ZeroAtF()", GetFD (f), GetFP (f), (u64)off, (u64)off + count,
		count, off < f->max_off ? " <" : "");
...
int WrapperReadSector (void *handle, u32 lba, u32 count, void *iobuf)
{
	...
	TRACE ("WrapperReadSector(fd=%d,lba=%x,count=%x) sector-size=%u\n", GetFD (&sf->f), lba, count,
		sf->f.sector_size);

	if (SIGINT_level > 1)
		return ERR_INTERRUPT;

	return ReadAtF (&sf->f, (off_t)lba * sf->f.sector_size, iobuf, count * sf->f.sector_size);
}
...
int WrapperWriteSector (void *handle, u32 lba, u32 count, void *iobuf)
{
	...
	return WriteAtF (&sf->f, (off_t)lba * sf->f.sector_size, iobuf, count * sf->f.sector_size);
}
...
{
	return S_ISREG (mode) ? FM_PLAIN : S_ISBLK (mode) ? FM_BLKDEV : S_ISCHR (mode) ? FM_CHRDEV : 0;
}
...
enumError SaveFileOpt (ccp path1, // NULL or part #1 of path
...
void SetFileID (WFile_t *f, const void *new_id, int id_length)
{
	DASSERT (f);
	DASSERT (id_length >= 0);
...
bool SetPatchFileID (WFile_t *f, const void *new_id, int id_length)
{
	DASSERT (f);
	DASSERT (id_length >= 0);
...
FileAttrib_t *CopyFileAttribDiscInfo (FileAttrib_t *dest, const struct WDiscInfo_t *src)
{
	DASSERT (src);
	DASSERT (dest);
...
static enumError GetFileMapHelper_FIEMAP (FileMap_t *fm, // file map
...
)
{
	...
	while (last_off < file->st.st_size)
	{
		...
		for (; fe < fe_end; fe++)
		{
			AppendFileMap (fm, fe->fe_logical + file->split_off, fe->fe_physical, fe->fe_length);
...
static enumError GetFileSystemMap_FIEMAP (FileMap_t *fm, // file map
	WFile_t *file // file to analyze
)
{
	DASSERT (fm);
	...
	PRINT ("ALLOC FIEMAP: %zu + %zu * %u = %u\n", sizeof (*fmap), sizeof (*fmap->fm_extents),
		MAX_ELEM, ALLOC_SIZE);
...
static enumError GetFileSystemMap_FIBMAP (FileMap_t *fm, // file map
	WFile_t *file // file to analyze
)
{
	DASSERT (fm);
	...
	if (ioctl (file->fd, FIGETBSZ, &blocksize) < 0)
		return ERROR1 (ERR_READ_FAILED, "Can't read block size (FIGETBSZ): %s\n", file->fname);
	...
	for (block = 0; block < n_blocks; block++)
	{
		...
		AppendFileMap (
			fm, block * (u64)blocksize + file->split_off, param * (u64)blocksize, blocksize);
	}
...
enumError GetFileSystemMap (FileMap_t *fm, // file map
...
)
{
	...
	if (S_ISREG (file->st.st_mode))
	{
		...
		{
#if HAVE_FIEMAP && HAVE_FIBMAP
			return ERROR1 (ERR_READ_FAILED, "Can't read file mapping (FIEMAP+FIBMAP failed): %s\n",
				file->fname);
#elif HAVE_FIEMAP
			return ERROR1 (
				ERR_READ_FAILED, "Can't read file mapping (FIEMAP failed): %s\n", file->fname);
#elif HAVE_FIBMAP
			return ERROR1 (
				ERR_READ_FAILED, "Can't read file mapping (FIBMAP failed): %s\n", file->fname);
#endif
		}
	}
	return stat;
	...
	else if (GetWinFileMap (fm, file->fd, file->split_off, file->st.st_size))
		err = ERR_READ_FAILED;

	return err ? ERROR0 (err, "Can't read file mapping: %s\n", file->fname) : ERR_OK;
		...
		if (sf.f.ftype & FT_A_ISO)
		{
			...
			if (disc)
			{
				...
				for (ip = 0; ip < disc->n_part; ip++)
				{
					...
					if (part->is_enabled && !wd_load_part (part, true, false, true) && part->cert)
					{
		...
		else if (sf.f.ftype & (FT_ID_SIG_BIN | FT_ID_CERT_BIN | FT_ID_TIK_BIN | FT_ID_TMD_BIN))
		{
			...
			if (!err)
			{
				...
				else if (sf.f.ftype & FT_ID_TMD_BIN)
				{
					...
					skip = sizeof (wd_tmd_t) + ntohs (tmd->n_content) * sizeof (wd_tmd_content_t);
				}
...
void SortGlobalCert (uint smode)
{
	...
	int (*func) (const void *, const void *) = smode & SORT_REVERSE ? sort_cert_reverse : sort_cert;
