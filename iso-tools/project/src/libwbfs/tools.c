...
enumError wd_print_error (ccp func, // calling function, use macro __FUNCTION__
...
u32 wd_align32 (u32 number, // object of aligning
...
u64 wd_align64 (u64 number, // object of aligning
...
u64 wd_align_part (u64 number, // object of aligning
	u64 align, // NULL or valid align factor
...
char *wd_print_size (char *buf, // result buffer
...
)
{
	...
	switch (mode & WD_SIZE_M_MODE)
	{
		...
		case WD_SIZE_GC:
			snprintf (
				buf, buf_size, aligned ? "%5.2f GC" : "%4.2f GC", (double)size / GC_DISC_SIZE);
			break;
		...
		default:
			buf = force_1000 ? wd_print_size_1000 (buf, buf_size, size, aligned)
...
char *wd_print_size_1000 (char *buf, // result buffer
									 // NULL: use a local circulary static buffer
	size_t buf_size, // size of 'buf', ignored if buf==NULL
	u64 size, // size to print
...
char *wd_print_size_1024 (char *buf, // result buffer
									 // NULL: use a local circulary static buffer
	size_t buf_size, // size of 'buf', ignored if buf==NULL
	u64 size, // size to print
	bool aligned // true: print exact 4+4 chars for num+unit
...
int wd_normalize_indent (int indent // base vlaue to normalize
)
{
...
void wd_print_byte_tab (FILE *f, // valid output file
