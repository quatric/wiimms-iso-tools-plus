...
{
	static const u8 square[16] = "\x00\x01\x04\x05\x10\x11\x14\x15\x40\x41\x44\x45\x50\x51\x54\x55";
...
)
{
	switch (stat)
	{
		case CERT_SIG_OK:
			return "Signature ok";
		case CERT_SIG_FAKE_SIGNED:
			return "Signature ok but fake signed";
		case CERT_HASH_FAILED:
			return "Signature ok but hash failed";

		case CERT_HASH_OK:
			return "Signature wrong but hash is ok";
		case CERT_FAKE_SIGNED:
			return "Signature wrong but fake signed";
		case CERT_SIG_FAILED:
			return "Signature wrong";

		case CERT_ERR_TYPE_MISSMATCH:
			return "Different types of signature and key";
		case CERT_ERR_NOT_SUPPORTED:
			return "This kind of signature is not supported";
		case CERT_ERR_NOT_FOUND:
			return "Certificate not found";
		case CERT_ERR_INVALID_SIG:
...
)
{
	switch (stat)
	{
		...
		case CERT_ERR_TYPE_MISSMATCH:
			return "CERT TYPE MISMATCH";
		case CERT_ERR_NOT_SUPPORTED:
			return "CERT TYPE NOT SUPPORTED";
		case CERT_ERR_NOT_FOUND:
...
int cert_get_data_size (u32 key_type)
{
	...
	{
		data_size
			= ALIGN32 (data_size, WII_CERT_ALIGN) + ALIGN32 (sizeof (cert_data_t), WII_CERT_ALIGN);
	}
	return data_size;
}
...
static BZ2Manager_t cert_mgr[] = { { cert_ca00000001_bz2, sizeof (cert_ca00000001_bz2), 0, 0 },
...
static const cert_item_t *FindCertItemHelper (ccp issuer, // name to search
...
)
{
	...
	if (data && data_size > 0)
	{
		...
		while (ptr < end)
		{
			...
			cert_item_t *item = cert_append_item (cc, data->issuer, data->key_id, uniq);
			DASSERT (item);
			...
			PRINT ("s=%04x k=%04x : d=%04x,04x c=%04x : t=%08x,%08x : %s . %s\n", item->sig_size,
				item->key_size, item->data_size, item->cert_size, ntohl (head->sig_type),
				ntohl (data->key_type), data->issuer, data->key_id);
		}
	}
...
cert_stat_t cert_check (const cert_chain_t *cc, // NULL or pointer to cert chain
...
)
{
	...
	PRINT ("FOUND: %s . %s [%s]\n", item->data->issuer, item->data->key_id,
		item->head ? cert_get_signature_name (ntohl (item->head->sig_type), "?") : "?");
	if (cert_found)
		*cert_found = item;
	...
	if (key_type == 2)
	{
		...
		static u8 ec_n[30] = "\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
							 "\x13\xe9\x74\xe7\x2f\x8a\x69\x22\x03\x1d\x26\x03\xcf\xe0\xd7";

		// base point
		static u8 ec_g[60] = "\x00\xfa\xc9\xdf\xcb\xac\x83\x13\xbb\x21\x39\xf1\xbb\x75\x5f"
...
#define ESTIMATE_EC_TIME 10
	...
	else
	{
		...
		static const u8 ber[16] = { 0x00, 0x30, 0x21, 0x30, 0x09, 0x06, 0x05, 0x2b, 0x0e, 0x03,
			0x02, 0x1a, 0x05, 0x00, 0x04, 0x14 };
		const int ber_index = sig_len - 36;
		res_is_sig = buf[0] == 0x00 && buf[1] == 0x01 && buf[2] == 0xff
			&& !memcmp (buf + 2, buf + 3, sig_len - ber_index - 3)
			&& !memcmp (buf + ber_index, ber, sizeof (ber));
	}

	if (res_is_sig)
		return sig_ok ? CERT_SIG_OK : fake_ok ? CERT_SIG_FAKE_SIGNED : CERT_SIG_FAILED;

	return sig_ok ? CERT_HASH_OK : fake_ok ? CERT_FAKE_SIGNED : CERT_HASH_FAILED;
...
)
{
	...
	const int tmd_size = sizeof (wd_tmd_t) + ntohs (tmd->n_content) * sizeof (wd_tmd_content_t);
