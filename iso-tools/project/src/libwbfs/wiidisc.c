...
const u8 *wd_get_common_key (wd_ckey_index_t ckey_index // index of common key
)
{
	wd_setup_common_key ();
...
const u8 *wd_set_common_key (wd_ckey_index_t ckey_index, // index of common key
...
void wd_decrypt_title_key (const wd_ticket_t *tik, // pointer to ticket
...
)
{
	...
	const wd_ckey_index_t ckey_index
		= tik->common_key_index < WD_CKEY__N ? tik->common_key_index : 0;
	aes_key_t akey;
	wd_aes_set_key (&akey, common_key[ckey_index]);
	wd_aes_decrypt (&akey, iv, &tik->title_key, title_key, WII_KEY_SIZE);

	TRACE ("| IV:  ");
	TRACE_HEXDUMP (0, 0, 1, WII_KEY_SIZE, iv, WII_KEY_SIZE);
	TRACE ("| IN:  ");
	TRACE_HEXDUMP (0, 0, 1, WII_KEY_SIZE, &tik->title_key, WII_KEY_SIZE);
	TRACE ("| OUT: ");
	TRACE_HEXDUMP (0, 0, 1, WII_KEY_SIZE, title_key, WII_KEY_SIZE);
}

///////////////////////////////////////////////////////////////////////////////

void wd_encrypt_title_key (wd_ticket_t *tik, // pointer to ticket (modified)
...
void wd_decrypt_sectors (wd_part_t *part, // NULL or pointer to partition
...
)
{
	...
	if (hash_dest)
	{
		...
		while (n_sectors-- > 0)
		{
			...
			wd_aes_decrypt (akey, iv0, src1, hdest, WII_SECTOR_HASH_SIZE);

			// decrypt data
			wd_aes_decrypt (akey, src1 + WII_SECTOR_IV_OFF, src1 + WII_SECTOR_HASH_SIZE, dest,
	...
	else
	{
		while (n_sectors-- > 0)
		{
			...
			// decrypt data
			wd_aes_decrypt (akey, src1 + WII_SECTOR_IV_OFF, src1 + WII_SECTOR_HASH_SIZE,
				dest + WII_SECTOR_HASH_SIZE, WII_SECTOR_DATA_SIZE);

			// decrypt header
			wd_aes_decrypt (akey, iv0, src1, dest, WII_SECTOR_HASH_SIZE);
...
void wd_encrypt_sectors (wd_part_t *part, // NULL or pointer to partition
	const aes_key_t *akey, // aes key, if NULL use 'part'
	const void *sect_src, // pointer to first source sector
...
)
{
	...
	if (hash_src)
	{
		...
		while (n_sectors-- > 0)
		{
			...
			// encrypt header
			wd_aes_encrypt (akey, iv0, hsrc, tempbuf, WII_SECTOR_HASH_SIZE);

			// encrypt data
			wd_aes_encrypt (akey, tempbuf + WII_SECTOR_IV_OFF, src, tempbuf + WII_SECTOR_HASH_SIZE,
				WII_SECTOR_DATA_SIZE);
	...
	else
	{
		...
		{
			// encrypt header
			wd_aes_encrypt (akey, iv0, src, dest, WII_SECTOR_HASH_SIZE);

			// encrypt data
			wd_aes_encrypt (akey, dest + WII_SECTOR_IV_OFF, src + WII_SECTOR_HASH_SIZE,
				dest + WII_SECTOR_HASH_SIZE, WII_SECTOR_DATA_SIZE);
...
void wd_split_sectors (const void *sect_src, // pointer to first source sector
...
void wd_join_sectors (const void *data_src, // pointer to data source
...
const char *wd_disc_type_name[] = { "unknown", "GameCube", "Wii", 0 };

//-----------------------------------------------------------------------------

const char *wd_get_disc_type_name (wd_disc_type_t disc_type, // disc type
	ccp not_found // result if no name found
)
{
	return (unsigned)disc_type < WD_DT__N ? wd_disc_type_name[disc_type] : not_found;
}

///////////////////////////////////////////////////////////////////////////////

const char *wd_part_name[] = { "DATA", "UPDATE", "CHANNEL", 0 };

//-----------------------------------------------------------------------------

const char *wd_get_part_name (u32 ptype, // partition type
	ccp not_found // result if no name found
)
{
	return ptype < sizeof (wd_part_name) / sizeof (*wd_part_name) - 1 ? wd_part_name[ptype]
																	  : not_found;
}

///////////////////////////////////////////////////////////////////////////////

char *wd_print_part_name (char *buf, // result buffer
...
)
{
	...
	ccp id4 = (ccp)&ptype;
	const bool is_id = ISALNUM (id4[0]) && ISALNUM (id4[1]) && ISALNUM (id4[2]) && ISALNUM (id4[3]);
	const ccp name
		= ptype < sizeof (wd_part_name) / sizeof (*wd_part_name) - 1 ? wd_part_name[ptype] : 0;
...
char *wd_print_id (const void *id, // ID to convert in printable format
...
char *wd_print_id_col (const void *id, // NULL | EMPTY | ID to convert
	size_t id_len, // len of 'id'
...
int wd_rename (void *p_data, // pointer to ISO data
...
enumError wd_read_raw (wd_disc_t *disc, // valid disc pointer
...
)
{
	...
	if (dest_buf)
	{
		...
		if (skip4)
		{
			...
			noTRACE ("WD-READ/PRE:  %9llx %5x -> %5zx %5x, skip=%x,%x\n", (u64)off1 << 2,
				HD_SECTOR_SIZE, dest - (u8 *)dest_buf, copy_len, skip, skip4);
		...
		if (copy_len > 0)
		{
			noTRACE ("WD-READ/MAIN: %9llx %5x -> %5zx %5x\n", (u64)offset4 << 2, copy_len,
				dest - (u8 *)dest_buf, copy_len);
		...
		if (len > 0)
		{
			...
			noTRACE ("WD-READ/POST: %9llx %5x -> %5zx %5x\n", (u64)offset4 << 2, HD_SECTOR_SIZE,
				dest - (u8 *)dest_buf, len);
	...
	if (usage_id)
	{
		...
		u32 end_block
			= disc_offset4 + (read_size + WII_SECTOR_SIZE - 1 >> 2) >> WII_SECTOR_SIZE_SHIFT - 2;
		if (end_block > WII_MAX_SECTORS)
			end_block = WII_MAX_SECTORS;
		noTRACE ("mark %x+%x => %x..%x [%x]\n", disc_offset4, read_size, first_block, end_block,
			end_block - first_block);

		if (first_block < end_block)
		{
			memset (disc->usage_table + first_block, usage_id, end_block - first_block);
...
enumError wd_read_part_raw (wd_part_t *part, // valid pointer to a disc partition
...
{
	DASSERT (part);
	return wd_read_raw (part->disc, part->part_off4 + offset4, dest_buf, read_size,
		mark_block ? part->usage_id : 0);
}

///////////////////////////////////////////////////////////////////////////////

enumError wd_read_part_block (wd_part_t *part, // valid pointer to a disc partition
...
)
{
	...
	if (disc->block_part != part || disc->block_num != block_num)
	{
		...
		err = wd_read_raw (part->disc, part->data_off4 + block_num * WII_SECTOR_SIZE4,
			disc->temp_buf, WII_SECTOR_SIZE, mark_block ? part->usage_id | WD_USAGE_F_CRYPT : 0);
		...
		else
		{
			...
			if (part->is_encrypted)
			{
				...
				// decrypt data
				wd_aes_decrypt (&disc->akey, disc->temp_buf + WII_SECTOR_IV_OFF,
					disc->temp_buf + WII_SECTOR_HASH_SIZE, disc->block_buf, WII_SECTOR_DATA_SIZE);
			}
			else
				memcpy (
					disc->block_buf, disc->temp_buf + WII_SECTOR_HASH_SIZE, WII_SECTOR_DATA_SIZE);
		}
	}
...
enumError wd_read_part (wd_part_t *part, // valid pointer to a disc partition
...
)
{
	...
	if (part->is_gc)
	{
		...
		return wd_read_raw (disc, off, dest_buf, read_size, mark_block ? part->usage_id : 0);
	}
	...
	while (read_size && err == ERR_OK)
	{
		...
		err = wd_read_part_block (part, data_offset4 / WII_SECTOR_DATA_SIZE4, temp, mark_block);
...
void wd_mark_part (wd_part_t *part, // valid pointer to a disc partition
	u32 data_offset4, // partition data offset/4
...
)
{
	...
	if (part->is_gc)
	{
		...
		end_block
			= (data_offset4 + data_size + WII_SECTOR_SIZE - 1) / WII_SECTOR_SIZE + block_delta;
	...
	else
	{
		...
		end_block
			= ((data_size + WII_SECTOR_DATA_SIZE - 1 >> 2) + data_offset4) / WII_SECTOR_DATA_SIZE4
			+ block_delta;
...
u64 wd_calc_disc_offset (wd_part_t *part, // valid pointer to a disc partition
...
)
{
	...
	return (u64)(part->data_sector + block) * WII_SECTOR_SIZE + (offset_in_block << 2)
...
wd_sector_status_t wd_get_part_sector_status (wd_part_t *part, // valid pointer to a disc partition
	u32 block_num, // block number of partition
...
)
{
	...
	enumError err = wd_read_raw (disc, abs_block_num * WII_SECTOR_SIZE4, rdbuf, WII_SECTOR_SIZE, 0);
	if (err)
	...
	if (!(stat & (WD_SS_HASH_CLEARED | WD_SS_F_SKELETON)))
	{
		...
		for (h0idx = 0; h0idx < WII_N_ELEMENTS_H0; h0idx++)
		{
			...
			SHA1 (dcbuf + WII_SECTOR_HASH_SIZE + h0idx * WII_H0_DATA_SIZE, WII_H0_DATA_SIZE, hash);
...
wd_sector_status_t wd_get_disc_sector_status (wd_disc_t *disc, // valid disc pointer
...
)
{
	...
	if (!usage)
	{
		...
		for (pi = disc->n_part - 1; pi >= 0; pi--)
		{
			...
			if (block_off >= off && wd_load_part (test_part, false, false, silent)
	...
	DASSERT (rdbuf);
	enumError err = wd_read_raw (disc, block_num * WII_SECTOR_SIZE4, rdbuf, WII_SECTOR_SIZE, 0);
	if (err)
		return WD_SS_READ_ERROR | WD_SS_F_PART_DATA;
...
static char *proceed_info_tab (char *buf, // destination buffer
...
ccp wd_log_sector_status (char *buf, // result buffer
									 // If NULL, a local circulary static buffer is used
	size_t buf_size, // size of 'buf', ignored if buf==NULL
	wd_sector_status_t sect_stat // sector status
)
{
	static info_t info_tab[] = { { "err:sect", WD_SS_INVALID_SECTOR, WD_SS_INVALID_SECTOR },
		...
		{ "Dz", WD_SS_DATA_ZEROED, WD_SS_DATA_CLEARED }, { "Hc", WD_SS_HASH_CLEARED, 0 },
		{ "Dc", WD_SS_DATA_CLEARED, 0 },

		{ "enc", WD_SS_ENCRYPTED, 0 }, { "dec", WD_SS_DECRYPTED, 0 },

		{ "pdata", WD_SS_F_PART_DATA, 0 }, { "scrub", WD_SS_F_SCRUBBED, 0 },
...
ccp wd_print_sector_status (char *buf, // result buffer
									   // If NULL, a local circulary static buffer is used
	size_t buf_size, // size of 'buf', ignored if buf==NULL
	wd_sector_status_t sect_stat, // sector status
...
{
	static info_t info_tab_sect[] = { { "encrypted", WD_SS_ENCRYPTED, 0 },
		{ "decrypted", WD_SS_DECRYPTED, 0 }, { "skeleton", WD_SS_F_SKELETON, WD_SS_F_SCRUBBED },
		{ "scrubbed", WD_SS_F_SCRUBBED, 0 },

		{ 0, 0, 0 } };

	static info_t info_tab_sig[] = { { "not signed", CERT_F_HASH_FAILED, CERT_F_HASH_OK },
		{ "fake signed", CERT_F_HASH_FAKED, CERT_F_HASH_OK }, { "well signed", CERT_F_HASH_OK, 0 },
...
)
{
	...
	noPRINT (
		"wd_is_block_encrypted(%d) -> %04x '%s'\n", block_num, ss, wd_log_sector_status (0, 0, ss));
	return ss & WD_SS_ENCRYPTED						? 1
		: ss & (WD_SS_DECRYPTED | WD_SS_F_SKELETON) ? 0
													: unknown_result;
}

///////////////////////////////////////////////////////////////////////////////

bool wd_is_part_scrubbed (wd_part_t *part, // valid pointer to a disc partition
	bool silent // true: don't print error messages
)
{
	DASSERT (part);
	...
	if (!part->scrub_test_done)
	{
		...
		for (sector = part->data_sector;
			sector < part->end_sector && try_count && !(part->sector_stat & WD_SS_F_SCRUBBED);
			sector++)
		{
...
bool wd_is_disc_scrubbed (wd_disc_t *disc, // valid disc pointer
	bool silent // true: don't print error messages
)
{
	DASSERT (disc);
...
static wd_part_t *wd_initialize_part (wd_disc_t *disc, // a valid disc pointer
...
static wd_disc_t *wd_open_gc_disc (wd_disc_t *disc, // a valid disc pointer
...
wd_disc_t *wd_open_disc (wd_read_func_t read_func, // read function, always valid
...
)
{
	...
	enumError err
		= wd_read_raw (disc, 0, disc->temp_buf, WII_BOOT_SIZE + WII_BI2_SIZE, WD_USAGE_DISC);
	...
	if (!err)
		err = wd_read_raw (
			disc, WII_PTAB_REF_OFF >> 2, &disc->ptab_info, sizeof (disc->ptab_info), WD_USAGE_DISC);
	...
	if (!err)
	{
		...
		for (n_part = ipt = 0; ipt < WII_MAX_PTAB && !err; ipt++)
		{
			...
			if (n)
			{
				...
				err = wd_read_raw (disc, ntohl (pt->off4), pe, n * sizeof (*pe), WD_USAGE_DISC);
				if (!err)
				{
					int pi;
					for (pi = 0; pi < n; pi++, pe++)
	...
	if (!err)
		err = wd_read_raw (
			disc, WII_REGION_OFF >> 2, &disc->region, sizeof (disc->region), WD_USAGE_DISC);

	if (!err)
		err = wd_read_raw (
			disc, WII_MAGIC2_OFF >> 2, &disc->magic2, sizeof (disc->magic2), WD_USAGE_DISC);
	...
	if (!err)
	{
		...
		for (sector = 0; sector < WII_PART_OFF / WII_SECTOR_SIZE; sector++)
			...
			{
				err = wd_read_raw (disc, sector * WII_SECTOR_SIZE4, disc->temp_buf, WII_SECTOR_SIZE,
					WD_USAGE_UNUSED);
				if (err)
					break;
...
wd_disc_t *wd_dup_disc (wd_disc_t *disc // NULL or a valid disc pointer
)
{
...
void wd_close_disc (wd_disc_t *disc // NULL or valid disc pointer
)
{
...
bool wd_disc_has_ptab (wd_disc_t *disc // valid disc pointer
)
{
	DASSERT (disc);
...
bool wd_disc_has_region_setting (wd_disc_t *disc // valid disc pointer
)
{
	DASSERT (disc);
	return disc->disc_type != WD_DT_GAMECUBE;
...
bool wd_part_has_ticket (wd_part_t *part // valid disc partition pointer
)
{
	DASSERT (part);
	return !part->is_gc;
...
bool wd_part_has_tmd (wd_part_t *part // valid disc partition pointer
)
{
	DASSERT (part);
...
bool wd_part_has_cert (wd_part_t *part // valid disc partition pointer
)
{
	DASSERT (part);
...
bool wd_part_has_h3 (wd_part_t *part // valid disc partition pointer
)
{
	DASSERT (part);
...
wd_part_t *wd_get_part_by_index (wd_disc_t *disc, // valid disc pointer
...
wd_part_t *wd_get_part_by_usage (wd_disc_t *disc, // valid disc pointer
...
wd_part_t *wd_get_part_by_ptab (wd_disc_t *disc, // valid disc pointer
...
)
{
	...
	for (i = 0; i < disc->n_part; i++, part++)
		if (part->ptab_index == ptab_index && part->ptab_part_index == ptab_part_index)
		{
...
wd_part_t *wd_get_part_by_type (wd_disc_t *disc, // valid disc pointer
...
static enumError wd_check_part_offset (wd_part_t *part, // valid disc partition pointer
		...
		if (off > iso_size)
		{
			if (!silent)
				WD_ERROR (ERR_WPART_INVALID,
					...
					wd_print_part_name (0, 0, part->part_type, WD_PNAME_NUM_INFO), name, off,
					iso_size, part->disc->error_term);
...
enumError wd_load_part (wd_part_t *part, // valid disc partition pointer
	...
	if (load_now)
	{
		...
		else
		{
			...
			TRACE ("part=%llx,%llx, tmd=%llx,%x, cert=%llx,%x, h3=%llx, data=%llx,%llx\n",
				(u64)part->part_off4 << 2, part->part_size, (u64)ph->tmd_off4 << 2, ph->tmd_size,
				(u64)ph->cert_off4 << 2, ph->cert_size, (u64)ph->h3_off4 << 2,
				...
				|| ((u64)ph->tmd_off4 << 2) + ph->tmd_size > ((u64)ph->data_off4 << 2))
			{
				if (!silent)
					WD_ERROR (ERR_WPART_INVALID, "Invalid TMD size (0x%x) in partition '%s'%s",
						ph->tmd_size, wd_print_part_name (0, 0, part->part_type, WD_PNAME_NUM_INFO),
						disc->error_term);
				return ERR_WPART_INVALID;
			}
			...
			part->is_encrypted
				= wd_is_block_encrypted (part, 0, 1, false) && !tmd_is_marked_not_encrypted (tmd);
		...
		enumError err = wd_read_part (
			part, WII_BOOT_OFF >> 2, disc->temp_buf, WII_BOOT_SIZE + WII_BI2_SIZE, true);
		if (err)
		{
			if (!silent)
				WD_ERROR (ERR_WPART_INVALID, "Can't read BOOT.BIN of partition '%s'%s",
					wd_print_part_name (0, 0, part->part_type, WD_PNAME_NUM_INFO),
					disc->error_term);
			return ERR_WPART_INVALID;
		}
		...
		// "setup.txt"
		part->setup_txt_len = snprintf ((char *)disc->temp_buf, sizeof (disc->temp_buf),
			...
			"!part-offset = 0x%llx\n"
			"\n",
			disc_type, image_type, part_id, part_title, (u64)part->part_off4 << 2);
		part->setup_txt = MEMDUP (disc->temp_buf, part->setup_txt_len);

		// "setup.sh"
		part->setup_sh_len = snprintf ((char *)disc->temp_buf, sizeof (disc->temp_buf),
			"DISC_TYPE=\"%s\"\n"
			"IMAGE_TYPE=\"%s\"\n"
			"IMAGE_EXT=\"%s\"\n"
			"PART_ID=\"%.6s\"\n"
			"PART_NAME=\"%s\"\n",
			disc_type, image_type, image_ext, part_id, norm_title);
		part->setup_sh = MEMDUP (disc->temp_buf, part->setup_sh_len);

		// "setup.bat"
		part->setup_bat_len = snprintf ((char *)disc->temp_buf, sizeof (disc->temp_buf),
			"set DISC_TYPE=%s\r\n"
			"set IMAGE_TYPE=%s\r\n"
			"set IMAGE_EXT=%s\r\n"
			"set PART_ID=%.6s\r\n"
		...
		else
		{
			...
			if (dol_size > boot->fst_off4 - boot->dol_off4 << 2)
			{
				WDPRINT ("!!! DOL-SIZE: %x <= %x & off=%x\n", dol_size,
					boot->fst_off4 - boot->dol_off4 << 2, boot->dol_off4);
				if (!silent)
			...
			for (fst++; fst < fst_end; fst++)
				if (fst->is_dir)
				{
					...
					if (part->part_type == WD_PART_UPDATE)
					{
						...
						in_wad_dir
							= fname < (ccp)part->fst + fst_size && !strcasecmp (fname, "_sys");
					}
				}
				else
				{
		...
		if (part->is_gc)
		{
			part->data_sector = part->data_off4 / WII_SECTOR_SIZE4;
			part->end_sector
				= part->data_sector + (part->max_marked + WII_SECTOR_SIZE - 1) / WII_SECTOR_SIZE;
			...
			noPRINT (">> S=%x .. %x / %llx .. %llx / %llx / %llx\n", part->data_sector,
				part->end_sector, (u64)part->data_off4 << 2, part->part_size, part->max_marked,
				disc->iso_size);

			if (wd_check_part_offset (
					part, (u64)part->data_off4 << 2, part->part_size, "PART", silent)
				> ERR_WARNING)
				return ERR_WPART_INVALID;
		}
		else
		...
		mgr_sect = (mgr_sect - part->data_sector + WII_GROUP_SECTORS - 1) / WII_GROUP_SECTORS
				* WII_GROUP_SECTORS
			+ part->data_sector;
		...
		WDPRINT ("PART #%u, sectors: %llx .. %llx .. %llx .. %llx\n", part->index,
			(u64)WII_SECTOR_SIZE * part->data_sector, (u64)WII_SECTOR_SIZE * part->end_mgr_sector,
			(u64)WII_SECTOR_SIZE * part->end_sector, (u64)WII_SECTOR_SIZE * WII_MAX_SECTORS);
		...
		for (p2 = 0; p2 < disc->n_part; p2++)
		{
			...
			if (part2->part_off4 >= part->part_off4 && part2->part_off4 < end4a)
			{
			...
			if (part2->is_loaded && part2->is_valid)
			{
				...
				if (part->part_off4 >= part2->part_off4 && part->part_off4 < end4b)
				{
					part->is_overlay = true;
					part2->is_overlay = true;
					continue;
	...
	if (part->is_valid)
	{
		if (ph->cert_off4)
		{
			if (!part->cert && load_cert)
			{
				...
				wd_read_part_raw (part, ph->cert_off4, part->cert, ph->cert_size, true);
			}
			else if (load_now)
...
enumError wd_load_all_part (wd_disc_t *disc, // valid disc pointer
	bool load_cert, // true: load cert data too
	bool load_h3, // true: load h3 data too
	bool silent // true: don't print error messages
)
...
enumError wd_calc_fst_statistics (wd_disc_t *disc, // valid disc pointer
...
const cert_chain_t *wd_load_cert_chain (wd_part_t *part, // valid disc partition pointer
...
cert_stat_t wd_get_cert_ticket_stat (wd_part_t *part, // valid disc partition pointer
	bool silent // true: don't print errors while loading cert
)
{
	DASSERT (part);
...
cert_stat_t wd_get_cert_tmd_stat (wd_part_t *part, // valid disc partition pointer
	bool silent // true: don't print errors while loading cert
)
{
	DASSERT (part);
...
ccp wd_get_sig_status_short_text (cert_stat_t sig_stat)
{
	return sig_stat & CERT_F_HASH_FAILED ? "no-sig"
		: sig_stat & CERT_F_HASH_FAKED	 ? "faked"
		: sig_stat & CERT_F_HASH_OK		 ? "signed"
										 : 0;
}

///////////////////////////////////////////////////////////////////////////////

ccp wd_get_sig_status_text (cert_stat_t sig_stat)
{
	return sig_stat & CERT_F_HASH_FAILED ? "not signed"
		: sig_stat & CERT_F_HASH_FAKED	 ? "fake signed"
		: sig_stat & CERT_F_HASH_OK		 ? "well signed"
										 : 0;
}

///////////////////////////////////////////////////////////////////////////////

char *wd_print_sig_status (char *buf, // result buffer
									  // If NULL, a local circulary static buffer is used
	size_t buf_size, // size of 'buf', ignored if buf==NULL
...
)
{
	...
	ccp enc_info = !add_enc_info ? ""
		: part->is_encrypted	 ? " Partition is encrypted"
								 : " Partition is decrypted";
	ccp scrub_info = !add_enc_info			 ? ""
		: wd_is_part_scrubbed (part, silent) ? " and scrubbed."
											 : ".";
	...
	if (tik_text == tmd_text)
	{
		if (!tik_text)
			snprintf (
				buf, buf_size, "Signing of TICKET & TMD is unknown.%s%s", enc_info, scrub_info);
		else
			snprintf (buf, buf_size, "TICKET & TMD are %s.%s%s", tik_text, enc_info, scrub_info);
	}
	else
	{
		if (!tik_text)
			snprintf (buf, buf_size, "Signing of TICKET is unknown. TMD is %s.%s%s", tmd_text,
				enc_info, scrub_info);
		else if (!tmd_text)
			snprintf (buf, buf_size, "TICKET is %s. Signing of TMD is unknown.%s%s", tik_text,
				enc_info, scrub_info);
		else
		{
			DASSERT (tik_text && tmd_text);
			snprintf (buf, buf_size, "TICKET is %s. TMD is %s.%s%s", tik_text, tmd_text, enc_info,
				scrub_info);
		}
	}
...
char *wd_print_part_status (char *buf, // result buffer
									   // If NULL, a local circulary static buffer is used
	size_t buf_size, // size of 'buf', ignored if buf==NULL
	wd_part_t *part, // valid disc partition pointer
	bool silent // true: don't print errors while loading cert
)
{
	...
	if (part->is_enabled)
	{
		...
		ccp sign_info = wd_get_sig_status_short_text (
			wd_get_cert_ticket_stat (part, silent) | wd_get_cert_tmd_stat (part, silent));
...
enumError wd_calc_part_status (wd_part_t *part, // valid pointer to a disc partition
	bool silent // true: don't print error messages
)
{
	DASSERT (part);
...
enumError wd_calc_disc_status (wd_disc_t *disc, // valid disc pointer
	bool silent // true: don't print error messages
)
{
	DASSERT (disc);
...
void wd_initialize_select (wd_select_t *select // valid pointer to a partition selector
)
{
	DASSERT (select);
...
void wd_reset_select (wd_select_t *select // valid pointer to a partition selector
)
{
	DASSERT (select);
	FREE (select->list);
...
void wd_copy_select (wd_select_t *dest, // valid pointer to a partition selector
...
void wd_move_select (wd_select_t *dest, // valid pointer to a partition selector
...
void wd_print_select (FILE *f, // valid output file
...
)
{
	...
	fprintf (
		f, "%*s%s all partitions (default rule)\n", indent, "", default_deny ? "DENY " : "ALLOW");
}

///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////

bool wd_is_part_selected (wd_part_t *part, // valid pointer to a disc partition
	...
	for (; item < end; item++)
	{
		...
		switch (item->mode & WD_SM_M_MODE)
		{
			...
			case WD_SM_ALLOW_PTAB_INDEX:
				match = part->is_gc
					? -1
					: item->table == part->ptab_index && item->part == part->ptab_part_index;
				break;
	...
	)
{
	...
	noPRINT ("wd_select(%p,%p) recom=%d chg=%d\n", disc, select, disc->patch_ptab_recommended,
		selection_changed);
...
const char wd_usage_name_tab[256] = { ".*ABCDEFGHIJKLMNOPQRSTUVWXYZABCD"
...
const u8 wd_usage_class_tab[256] = { 0, 1, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2,
...
u8 *wd_calc_usage_table (wd_disc_t *disc // valid disc partition pointer
)
{
	DASSERT (disc);
	wd_load_all_part (disc, false, false, false);
...
u8 *wd_filter_usage_table (wd_disc_t *disc, // valid disc pointer
...
)
{
	...
	for (ip = 0; ip < disc->n_part; ip++)
	{
		...
		if (part->is_valid && part->is_enabled)
		{
			...
			if (disc->whole_part && !part->is_overlay)
			{
				...
				u32 end_block
					= (part->ph.data_size4 + WII_SECTOR_SIZE4 - 1) / WII_SECTOR_SIZE4 + first_block;
				if (end_block > WII_MAX_SECTORS)
					end_block = WII_MAX_SECTORS;

				noTRACE ("mark %llx+%llx => %x..%x [%x]\n", (u64)part->data_off4 << 2,
					(u64)part->ph.data_size4, first_block, end_block, end_block - first_block);

				if (first_block < end_block)
					memset (usage_table + first_block, part->usage_id | WD_USAGE_F_CRYPT,
						end_block - first_block);
			}
		}
		else
	...
	u32 n_sect = disc->iso_size ? (disc->iso_size + WII_SECTOR_SIZE - 1) / WII_SECTOR_SIZE
								: WII_MAX_SECTORS;
...
u64 wd_count_used_disc_size (wd_disc_t *disc, // valid pointer to a disc
...
{
	return wd_count_used_disc_blocks (disc, block_size, select) * (u64)block_size
		* (u64)WII_SECTOR_SIZE;
}

///////////////////////////////////////////////////////////////////////////////

u32 wd_count_used_disc_blocks (wd_disc_t *disc, // valid pointer to a disc
	int block_size, // if >1: count every 'block_size'
					//        continuous sectors as one block
					//        and return the block count
					// if <0: like >1, but give the result as multiple
...
u32 wd_count_used_blocks (const u8 *usage_table, // valid pointer to usage table
	int block_size // if >1: count every 'block_size'
				   //        continuous sectors as one block
				   //        and return the block count
				   // if <0: like >1, but give the result as multiple
...
bool wd_is_block_used (const u8 *usage_table, // valid pointer to usage table
...
static int wd_iterate_fst_helper (wd_iterator_t *it, // valid pointer to iterator data
...
int wd_iterate_files (wd_disc_t *disc, // valid pointer to a disc
	wd_file_func_t func, // call back function
		...
		for (part = disc->part; part < part_end; part++)
			if (part->is_valid && part->is_enabled)
			{
				...
				for (part2 = disc->part; part2 < part; part2++)
					if (part2->is_valid && part2->is_enabled && part2->part_type == part->part_type)
					{
	...
	for (part = disc->part; part < part_end && !stat; part++)
	{
		...
		switch (prefix_mode)
		{
			...
			case WD_IPM_PART_INDEX:
				snprintf (it.prefix, sizeof (it.prefix), "P%u.%u/", part->ptab_index,
					part->ptab_part_index);
				break;

			case WD_IPM_COMBI:
				snprintf (it.prefix, sizeof (it.prefix), "P%u.%u-%s/", part->ptab_index,
					part->ptab_part_index,
			...
			default:
				wd_print_part_name (
					it.prefix, sizeof (it.prefix) - 1, part->part_type, WD_PNAME_P_NAME);
				strcat (it.prefix, "/");
				break;
		}
		...
		it.icm = WD_ICM_DIRECTORY;
		it.off4 = 0;
		it.size = part->is_gc ? SYS_DIR_COUNT_GC + SYS_FILE_COUNT_GC + part->fst_n - 1
		...
		stat = wd_iterate_fst_helper (&it, part->fst, func, part, ignore_files, 0, part->is_gc);
		if (stat)
			break;
...
int wd_iterate_fst_files (const wd_fst_item_t *fst_base, // valid pointer to FST data
	wd_file_func_t func, // call back function
	void *param // user defined parameter
)
{
...
)
{
	...
	if (stat == 1)
	{
		...
		WDPRINT ("fst_end=%p new_end=%p, n=%u->%u\n", fst_end, part->fst + part->fst_n, n_fst,
			part->fst_n);
		HEXDUMP16 (0, 0, fst_end, 16);
...
void wd_initialize_print_fst (wd_print_fst_t *pf, // valid pointer
...
)
{
	...
	char buf[50];
	const int fw_offset
		= max_off4 > 0 ? snprintf (buf, sizeof (buf), "%llx", (u64)max_off4 << 2) : 9;
	...
	if (mode & WD_PFST_SIZE_HEX)
		pf->fw_size_hex = max_size > 0 ? snprintf (buf, sizeof (buf), "%x", max_size) : 7;

	if (mode & WD_PFST_SIZE_DEC)
		pf->fw_size_dec = max_size > 0 ? snprintf (buf, sizeof (buf), "%u", max_size) : 8;
}

///////////////////////////////////////////////////////////////////////////////

void wd_print_fst_header (wd_print_fst_t *pf, // valid pointer
...
void wd_print_fst_item (wd_print_fst_t *pf, // valid pointer
	wd_part_t *part, // valid pointer to a disc partition
...
)
{
	...
	switch (icm)
	{
		...
		case WD_ICM_DATA:
			...
			if (pf->fw_offset)
				fprintf (pf->f, "%*llx%c ", pf->fw_offset, offset, icm == WD_ICM_FILE ? '+' : ' ');
			if (pf->fw_size_hex)
...
static int wd_print_fst_item_wrapper (struct wd_iterator_t *it // iterator struct with all infos
)
{
	DASSERT (it);
...
void wd_print_fst (FILE *f, // valid output file
	int indent, // indention of the output
	wd_disc_t *disc, // valid pointer to a disc
	wd_ipm_t prefix_mode, // prefix mode
...
void wd_reset_memmap (wd_memmap_t *mm // NULL or patching data
)
{
	if (mm)
	{
...
static u32 wd_insert_patch_helper (wd_memmap_t *mm, // NULL or patching data
...
wd_memmap_item_t *wd_find_memmap (wd_memmap_t *mm, // patching data
...
wd_memmap_item_t *wd_insert_memmap (wd_memmap_t *mm, // patching data
	u32 mode, // memmap mode (e.g. wd_patch_mode_t)
	u64 offset, // offset of object
	u64 size // size of object
)
...
wd_memmap_item_t *wd_insert_memmap_alloc (wd_memmap_t *mm, // patching data
	u32 mode, // memmap mode (e.g. wd_patch_mode_t)
	u64 offset, // offset of object
	u64 size // size of object
)
...
int wd_insert_memmap_disc_part (wd_memmap_t *mm, // patching data
	wd_disc_t *disc, // valid disc pointer
...
)
{
	...
	for (ip = 0; ip < disc->n_part; ip++)
	{
		wd_part_t *part = disc->part + ip;
		if (part->is_valid && part->is_enabled)
			count += wd_insert_memmap_part (mm, part, func, param, wii_head_mode, wii_mgr_mode,
				wii_data_mode, gc_mgr_mode, gc_data_mode);
	}
...
int wd_insert_memmap_part (wd_memmap_t *mm, // patching data
	wd_part_t *part, // valid pointer to a disc partition
...
)
{
	...
	{
		snprintf (
			intro, sizeof (intro), "GC P.%u, %s", part->index, wd_print_id (&part->boot, 6, 0));
	...
	{
		snprintf (intro, sizeof (intro), "P.%u.%u, %s", part->ptab_index, part->ptab_part_index,
			wd_print_part_name (0, 0, part->part_type, WD_PNAME_NUM_INFO));
		...
		if (wii_head_mode != WD_PAT_IGNORE)
		{
			...
			wd_memmap_item_t *item = wd_insert_memmap (mm, wii_head_mode, head_off, head_size);
			DASSERT (item);
	...
	if (wii_mgr_mode != WD_PAT_IGNORE && wii_mgr_mode == wii_data_mode)
	{
		...
		wd_memmap_item_t *item
			= wd_insert_memmap (mm, wii_mgr_mode, (u64)part->data_sector * WII_SECTOR_SIZE,
	...
	else
	{
		...
		if (wii_data_mode != WD_PAT_IGNORE && part->end_mgr_sector < part->end_sector)
		{
			...
			wd_memmap_item_t *item
				= wd_insert_memmap (mm, wii_data_mode, (u64)part->end_mgr_sector * WII_SECTOR_SIZE,
...
void wd_print_memmap (FILE *f, // valid output file
	int indent, // indention of the output
...
)
{
	...
	for (item = mm->item; item < last; item++)
	{
		...
		fprintf (f, "%*s%c %9llx .. %9llx %9llx  %s\n", indent, "",
			item->offset < prev_end ? '!' : ' ', item->offset, end, item->size, item->info);
		if (prev_end < end)
			prev_end = end;
	}
}
...
wd_memmap_item_t *wd_insert_patch_ticket (wd_part_t *part // valid pointer to a disc partition
)
{
	...
	wd_memmap_item_t *item = wd_insert_memmap (
		&part->disc->patch, WD_PAT_PART_TICKET, (u64)part->part_off4 << 2, WII_TICKET_SIZE);
	DASSERT (item);
	item->index = part->index;
	item->data = &part->ph.ticket;

	snprintf (item->info, sizeof (item->info), "ticket, id=%s, ckey=%u",
		wd_print_id (part->ph.ticket.title_id + 4, 4, 0), part->ph.ticket.common_key_index);
...
wd_memmap_item_t *wd_insert_patch_tmd (wd_part_t *part // valid pointer to a disc partition
)
{
	if (!wd_part_has_tmd (part))
		return 0;

	wd_memmap_item_t *item = wd_insert_memmap (&part->disc->patch, WD_PAT_PART_TMD,
		(u64)(part->part_off4 + part->ph.tmd_off4) << 2, part->ph.tmd_size);
	DASSERT (item);
	item->index = part->index;
	item->data = part->tmd;
	...
	if (high == 1 && low < 0x100)
		snprintf (item->info, sizeof (item->info), "tmd, id=%s, ios=%u",
			wd_print_id (part->tmd->title_id + 4, 4, 0), low);
	else
		snprintf (item->info, sizeof (item->info), "tmd, id=%s, sys=%x-%x",
...
wd_memmap_item_t *wd_insert_patch_fst (wd_part_t *part // valid pointer to a disc partition
)
{
	wd_insert_patch_tmd (part);
	wd_memmap_item_t *item = wd_insert_memmap (
		&part->patch, WD_PAT_DATA, (u64)part->boot.fst_off4 << 2, (u64)part->boot.fst_size4 << 2);
	DASSERT (item);
	item->index = part->index;
	item->data = part->fst;
	part->sign_tmd = true;

	snprintf (item->info, sizeof (item->info), "fst.bin, N=%u", ntohl (part->fst->size));
...
void wd_print_disc_patch (FILE *f, // valid output file
	int indent, // indention of the output
	wd_disc_t *disc, // valid pointer to a disc
...
void wd_calc_group_hashes (const u8 *group_data, // group data space
...
)
{
	...
	for (d = 0; d < WII_GROUP_SECTORS; d++)
	{
		...
		u8 *h1 = group_hash + (d / WII_N_ELEMENTS_H1) * WII_N_ELEMENTS_H1 * WII_SECTOR_HASH_SIZE
			+ (d % WII_N_ELEMENTS_H1) * WII_HASH_SIZE + 0x280;
...
static enumError wd_rap_part_sectors (wd_part_t *part, // valid partition pointer
...
)
{
	...
	if (or_rel & WD_RELOC_F_COPY)
	{
		...
		if (is_encrypted && decrypt)
			buf += WII_SECTOR_SIZE;
		const enumError err = wd_read_raw (disc, src * WII_SECTOR_SIZE4, buf, size, 0);
		if (err)
			return err;
	}
	else
				...
				switch (item->mode)
				{
					...
					case WD_PAT_DATA:
						memcpy (buf + (overlap1 - off1),
							(u8 *)item->data + (overlap1 - item->offset), overlap2 - overlap1);
						break;
...
static enumError wd_rap_sectors (wd_disc_t *disc, // valid disc pointer
	...
	while (sector < end_sector)
	{
		...
		if (rel & WD_RELOC_M_CRYPT)
		{
			...
			if (pidx < disc->n_part)
			{
				...
				u32 sect1 = (sector - part->data_sector) / WII_GROUP_SECTORS * WII_GROUP_SECTORS
					+ part->data_sector;
...
enumError wd_read_and_patch (wd_disc_t *disc, // valid disc pointer
...
u32 wd_get_ptab_sector (wd_disc_t *disc // valid disc pointer
)
{
	DASSERT (disc);
	return disc->disc_type == WD_DT_GAMECUBE ? 0 : WII_PTAB_SECTOR;
}
	...
	)
{
	...
	{
		memcpy (sector_data, &disc->dhead, GC_MULTIBOOT_PTAB_OFF + GC_MULTIBOOT_PTAB_SIZE);
		if (!(disc->disc_attrib & WD_DA_GC_MULTIBOOT))
			return true;
		...
		WDPRINT (
			"PATCH-PTAB: np=%u, max_part_off4=%x, is_dvd9=%d\n", n_part, max_part_off4, is_dvd9);
...
bool wd_patch_id (void *dest_id, // destination, size=id_len, not 0 term
	...
	)
{
	...
	if (new_id && !(disc->disc_attrib & WD_DA_GC_MULTIBOOT))
	{
		...
		{
			stat = true;
			wd_memmap_item_t *item = wd_insert_memmap_alloc (&disc->patch, WD_PAT_DATA, 0, 6);
			DASSERT (item);
			snprintf (item->info, sizeof (item->info), "disc id: %s -> %s", wd_print_id (src, 6, 0),
				wd_print_id (id6, 6, 0));
	...
	if (new_name)
	{
		...
		if (memcmp (name, disc->dhead.disc_title, sizeof (name)))
		{
			stat = true;
			wd_memmap_item_t *item
				= wd_insert_memmap_alloc (&disc->patch, WD_PAT_DATA, WII_TITLE_OFF, WII_TITLE_SIZE);
			DASSERT (item);
	...
	)
{
	...
	if (disc->disc_type == WD_DT_GAMECUBE)
	{
		...
		wd_memmap_item_t *item = wd_insert_memmap_alloc (
			&disc->patch, WD_PAT_DATA, WII_BI2_OFF + WII_BI2_REGION_OFF, sizeof (u32));
	...
	wd_memmap_item_t *item
		= wd_insert_memmap_alloc (&disc->patch, WD_PAT_DATA, WII_REGION_OFF, WII_REGION_SIZE);
	snprintf (item->info, sizeof (item->info), "region: %x", new_region);
	...
	)
{
	...
	if ((unsigned)ckey_index < WD_CKEY__N && ckey_index != part->ph.ticket.common_key_index)
	{
	...
	)
{
	...
	if (new_boot_id && modify & WD_MODIFY_BOOT)
	{
		...
		if (wd_patch_id (id6, src, new_boot_id, 6))
		{
			...
			DASSERT (item);
			item->index = part->index;
			snprintf (item->info, sizeof (item->info), "boot id: %s -> %s", wd_print_id (src, 6, 0),
				wd_print_id (id6, 6, 0));
			memcpy (item->data, id6, 6);
			stat = true;
		}
	...
	)
{
	...
	if (modify & WD_MODIFY_BOOT)
	{
		...
		{
			wd_memmap_item_t *item = wd_insert_memmap_alloc (
				&part->patch, WD_PAT_DATA, WII_BOOT_OFF + WII_TITLE_OFF, WII_TITLE_SIZE);
			DASSERT (item);
...
static int wd_patch_main_iterator (struct wd_iterator_t *it // iterator struct with all infos
)
{
	...
	if (pm->patch_main && !pm->main && !strcmp (it->path, "DATA/sys/main.dol"))
	{
	...
	wd_memmap_item_t *item
		= wd_insert_memmap_alloc (&it->part->patch, WD_PAT_DATA, it->off4 << 2, it->size);
...
int wd_patch_main (wd_patch_main_t *pm, // result only, will be initialized
...
)
{
	...
	return wd_iterate_files (
		disc, wd_patch_main_iterator, pm, patch_staticr ? 0 : 1, WD_IPM_PART_NAME);
}
...
static void wd_mark_disc_reloc (wd_reloc_t *reloc, // valid pointer to relocation table
...
static void wd_mark_part_reloc (wd_part_t *part, // NULL (=disc) or pointer to partition
...
wd_reloc_t *wd_calc_relocation (wd_disc_t *disc, // valid disc pointer
...
{
	DASSERT (disc);
	TRACE ("wd_calc_relocation(%p,%d,%x,%x,%p,%d)\n", disc, encrypt, trim_mode, trim_align, select,
		force);
	...
	{
		wd_memmap_item_t *item = disc->disc_type == WD_DT_GAMECUBE
			? wd_insert_memmap (
				  &disc->patch, WD_PAT_DATA, 0, GC_MULTIBOOT_PTAB_OFF + GC_MULTIBOOT_PTAB_SIZE)
			: wd_insert_memmap (&disc->patch, WD_PAT_DATA, WII_PTAB_REF_OFF, WII_MAX_PTAB_SIZE);
		DASSERT (item);
		snprintf (
			item->info, sizeof (item->info), "use %u of %u partitions", enable_count, disc->n_part);
		item->data = &disc->ptab;
		...
		if (disc->disc_attrib & WD_DA_GC_MULTIBOOT)
		{
			...
			if ((!part->is_valid || !part->is_enabled) && !part->data_off4)
			{
				...
				item = wd_insert_memmap (&disc->patch, WD_PAT_ZERO, sizeof (disc->dhead),
					WII_SECTOR_SIZE - sizeof (disc->dhead));
				DASSERT (item);
				snprintf (item->info, sizeof (item->info), "remove boot partition (zero data)");
			}
		}
	}
	...
	for (idx = 0; idx < WII_MAX_SECTORS; idx++)
	{
		...
		{
			reloc[idx]
				= ((uval & WD_USAGE__MASK) - WD_USAGE_PART_0) << WD_RELOC_S_PART | WD_RELOC_F_COPY;
		...
		if (part->patch.used)
		{
			...
			if (part->h3)
			{
				item = wd_insert_memmap (&part->disc->patch, WD_PAT_DATA,
					(u64)(part->part_off4 + part->ph.h3_off4) << 2, WII_H3_SIZE);
				DASSERT (item);
				item->index = part->index;
				item->data = part->h3;
				snprintf (item->info, sizeof (item->info), "h3");
				wd_mark_disc_reloc (
					reloc, item->offset, item->size, WD_RELOC_F_PATCH | WD_RELOC_F_LAST);
			}
...
void wd_print_relocation (FILE *f, // valid output file
	int indent, // indention of the output
...
)
{
	...
	while (rel < end)
	{
		...
		if (count > 1)
			fprintf (f, "%*s%9llx  %5x .. %5x :%5x : %5x .. %5x :", indent, "",
				(u64)WII_SECTOR_SIZE * dest, dest, dest + count - 1, count, src, src + count - 1);
		else
			fprintf (f, "%*s%9llx  %14x :    1 : %14x :", indent, "", (u64)WII_SECTOR_SIZE * dest,
				dest, src);
		...
		fprintf (f, " %s %s %s %s %s\n",
			val & WD_RELOC_F_ENCRYPT	   ? "enc"
				: val & WD_RELOC_F_DECRYPT ? "dec"
										   : " - ",
			val & WD_RELOC_F_COPY ? "copy" : " -  ", val & WD_RELOC_F_PATCH ? "patch" : "  -  ",
			val & WD_RELOC_F_HASH ? "hash" : " -  ", val & WD_RELOC_F_LAST ? "last" : " -");
	}
}

///////////////////////////////////////////////////////////////////////////////

void wd_print_disc_relocation (FILE *f, // valid output file
	int indent, // indention of the output
	wd_disc_t *disc, // valid pointer to a disc
	bool print_title // true: print table titles
)
{
	...
	wd_print_relocation (f, indent,
		wd_calc_relocation (disc, true, disc->trim_mode, disc->trim_align, 0, false), print_title);
}

///////////////////////////////////////////////////////////////////////////////

wd_trim_mode_t wd_get_relococation_trim (wd_trim_mode_t trim_mode, // trim mode to check
...
)
{
	...
	if (trim_align)
	{
		...
		{
			const u32 align0 = *trim_align > WII_GROUP_SIZE ? WII_GROUP_SIZE
				: *trim_align								? *trim_align
				: disc_type == WD_DT_GAMECUBE				? GC_GOOD_PART_ALIGN
															: WII_SECTOR_SIZE;
...
wd_file_list_t *wd_initialize_file_list (wd_file_list_t *fl // NULL or working file list
...
void wd_reset_file_list (wd_file_list_t *fl, // NULL or working file list to reset (free data)
...
wd_file_list_t *wd_create_file_list (wd_file_list_t *fl, // NULL or working file list
														 // If NULL: allocate a file list
...
wd_file_t *wd_insert_file (wd_file_list_t *fl, // working file list
...
wd_file_t *wd_insert_directory (wd_file_list_t *fl, // working file list
...
wd_fst_item_t *wd_create_file_list_fst (wd_file_list_t *fl, // working file list
...
bool wd_is_directory (wd_file_t *file // pointer to a file
)
{
	DASSERT (file);
...
void wd_print_disc (FILE *f, // valid output file
	int indent, // indention of the output
	wd_disc_t *disc, // valid disc pointer
...
)
{
	...
	fprintf (f, "%*sRead func+data:     %p %p\n", indent, "", disc->read_func, disc->read_data);

	fprintf (
		f, "%*sFile size:          %llx/hex = %llu\n", indent, "", disc->iso_size, disc->iso_size);
	...
	for (ip = 0; ip < disc->n_part; ip++)
	{
		...
		fprintf (f,
			...
			indent, "", part->index, part->ptab_index, part->ptab_part_index, part->is_loaded,
			part->is_valid, part->is_enabled, part->is_encrypted,
		...
		if (load_part && part->is_valid)
		{
			...
			fprintf (f, "%*s  TICKET:      %9llx .. %9llx, %9zx\n", indent, "", off,
				off + sizeof (part->ph), sizeof (part->ph));

			if (part->ph.tmd_off4)
			{
				off = (part->part_off4 + part->ph.tmd_off4) << 2;
				fprintf (f, "%*s  TMD:         %9llx .. %9llx, %9x\n", indent, "", off,
					off + part->ph.tmd_size, part->ph.tmd_size);
			}
			...
			if (part->ph.cert_off4)
			{
				...
				fprintf (f, "%*s  CERT:        %9llx .. %9llx, %9x,  loaded=%d\n", indent, "", off,
					off + part->ph.cert_size, part->ph.cert_size, part->cert != 0);
			}
			...
			if (part->ph.h3_off4)
			{
				...
				fprintf (f, "%*s  H3:          %9llx .. %9llx, %9x,  loaded=%d\n", indent, "", off,
					off + WII_H3_SIZE, WII_H3_SIZE, part->h3 != 0);
			}
...
void wd_print_disc_usage_tab (FILE *f, // valid output file
	int indent, // indention of the output
	wd_disc_t *disc, // valid disc pointer
...
void wd_print_usage_tab (FILE *f, // valid output file
	int indent, // indention of the output
...
{
	wd_print_byte_tab (f, indent, usage_tab, (iso_size + WII_SECTOR_SIZE - 1) / WII_SECTOR_SIZE,
		WII_MAX_SECTORS, WII_SECTOR_SIZE, wd_usage_name_tab, print_all);
}

///////////////////////////////////////////////////////////////////////////////

static void wd_print_files (wd_part_t *part, // pointer to partition
...
)
{
	...
	while (off < end)
	{
		...
		u32 start = sect;
		while (sect < WII_MAX_SECTORS && (utab[sect] & WD_USAGE__MASK) != usage_id)
			sect++;
		if (start < sect)
		{
...
static void wd_print_gc_mem (wd_disc_t *disc, // valid disc pointer
	wd_mem_func_t func, // valid function pointer
	void *param // user defined parameter
)
{
	...
	for (; part < part_end; part++)
	{
		...
		WDPRINT ("PART #%u, off=%9llx, size=%9llx, doff4=%8x, sect=%5x..%5x\n", part->index,
			base_off, part->part_size, part->data_off4, part->data_sector, part->end_sector);

		char *dest = msg;
		if (disc->n_part || !have_0_part)
			dest += snprintf (
				msg, sizeof (msg), "%sP.%u: ", part->index & 1 ? "" : " ", part->index);
		...
		snprintf (dest, msgsize, "boot.bin, magic=%02x-%02x-%02x-%02x, id=%s", m[0], m[1], m[2],
			m[3], wd_print_id (boot->dhead.id6.id6, 6, 0));
	...
	if (!have_0_part)
	{
		snprintf (msg, sizeof (msg), "GameCube multiboot header, id=%.6s%s", disc->dhead.id6.id6,
...
void wd_print_mem (wd_disc_t *disc, // valid disc pointer
	wd_mem_func_t func, // valid function pointer
	void *param // user defined parameter
)
{
	...
	snprintf (msg, sizeof (msg), "Header, magic=%02x-%02x-%02x-%02x, id=%s", m[0], m[1], m[2], m[3],
		wd_print_id (disc->dhead.id6.id6, 6, 0));
	func (param, 0, sizeof (disc->dhead), msg);

	//----- partition tables

	snprintf (msg, sizeof (msg), "Partition address table, N=%u+%u+%u+%u",
		ntohl (disc->ptab_info[0].n_part), ntohl (disc->ptab_info[1].n_part),
		ntohl (disc->ptab_info[2].n_part), ntohl (disc->ptab_info[3].n_part));
	...
	for (ip = 0; ip < WII_MAX_PTAB; ip++)
	{
		...
		if (np)
		{
			...
			snprintf (msg, sizeof (msg), "Partition table #%u with %u partition%s", ip, np,
				np == 1 ? "" : "s");
	...
	snprintf (msg, sizeof (msg), "Region settings, region=%x", ntohl (disc->region.region));
	func (param, WII_REGION_OFF, sizeof (disc->region), msg);

	//----- magic2

	m = (u8 *)&disc->magic2;
	snprintf (msg, sizeof (msg), "Magic2: %02x-%02x-%02x-%02x", m[0], m[1], m[2], m[3]);
	...
	for (ip = 0; ip < disc->n_part; ip++)
	{
		...
		char *dest = msg
			+ snprintf (
				msg, sizeof (msg), "P.%u.%u %s: ", part->ptab_index, part->ptab_part_index, pname);
		size_t msgsize = msg + sizeof (msg) - dest;
		...
		else if (part->is_enabled)
		{
			...
			snprintf (dest, msgsize, "ticket, id=%s", wd_print_id (ph->ticket.title_id + 4, 4, 0));
			...
			if (ph->tmd_off4)
			{
				...
				if (hi == 1 && lo < 0x100)
					snprintf (dest, msgsize, "tmd, ios=%u, id=%s", lo,
						wd_print_id (part->tmd->title_id + 4, 4, 0));
				else
					snprintf (dest, msgsize, "tmd, sys=%08x-%08x, id=%s", hi, lo,
						wd_print_id (part->tmd->title_id + 4, 4, 0));
	...
	if (disc->iso_size)
		func (param, disc->iso_size, 0, "-- End of file/disc --");
}
