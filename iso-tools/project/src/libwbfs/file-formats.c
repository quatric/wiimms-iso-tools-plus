...
int validate_file_format_sizes (int trace_sizes)
{
	...
	if (trace_sizes)
	{
		...
		ASSERT (!clear_part_control (&pc, WII_TMD_GOOD_SIZE, 0xa00, 0x1000000));
...
uint calc_dol_records (dol_record_t *rec, // pointer to at least DOL_N_SECTIONS records
...
)
{
	...
	for (;;)
	{
		...
		{
			if (dh.sect_off[i] && dh.sect_size[i] && dh.sect_addr[i] < last_addr
		...
		if (dest > rec && dest[-1].delta == dest->delta && dest->addr + dest->size == dest[-1].addr)
		{
...
const dol_record_t *search_dol_record (dol_record_t *rec, // pointer to at least records
...
ccp wd_get_compression_name (wd_compression_t compr, // compression mode
...
ccp wd_print_compression (char *buf, // result buffer
...
)
{
	...
	else
	{
		...
		if (compr_method == WD_COMPR__N)
			...
			snprintf (buf, buf_size, "%u.%u%s %s.%u%s", compr_method, compr_level, cbuf, name,
				compr_level, cbuf);
		else
			snprintf (buf, buf_size, "%u%s %s%s", compr_method, cbuf, name, cbuf);
	}
...
void header_128_setup (wd_header_128_t *dhead, // valid pointer
...
void header_setup (wd_header_t *dhead, // valid pointer
	const void *id6, // NULL or pointer to ID
	ccp disc_title, // NULL or pointer to disc title (truncated)
	bool is_gc // true: GameCube setup
)
...
wd_disc_type_t get_header_128_disc_type (wd_header_128_t *dhead, // valid pointer
...
)
{
	...
	if (ntohl (dhead->gc_magic) == GC_MAGIC)
	{
		if (attrib)
		{
			...
			if (!memcmp (id6, "GCOPDV", 6) || !memcmp (id6, "COBRAM", 6)
				|| !memcmp (id6, "GGCOSD", 6) || !memcmp (id6, "RGCOSD", 6))
			{
...
wd_disc_type_t get_header_disc_type (wd_header_t *dhead, // valid pointer
	wd_disc_attrib_t *attrib // not NULL: store disc attributes
)
{
...
ccp wd_print_age_rating (char *buf, // result buffer
									// If NULL, a local circulary static buffer is used
	size_t buf_size, // size of 'buf', ignored if buf==NULL
...
)
{
	...
	snprintf (buf, buf_size, "Jap=%u USA=%u ?=%u Eur=%u,%u,%u,%u,%u,%u Kor=%u",
		age_rating[WD_AGE_JAPAN], age_rating[WD_AGE_USA], age_rating[WD_AGE_UNKNOWN],
		age_rating[WD_AGE_EUROPE1], age_rating[WD_AGE_EUROPE2], age_rating[WD_AGE_EUROPE3],
		age_rating[WD_AGE_EUROPE4], age_rating[WD_AGE_EUROPE5], age_rating[WD_AGE_EUROPE6],
...
static int setup_part_control_helper (wd_part_control_t *pc)
{
	...
	u8 *ptr = pc->tmd ? (u8 *)pc->tmd : pc->part_bin + ALIGN32 (sizeof (wd_part_header_t), 32);
	pc->tmd = (wd_tmd_t *)ptr;
	...
	TRACE (" - CON0: %p : %8zx + %9zx [N=%u/%u]\n", pc->tmd_content,
		(ntohl (head->tmd_off4) << 2) + sizeof (wd_tmd_t), sizeof (wd_tmd_content_t),
		pc->tmd->n_content, n_content);

	// setup cert

	ptr = pc->cert ? (u8 *)pc->cert : (u8 *)pc->tmd + ALIGN32 (pc->tmd_size, 32);
	pc->cert = ptr;
	...
	if (pc->tmd_size < WII_TMD_GOOD_SIZE || pc->cert_size < 0x10
		|| pc->data_off < sizeof (pc->part_bin)
	...
	if (tmd_beg < head_end || tmd_end > part_end || cert_beg < head_end || cert_end > part_end
...
int clear_part_control (wd_part_control_t *pc, u32 tmd_size, u32 cert_size, u64 data_size)
{
	ASSERT (pc);
...
int setup_part_control (wd_part_control_t *pc)
{
	...
	pc->tmd = (wd_tmd_t *)(pc->part_bin + (ntohl (head->tmd_off4) << 2));
...
ccp wpat_get_type_name (wpat_type_t type, ccp return_if_invalid)
{
	...
	return !res ? return_if_invalid : type & WPAT_F_TOC ? res : res + 4;
}
