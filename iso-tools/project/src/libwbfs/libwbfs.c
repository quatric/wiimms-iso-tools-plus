...
be64_t wbfs_setup_inode_info (wbfs_t *p, wbfs_inode_info_t *ii, bool is_valid, int is_changed)
{
	ASSERT (p);
	ASSERT (p->head);
	ASSERT (ii);
...
wbfs_t *wbfs_open_partition (rw_sector_callback_t read_hdsector,
	rw_sector_callback_t write_hdsector, void *callback_data, int hd_sector_size, int num_hd_sector,
	u32 part_lba, int reset)
{
	wbfs_param_t par;
...
void wbfs_setup_lists (wbfs_t *p // valid WBFS descriptor
)
{
	DASSERT (p);
...
wbfs_t *wbfs_open_partition_param (wbfs_param_t *par)
{
	...
	if (par->reset > 0)
	{
		...
		if (par->wbfs_sector_size > 0)
			...
			head->wbfs_sec_sz_s = wbfs_calc_size_shift (
				head->hd_sec_sz_s, par->num_hd_sector, par->old_wii_sector_calc);
	}
	else
	{
	...
	wbfs_calc_geometry (
		p, wbfs_ntohl (head->n_hd_sec), 1 << head->hd_sec_sz_s, 1 << head->wbfs_sec_sz_s);
...
int wbfs_calc_size_shift (u32 hd_sec_sz_s, u32 num_hd_sector, int old_wii_sector_calc)
{
	...
	int shift_count;
	for (shift_count = WBFS_MIN_SECTOR_SHIFT; shift_count <= WBFS_MAX_SECTOR_SHIFT; shift_count++)
	{
		// ensure that wbfs_sec_sz is big enough to address every blocks using 16 bits
...
void wbfs_calc_geometry (wbfs_t *p, // pointer to wbfs_t, p->head must be NULL or valid
...
)
{
	...
	p->freeblks_lba_count = fb_memsize >> p->hd_sec_sz_s;

	//----- here we correct the previous wrong calulations

	p->n_wii_sec = p->n_hd_sec / (p->wii_sec_sz / p->hd_sec_sz);
	const u32 n_wbfs_sec = p->n_wii_sec >> (p->wbfs_sec_sz_s - p->wii_sec_sz_s);
	p->n_wbfs_sec = n_wbfs_sec < WBFS_MAX_SECTORS ? n_wbfs_sec : WBFS_MAX_SECTORS - 1;
...
void wbfs_sync (wbfs_t *p)
{
	...
	if (p->write_hdsector)
	{
		...
		if (p->used_block && p->used_block_dirty)
		{
			...
			if (p->freeblks)
				p->write_hdsector (p->callback_data, p->part_lba + p->freeblks_lba,
					p->freeblks_lba_count, p->freeblks);
	...
	wbfs_t *p, u32 slot, wbfs_disc_info_t *info, int force_open)
{
	...
	if (*d->header->dhead)
	{
		...
		noTRACE ("MAGIC: %08x %08x => %d %d\n", wii_magic, gc_magic, wii_magic == WII_MAGIC,
			gc_magic == GC_MAGIC);
...
wbfs_disc_t *wbfs_open_disc_by_slot (wbfs_t *p, u32 slot, int force_open)
{
	...
	TRACE ("LIBWBFS: wbfs_open_disc_by_slot(%p,slot=%u,force=%d) max=%u, disctab=%u\n", p, slot,
		p->max_disc, p->head->disc_table[slot], force_open);
	...
	if (p->read_hdsector (
			p->callback_data, p->part_lba + 1 + slot * disc_info_sz_lba, disc_info_sz_lba, info))
	{
		wbfs_free (info);
		return 0;
	}
...
wbfs_disc_t *wbfs_create_disc (wbfs_t *p, // valid WBFS descriptor
...
)
{
	...
	if (disc_id)
	{
		...
		if (slot >= 0)
		{
			wbfs_error ("Disc with id '%s' already exists.", wd_print_id (disc_id, 6, 0));
			return 0;
		}
	}
...
int wbfs_sync_disc_header (wbfs_disc_t *d)
{
	...
	wbfs_t *p = d->p;
	const u32 disc_info_sz_lba = p->disc_info_sz >> p->hd_sec_sz_s;
	return p->write_hdsector (p->callback_data, p->part_lba + 1 + d->slot * disc_info_sz_lba,
...
wbfs_inode_info_t *wbfs_get_inode_info (wbfs_t *p, wbfs_disc_info_t *info, int clear_mode)
{
	ASSERT (p);
	ASSERT (info);
	wbfs_inode_info_t *iinfo = (wbfs_inode_info_t *)(info->dhead + WBFS_INODE_INFO_OFF);
...
uint wbfs_get_fragments (const u16 *wlba_tab, // valid wlba table in network byte order
...
uint wbfs_get_disc_fragments (wbfs_disc_t *d, // valid wbfs disc
	uint *disc_blocks // not NULL: store number of disc blocks
)
{
	DASSERT (d);
	...
	{
		d->n_fragments = wbfs_get_fragments (
			d->header->wlba_table, d->p->n_wbfs_sec_per_disc, &d->disc_blocks);
	}
	if (disc_blocks)
...
int wbfs_rename_disc (wbfs_disc_t *d, // pointer to an open disc
...
)
{
	...
	int do_sync = 0;
	if (chg_wbfs_head && wd_rename (d->header->dhead, new_id, new_title))
	{
	...
	if (do_sync)
	{
		TRACE ("wbfs_rename_disc() now=%llu i=%llu m=%llu c=%llu a=%llu d=%llu\n", ntoh64 (now),
			ntoh64 (iinfo->itime), ntoh64 (iinfo->mtime), ntoh64 (iinfo->ctime),
			ntoh64 (iinfo->atime), ntoh64 (iinfo->dtime));
...
int wbfs_touch_disc (wbfs_disc_t *d, // pointer to an open disc
...
void wbfs_print_block_usage (FILE *f, // valid output file
...
)
{
	...
	wbfs_print_usage_tab (f, indent, p->used_block, p->n_wbfs_sec, p->wbfs_sec_sz, print_all);
}

///////////////////////////////////////////////////////////////////////////////

const char wbfs_usage_name_tab[256] = { ".x23456789ABCDEFGHIJKLMNOPQRSTUV"
										"WXYZ++++++++++++++++++++++++++++"
										"++++++++++++++++++++++++++++++++"
										"+++++++++++++++++++++++++++++++*"
...
void wbfs_print_usage_tab (FILE *f, // valid output file
	int indent, // indention of the output
...
{
	wd_print_byte_tab (f, indent, used_block, block_used_sz, block_used_sz, sector_size,
		wbfs_usage_name_tab, print_all);
}
...
enumError wbfs_get_disc_info (wbfs_t *p, // valid wbfs descriptor
	...
	for (slot = 0; slot < p->max_disc; slot++)
		if (p->head->disc_table[slot])
			if (count++ == index)
			{
				if (slot_found)
					*slot_found = slot;
				return wbfs_get_disc_info_by_slot (
					p, slot, header, header_size, disc_type, disc_attrib, size4, n_fragments);
			}
...
enumError wbfs_get_disc_info_by_slot (wbfs_t *p, // valid wbfs descriptor
...
)
{
	...
	const u32 disc_info_sz_lba = p->disc_info_sz >> p->hd_sec_sz_s;
	p->read_hdsector (
		p->callback_data, p->part_lba + 1 + slot * disc_info_sz_lba, 1, p->tmp_buffer);
	...
	{
		const wd_disc_type_t dt = get_header_disc_type ((wd_header_t *)p->tmp_buffer, disc_attrib);
		if (disc_type)
			*disc_type = dt;
	}
	...
	if (size4 || n_fragments)
	{
		...
		p->read_hdsector (
			p->callback_data, p->part_lba + 1 + slot * disc_info_sz_lba, disc_info_sz_lba, header);

		sec_used = wbfs_sector_used (p, header);
		if (size4)
			*size4 = sec_used << (p->wbfs_sec_sz_s - 2);
		if (n_fragments)
			*n_fragments = wbfs_get_fragments (header->wlba_table, p->n_wbfs_sec_per_disc, 0);
		wbfs_iofree (header);
	}
	return ERR_OK;
}
...
id6_t *wbfs_load_id_list (wbfs_t *p, int force_reload)
{
	...
	TRACE ("LIBWBFS: +wbfs_load_id_list(%p,%d) id_list_size=%u*%u=%d\n", p, force_reload,
		p->max_disc, id_item_size, id_list_size);
...
int wbfs_find_slot (wbfs_t *p, const u8 *disc_id)
{
	...
	u32 slot;
	for (slot = 0; slot < p->max_disc; slot++)
		if (p->head->disc_table[slot] && !memcmp (p->id_list[slot], disc_id, id_item_size))
		{
...
u32 *wbfs_free_freeblocks (wbfs_t *p)
{
	...
	{
		if (p->freeblks
			&& ((u8 *)p->freeblks < p->block0 || (u8 *)p->freeblks > p->block0 + p->wbfs_sec_sz))
		{
			wbfs_iofree (p->freeblks);
		}
		p->freeblks = (u32 *)(p->block0 + (p->part_lba + p->freeblks_lba) * p->hd_sec_sz);
		return p->freeblks;
	}
...
int wbfs_calc_used_blocks (wbfs_t *p, // valid WBFS descriptor
...
)
{
	...
	{
		TRACE ("READ BLOCK0\n");
		int read_stat = p->read_hdsector (
			p->callback_data, p->part_lba, p->wbfs_sec_sz / p->hd_sec_sz, block0);
		if (read_stat)
		{
			if (!store_block0)
				wbfs_iofree (block0);
	...
	u32 *fbt = fbt0;
	TRACE ("block0=%p..%p, fbt=%p [%zx]\n", block0, block0 + p->wbfs_sec_sz, fbt,
		(ccp)fbt - (ccp)block0);
	u8 *dest = used + 1;
	u32 i;
	...
	for (slot = 0; slot < p->max_disc; slot++)
	{
		...
		if (slot_info)
		{
			...
			noPRINT ("NEW WBFS INTERFACE: check slot %u: stat=%x, off=%zx, id=%s\n", slot,
				slot_info, (u8 *)info - block0, wd_print_id (info, 6, 0));
			...
			for (bl = 0; bl < p->n_wbfs_sec_per_disc; bl++)
			{
				...
				if (wlba >= p->n_wbfs_sec)
				{
					...
					if (func)
					{
						const uint msg_len = snprintf (msg, sizeof (msg),
							"Invalid WBFS block (%u>%u) at slot #%u [%s].", wlba, p->n_wbfs_sec - 1,
							slot, id6);
				...
				else if (wlba > 0)
				{
					...
					if (used[wlba] & FREE_MARK)
					{
						...
						{
							const uint msg_len = snprintf (msg, sizeof (msg),
								"WBFS block #%u marked free, but used by slot #%u [%s].", wlba,
								slot, id6);
							func (p, WBFS_CHK_FREE_BLOCK_USED, slot, id6, wlba, 0, msg, msg_len,
								param);
						}
					}
			...
			{
				PRINT_IF (
					!func, "!!! NEW WBFS INTERFACE: disc @ slot %u does'n have any block\n", slot);
	...
	for (i = 1; i < p->n_wbfs_sec; i++)
	{
		...
		{
			PRINT_IF (
				!func, "!!! NEW WBFS INTERFACE: block %u* used: #%x [%02x]\n", ucnt, i, used[i]);
			pass2_needed = true;
			if (func)
			{
				const uint msg_len
					= snprintf (msg, sizeof (msg), "WBFS block #%u used %u times.", i, ucnt);
			...
			if (slot_info)
			{
				...
				for (bl = 0; bl < p->n_wbfs_sec_per_disc; bl++)
				{
					...
					if (wlba > 0 && wlba < p->n_wbfs_sec && used[wlba] > 1)
					{
						...
						{
							const uint msg_len = snprintf (msg, sizeof (msg),
								"Disc at slot #%u [%s] use shared block %u (used %u* total).", slot,
								p->id_list[slot], wlba, used[wlba]);
							func (p, WBFS_CHK_SHARED_BLOCK, slot, p->id_list[slot], bl, used[wlba],
								msg, msg_len, param);
						}
					}
				}
...
u32 wbfs_alloc_block (wbfs_t *p, // valid WBFS descriptor
...
void wbfs_free_block (wbfs_t *p, u32 bl)
{
	...
	if (bl > 0 && bl < p->n_wbfs_sec && p->used_block[bl] > 0 && p->used_block[bl] < 127)
	{
		if (!--p->used_block[bl])
			p->used_block_dirty = p->is_dirty = true;
	}
...
u32 wbfs_add_disc (wbfs_t *p, wd_read_func_t read_src_wii_disc, void *callback_data,
	progress_callback_t spinner, const wd_select_t *psel, int copy_1_1)
{
	wbfs_param_t par;
	memset (&par, 0, sizeof (par));
...
u32 wbfs_add_disc_param (wbfs_t *p, wbfs_param_t *par)
{
	...
	if (!disc)
	{
		disc = wd_open_disc (par->read_src_wii_disc, par->callback_data, par->iso_size, 0, 0, 0);
		if (!disc)
	...
	PRINT ("ADD, TOTAL BLOCKS= %u*%u*%u = %llu\n", total_blocks, wii_sec_per_wbfs_sect,
		WII_SECTOR_SIZE, (u64)total_blocks * wii_sec_per_wbfs_sect * WII_SECTOR_SIZE);
	...
	if (total_blocks > free_blocks)
	{
		wbfs_error ("New discs needs %u wbfs blocks (%s)"
			...
			total_blocks, wd_print_size (0, 0, p->wbfs_sec_sz, false, WD_SIZE_AUTO), free_blocks);
		goto error;
	}
	...
	if (p->balloc_mode == WBFS_BA_AVOID_FRAG
		|| p->balloc_mode == WBFS_BA_AUTO && p->hd_sec_sz * (u64)p->n_hd_sec >= 20 * (u64)GiB)
	{
		...
		if (wd_is_block_used (used, i, wii_sec_per_wbfs_sect))
		{
			...
			while (subsec < wii_sec_per_wbfs_sect)
			{
				...
				if (wiisec < WII_MAX_SECTORS && used[wiisec])
				{
					...
					// [[2do]] use wd_read_and_patch()
					if (par->read_src_wii_disc (
							par->callback_data, wiisec * (p->wii_sec_sz >> 2), size, dest))
			...
			if (i == ptab_index)
				wd_patch_ptab (disc, copy_buffer + ptab_off - i * p->wbfs_sec_sz, false);
#endif
	...
	memcpy (info->dhead + WBFS_INODE_INFO_OFF, &par->iinfo, sizeof (par->iinfo));
...
u32 wbfs_add_phantom (wbfs_t *p, ccp phantom_id, u32 wii_sectors)
{
	...
	snprintf ((char *)info->dhead + WII_TITLE_OFF, WII_TITLE_SIZE,
		"Phantom %.6s @ slot %u -> not a real disc, for tests only!", phantom_id, slot);
	...
	for (i = 0; i < max_wbfs_sect; i++)
	{
		...
		if (i == 0)
			p->write_hdsector (
				p->callback_data, p->part_lba + bl * (p->wbfs_sec_sz / p->hd_sec_sz), 1, info);
...
u32 wbfs_rm_disc (wbfs_t *p, // valid WBFS descriptor
...
{
	TRACE ("LIBWBFS: +wbfs_rm_disc(%p,%.6s,%d,%d)\n", p, discid ? (ccp)discid : "-", slot,
		free_slot_only);
	DASSERT (p);
	DASSERT (p->head);

	wbfs_disc_t *d
		= discid ? wbfs_open_disc_by_id6 (p, discid) : wbfs_open_disc_by_slot (p, slot, false);
	if (!d)
		return 1;
	slot = d->slot;
...
#endif
		const u32 disc_info_sz_lba = p->disc_info_sz >> p->hd_sec_sz_s;
		p->write_hdsector (p->callback_data, p->part_lba + 1 + slot * disc_info_sz_lba,
			disc_info_sz_lba, d->header);
	}
	wbfs_close_disc (d);
...
u32 wbfs_trim (wbfs_t *p) // trim the file-system to its minimum size
{
	...
	wbfs_calc_geometry (
		p, max_block << p->wbfs_sec_sz_s - p->hd_sec_sz_s, p->hd_sec_sz, p->wbfs_sec_sz);
	p->used_block_dirty = p->is_dirty = true;
	wbfs_sync (p);
