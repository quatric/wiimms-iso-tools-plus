...
static void gkey (aes_key_t *akey, int nb, int nk, char *key)
{
	...
	for (j = akey->Nk, k = 0; j < N; j += akey->Nk, k++)
	{
		...
		else
		{
			for (i = 1; i < 4 && (i + j) < N; i++)
				akey->fkey[i + j] = akey->fkey[i + j - akey->Nk] ^ akey->fkey[i + j - 1];
			if ((j + 4) < N)
				akey->fkey[j + 4] = akey->fkey[j + 4 - akey->Nk] ^ SubByte (akey->fkey[j + 3]);
	...
	for (j = 0; j < akey->Nb; j++)
		akey->rkey[j + N - akey->Nb] = akey->fkey[j];
	for (i = akey->Nb; i < N - akey->Nb; i += akey->Nb)
	{
		k = N - akey->Nb - i;
		for (j = 0; j < akey->Nb; j++)
			akey->rkey[k + j] = InvMixCol (akey->fkey[i + j]);
	}
	for (j = N - akey->Nb; j < N; j++)
		akey->rkey[j - N + akey->Nb] = akey->fkey[j];
}
...
static void encrypt (const aes_key_t *akey, char *buff)
{
	...
	{ /* akey->Nr is number of rounds. May be odd. */
		...
		{ /* deal with each 32-bit element of the State */
			...
			y[j] = akey->fkey[k++] ^ ftable[(u8)x[j]] ^ ftable8[(u8)(x[akey->fi[m]] >> 8)]
...
static void decrypt (const aes_key_t *akey, char *buff)
{
	...
	{ /* akey->Nr is number of rounds. May be odd. */
		...
		for (m = j = 0; j < akey->Nb; j++, m += 3)
		{ /* This is the time-critical bit */
			y[j] = akey->rkey[k++] ^ rtable[(u8)x[j]] ^ rtable8[(u8)(x[akey->ri[m]] >> 8)]
...
// CBC mode decryption
void wd_aes_decrypt (
	const aes_key_t *akey, const void *p_iv, const void *p_inbuf, void *p_outbuf, u64 len)
{
	const u8 *iv = p_iv;
