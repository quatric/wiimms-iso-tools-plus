...
{
	static const KeywordTab_t tab[] = { { 0, "AUTO", 0, ENCODE_MASK },
...
static const enumEncoding encoding_m_tab[] = { ENCODE_M_HASH, ENCODE_M_CRYPT, ENCODE_M_SIGN, 0 };

enumEncoding SetEncoding (enumEncoding val, enumEncoding set_mask, enumEncoding default_mask)
{
...
enumRegion ScanRegion (ccp arg)
{
	static const KeywordTab_t tab[]
		= { { REGION_JAP, "JAPAN", "JAP", 0 }, { REGION_USA, "USA", 0, 0 },
			  { REGION_EUR, "EUROPE", "EUR", 0 }, { REGION_KOR, "KOREA", "KOR", 0 },

			  { REGION__AUTO, "AUTO", 0, 0 }, { REGION__FILE, "FILE", 0, 0 },
...
{
	static ccp tab[] = { "Japan", "USA", "Europe", "Korea" };

	return (unsigned)region < sizeof (tab) / sizeof (*tab) ? tab[region] : unkown_value;
}
...
{
	static const KeywordTab_t tab[]
		= { { WD_CKEY_STANDARD, "STANDARD", 0, 0 }, { WD_CKEY_KOREA, "KOREAN", 0, 0 },
...
static bool PatchHost (u8 *data, uint size, bool patch, PatchStat_t *pstat)
{
	...
	static const struct tab_t tab[] = { { "SC7", 0x8023c954 }, // Call of Duty Black Ops
	...
	for (ptr = tab; ptr->id[0]; ptr++)
	{
		...
		PRINT ("FOUND: %08x %s : %08x %s\n", off_id, PrintID (data + off_id, 4, 0), off_host,
			PrintID (data + off_host, 4, 0));
...
uint PatchDomain (bool is_dol, u8 *data, uint size, ccp title, bool patch, PatchStat_t *pstat)
{
	...
	for (;;)
	{
		...
		{
			if (opt_domain && next_n[-1] == '.' && !memcmp (next_n, nin_wifi_net, nin_wifi_net_len))
			{
			...
			else if (opt_domain && !memcmp (next_n - 5, sake_gamespy_com, sake_gamespy_com_len))
			{
...
int patch_main_and_rel (wd_disc_t *disc, AnalyzeFile_t *af)
{
	...
	{
		stat |= PatchDomain (
			true, pm.main->data, pm.main->size, "MAIN", af->patch_dol, &af->pstat_main);
		if (opt_security_fix)
		{
	...
	if (pm.staticr)
		stat |= PatchDomain (
			false, pm.staticr->data, pm.staticr->size, "SREL", af->patch_dol, &af->pstat_staticr);
...
{
	static const KeywordTab_t tab[] = { { WD_MODIFY__NONE, "NONE", "-", WD_MODIFY__ALL },
		{ WD_MODIFY__ALL, "ALL", 0, WD_MODIFY__ALL },
		{ WD_MODIFY__AUTO, "AUTO", 0, WD_MODIFY__ALL }, { WD_MODIFY__TT, "TT", 0, 0 },

		{ WD_MODIFY_DISC, "DISC", 0, 0 }, { WD_MODIFY_BOOT, "BOOT", 0, 0 },
		{ WD_MODIFY_TICKET, "TICKET", "TIK", 0 }, { WD_MODIFY_TMD, "TMD", 0, 0 },
...
static int ScanOptIdHelper (ccp p_arg, ccp opt_name, ccp *id_ptr, char *buf, int max_len)
{
	DASSERT (opt_name);
	DASSERT (id_ptr);
	DASSERT (buf);
...
{
	return ScanOptIdHelper (arg, "id", &modify_id, modify_id_buf, sizeof (modify_id_buf) - 1);
}

int ScanOptDiscId (ccp arg)
{
	return ScanOptIdHelper (
		arg, "disc-id", &modify_disc_id, modify_disc_id_buf, sizeof (modify_disc_id_buf) - 1);
}

int ScanOptBootId (ccp arg)
{
	return ScanOptIdHelper (
		arg, "boot-id", &modify_boot_id, modify_boot_id_buf, sizeof (modify_boot_id_buf) - 1);
}

int ScanOptTicketId (ccp arg)
{
	return ScanOptIdHelper (arg, "ticket-id", &modify_ticket_id, modify_ticket_id_buf,
		sizeof (modify_ticket_id_buf) - 1);
}

int ScanOptTmdId (ccp arg)
{
	return ScanOptIdHelper (
		arg, "tmd-id", &modify_tmd_id, modify_tmd_id_buf, sizeof (modify_tmd_id_buf) - 1);
}

int ScanOptWbfsId (ccp arg)
{
	return ScanOptIdHelper (
		arg, "wbfs-id", &modify_wbfs_id, modify_wbfs_id_buf, sizeof (modify_wbfs_id_buf) - 1);
}
...
{
	NormID (WD_MODIFY__AUTO | WD_MODIFY_DISC, &modify_disc_id, modify_disc_id_buf,
		sizeof (modify_disc_id_buf) - 1);
	NormID (WD_MODIFY__AUTO | WD_MODIFY_BOOT, &modify_boot_id, modify_boot_id_buf,
		sizeof (modify_boot_id_buf) - 1);
	NormID (WD_MODIFY__AUTO | WD_MODIFY_TICKET, &modify_ticket_id, modify_ticket_id_buf,
		sizeof (modify_ticket_id_buf) - 1);
	NormID (WD_MODIFY__AUTO | WD_MODIFY_TMD, &modify_tmd_id, modify_tmd_id_buf,
		sizeof (modify_tmd_id_buf) - 1);
	NormID (WD_MODIFY__AUTO | WD_MODIFY_WBFS, &modify_wbfs_id, modify_wbfs_id_buf,
		sizeof (modify_wbfs_id_buf) - 1);
}

//-----------------------------------------------------------------------------

bool PatchId (void *dest_id, // destination with 'maxlen' byte
...
bool CopyPatchId (void *dest, // destination with 'maxlen' byte
...
{
	return opt_hook >= 0 && CopyPatchId (dest_id6, source_id6, modify_disc_id, 6, true);
}
...
wd_trim_mode_t ScanTrim (ccp arg, // argument to scan
	ccp err_text_extend // error message extention
)
{
	static const KeywordTab_t tab[] = { { WD_TRIM_DEFAULT, "DEFAULT", 0, WD_TRIM_M_ALL },

		{ WD_TRIM_NONE, "NONE", "-", WD_TRIM_ALL }, { WD_TRIM_ALL, "ALL", 0, WD_TRIM_ALL },
...
int ScanOptTrim (ccp arg // argument to scan
)
{
...
int ScanOptAlign (ccp p_arg)
{
	...
	while (arg && *arg && *align_ptr)
	{
		...
		*dest = 0;
		if (dest > iobuf
			&& ScanSizeOptU32 (*align_ptr, // u32 * num
				iobuf, // ccp source
				1, // default_factor1
				0, // int force_base
				"align", // ccp opt_name
		...
		{
			ERROR0 (ERR_SEMANTIC, "Option --align: Ascending order expected: %.*s\n",
	...
	if (*arg)
	{
		ERROR0 (ERR_SEMANTIC, "Option --align: End of parameter expected: %.20s\n", arg);
		return 1;
	}
...
{
	return ScanSizeOptU32 (&opt_align_part, // u32 * num
			   arg, // ccp source
			   1, // default_factor1
			   0, // int force_base
...
{
	return ScanSizeOptU64 (&opt_disc_size, arg, GiB, 0, "disc-size", 0,
			   WII_SECTOR_SIZE * (u64)WII_MAX_SECTORS, WII_SECTOR_SIZE, 0, true)
		!= ERR_OK;
}
...
static uint security_patch (AnalyzeFile_t *af, FILE *log)
{
	...
	static const u8 opCodeChainP2P_v1[22] = { 32, 32, 21, 21, 21, 21, 20, 20, 31, 40, 21, 20, 20,
		31, 31, 10, 20, 36, 21, 44, 36, 16 };
	static const u8 opCodeChainP2P_v2[22] = { 32, 32, 21, 21, 20, 21, 20, 21, 31, 40, 21, 20, 20,
		31, 31, 10, 20, 36, 21, 44, 36, 16 };

	// opcode lists for MASTER:
	static const u8 opCodeChainMASTER_v1[22] = { 21, 21, 21, 21, 40, 20, 20, 20, 20, 31, 31, 14, 31,
		20, 21, 44, 21, 36, 36, 18, 11, 16 };
	static const u8 opCodeChainMASTER_v2[22] = { 21, 21, 21, 21, 40, 20, 20, 20, 20, 31, 31, 14, 31,
		20, 21, 36, 21, 44, 36, 18, 11, 16 };
	...
	for (u8 *cur = dol_start; cur < dol_end; cur += 4)
	{
		if (!memcmp (cur, &gt2locator, 8))
		{
			...
			if (found_opcode_chain_P2P_v1 || found_opcode_chain_P2P_v2)
			{
				...
				if (get_immediate_value (cur + 0x0c) == 0x0c
					...
					&& get_immediate_value (cur + 0x60) == 0x18)
				{
					if (log)
					{
						...
						fprintf (log, "PATCH:  P2P[%c%c] %#08zx : %s\n",
							...
							found_opcode_chain_P2P_v2 ? '2' : '-', cur - dol_head, fname);
					}
					...
					if (af->patch_dol)
					{
						...
						{
							write_be32 (
								cur + 0x14, 0x88010011 | reg_cmp << 21); // lbz reg_cmp, 0x11(r1)
							write_be32 (
								cur + 0x18, 0x28000080 | reg_cmp << 16); // cmplwi reg_cmp, 0x80
							write_be32 (cur + 0x24, 0x41810064); // bgt- +0x64
							write_be32 (cur + 0x28, 0x60000000); // nop
							write_be32 (cur + 0x2c, 0x60000000); // nop
							write_be32 (
								cur + 0x34, 0x3C005A00 | reg_cmp << 21); // lis reg_cmp, 0x5a00
							write_be32 (cur + 0x48,
								0x7C000000 | reg_cmp << 16
						...
						else
						{
							...
							write_be32 (cur + 0x34, 0x7c000000 | reg_cmp << 16 | reg_load << 11);
			...
			else if (found_opcode_chain_MASTER_v1 || found_opcode_chain_MASTER_v2)
			{
				...
				if (get_immediate_value (MASTERopcodeChain + 0x10) == 0x12
					...
					&& (v3c == 0x12 && v44 == 0x0c || v3c == 0x0c && v44 == 0x12))
				{
					if (log)
					{
						...
						fprintf (log, "PATCH:  MAS[%c%c] %#08zx : %s\n",
							found_opcode_chain_MASTER_v1 ? '1' : '-',
							found_opcode_chain_MASTER_v2 ? '2' : '-', MASTERopcodeChain - dol_head,
							fname);
					}
					...
					if (af->patch_dol)
					{
						...
						write_be32 (p + 0x04,
							0x7c00042c | rY << 21 | 3 << 16 | rX << 11); // lwbrx rY, r3, rX;
						write_be32 (p + 0x14, 0x9000000c | rY << 21 | 1 << 16); // stw rY, 0xc(r1);
						write_be32 (p + 0x18, 0x88000011 | rY << 21 | 1 << 16); // lbz rY, 0x11(r1);
	...
	{
		log_count++;
		fprintf (log, "FAIL: %12d %2d %2d : %s\n", hasGT2Error, af->pstat_main.n_p2p,
			af->pstat_main.n_master, fname);
	}
...
void AnalyzeFile (AnalyzeFile_t *af)
{
	...
	if (af->dol)
	{
		...
		{
			PatchDomain (
				true, (u8 *)af->dol_patch, af->dol_load_size, 0, af->patch_dol, &af->pstat_main);
...
enumError RewriteModifiedSF (SuperFile_t *fi, // valid input file
...
void SetupWritePatch (WritePatch_t *pat // patch data structure
)
{
	DASSERT (pat);
...
enumError CloseWritePatch (WritePatch_t *pat // patch data structure
)
{
	DASSERT (pat);
	...
	if (pat->file)
	{
		...
		if (!err && fwrite (&thead, sizeof (thead), 1, pat->file) != 1)
			err = ERROR1 (ERR_WRITE_FAILED, "Writing patch toc header failed: %s", pat->fname);
...
enumError CreateWritePatch (WritePatch_t *pat, // patch data structure
...
)
{
	...
	if (!f)
		return ERROR1 (ERR_CANT_CREATE, "Can't create patch file: %s\n", filename);
...
enumError CreateWritePatchF (WritePatch_t *pat, // patch data structure
...
)
{
	...
	if (fwrite (&hd, sizeof (hd), 1, pat->file) != 1)
		return ERROR1 (ERR_WRITE_FAILED, "Writing patch header failed: %s", pat->fname);

	return WritePatchComment (
		pat, "%s", "Creator: " TOOLSET_SHORT " v" VERSION " r" REVISION " " SYSTEM2);
}

///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////

enumError WritePatchComment (WritePatch_t *pat, // patch data structure
...
void SetupReadPatch (ReadPatch_t *pat // patch data structure
)
{
	DASSERT (pat);
	memset (pat, 0, sizeof (*pat));
...
enumError CloseReadPatch (ReadPatch_t *pat // patch data structure
)
{
	DASSERT (pat);
...
enumError OpenReadPatch (ReadPatch_t *pat, // patch data structure
...
)
{
	...
	if (!f)
		return ERROR1 (ERR_CANT_OPEN, "Can't open patch file: %s\n", filename);
...
enumError OpenReadPatchF (ReadPatch_t *pat, // patch data structure
...
enumError SupportedVersionReadPatch (ReadPatch_t *pat, // patch data structure
	bool silent // true: suppress error message
)
{
	DASSERT (pat);
	pat->is_compatible
		= WIT_PATCH_VERSION >= pat->compatible && pat->version >= WIT_PATCH_READ_COMPATIBLE;
	if (!pat->is_compatible)
	{
		...
		if (WIT_PATCH_READ_COMPATIBLE < WIT_PATCH_VERSION)
			return ERROR0 (ERR_INVALID_VERSION,
				...
				PrintVersion (0, 0, pat->version), PrintVersion (0, 0, WIT_PATCH_READ_COMPATIBLE),
				PrintVersion (0, 0, WIT_PATCH_VERSION), pat->fname);

		return ERROR0 (ERR_INVALID_VERSION, "Patch version %s not supported (%s expected): %s\n",
			PrintVersion (0, 0, pat->version), PrintVersion (0, 0, WIT_PATCH_VERSION), pat->fname);
	}
...
enumError GetNextReadPatch (ReadPatch_t *pat // patch data structure
)
{
	DASSERT (pat);
