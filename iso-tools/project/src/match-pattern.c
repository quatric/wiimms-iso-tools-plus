...
static const struct macro_tab_t macro_tab[] = { { 4, "base", "+/*$" }, { 6, "nobase", "-/*$" },
	{ 4, "disc", "+/disc/" }, { 6, "nodisc", "-/disc/" }, { 3, "sys", "+/sys/" },
	{ 5, "nosys", "-/sys/" }, { 5, "files", "+/files/" }, { 7, "nofiles", "-/files/" },
...
int AddFilePattern (ccp arg, int pattern_index)
{
	...
	while (*arg)
	{
		...
		if (!ok && *arg != '+' && *arg != '-' && *arg != ':' && *arg != '=')
			return ERROR0 (
				ERR_SYNTAX, "File pattern rule must begin with '+', '-' or ':' => %.20s\n", arg);
		...
		if (*start == ':' || *start == '=')
		{
			...
			if (!tab->len)
			{
				if (!strcmp (start, "negate"))
				{
					...
					return ERROR0 (
						ERR_SYNTAX, "Macro '%.*s' not found: :%.20s\n", len, start, start);
			}
		}
		else
		{
...
bool SetupFilePattern (FilePattern_t *pat)
{
	...
	if (pat->is_dirty)
	{
		...
		else
		{
			...
			ASSERT (first);
			if (!strcmp (first, "+") || !strcmp (first, "+*") || !strcmp (first, "+**"))
			{
				pat->match_all = true;
			}
			else if (!strcmp (first, "-") || !strcmp (first, "-*") || !strcmp (first, "-**"))
			{
				pat->match_none = true;
			}
		}
#ifdef DEBUG
		TRACE ("FILE PATTERN: N=%u, all=%d, none=%d\n", pat->rules.used, pat->match_all,
			pat->match_none);
...
bool MatchFilePattern (FilePattern_t *pat, // filter rules
...
int MatchFilePatternFST (struct wd_iterator_t *it // iterator struct with all infos
)
{
	DASSERT (it);
	// result>0: ignore this file
	return it->icm >= WD_ICM_DIRECTORY && !MatchFilePattern (it->param, it->fst_name, '/');
}
