...
static enumError read_src (wiiu_src_t *src, void *buf, size_t count)
{
	...
	{
		if (fseeko (src->f, (off_t)src->offset, SEEK_SET) || fread (buf, 1, count, src->f) != count)
...
enumError XInfoWiiU (ccp source, xformat_t format)
{
	...
	if (src.is_wux)
	{
		...
		printf ("      WUX sectors: %u of %u stored (0x%x bytes each)\n",
			src.wux.n_stored ? src.wux.n_stored : 0, src.wux.n_index, src.wux.sector_size);
	}
...
enumError XConvertWiiU (ccp source, // source container
...
)
{
	...
	if (dest_wux)
	{
		...
		// equivalent of it.
		err = OpenWriteWUX (&wux, out, dest, src.image_size, src.is_wux ? src.wux.sector_size : 0);
		if (err)
			goto abort;
	}
...
#define WIIU_COMMON_KEY "\xd7\xb0\x04\x02\x65\x9b\xa2\xab\xd2\xcb\x0d\xb2\x7f\xa2\xb6\x56"
#define WIIU_COMMON_DEV_KEY "\x2f\x5c\x1b\x29\x44\xe7\xfd\x6f\xc3\x97\x96\x4b\x05\x76\x91\xfa"
...
static enumError load_disc_key (ccp source, u8 key[16])
{
	...
	if (i < 16)
		return ERROR0 (
			ERR_INVALID_DATA, "Disc key for %s is not 32 valid hex characters.\n", source);
...
static enumError decrypt_partition (wiiu_src_t *src, // open source image
...
static enumError read_partition_table (
	wiiu_src_t *src, const aes_key_t *akey, wiiu_part_t **part_list, uint *n_part)
{
	*part_list = 0;
	*n_part = 0;
...
static enumError parse_fst (u8 *data, uint size, wiiu_fst_t *fst)
{
	...
	if (!n_entry || n_entry > 0x100000 || (u64)fentry_off + (u64)n_entry * 0x10 > size)
...
typedef void (*wiiu_fst_visit_cb) (
	void *ctx, const wiiu_fst_t *fst, uint entry_idx, ccp full_path, uint depth);
...
static enumError extract_content_hashed (wiiu_src_t *src, u64 content_abs_offset, u64 file_offset,
	u64 size, const aes_key_t *title_akey, u16 content_id, wiiu_sink_t *sink)
{
	...
	while (size > 0 && !err)
	{
		...
		{
			err = ERROR0 (
				ERR_INVALID_DATA, "H0 hash mismatch decrypting content id %u\n", content_id);
			break;
		}
...
static enumError extract_content_simple (wiiu_src_t *src, u64 content_abs_offset, u64 file_offset,
	u64 size, const aes_key_t *title_akey, u16 content_id, wiiu_sink_t *sink)
{
	enum
	{
...
static enumError read_disc_key_region (
	wiiu_src_t *src, u64 abs_offset, u64 size, const aes_key_t *akey,
...
)
{
	...
	for (u64 c = 0; c < n_cluster; c++)
		wd_aes_decrypt (akey, zero_iv, buf + c * WIIU_SECTOR_SIZE, plain + c * WIIU_SECTOR_SIZE,
			WIIU_SECTOR_SIZE);
	FREE (buf);
	...
	wiiu_src_t *src, u64 part_offset, u32 *header_size, u32 *fst_size)
{
	...
	if (be32 (hdr) != WIIU_PART_HEADER_MAGIC)
		return ERROR0 (ERR_INVALID_DATA, "Bad partition header magic at 0x%llx\n", part_offset);
...
static enumError read_disc_key_fst (wiiu_src_t *src, const aes_key_t *akey, u64 part_offset,
	u32 header_size, u32 fst_size, wiiu_fst_t *fst)
{
...
static enumError read_gm_fst (wiiu_src_t *src, const aes_key_t *title_akey, u64 part_offset,
	u32 header_size,
...
static enumError derive_title_key (
	const u8 *tmd, uint tmd_len, const u8 *tik, uint tik_len, u8 title_key[16])
{
...
static enumError decrypt_fst_file (wiiu_src_t *src, const aes_key_t *akey, u64 part_offset,
	u32 header_size, const wiiu_fst_t *fst, const wiiu_fentry_t *e, u8 **out, uint *out_len)
{
...
static void si_visit (void *vctx, const wiiu_fst_t *fst, uint idx, ccp full_path, uint depth)
{
	...
	ctx->err = decrypt_fst_file (
		ctx->src, ctx->akey, ctx->part_offset, ctx->header_size, fst, e, dst_buf, dst_len);
}
...
static void gm_visit (void *vctx, const wiiu_fst_t *fst, uint idx, ccp full_path, uint depth)
{
	...
	{
		ctx->err
			= ERROR0 (ERR_INVALID_DATA, "Bad content id %u for %s\n", e->content_id, full_path);
		return;
	}
	...
	if (!tc)
	{
		ctx->err = ERROR0 (ERR_INVALID_DATA, "No TMD content with index %u (needed by %s)\n",
			e->content_id, full_path);
		return;
	}
	...
	if (verbose >= 0)
		printf ("    %-48s %10u bytes (cid=%u%s)\n", full_path, e->b, e->content_id,
			hashed ? " hashed" : "");

	ctx->err = hashed ? extract_content_hashed (ctx->src, content_base, file_off, e->b,
							ctx->title_akey, e->content_id, &sink)
					  : extract_content_simple (ctx->src, content_base, file_off, e->b,
							ctx->title_akey, e->content_id, &sink);
...
enumError XExtractWiiU (ccp source, xformat_t format, ccp dest)
{
	...
	for (uint i = 0; i < n_part && !err; i++)
	{
		...
		if (!strncasecmp (p->name, "GM", 2))
		{
			...
			{
				printf (
					"  partition %-4s : skipped - no matching ticket found in the SI partition\n",
					p->name);
				continue;
			}

			u8 title_key[16];
			err = derive_title_key (
				match->tmd, match->tmd_len, match->tik, match->tik_len, title_key);
			if (err)
				break;
