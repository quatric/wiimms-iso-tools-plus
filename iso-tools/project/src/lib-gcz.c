...
int ScanOptGCZBlock (ccp arg)
{
	...
	u32 block_size;
	enumError stat = ScanSizeOptU32 (&block_size, // u32 * num
						 arg, // ccp source
						 1, // default_factor1
						 0, // int force_base
	...
	WFile_t *f, uint block, u32 adler32, const void *data, uint data_size)
{
	...
	if (!f->disable_errors)
		ERROR0 (ERR_GCZ_INVALID, "Wrong checksum for block block #%u (need=%08x,have=%08x]: %s\n",
...
)
{
	...
	if (file_size)
	{
		const u64 total = sizeof (GCZ_Head_t) + 12 * head->num_blocks + head->num_blocks;
		if (total > file_size)
			return 0;
	}
...
)
{
	...
	const enumFileType ftype
		= AnalyzeGCZ (&gcz->head, sizeof (gcz->head), f->st.st_size, &gcz->head);
...
)
{
	...
	while (count)
	{
		...
		else if (block != gcz->block)
		{
			...
			u32 read_size = (block == gcz->head.num_blocks - 1 ? gcz->head.compr_size
															   : le64 (gcz->offset + block + 1))
				- read_off;
			...
			if (read_size > gcz->head.block_size)
			{
				if (!f->disable_errors)
					ERROR0 (ERR_GCZ_INVALID,
						...
						gcz->head.num_blocks, read_size, f->fname);
				return ERR_GCZ_INVALID;
			}
			...
			if (read_off < 0)
			{
				...
				noPRINT ("GCZ/RAW: b=%x, read=%llx + %x, copy=%x +%x\n", block, read_off, read_size,
					copy_delta, copy_size);
				enumError err = ReadAtF (f, read_off, gcz->data, read_size);
				if (!err)
					err = CheckAdler32 (
						f, block, le32 (gcz->checksum + block), gcz->data, read_size);
				if (err)
					return err;
			}
			else
			{
				...
				noPRINT ("GCZ/UNZIP: b=%x, read=%llx + %x, copy=%x +%x\n", block, read_off,
					read_size, copy_delta, copy_size);
				enumError err = ReadAtF (f, read_off, gcz->cdata, read_size);
				if (!err)
					err = CheckAdler32 (
						f, block, le32 (gcz->checksum + block), gcz->cdata, read_size);
				if (err)
					return err;
				...
				if (stat < 0)
					goto inflate_err;
				PRINT ("block %u, read=%x, cksum %08x %08x\n", block, read_size, (u32)zs.adler,
					le32 (gcz->checksum + block));
			}
			if (read_size < gcz->head.block_size)
...
)
{
	...
	DASSERT (sf->gcz);
	TRACE ("#G# -----\n");
	PRINT (TRACE_RDWR_FORMAT, "#G# ReadGCZ()", GetFD (&sf->f), GetFP (&sf->f), (u64)off,
		(u64)off + count, count, "");
...
static enumError write_block (struct SuperFile_t *sf, // file
...
)
{
	...
	if (gcz->block >= gcz->head.num_blocks)
	{
		if (!sf->f.disable_errors)
			ERROR0 (ERR_WRITE_FAILED, "GCZ files can't grow (block %u/%u): %s\n", gcz->block,
				gcz->head.num_blocks, sf->f.fname);
	...
	else
	{
		...
		if (try_zip)
		{
			...
			if (deflateInit (&zs, 9) == Z_OK && deflate (&zs, Z_FINISH) == Z_STREAM_END
	...
	noPRINT ("WRITE BLOCK #%d [raw=%d,z=%d,%d,size=%x]\n", gcz->block, flag != 0,
		data == gcz->zero_data, is_zero, size);
...
static enumError flush_block (struct SuperFile_t *sf, // file
...
)
{
	...
	if (!IS_M1 (gcz->block))
	{
		...
		{
			if (!sf->f.disable_errors)
				ERROR0 (ERR_WRITE_FAILED, "GCZ file must be created sequential: %s\n", sf->f.fname);
			return ERR_WRITE_FAILED;
		}
...
)
{
	...
	if (!image_size)
	{
		...
		if (!image_size)
		{
			...
			if (!image_size)
				return ERROR0 (ERR_INTERNAL, "%p %llu %llu %llu", sf->src,
					sf->src ? sf->src->file_size : 0ull, sf->source_size, sf->file_size);
		}
	}
	...
	gcz->head.num_blocks = (image_size + gcz->head.block_size - 1) / gcz->head.block_size;
...
)
{
	...
	return err ? err : WriteAtF (&sf->f, sizeof (hd), gcz->offset, gcz->head.num_blocks * 12);
...
)
{
	...
	TRACE ("#G# -----\n");
	noPRINT (TRACE_RDWR_FORMAT, "#G# WriteGCZ()", GetFD (&sf->f), GetFP (&sf->f), (u64)off,
		(u64)off + count, count, "");
