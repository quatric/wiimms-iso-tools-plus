		...
		else if (!strcasecmp (n->name, "options"))
		{
			for (const xml_node_t *snode = n->first_child; snode; snode = snode->next_sibling)
			{
				if (!strcasecmp (snode->name, "section"))
				{
					...
					for (const xml_node_t *onode = snode->first_child; onode;
						onode = onode->next_sibling)
					{
						if (!strcasecmp (onode->name, "option"))
						{
							...
							for (const xml_node_t *cnode = onode->first_child; cnode;
								cnode = cnode->next_sibling)
							{
								if (!strcasecmp (cnode->name, "choice"))
								{
									...
									for (const xml_node_t *pnode = cnode->first_child; pnode;
										pnode = pnode->next_sibling)
									{
										if (!strcasecmp (pnode->name, "patch"))
										{
											riiv_patch_ref_t *pref
												= CALLOC (1, sizeof (riiv_patch_ref_t));
						...
						for (riiv_section_t *sec = doc->sections; sec; sec = sec->next)
						{
							for (riiv_option_t *opt = sec->options; opt; opt = opt->next)
							{
								if (opt->id && !strcasecmp (opt->id, macro_id))
								{
									...
									for (riiv_choice_t *src_ch = opt->choices; src_ch;
										src_ch = src_ch->next)
									{
										...
										for (riiv_patch_ref_t *src_pref = src_ch->patch_refs;
											src_pref; src_pref = src_pref->next)
										{
											riiv_patch_ref_t *new_pref
												= CALLOC (1, sizeof (riiv_patch_ref_t));
...
static void select_option_choice (riiv_option_t *opt, const char *choice_str)
{
	...
	while (*choice_str && *choice_str <= ' ')
		choice_str++;
	if (!strcasecmp (choice_str, "none") || !strcasecmp (choice_str, "disabled")
		|| !strcmp (choice_str, "0"))
	{
			...
			if (eq)
			{
				*eq = '\0';
				...
				for (riiv_section_t *s = doc->sections; s; s = s->next)
				{
					...
					{
						if ((o->id && !strcasecmp (o->id, opt_name))
							|| !strcasecmp (o->name, opt_name))
						{
...
static void prompt_interactive_choices (
	riiv_doc_t *doc, RiivolutionOptions_t *opt, char *id6, char *game_title, size_t title_size)
{
	if (!doc)
		return;
	...
	if (doc->sections)
	{
		for (riiv_section_t *sec = doc->sections; sec; sec = sec->next)
		{
			...
			for (riiv_option_t *op = sec->options; op; op = op->next)
			{
				...
				{
					printf ("  [%u] %s%s\n", idx, ch->name,
						(idx == (uint)op->selected_choice) ? " (Current)" : "");
				}
				...
				if (fgets (line, sizeof (line), stdin))
				{
					...
					while (*p && (unsigned char)*p <= ' ')
						p++;
					char *end = p + strlen (p);
					while (end > p && (unsigned char)*(end - 1) <= ' ')
						*(--end) = '\0';
	...
	ccp cur_title
		= opt->custom_name ? opt->custom_name : (game_title[0] ? game_title : "Modded Game");
...
static active_patch_t *resolve_active_patches (
	riiv_doc_t *doc, const RiivolutionOptions_t *opt, const char *game_id6)
{
	(void)opt;
...
enumError RiivolutionInspect (riiv_doc_t *doc, int verbose)
{
	...
	printf ("  Game Filter:   Game=%s, Dev=%s, Disc=%d, Ver=%d\n",
		...
		doc->filter_developer ? doc->filter_developer : "Any", doc->filter_disc,
		doc->filter_version);
	...
	if (doc->filter_regions.used)
	{
		...
		for (uint i = 0; i < doc->filter_regions.used; i++)
			printf ("%s%s", doc->filter_regions.field[i],
				(i + 1 < doc->filter_regions.used) ? ", " : "");
		putchar ('\n');
	}
		...
		for (riiv_section_t *sec = doc->sections; sec; sec = sec->next)
		{
			...
			for (riiv_option_t *o = sec->options; o; o = o->next)
			{
				printf ("    Option: \"%s\"%s%s%s (Default: %u)\n", o->name, o->id ? " [id=" : "",
					o->id ? o->id : "", o->id ? "]" : "", o->default_choice);
				uint c_idx = 1;
				for (riiv_choice_t *ch = o->choices; ch; ch = ch->next, c_idx++)
				{
	...
	if (doc->patches)
	{
		...
		for (riiv_patch_t *p = doc->patches; p; p = p->next)
		{
			...
			printf ("  Patch: \"%s\" (root=\"%s\") -> Files: %u, Folders: %u, Memory: %u, "
					"Savegame: %u\n",
			...
			{
				for (riiv_file_t *f = p->files; f; f = f->next)
					printf ("    FILE:   ext=\"%s\" -> disc=\"%s\" (resize=%d, create=%d, "
							"off=0x%x, foff=0x%x, len=%u)\n",
						f->external, f->disc, f->resize, f->create, f->offset, f->fileoffset,
						f->length);
				for (riiv_folder_t *fo = p->folders; fo; fo = fo->next)
					printf (
						"    FOLDER: ext=\"%s\" -> disc=\"%s\" (resize=%d, create=%d, rec=%d)\n",
						fo->external, fo->disc, fo->resize, fo->create, fo->recursive);
				for (riiv_memory_t *m = p->memories; m; m = m->next)
					printf ("    MEMORY: addr=0x%08X (val_bytes=%u, vfile=%s, orig_bytes=%u, "
							"search=%d)\n",
						m->offset, m->value_len, m->valuefile ? m->valuefile : "none",
						m->original_len, m->search);
				for (riiv_savegame_t *s = p->savegames; s; s = s->next)
...
static bool resolve_case_insensitive_path (
	const char *base, const char *rel, char *out, size_t out_size)
{
...
static bool find_file_in_tree (
	const char *dir, const char *target_filename, char *out, size_t out_size)
{
	DIR *d = opendir (dir);
	if (!d)
		return false;
...
static bool resolve_external_path (const active_patch_t *ap, const char *sd_root,
	const char *xml_dir, const char *external_raw, char *out_path, size_t out_size)
{
	...
	bool create, bool resize, u32 offset, u32 fileoffset, u32 length, int verbose)
{
	...
	{
		size_t chunk
			= (to_copy - copied) > sizeof (buf) ? sizeof (buf) : (size_t)(to_copy - copied);
	...
	if (verbose >= 1)
		printf ("  + Patched %llu bytes into %s at offset 0x%x\n", (unsigned long long)copied,
			dst_disc_file, offset);
	return ERR_OK;
}

static enumError apply_folder_patch_recursive (const char *src_dir, const char *dst_dir,
	const char *files_dir, bool create, bool resize, bool recursive, u32 length, int verbose)
{
	...
	while ((ent = readdir (d)) != NULL)
	{
		...
		if (S_ISDIR (st.st_mode))
		{
			if (recursive)
			{
				...
				apply_folder_patch_recursive (
					child_src, child_dst, files_dir, create, resize, recursive, length, verbose);
			}
		}
	...
	const char *sd_root, const char *xml_dir, int verbose)
{
	...
	for (active_patch_t *ap = active_patches; ap; ap = ap->next)
	{
		for (riiv_memory_t *m = ap->patch->memories; m; m = m->next)
		{
			...
			{
				char vpath[PATH_MAX];
				if (resolve_external_path (
						ap, sd_root, xml_dir, m->valuefile, vpath, sizeof (vpath)))
				{
					size_t v_size = 0;
					if (LoadFileAlloc (vpath, 0, 0, &payload, &v_size, 32 * MiB, 0, 0, false)
						== ERR_OK)
					{
			...
			// Ignore Return-to-Riivolution channel launcher hooks (Title ID 00010001-RIIV /
			// 0x52494956) which break standalone ISO/WBFS execution by hijacking _OSBootDOL
			if (m->offset == 0x80048C10 || (m->offset >= 0x80003400 && m->offset <= 0x80003430))
			{
				if (verbose >= 1)
					printf ("  - Skipping Return-to-Riivolution channel hook at 0x%08X (standalone "
							"disc)\n",
						m->offset);
				skipped_count++;
				if (free_payload)
				...
				for (int s = 0; s < DOL_MAX_SECT; s++)
				{
					...
					for (u32 off = 0; off <= sect_limit; off += align)
					{
						if (!memcmp (sect_data + off, m->original, m->original_len))
						{
							...
							if (verbose >= 1)
								printf ("  + Matched search pattern at 0x%08X (DOL off 0x%llX), "
										"wrote %u bytes\n",
									match_addr, (unsigned long long)foff, payload_len);

							if (m->ocarina && m->offset > 0)
							{
								for (u32 b_off = off + payload_len; b_off + 4 <= ctx.sect_size[s];
									b_off += 4)
								{
									if (be32 (sect_data + b_off) == 0x4e800020)
									{
										...
										if (verbose >= 1)
											printf ("  + Patched Ocarina hook at 0x%08X -> branch "
													"to 0x%08X (0x%08X)\n",
			...
			else
			{
				...
				if (foff >= 0)
				{
					if (m->original && m->original_len > 0)
					{
						...
						{
							if (verbose >= 2)
								printf ("  - Original bytes mismatch at 0x%08X (DOL off 0x%llX), "
										"skipping region-specific patch\n",
					...
					if (foff + payload_len <= (s64)ctx.size)
					{
						...
						if (verbose >= 1)
							printf ("  + Patched 0x%08X (DOL off 0x%llX) with %u bytes\n", addr,
								(unsigned long long)foff, payload_len);
					}
					else
					{
						skipped_count++;
				...
				else
				{
					...
					if (extend_idx >= 0)
					{
						...
						applied_count++;
						if (verbose >= 1)
							printf ("  + Extended DOL section #%d to 0x%08X (wrote %u bytes at off "
									"0x%X)\n",
					...
					if (empty_idx < 0)
					{
						if (verbose >= 1)
							printf (
								"  ! No empty DOL section available for address 0x%08X\n", addr);
						skipped_count++;
						if (free_payload)
							FREE (payload);
						continue;
					...
					if (verbose >= 1)
						printf ("  + Created new DOL section #%d at 0x%08X (file off 0x%X, size %u "
								"bytes)\n",
...
static enumError inject_gct_into_dol (const char *dol_path, const char *gct_path, int verbose)
{
	...
	char cmd[PATH_MAX * 3];
	snprintf (cmd, sizeof (cmd), "wstrt patch \"%s\" --add-section \"%s\" %s 2>&1", dol_path,
		gct_path, verbose >= 2 ? "-vv" : (verbose >= 1 ? "-v" : "-q"));
	...
	if (ret != 0)
	{
		const char *wstrt_paths[] = { "/Users/larsen/wiimms-szs-tools-nintendo/project/bin/wstrt",
			"/usr/local/bin/wstrt", "/opt/homebrew/bin/wstrt", NULL };
		for (int i = 0; wstrt_paths[i]; i++)
		{
			if (stat (wstrt_paths[i], &st) == 0 && (st.st_mode & S_IXUSR))
			{
				snprintf (cmd, sizeof (cmd), "\"%s\" patch \"%s\" --add-section \"%s\" %s 2>&1",
					wstrt_paths[i], dol_path, gct_path,
					verbose >= 2 ? "-vv" : (verbose >= 1 ? "-v" : "-q"));
				ret = system (cmd);
				if (ret == 0)
					break;
			}
...
static void determine_sd_root (
	const RiivolutionOptions_t *opt, const char *xml_dir, char *out_root, size_t out_size)
{
...
enumError RiivolutionBuild (RiivolutionOptions_t *opt)
{
	...
	else
	{
		...
		if (!disc)
		{
			...
			RiivolutionFreeDoc (doc);
			return ERROR0 (
				ERR_CANT_OPEN, "Cannot read disc header from image: %s\n", opt->source_image);
		}
	...
	if (!opt->ignore_regions && id6[0])
	{
		if (doc->filter_game && strncasecmp (id6, doc->filter_game, strlen (doc->filter_game)) != 0)
		{
			...
			return ERROR0 (ERR_WRONG_FILE_TYPE,
				"Game ID mismatch: base game '%s' does not match XML filter '%s' (use "
				"--ignore-regions to override).\n",
				id6, doc->filter_game);
		}
		...
		if (doc->filter_regions.used > 0)
		{
			...
			if (!reg_matched)
			{
				...
				RiivolutionFreeDoc (doc);
				return ERROR0 (ERR_WRONG_FILE_TYPE,
					"Region mismatch: game region '%c' not in XML supported regions (use "
					"--ignore-regions to override).\n",
					id6[3]);
			}
		}
	...
	if (opt->testmode)
	{
		...
		printf ("\nTest mode passed: mod XML, game ID, and patches verified without making "
				"modifications.\n");
		ResetSF (&sfi, 0);
	...
	if (source_is_dir)
	{
			...
			|| opt->dest_path[strlen (opt->dest_path) - 1] == '/')
		{
			...
			apply_folder_patch_recursive (
				opt->source_image, work_dir, work_dir, true, true, true, 0, opt->verbose);
		}
		else
		{
			StringCopyS (work_dir, sizeof (work_dir), opt->source_image);
	...
	else
	{
		...
		if (!mkdtemp (temp_dir))
		{
			...
			RiivolutionFreeDoc (doc);
			return ERROR0 (
				ERR_CANT_CREATE, "Failed to create temporary directory for extraction.\n");
		}
		is_temp = true;
	...
	for (active_patch_t *ap = active_patches; ap; ap = ap->next)
	{
		...
		for (riiv_file_t *f = ap->patch->files; f; f = f->next)
		{
			...
			if (!resolve_external_path (
					ap, sd_root, xml_dir, f->external, src_path, sizeof (src_path)))
			{
				if (opt->verbose >= 1)
			...
			bool is_dol = (!strcasecmp (subst_disc, "main.dol")
				|| !strcasecmp (subst_disc, "/main.dol") || !strcasecmp (subst_disc, "sys/main.dol")
			...
			apply_file_patch_direct (src_path, dst_path, f->create, f->resize, f->offset,
				f->fileoffset, f->length, opt->verbose);
		}
		...
		for (riiv_folder_t *fold = ap->patch->folders; fold; fold = fold->next)
		{
			...
			if (!resolve_external_path (
					ap, sd_root, xml_dir, fold->external, src_dir, sizeof (src_dir)))
			{
				if (opt->verbose >= 1)
			...
			FREE (subst_disc);
			apply_folder_patch_recursive (src_dir, dst_dir, files_dir, fold->create, fold->resize,
				fold->recursive, fold->length, opt->verbose);
		}
	}
	...
	if (out_oft == OFT_FST)
	{
		if (strcmp (work_dir, final_dest))
		{
			mkdir_p (final_dest);
			apply_folder_patch_recursive (
				work_dir, final_dest, final_dest, true, true, true, 0, opt->verbose);
		}
		if (opt->verbose >= 0)
