...
static enumError CloseHelperSF (SuperFile_t *sf, // file to close
	...
	else
	{
		...
		if (!err && sf->f.is_writing)
		{
			...
			if (sf->wbfs)
			{
				...
				if (!err)
					err = sf->wbfs->is_growing ? TruncateWBFS (sf->wbfs)
...
enumError Close2SF (SuperFile_t *sf, // file to close
	SuperFile_t *remove_sf, // not NULL & 'sf' finished without error:
							// close & remove it before renaming 'sf'
...
enumError SetupReadWBFS (SuperFile_t *sf)
{
	...
	err = sf->f.slot >= 0 ? OpenWDiscSlot (wbfs, sf->f.slot, false)
	...
	snprintf (iobuf, sizeof (iobuf), "%s [%s]", GetTitle (sf->f.id6_dest, (ccp)dh->disc_title),
		sf->f.id6_dest);
...
enumError OpenSF (SuperFile_t *sf, ccp fname, bool allow_non_iso, bool open_modify)
{
	ASSERT (sf);
	CloseSF (sf, 0);
	...
	return allow_non_iso ? SetupReadSF (sf) : SetupReadISO (sf);
}

///////////////////////////////////////////////////////////////////////////////

enumError CreateSF (SuperFile_t *sf, // file to setup
...
enumError PreallocateSF (SuperFile_t *sf, // file to operate
...
enumError SetupWriteSF (SuperFile_t *sf, // file to setup
...
)
{
	...
	switch (sf->iod.oft)
	{
		...
		default:
			return ERROR0 (ERR_INTERNAL, "Unknown output file format: %d\n", sf->iod.oft);
	}
}
...
static enumError ReadDiscWrapper (SuperFile_t *sf, off_t off, void *buf, size_t count)
{
	DASSERT (sf);
	DASSERT (sf->disc1);
...
void CloseDiscSF (SuperFile_t *sf // valid file pointer
)
{
	DASSERT (sf);
	wd_close_disc (sf->disc2);
...
wd_disc_t *OpenDiscSF (SuperFile_t *sf, // valid file pointer
...
)
{
	...
	const wd_modify_t modify = opt_modify & WD_MODIFY__AUTO ? WD_MODIFY__ALL : opt_modify;
	...
	else if (disable_patch_on_load <= 0)
	{
		...
		if (main_part)
		{
			...
			reloc |= wd_patch_part_id (
				main_part, modify, modify_disc_id, modify_boot_id, modify_ticket_id, modify_tmd_id);
			if (modify_name)
		...
		else if (modify_id && strlen (modify_id) > 3 && modify_id[3] != '.'
		...
		if (!reloc && disc->disc_type != WD_DT_GAMECUBE
	...
	if (SetupFilePattern (pat) && pat->rules.used
...
void SubstFileNameSF (SuperFile_t *fo, // output file
...
int SubstFileNameBuf (char *fname_buf, // output buf
...
)
{
	...
	return SubstFileName (fname_buf, fname_size, fi->f.id6_dest, disc_name,
		fi->f.path ? fi->f.path : fi->f.fname, fname, dest_arg, oft);
}

///////////////////////////////////////////////////////////////////////////////

int SubstFileName (char *fname_buf, // output buf
	size_t fname_size, // size of output buf
	ccp id6, // id6
...
)
{
	...
	char x_buf[1000];
	snprintf (x_buf, sizeof (x_buf), "%s [%s]%s", title, id6, oft_info[oft].ext1);
	...
	if (oft == OFT_WBFS)
	{
		snprintf (plus_buf, sizeof (plus_buf), "%s%s", id6, oft_info[oft].ext1);
		plus_name = plus_buf;
	}
	else
		plus_name = x_buf;

	SubstString_t subst_tab[] = { { 'i', 'I', 0, id6 }, { 'n', 'N', 0, disc_name },
		{ 't', 'T', 0, title }, { 'e', 'E', 0, oft_info[oft].ext1 + 1 }, { 'p', 'P', 1, src_path },
		{ 'f', 'F', 1, fname }, { 'g', 'G', 1, pure_name }, { 'h', 'H', 1, ext },
		{ 'x', 'X', 0, x_buf }, { 'y', 'Y', 0, y_buf }, { '+', '+', 0, plus_name },
		{ 0, 0, 0, 0 } };
...
enumError PatchSF (SuperFile_t *sf, // valid file pointer
...
)
{
	...
	if (disc && disc->reloc)
	{
		...
		for (idx = 0; idx < WII_MAX_SECTORS && !err; idx++, reloc++)
			if (*reloc & (WD_RELOC_F_PATCH | WD_RELOC_F_HASH) && !(*reloc & WD_RELOC_F_LAST))
			{
...
static enumError SparseHelperByte (SuperFile_t *sf, off_t off, const void *buf, size_t count,
	WriteFunc write_func, size_t min_chunk_size)
{
	...
	TRACE (TRACE_RDWR_FORMAT, "#SH# SparseHelperB()", GetFD (&sf->f), GetFP (&sf->f), (u64)off,
		(u64)off + count, count, "");
...
enumError SparseHelper (SuperFile_t *sf, off_t off, const void *buf, size_t count,
	WriteFunc write_func, size_t min_chunk_size)
{
	ASSERT (sf);
	ASSERT (write_func);

	TRACE (TRACE_RDWR_FORMAT, "#SH# SparseHelper()", GetFD (&sf->f), GetFP (&sf->f), (u64)off,
		(u64)off + count, count, "");
	...
	if (remaining_len)
	{
		...
		const enumError err
			= SparseHelperByte (sf, woff, end, remaining_len, write_func, min_chunk_size);
		if (err)
			return err;
	}
...
off_t DataBlockSF (SuperFile_t *sf, // valid file
...
off_t UnionDataBlockSF (SuperFile_t *sf1, // first file
...
uint GetFileMapSF (SuperFile_t *sf, // valid super files
...
enumError ReadSwitchSF (SuperFile_t *sf, off_t off, void *buf, size_t count)
{
	...
	switch (sf->iod.oft)
	{
		case OFT_WDF1:
		case OFT_WDF2:
			return ReadWDF (sf, off, buf, count);
		case OFT_WIA:
			return ReadWIA (sf, off, buf, count);
		case OFT_GCZ:
			return ReadGCZ (sf, off, buf, count);
		case OFT_CISO:
			return ReadCISO (sf, off, buf, count);
		case OFT_WBFS:
			return ReadWBFS (sf, off, buf, count);
		case OFT_FST:
			return ReadFST (sf, off, buf, count);
		default:
			return ReadISO (sf, off, buf, count);
	}
}
...
enumError WriteSwitchSF (SuperFile_t *sf, off_t off, const void *buf, size_t count)
{
	...
	{
		case OFT_PLAIN:
			return WriteISO (sf, off, buf, count);
		case OFT_WDF1:
		case OFT_WDF2:
			return WriteWDF (sf, off, buf, count);
		case OFT_WIA:
			return WriteWIA (sf, off, buf, count);
		case OFT_GCZ:
			return WriteGCZ (sf, off, buf, count);
		case OFT_CISO:
			return WriteCISO (sf, off, buf, count);
		case OFT_WBFS:
			return WriteWBFS (sf, off, buf, count);
		default:
			return ERROR0 (ERR_INTERNAL, 0);
	}
}
...
enumError WriteSparseSwitchSF (SuperFile_t *sf, off_t off, const void *buf, size_t count)
{
	...
	{
		case OFT_PLAIN:
			return WriteSparseISO (sf, off, buf, count);
		case OFT_WDF1:
		case OFT_WDF2:
			return WriteSparseWDF (sf, off, buf, count);
		case OFT_WIA:
			return WriteSparseWIA (sf, off, buf, count);
		case OFT_GCZ:
			return WriteGCZ (sf, off, buf, count);
		case OFT_CISO:
			return WriteSparseCISO (sf, off, buf, count);
		case OFT_WBFS:
			return WriteWBFS (sf, off, buf, count); // no sparse support
		default:
			return ERROR0 (ERR_INTERNAL, 0);
	}
}
...
enumError WriteZeroSwitchSF (SuperFile_t *sf, off_t off, size_t count)
{
	...
	{
		case OFT_PLAIN:
			return WriteZeroISO (sf, off, count);
		case OFT_WDF1:
		case OFT_WDF2:
			return WriteZeroWDF (sf, off, count);
		case OFT_WIA:
			return WriteZeroWIA (sf, off, count);
		case OFT_GCZ:
			return WriteZeroGCZ (sf, off, count);
		case OFT_CISO:
			return WriteZeroCISO (sf, off, count);
		case OFT_WBFS:
			return WriteZeroWBFS (sf, off, count);
		default:
			return ERROR0 (ERR_INTERNAL, 0);
	}
}
...
enumError ReadISO (SuperFile_t *sf, off_t off, void *buf, size_t count)
{
	...
	DASSERT_MSG (err || sf->f.cur_off == (off_t)-1 || sf->f.cur_off == off, "%llx : %llx\n",
...
off_t DataBlockStandard (SuperFile_t *sf, off_t off, size_t hint_align, off_t *block_size)
{
	if (block_size)
...
enumError SetSizeSF (SuperFile_t *sf, off_t off)
{
	...
	{
		case OFT_PLAIN:
			return opt_truncate ? ERR_OK : SetSizeF (&sf->f, off);
		case OFT_WDF1:
		case OFT_WDF2:
		case OFT_WIA:
		case OFT_GCZ:
...
{
	return opt_disc_size ? opt_disc_size
		: is_gc			 ? GC_DISC_SIZE
...
{
	TRACE (TRACE_RDWR_FORMAT, "#B# ReadWBFS()", GetFD (&sf->f), GetFP (&sf->f), (u64)off,
		(u64)off + count, count, "");
	...
	while (count > 0)
	{
		...
		if (wlba)
		{
			enumError err = ReadAtF (&sf->f, (off_t)w->wbfs_sec_sz * wlba + bl_off, buf, max_count);
			if (err)
				return err;
		}
		else
...
off_t DataBlockWBFS (SuperFile_t *sf, off_t off, size_t hint_align, off_t *block_size)
{
	DASSERT (sf->wbfs);
	DASSERT (sf->wbfs->disc);
	DASSERT (sf->wbfs->disc->header);
...
void FileMapWBFS (SuperFile_t *sf, FileMap_t *fm)
{
	...
	for (block = 0; block < w->n_wbfs_sec_per_disc; block++)
	{
		...
		if (src_block)
			AppendFileMap (fm, block_size * block, block_size * src_block, block_size);
	}
}
...
{
	TRACE (TRACE_RDWR_FORMAT, "#B# WriteWBFS()", GetFD (&sf->f), GetFP (&sf->f), (u64)off,
		(u64)off + count, count, "");
	...
	while (count > 0)
	{
		...
		if (bl >= w->n_wbfs_sec_per_disc)
		{
			...
			if (!sf->f.disable_errors)
				ERROR0 (ERR_WRITE_FAILED, "Can't write behind WBFS limit [%c=%d]: %s\n",
		...
		if (!wlba)
		{
			...
			if (wlba == WBFS_NO_BLOCK)
			{
				...
				if (sf->f.max_error < ERR_WRITE_FAILED)
					sf->f.max_error = ERR_WRITE_FAILED;
				if (!sf->f.disable_errors)
					ERROR0 (ERR_WRITE_FAILED, "Can't allocate a free WBFS block [%c=%d]: %s\n",
						GetFT (&sf->f), GetFD (&sf->f), sf->f.fname);
				return ERR_WRITE_FAILED;
			}
		...
		enumError err = WriteAtF (&sf->f, (off_t)w->wbfs_sec_sz * wlba + bl_off, buf, max_count);
		if (err)
			return err;
...
int WrapperWriteSF (void *p_sf, u32 lba, u32 count, void *iobuf)
{
	...
	return sf->iod.write_func (sf, (off_t)lba * WII_SECTOR_SIZE, iobuf, count * WII_SECTOR_SIZE);
}
...
int WrapperWriteSparseSF (void *p_sf, u32 lba, u32 count, void *iobuf)
{
	...
	return sf->iod.write_sparse_func (
		sf, (off_t)lba * WII_SECTOR_SIZE, iobuf, count * WII_SECTOR_SIZE);
}
...
void PrintProgressSF (u64 p_done, u64 p_total, void *param)
{
	...
	TRACE ("PrintProgressSF(%llu,%llu), sf=%p, fd=%d, sh-prog=%d\n", p_done, p_total, sf,
		sf ? GetFD (&sf->f) : -2, sf ? sf->show_progress : -1);
	...
	noTRACE (" - sec=%d->%d, rate=%d, total=%llu=%llu+%llu\n", sf->progress_last_view_sec, view_sec,
		rate, total, sf->f.bytes_read, sf->f.bytes_written);
	...
	if (sf->progress_last_view_sec != view_sec && rate > 0 && total > 0)
	{
		...
		if (print_sections)
		{
			printf ("[progress]\n"
					...
					"mib-per-sec=%3.1f\n"
					"\n",
				sf->progress_verb, percent, elapsed, time1, percent >= 10 || view_sec >= 10, eta,
				time2, (total + MiB / 2) / MiB, (double)total * 1000 / MiB / elapsed);
		}
		else
		{
			int wd;
			if (percent < 10 && view_sec < 10)
				printf ("%*s%3d%% %s in %s (%3.1f MiB/sec)  %n\r", sf->indent, "", percent,
					sf->progress_verb, time1, (double)total * 1000 / MiB / elapsed, &wd);
			else
				printf ("%*s%3d%% %s in %s (%3.1f MiB/sec) -> ETA %s   %n\r", sf->indent, "",
					percent, sf->progress_verb, time1, (double)total * 1000 / MiB / elapsed, time2,
					&wd);
...
void PrintSummarySF (SuperFile_t *sf)
{
	...
	if (sf->show_summary)
	{
		...
		if (sf->source_size)
		{
			...
			if (sf->f.max_off)
				snprintf (ratebuf, sizeof (ratebuf), ", compressed to %4.2f%%",
		...
		if (print_sections)
		{
			printf ("[progress:summary]\n"
					...
					"mib-total=%llu\n"
					"mib-per-sec=%3.1f\n",
				sf->progress_verb, elapsed, tim, (total + MiB / 2) / MiB,
				(double)total * 1000 / MiB / elapsed);

			if (*ratebuf)
				printf (
					"compression-percent=%4.2f\n", 100.0 * (double)sf->f.max_off / sf->source_size);

			putchar ('\n');
		}
		else if (total)
		{
			snprintf (buf, sizeof (buf), "%*s%4llu MiB %s in %s, %4.1f MiB/sec%s", sf->indent, "",
				(total + MiB / 2) / MiB, sf->progress_verb, tim,
				(double)total * 1000 / MiB / elapsed, ratebuf);
		}
		else
			snprintf (buf, sizeof (buf), "%*sFinished in %s%s", sf->indent, "", tim, ratebuf);
	}
...
void DefineProgressChunkSF (SuperFile_t *sf, // valid file
...
void PrintProgressChunkSF (SuperFile_t *sf, // valid file
...
)
{
	if (sf && sf->show_progress && sf->progress_chunk_size)
	{
		...
		u64 done = last_done + chunk_done * sf->progress_data_size / sf->progress_chunk_size
...
enumFileType AnalyzeFT (WFile_t *f)
{
	...
	const uint max_read = f->st.st_size < sizeof (buf1) ? f->st.st_size : sizeof (buf1);
	...
	else if (IsWIA (buf1, sizeof (buf1), id6, &disc_type, 0))
	{
		...
		ft |= disc_type == WD_DT_GAMECUBE ? FT_ID_GC_ISO | FT_A_ISO | FT_A_GC_ISO | FT_A_WIA
	...
	else
	{
#ifdef DEBUG
		{
			...
			for (i = 0; i < 4; i++, ptr += 8)
			{
				TRACE ("ANALYZE/HEAD: \"%c%c%c%c%c%c%c%c\"  "
					...
					ptr[7] >= ' ' && ptr[7] < 0x7f ? ptr[7] : '.', (u8)ptr[0], (u8)ptr[1],
					(u8)ptr[2], (u8)ptr[3], (u8)ptr[4], (u8)ptr[5], (u8)ptr[6], (u8)ptr[7]);
			}
		}
#endif // DEBUG
		...
		if (!err)
		{
				...
				&& !memcmp (buf2, WDF_MAGIC, WDF_MAGIC_SIZE))
			{
				...
				if (wc->data_size >= 6)
				{
					...
					const u32 ldlen = wc->data_size < sizeof (buf2) ? wc->data_size : sizeof (buf2);
		...
		TRACE ("ISO ID6+MAGIC:  \"%c%c%c%c%c%c\"    %02x %02x %02x %02x %02x %02x / %08x\n",
			...
			data_ptr[5] >= ' ' && data_ptr[5] < 0x7f ? data_ptr[5] : '.', (u8)data_ptr[0],
			(u8)data_ptr[1], (u8)data_ptr[2], (u8)data_ptr[3], (u8)data_ptr[4], (u8)data_ptr[5],
	...
	// [[2do]] [[ft-id]]
	ASSERT_MSG (Count1Bits64 (ft & FT__ID_MASK) <= 1, "ft = %llx, nbits = %u\n", (u64)ft,
		Count1Bits64 (ft & FT__ID_MASK));
	return f->ftype = ft;
}
...
enumFileType AnalyzeMemFT (const void *preload_buf, off_t file_size)
{
	...
	TRACE ("AnalyzeMemFT(,%llx) id=%08x magic=%08x\n", (u64)file_size, be32 (data),
		be32 (data + WII_MAGIC_OFF));
	...
	const bool is_root_sig
		= sig_size && sig_size < file_size && !memcmp (preload_buf + head_size, "Root", 4);
	if (is_root_sig)
	{
		//----- test CERT.bin
		...
		if (!(file_size & WII_CERT_ALIGN - 1))
		{
			...
			if (data && (ccp)data + sizeof (data) < (ccp)preload_buf + file_size
	...
	if (sig_size && file_size >= 0x108 && !memcmp (preload_buf + 0x80, "Root-", 5)
...
enumError XPrintErrorFT (XPARM WFile_t *f, enumFileType err_mask)
{
	...
	TRACE ("PRINT FILETYPE ERROR: ft=%llx mask=%lx -> %lx,%lx\n", (u64)f->ftype, err_mask, and_mask,
		nand_mask);

	if ((f->split_f ? f->split_f[0]->fd : f->fd) == -1)
		stat = PrintError (XERROR0, ERR_READ_FAILED, "File is not open: %s\n", f->fname);

	else if (and_mask & FT_ID_DIR)
		stat = PrintError (XERROR0, ERR_WRONG_FILE_TYPE, "Is a directory: %s\n", f->fname);

	else if (and_mask & FT_A_WDISC)
		stat = PrintError (
			XERROR0, ERR_WRONG_FILE_TYPE, "Single images of a WBFS not allowed: %s\n", f->fname);

	else if (nand_mask & FT_A_SEEKABLE)
		stat = PrintError (
			XERROR0, ERR_WRONG_FILE_TYPE, "Wrong file type (not seekable): %s\n", f->fname);

	else if (nand_mask & (FT_A_REGFILE | FT_A_BLOCKDEV | FT_A_CHARDEV))
		stat = PrintError (XERROR0, ERR_WRONG_FILE_TYPE, "Wrong file type: %s\n", f->fname);

	else if (nand_mask & FT_ID_WBFS)
		stat = PrintError (XERROR0, ERR_WRONG_FILE_TYPE, "WBFS expected: %s\n", f->fname);

	else if (nand_mask & FT_ID_GC_ISO || nand_mask & FT_A_GC_ISO)
		stat = PrintError (
			XERROR0, ERR_WRONG_FILE_TYPE, "GameCube ISO image expected: %s\n", f->fname);

	else if (nand_mask & FT_ID_WII_ISO || nand_mask & FT_A_WII_ISO)
		stat = PrintError (XERROR0, ERR_WRONG_FILE_TYPE, "Wii ISO image expected: %s\n", f->fname);

	else if (nand_mask & FT_A_ISO)
		stat = PrintError (
			XERROR0, ERR_WRONG_FILE_TYPE, "GameCube or Wii ISO image expected: %s\n", f->fname);

	else if (nand_mask & FT_M_WDF)
		stat = PrintError (XERROR0, ERR_WRONG_FILE_TYPE, "WDF expected: %s\n", f->fname);

	else if (nand_mask & FT_A_WIA)
		stat = PrintError (XERROR0, ERR_WRONG_FILE_TYPE, "WIA expected: %s\n", f->fname);

	else if (nand_mask & FT_A_CISO)
		stat = PrintError (XERROR0, ERR_WRONG_FILE_TYPE, "CISO expected: %s\n", f->fname);

	else if (nand_mask & FT_A_GCZ)
		stat = PrintError (XERROR0, ERR_WRONG_FILE_TYPE, "GCZ expected: %s\n", f->fname);
	// [[nkit]]
...
ccp GetNameFT (enumFileType ftype, int ignore)
{
	...
	switch (ftype & FT__ID_MASK)
	{
		...
		case FT_ID_FST:
			return ftype & FT_A_PART_DIR ? (ftype & FT_A_GC_ISO ? "FST/GC" : "FST/WII")
										 : (ftype & FT_A_GC_ISO ? "FST/GC+" : "FST/WII+");

		case FT_ID_WBFS:
			return ftype & FT_A_WDISC ? (ftype & FT_A_GC_ISO ? "WBFS/GC" : "WBFS/WII")
				: ignore > 1		  ? 0
				: ftype & FT_M_WDF	  ? "WDF+WBFS"
									  : "WBFS";
...
wd_disc_type_t FileType2DiscType (enumFileType ftype)
{
	...
	return ftype & FT_A_GC_ISO ? WD_DT_GAMECUBE : ftype & FT_A_WII_ISO ? WD_DT_WII : WD_DT_UNKNOWN;
}
...
enumError CopyImage (SuperFile_t *fi, // valid input file
...
enumError CopyImageName (SuperFile_t *fi, // valid input file
...
enumError NormalizeExtractPath (char *dest_dir, // result: pointer to path buffer
...
enumError ExtractImage (SuperFile_t *fi, // valid input file
...
)
{
	...
	InitializeFST (&fst);
	CollectFST (
		&fst, disc, GetDefaultFilePattern (), false, copy_image ? 2 : 0, prefix_mode, false);
	...
	if (!err && wfi.not_created_count)
	{
		if (wfi.not_created_count == 1)
			err = ERROR0 (ERR_CANT_CREATE, "1 file or directory not created\n");
		else
			err = ERROR0 (ERR_CANT_CREATE, "%d files and/or directories not created.\n",
...
enumError CopySF (SuperFile_t *in, SuperFile_t *out)
{
	...
	switch (in->iod.oft)
	{
		case OFT_WDF1:
		case OFT_WDF2:
			return CopyWDF (in, out);
		case OFT_WIA:
			return CopyWIA (in, out);
		case OFT_WBFS:
			return CopyWBFSDisc (in, out);
		default:
...
enumError CopyRawData (SuperFile_t *in, SuperFile_t *out, off_t off, off_t copy_size)
{
	ASSERT (in);
	ASSERT (out);
	TRACE ("+++ CopyRawData(%d,%d,%llx,%llx) +++\n", GetFD (&in->f), GetFD (&out->f), (u64)off,
		(u64)copy_size);
...
enumError CopyRawData2 (
	SuperFile_t *in, off_t in_off, SuperFile_t *out, off_t out_off, off_t copy_size)
{
	ASSERT (in);
	ASSERT (out);
...
enumError CopyWBFSDisc (SuperFile_t *in, SuperFile_t *out)
{
	...
	for (bl = 0; bl < w->n_wbfs_sec_per_disc; bl++)
	{
		...
		if (wlba)
		{
			err = ReadAtF (&in->f, (off_t)w->wbfs_sec_sz * wlba, copybuf, w->wbfs_sec_sz);
			if (err)
				goto abort;

			err = WriteSF (out, (off_t)w->wbfs_sec_sz * bl, copybuf, w->wbfs_sec_sz);
			if (err)
				goto abort;
...
)
{
	...
	{
		diff->mismatch_limit = opt_limit >= 0 ? opt_limit
			: long_count < 3				  ? 1
			: long_count < 4				  ? 10
											  : 0;
...
bool OpenDiffSource (Diff_t *diff, // valid diff structure
...
)
{
	...
	diff->active_block_size = diff_iso && diff->block_size < 0 ? WII_SECTOR_SIZE : diff->block_size;
...
{
	TRACE ("DIFF: CloseDiffSource(%p,%d) -> %d,%d,%d\n", diff, silent, diff->source_differ_count,
		diff->file_differ_count, diff->mismatch_count);
	DASSERT (diff);
	...
	{
		if (diff->diff_iso)
			fprintf (diff->logfile, "* ISOs identical: %s:%s : %s:%s\n",
				oft_info[diff->f1->iod.oft].name, diff->f1->f.fname,
				oft_info[diff->f2->iod.oft].name, diff->f2->f.fname);
		else
			fprintf (diff->logfile, "* Files identical: %s : %s\n", diff->f1->f.fname,
				diff->f2->f.fname);
	}

	PRINT_IF (print_stat, "DIFF: CLOSE/SRC:  %u:%u/%u | %u:%u/%u, %u:%u\n", diff->source_count,
		diff->source_differ_count, diff->source_differ_limit, diff->file_count,
		diff->file_differ_count, diff->file_differ_limit, diff->total_file_count,
	...
	return !diff->source_differ_limit || diff->source_differ_count < diff->source_differ_limit;
};
...
)
{
	...
	if (diff->mismatch_count || diff->mismatch_marked)
	{
		...
		if (!silent && diff->verbose >= 0 && !diff->info_level && diff->file_name)
		{
			ClearProgressLineSF (diff->f2);
			if (print_sections)
				fprintf (diff->logfile, "[mismatch:file]\nfile=%s%s\n\n", diff->file_prefix,
					diff->file_name);
			else
				fprintf (diff->logfile, "!   Files differ:     %s%s\n", diff->file_prefix,
					diff->file_name);
	...
	noPRINT_IF (print_stat, "DIFF: CLOSE/FILE: %u:%u/%u | %u:%u/%u, %u:%u\n", diff->source_count,
		diff->source_differ_count, diff->source_differ_limit, diff->file_count,
		diff->file_differ_count, diff->file_differ_limit, diff->total_file_count,
		diff->total_file_differ_count);
	...
	return !diff->file_differ_limit || diff->file_differ_count < diff->file_differ_limit;
}

///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////

static int get_mismatch_size (Diff_t *diff, // valid diff structure
...
)
{
	...
	else
	{
		...
		noPRINT ("get_mismatch_size(off=%llx, siz=%zx) incompl=%d, next=%llx\n", (u64)data_off,
			data_size, incompl, (u64)(incompl ? data_off + mlen : next_off));
	}
...
)
{
	...
	if (diff->data_size)
	{
		...
		if (diff->data_off + total_len == data_off)
		{
			...
			bool incomplete;
			int mis_len
				= get_mismatch_size (diff, diff->data_off, total_len + add_len, 0, 0, &incomplete);
	...
	while (src1 < end1 && (!diff->mismatch_limit || diff->mismatch_count < diff->mismatch_limit))
	{
		...
		int mis_len = get_mismatch_size (diff, doff, end1 - src1, &block, &next_off, &incomplete);
		...
		if (print_sections)
		{
			fprintf (diff->logfile,
				...
				"block=%lld\n"
				"block-size=%u\n",
				diff->file_prefix ? diff->file_prefix : "", diff->file_name ? diff->file_name : "",
				(u64)doff, block, diff->active_block_size > 0 ? diff->active_block_size : 0);
		}
		else
		{
...
static int DiffFile (Diff_t *diff // valid diff structure
)
{
	diff->total_mismatch_count++;
	diff->file_differ_count++;
...
static int DiffOnlyPart (Diff_t *diff, // valid diff structure
	int iso_index, // 1 | 2
	...
	if (verbose >= 0)
	{
		...
		if (print_sections)
			fprintf (diff->logfile,
				...
				"part-name=%s\n"
				"\n",
				iso_index, wp->index, wp->part_type, wd_get_part_name (wp->part_type, ""));
		else
			fprintf (diff->logfile, "!   Only in image #%u: Partition #%u, type=%x %s\n", iso_index,
				wp->index, wp->part_type, wd_get_part_name (wp->part_type, ""));
	}
...
static int DiffOnlyFile (Diff_t *diff, // valid diff structure
	int iso_index, // 1 | 2
	...
	if (verbose >= 0)
	{
		...
		if (print_sections)
			fprintf (diff->logfile,
				...
				"file-name=%s%s\n"
				"\n",
				iso_index, wp1->index, wp1->part_type, wd_get_part_name (wp1->part_type, ""),
				wp2->index, wp2->part_type, wd_get_part_name (wp2->part_type, ""), path1, path2);
		else
			fprintf (diff->logfile, "!   Only in image #%u: %s%s\n", iso_index, path1, path2);
	}
...
static bool DiffCheckSize (Diff_t *diff, // valid diff structure
...
)
{
	...
	{
		if (diff->file_name)
			fprintf (diff->logfile, "[mismatch:file-size]\nfile=%s%s\n", diff->file_prefix,
				diff->file_name);
		else
	...
	else
	{
		...
		fprintf (diff->logfile, "%llu%+lld=%llu%s\n", size1, size2 - size1, size2, postfix);
	}
...
enumError GetDiffStatus (Diff_t *diff // valid diff structure
)
{
	DASSERT (diff);
...
enumError DiffSF (Diff_t *diff, // valid diff structure
...
)
{
	...
	for (idx = 0; idx < sizeof (wdisc_usage_tab); idx++)
	{
		if (wd_usage_class_tab[wdisc_usage_tab[idx]] != wd_usage_class_tab[wdisc_usage_tab2[idx]])
		{
...
enumError DiffRawSF (Diff_t *diff // valid diff structure
)
{
	DASSERT (diff);
	...
	if ((attrib1 | attrib2) & OFT_A_NOSIZE)
	{
		...
		max_off = f1->file_size > f2->file_size ? f1->file_size : f2->file_size;
	}
	else
	{
...
enumError DiffFilesSF (Diff_t *diff, // valid diff structure
	...
	for (ip1 = 0; ip1 < fst1.part_used; ip1++)
	{
		...
		while (file1 < end1 && file2 < end2)
		{
			...
			if (file1->icm != file2->icm)
			{
				...
				if (diff_stat >= 0)
					fprintf (diff->logfile, "!   File types differ: %s%s\n", p1->path, file1->path);
				if (diff_stat <= 0)
					goto abort_source;
			}
...
enumError DiffPatchSF (Diff_t *diff, // valid diff structure
	struct FilePattern_t *pat, // NULL or file pattern
	wd_ipm_t pmode // prefix mode
)
{
...
static void oldPrintDiff (off_t off, ccp iobuf1, ccp iobuf2, size_t iosize)
{
	while (iosize > 0)
	{
		if (*iobuf1 != *iobuf2)
		{
			...
			{
				printf ("!");
				HexDump (stdout, 3, off, 9, 16, iobuf1, 16);
				printf ("!");
				HexDump (stdout, 3, off, 9, 16, iobuf2, 16);
			}
...
enumError oldDiffSF (SuperFile_t *f1, // first file
	SuperFile_t *f2, // second file
	int long_count, // relevant long count
	bool force_raw_mode // true: force raw mode
)
...
enumError oldDiffRawSF (SuperFile_t *f1, SuperFile_t *f2, int long_count)
{
	ASSERT (f1);
	ASSERT (f2);
	ASSERT (IsOpenSF (f1));
...
enumError oldDiffFilesSF (
	SuperFile_t *f1, SuperFile_t *f2, int long_count, FilePattern_t *pat, wd_ipm_t pmode)
{
	ASSERT (f1);
	ASSERT (f2);
	DASSERT (IsOpenSF (f1));
...
void InitializeIterator (Iterator_t *it, bool allow)
{
	...
	if (allow)
	{
		it->act_fst = opt_allow_fst >= OFFON_AUTO ? ACT_EXPAND : ACT_IGNORE;
...
static void IteratorProgress (Iterator_t *it, bool last_message, SuperFile_t *sf)
{
	...
	if (sf && it->scan_progress)
	{
		if (print_sections)
		{
			printf ("[progress:found]\n"
					...
					"size=%llu\n"
					"\n",
				oft_info[sf->iod.oft].name, sf->f.fname, (u64)sf->f.st.st_size);
		}
		else
		{
			if (active)
			...
			printf ("> %s found: %s:%s\n", it->progress_t_file, oft_info[sf->iod.oft].name,
				sf->f.fname);
		}
		flush = true;
	}
		...
		if (sec != it->progress_last_sec || last_message)
		{
			...
			if (print_sections)
			{
				printf ("[progress:scan%s]\n"
						...
						"found-name-2=%s\n"
						"\n",
					last_message ? ":term" : "", it->num_of_scans, it->num_of_dirs,
					it->num_of_files, it->progress_t_file, it->progress_t_files);
			}
			else
			{
				printf ("%u object%s scanned", it->num_of_scans, it->num_of_scans == 1 ? "" : "s");

				ccp tie = ",", term = "";
				if (it->num_of_dirs > 0)
				{
					printf (", %u director%s", it->num_of_dirs, it->num_of_dirs == 1 ? "y" : "ies");
					tie = " and";
					term = " found";
				}
				...
				if (it->num_of_files > 0)
				{
					printf ("%s %u %s", tie, it->num_of_files,
						it->num_of_files == 1 ? it->progress_t_file : it->progress_t_files);
					term = " found";
				}
...
static enumError SourceIteratorHelper (Iterator_t *it, ccp path, bool collect_fnames)
{
	ASSERT (it);
	ASSERT (it->func || collect_fnames);
	TRACE ("SourceIteratorHelper(%p,%s,%d) depth=%d/%d\n", it, path, collect_fnames, it->depth,
		it->max_depth);
...
check_file:
	...
	// [[2do]] [[ft-id]]
	if (it->act_wbfs >= ACT_EXPAND && (sf.f.ftype & (FT_ID_WBFS | FT_A_WDISC)) == FT_ID_WBFS)
	{
		WBFS_t wbfs;
		InitializeWBFS (&wbfs);
		if (!SetupWBFS (&wbfs, &sf, false, 0, false))
		{
			...
			int fw, slot;
			for (slot = max_disc - 1; slot >= 0 && !wbfs.wbfs->head->disc_table[slot]; slot--)
				;
			...
			*dest++ = '/';
				...
				for (slot = 0; slot < max_disc && it->num_of_files < job_limit; slot++)
				{
					if (!GetWDiscInfoBySlot (&wbfs, &wdisk, slot) && !IsExcluded (wdisk.id6))
					{
						...
						const time_t mtime
							...
							? ntoh64 (wdisk.dhead.iinfo.mtime)
							: 0;
			...
			u8 discbuf[512];
			u8 *disc_table = max_disc <= sizeof (discbuf) ? discbuf : MALLOC (sizeof (discbuf));
	...
	{
		const enumAction action = it->act_non_iso > it->act_known ? it->act_non_iso : it->act_known;
		if (action < ACT_ALLOW)
		{
			if (action == ACT_WARN)
				PrintErrorFT (&sf.f, FT_A_ISO);
	...
	{
		const enumAction action = sf.f.ftype & FT_ID_WBFS ? it->act_wbfs : it->act_non_iso;
		if (action < ACT_ALLOW)
		{
			if (action == ACT_WARN)
				PrintErrorFT (&sf.f, FT_A_ISO);
...
static enumError SourceIteratorStarter (Iterator_t *it, ccp path, bool collect_fnames)
{
	ASSERT (it);
...
enumError SourceIterator (
	Iterator_t *it, int warning_mode, bool current_dir_is_default, bool collect_fnames)
{
	ASSERT (it);
	ASSERT (it->func || collect_fnames);
	TRACE ("SourceIterator(%p) func=%p, act=%d,%d,%d\n", it, it->func, it->act_non_exist,
		it->act_non_iso, it->act_wbfs);

	if (current_dir_is_default && !source_list.used && !recurse_list.used && !opt_source_auto)
	...
	if (opt_source_auto > 0 && !it->auto_processed && it->act_wbfs >= ACT_ALLOW)
	{
		...
		for (ptr = wbfs_part_list.field, end = ptr + wbfs_part_list.used;
			err == ERR_OK && !SIGINT_level && ptr < end && it->num_of_files < job_limit; ptr++)
		{
			if (collect_fnames)
	...
	return warning_mode > 0 ? SourceIteratorWarning (it, err, warning_mode == 1) : err;
}

//-----------------------------------------------------------------------------

enumError SourceIteratorCollected (Iterator_t *it, // iterator info
...
)
{
	...
	for (idx = 0; idx < it->source_list.used && !SIGINT_level; idx++)
	{
		...
		it->job_count++;
		const enumError err = SourceIteratorStarter (it, it->source_list.field[idx]->arg, false);
		if (max_err < err)
			max_err = err;
	...
	return warning_mode > 0 ? SourceIteratorWarning (it, max_err, warning_mode == 1) : max_err;
}
...
enumError SourceIteratorWarning (Iterator_t *it, enumError max_err, bool silent)
{
	if (it->num_empty_dirs && max_err < ERR_NO_SOURCE_FOUND)
	{
		if (silent)
			...
			max_err = ERROR0 (
				ERR_NO_SOURCE_FOUND, "One directory scanned, but no valid source files found.\n");
	}
