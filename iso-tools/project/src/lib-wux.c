...
static enumError wux_read_at (WUX_t *wux, u64 off, void *buf, size_t count)
{
	...
	if (fread (buf, 1, count, wux->f) != count)
		return ERROR1 (
			ERR_READ_FAILED, "Can't read %zu bytes at %llu: %s\n", count, off, wux->fname);
	return ERR_OK;
}
...
)
{
	...
	const bool ok = h.magic0 == WUX_MAGIC0 && h.magic1 == WUX_MAGIC1
		&& h.sector_size >= WUX_MIN_SECTOR_SIZE && h.sector_size <= WUX_MAX_SECTOR_SIZE
		&& !(h.sector_size & h.sector_size - 1) // power of 2
		&& h.image_size > 0 && h.image_size <= WUX_MAX_IMAGE_SIZE;
...
)
{
	...
	if (n_index > 0x8000000)
		return ERROR0 (ERR_WRONG_FILE_TYPE, "WUX index table too large (%llu entries): %s\n",
			n_index, wux->fname);
	wux->n_index = (u32)n_index;

	wux->index = MALLOC (wux->n_index * sizeof (*wux->index));
	err = wux_read_at (wux, WUX_HEAD_SIZE, wux->index, wux->n_index * sizeof (*wux->index));
	if (err)
		return err;
...
)
{
	...
	if (off + count > wux->image_size)
		return ERROR0 (ERR_READ_FAILED, "Read beyond end of image (%llu+%zu > %llu): %s\n", off,
			count, wux->image_size, wux->fname);
	...
	while (count > 0)
	{
		...
		const u64 src = wux->data_offset + (u64)le32 (wux->index + idx) * wux->sector_size + skip;
...
)
{
	...
	if (sector_size < WUX_MIN_SECTOR_SIZE || sector_size > WUX_MAX_SECTOR_SIZE
		|| sector_size & sector_size - 1)
		return ERROR0 (ERR_INVALID_FILE, "Invalid WUX sector size: 0x%x\n", sector_size);
	if (!image_size || image_size > WUX_MAX_IMAGE_SIZE)
		return ERROR0 (ERR_INVALID_FILE, "Invalid WUX image size: %llu\n", image_size);
...
static enumError store_sector (WUX_t *wux, u32 image_index)
{
	...
	for (int i = wux->hash_tab[bucket]; i >= 0; i = wux->hash_list[i].next)
	{
		...
		const u64 off = wux->data_offset + (u64)wux->hash_list[i].sector * wux->sector_size;
		const off_t save = ftello (wux->f);
		const enumError err = wux_read_at (wux, off, wux->cmp_buf, wux->sector_size);
		if (err)
			return err;
	...
	{
		wux->hash_alloced *= 2;
		wux->hash_list = REALLOC (wux->hash_list, wux->hash_alloced * sizeof (*wux->hash_list));
	}
...
)
{
	...
	while (count > 0)
	{
		...
		if (wux->buf_fill == wux->sector_size)
		{
			...
			if (idx >= wux->n_index)
				return ERROR0 (
					ERR_WRITE_FAILED, "More data written than announced: %s\n", wux->fname);
...
)
{
	...
	if (wux->buf_fill)
	{
		...
		const u32 idx = wux->n_written++;
		if (idx >= wux->n_index)
			return ERROR0 (ERR_WRITE_FAILED, "More data written than announced: %s\n", wux->fname);
		const enumError err = store_sector (wux, idx);
		if (err)
			return err;
		wux->buf_fill = 0;
	...
	if (fseeko (wux->f, 0, SEEK_SET) || fwrite (head, 1, sizeof (head), wux->f) != sizeof (head)
