...
static enumError extract_range (ccp fname, const u8 *image, u64 image_size, u32 off, u32 size)
{
	if (!off || !size)
		return ERR_OK;
	if ((u64)off + size > image_size)
		return ERROR0 (
			ERR_INVALID_FILE, "Range 0x%x+0x%x is outside the image: %s\n", off, size, fname);
...
enumError XInfoNDS (ccp source)
{
	...
	else
	{
		...
		printf ("      title:    %s\n", title);
		printf ("      gamecode: %.4s   maker: %.2s   unit: 0x%02x\n", image + NDS_OFF_GAMECODE,
			image + NDS_OFF_MAKERCODE, image[NDS_OFF_UNITCODE]);
		printf ("      arm9:     0x%06x + 0x%06x\n", le32 (image + NDS_OFF_ARM9_ROM),
			le32 (image + NDS_OFF_ARM9_SIZE));
		printf ("      arm7:     0x%06x + 0x%06x\n", le32 (image + NDS_OFF_ARM7_ROM),
			le32 (image + NDS_OFF_ARM7_SIZE));
		printf ("      fnt:      0x%06x + 0x%06x\n", le32 (image + NDS_OFF_FNT_ROM),
			le32 (image + NDS_OFF_FNT_SIZE));
		printf ("      fat:      0x%06x + 0x%06x  (%u files)\n", le32 (image + NDS_OFF_FAT_ROM),
			le32 (image + NDS_OFF_FAT_SIZE), le32 (image + NDS_OFF_FAT_SIZE) / 8);
...
static enumError extract_dir (ccp dest, // base directory of the extraction
...
)
{
	...
	while (!err)
	{
		...
		if (type & 0x80)
		{
			...
			if (!err)
				err = extract_dir (dest, rel, image, image_size, fnt_off, fnt_size, fat_off,
					n_files, sub_id, depth + 1);
		}
		else
		{
			if (file_id >= n_files)
			...
			if (end < beg || end > image_size)
				return ERROR0 (
					ERR_INVALID_FILE, "FAT entry %u is invalid (0x%x..0x%x)\n", file_id, beg, end);
...
static enumError extract_overlays (ccp dest, // base directory
...
)
{
	...
	for (u32 pos = 0; pos + 32 <= ovl_size; pos += 32)
	{
		...
		char name[64];
		snprintf (name, sizeof (name), "%s/%s_%04u.bin", NDS_DIR_OVERLAY, prefix, (uint)(pos / 32));
...
enumError XExtractNDS (ccp source, ccp dest)
{
	...
	err = extract_range (
		buf, image, size, le32 (image + NDS_OFF_ARM7_ROM), le32 (image + NDS_OFF_ARM7_SIZE));
	if (err)
		goto abort;
	...
	err = extract_range (
		buf, image, size, le32 (image + NDS_OFF_OVL9_ROM), le32 (image + NDS_OFF_OVL9_SIZE));
	if (err)
		goto abort;

	PathCatPP (buf, sizeof (buf), dest, NDS_FN_OVL7);
	err = extract_range (
		buf, image, size, le32 (image + NDS_OFF_OVL7_ROM), le32 (image + NDS_OFF_OVL7_SIZE));
	if (err)
		goto abort;
...

	{
		...
		{
			if ((u64)fnt_off + fnt_size > size || (u64)fat_off + fat_size > size)
			{
				err = ERROR0 (ERR_INVALID_FILE, "FNT or FAT is outside the image: %s\n", source);
				goto abort;
			}

			err = extract_overlays (dest, "arm9", image, size, le32 (image + NDS_OFF_OVL9_ROM),
				le32 (image + NDS_OFF_OVL9_SIZE), fat_off, n_files);
			if (!err)
				err = extract_overlays (dest, "arm7", image, size, le32 (image + NDS_OFF_OVL7_ROM),
					le32 (image + NDS_OFF_OVL7_SIZE), fat_off, n_files);
			if (err)
				goto abort;

			err = extract_dir (
				dest, NDS_DIR_DATA, image, size, fnt_off, fnt_size, fat_off, n_files, 0xf000, 0);
			if (err)
				goto abort;
		}
	}
...
static enumError scan_dir (nds_fs_t *fs, uint dir_index, uint depth)
{
	...
	for (uint i = 0; i < n_names && !err; i++)
	{
		...
		if (fs->n_dirs >= 0x1000)
		{
			err = ERROR0 (ERR_INVALID_FILE, "Too many directories, the FNT allows 4096\n");
			break;
		}
...
static enumError build_fnt (nds_fs_t *fs, nds_buf_t *fnt)
{
	...
	for (uint i = 0; i < fs->n_dirs; i++)
	{
		...
		for (uint j = 0; j < dir->n_files; j++, file_index++)
		{
			...
			if (!len || len > 127)
				return ERROR0 (
					ERR_INVALID_FILE, "File name length %u is not usable: %s\n", len, f->path);
			buf_u8 (fnt, len);
		...
		for (uint j = 0; j < dir->n_subs; j++)
		{
			...
			if (!len || len > 127)
				return ERROR0 (ERR_INVALID_FILE, "Directory name length %u is not usable: %s\n",
					len, sub->path);
			buf_u8 (fnt, 0x80 | len);
