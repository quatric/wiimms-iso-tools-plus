...
static ccp overlay_info[] = { "no overlay", "simple overlay", "multiple hole overlay", 0 };
...
static enumError verify_mix (MixParam_t *mp, // valid pointer to mix param + data
...
)
{
	...
	for (mix = mp->mix; mix < mp->end_mix; mix++, dest_id++)
	{
		...
		const int max_sectors = WII_MAX_SECTORS
			- (mix->src_sector > mix->dest_sector ? mix->src_sector : mix->dest_sector);
		u8 *dest = utab + mix->dest_sector, *enddest = dest + max_sectors;
		const u8 *src = mix->disc->usage_table + mix->src_sector;

		noPRINT (">> V#%zu: sect=%5x..%5x -> %5x..%5x, n=%5x\n", mix - mp->mix, mix->src_sector,
			mix->src_sector + max_sectors, mix->dest_sector, mix->dest_sector + max_sectors,
			max_sectors);
		...
		for (; dest < enddest; dest++, src++)
		{
			if ((*src & WD_USAGE__MASK) == src_id)
			{
				...
				while (dest < enddest && (*src & WD_USAGE__MASK) == src_id)
				{
					if (*dest)
					{
						PRINT_IF (error_count < 20,
							...
							mix - mp->mix, dest - utab, *dest, dest_id | *src++ & WD_USAGE_F_CRYPT);
						error_count++;
					}
				...
				if (create_memmap)
				{
					...
					{
						MemMapItem_t *item
							= InsertMemMap (&mp->mm, (start - utab) * (u64)WII_SECTOR_SIZE,
								(dest - start) * (u64)WII_SECTOR_SIZE);
						DASSERT (item);
						item->index = mix - mp->mix;
						if (!mp->is_gc)
							snprintf (item->info, sizeof (item->info), "P.%0*u, %s, %s -> %s",
							...
							snprintf (item->info, sizeof (item->info), "Boot, %s",
								wd_print_id (mix->disc->dhead.id6.id6, 6, 0));
						else
							snprintf (item->info, sizeof (item->info), "P.%0*u, %s", part_fw,
								(int)(mix - mp->mix), wd_print_id (mix->disc->dhead.id6.id6, 6, 0));
					}
				}
			}
		}
	...
	if (error_count)
		return ERROR0 (ERR_INTERNAL, "Verification of mix failed for %u sector%s!\n", error_count,
			error_count == 1 ? "" : "s");
...
static enumError scan_mix_param (MixParam_t *mp // valid pointer to mix param + data
)
{
	DASSERT (mp);
	mp->disc_type = WD_DT_UNKNOWN;

	u32 max_part = InfoUI_wit.opt_used[OPT_OVERLAY] ? MAX_MIX_PERM : WII_MAX_PARTITIONS;
	...
	while (param)
	{
		...
		if (src_warn && !strchr (srcfile, '/') && !strchr (srcfile, '.'))
		{
			...
			ERROR0 (ERR_WARNING,
				...
				" to distinguish file names from keywords: %s",
				srcfile);
		}
		param = param->next;
		...
		while (scan_qualifier)
		{
			...
			if (param && param->next
				&& (!strcasecmp (param->arg, "select") || !strcasecmp (param->arg, "psel")))
			{
				param = param->next;
				AtExpandParam (&param);
			...
			if (param && param->next && !strcasecmp (param->arg, "as"))
			{
				param = param->next;
				AtExpandParam (&param);
				const enumError err = ScanPartTabAndType (
					&ptab, &ptab_valid, &ptype, &ptype_valid, param->arg, " ('as')");
				if (err)
					return err;
				param = param->next;
				scan_qualifier = true;
			...
			if (param && !strcasecmp (param->arg, "boot"))
			{
				...
				if (mp->have_boot_part)
					return ERROR0 (
						ERR_SEMANTIC, "Multiple usage of qualifier 'boot' is not allowed.");
			}
			...
			if (param && !strcasecmp (param->arg, "header"))
			{
				...
				if (mp->source_dhead)
					return ERROR0 (
						ERR_SEMANTIC, "Multiple usage of qualifier 'header' is not allowed.");
			}
			...
			if (param && !strcasecmp (param->arg, "region"))
			{
				...
				if (mp->source_region)
					return ERROR0 (
						ERR_SEMANTIC, "Multiple usage of qualifier 'region' is not allowed.");
			}
		}
		...
		if (mp->disc_type == WD_DT_UNKNOWN)
		{
			...
			return ERROR0 (
				ERR_SEMANTIC, "Combination of Wii and GameCube partitions is not possible.");

		if (!mp->is_gc && qual_boot)
			return ERROR0 (
				ERR_SEMANTIC, "Qualifier 'boot' is only allowed for GameCube partitions.");
		...
		for (part = disc->part; part < end_part; part++)
		{
			...
			if (mp->n_mix >= max_part)
				return ERROR0 (
					ERR_SEMANTIC, "Maximum supported partition count (%u) exceeded.\n", max_part);
			...
			PRINT ("N-BLOCKS=%d, A1=0+%x, A2=%x+%x\n", mix->n_blocks, mix->a1size, mix->a2off,
				mix->a2size);
		}
	...
	mp->start_sector = !mp->is_gc				  ? WII_PART_OFF / WII_SECTOR_SIZE
		: mp->have_boot_part || !mp->is_multiboot ? 0
	...
	PRINT ("MIX START SECTOR = %x ==> off=%x, align-sector=%x,  n-mix=%d\n", mp->start_sector,
		mp->start_sector * WII_SECTOR_SIZE, mp->sector_align, mp->n_mix);
	...
	for (mix = mp->mix; mix < end_mix; mix++)
	{
		...
		PRINT ("  - MIX #%02zu: %9llx .. %9llx -> %9llx .. %9llx\n", mix - mp->mix,
			WII_SECTOR_SIZE * (u64)mix->src_sector, WII_SECTOR_SIZE * (u64)mix->src_end_sector,
			WII_SECTOR_SIZE * (u64)mix->dest_sector, WII_SECTOR_SIZE * (u64)sector);
	}
...
static u32 mark_used_mix (MixFree_t *base, // pointer to free field
...
)
{
	...
	ASSERT_MSG (off + size <= ptr->size, "off+size=%x+%x=%x <= ptr->size=%x\n", off, size,
		off + size, ptr->size);

	if (!off)
	{
		noTRACE ("\t\t\t\t\t> %x..%x -> %x..%x [%u/%u]\n", ptr->off, ptr->off + ptr->size,
			ptr->off + size, ptr->off + ptr->size, (int)(ptr - base), used);
		ptr->off += size;
		ptr->size -= size;
		if (!ptr->size)
		{
	...
	if (end == ptr->size)
	{
		noTRACE ("\t\t\t\t\t> %x..%x -> %x..%x [%u/%u]\n", ptr->off, ptr->off + ptr->size, ptr->off,
			ptr->off + ptr->size - size, (int)(ptr - base), used);
		ptr->size -= size;
		return used;
	}
	...
	noTRACE ("\t\t\t\t\t> %x..%x -> %x..%x,%x..%x [%u/%u>%u]\n", old_off, old_off + old_size,
		ptr[0].off, ptr[0].off + ptr[0].size, ptr[1].off, ptr[1].off + ptr[1].size,
...
static void setup_sector_mix (Mix_t *mix)
{
	...
	{
		while (src_sector < mix->src_end_sector && (utab[src_sector] & WD_USAGE__MASK) == usage_id)
			src_sector++;
...
static void log_mix (MixParam_t *mp // valid pointer to mix param + data
)
{
	DASSERT (mp);
	...
	printf ("\nMix table (%d partitions, %s, total size=%llu MiB):\n\n", mp->n_mix,
		overlay_info[mp->overlay], mp->max_sector * (u64)WII_SECTOR_SIZE / MiB);
		...
		for (mix = mp->mix; mix < mp->end_mix; mix++)
		{
			if (mix->a2off)
			{
				...
				printf ("%7x..%5x + %5x..%5x : %9llx..%9llx : %u %s : %c %s\n", mix->dest_sector,
					mix->dest_sector + mix->a1size, mix->dest_sector + mix->a2off, end_sector,
					mix->dest_sector * (u64)WII_SECTOR_SIZE, end_sector * (u64)WII_SECTOR_SIZE,
					mix->ptab, wd_print_part_name (0, 0, mix->ptype, WD_PNAME_COLUMN_9),
			...
			else
			{
				...
				printf ("%7x..%5x                : %9llx..%9llx : %u %s : %c %s\n",
					mix->dest_sector, end_sector, mix->dest_sector * (u64)WII_SECTOR_SIZE,
					end_sector * (u64)WII_SECTOR_SIZE, mix->ptab,
...
static enumError create_output_image (MixParam_t *mp // valid pointer to mix param + data
)
{
	enumError err
		= CreateWFile (&mp->fo.f, 0, IOM_IS_IMAGE, InfoUI_wit.opt_used[OPT_OVERWRITE] ? 1 : 0);
	if (err)
		return err;
...
static enumError copy_partitions (MixParam_t *mp // valid pointer to mix param + data
)
{
	DASSERT (mp);
	...
	for (mi = 0; mi < mp->mm.used; mi++)
	{
		...
		u64 src_off
			= item->off + ((int)mix->src_sector - (int)mix->dest_sector) * (s64)WII_SECTOR_SIZE;
		if (verbose > 0)
			printf (" - copy P.%-2u %9llx -> %9llx .. %9llx, size=%9llx\n", index, src_off,
				(u64)item->off, (u64)item->off + (u64)item->size, (u64)item->size);

		const enumError err = CopyRawData2 (mix->sf, src_off, &mp->fo, item->off, item->size);
		if (err)
			return err;
	}
	...
	{
		Mix_t *mix = mp->mix + mi;
		u64 delta_off = ((int)mix->dest_sector - (int)mix->src_sector) * (s64)WII_SECTOR_SIZE;
		PRINT ("-> REWRITE #%02u: %16llx\n", mi, delta_off);

		const enumError err = RewriteModifiedSF (mix->sf, &mp->fo, 0, delta_off);
		if (err)
			return err;
	}
...
static enumError create_gc_mix (MixParam_t *mp, // valid pointer to mix param + data
...
)
{
	...
	else
	{
		...
		disc_info1 = is_dvd9 ? "Multiboot GameCube disc, format=DVD9" : "Multiboot GameCube disc";
	...
	*(u32 *)(head + WII_BI2_OFF + WII_BI2_REGION_OFF) = reg->region;
	...
	if (testmode || verbose >= 0)
	{
		...
		printf ("\n%sreate disc [%.6s] %s:%s\n"
				...
				"  Title:  %.64s\n"
				"  Region: %x [%s]\n\n",
			testmode ? "WOULD c" : "C", head, oft_info[oft].name, mp->fo.f.fname, disc_info1,
			disc_info2, head + WII_TITLE_OFF, regnum, GetRegionName (regnum, "?"));
	}
...
static enumError create_wii_mix (MixParam_t *mp, // valid pointer to mix param + data
	wd_header_t *dhead, // valid pointer to prepared disc header
	wd_region_t *reg // valid pointer to prepared region set
)
{
	...
	if (testmode || verbose >= 0)
	{
		...
		printf ("\n%sreate Wii disc [%.6s] %s:%s\n"
				"  Title:  %.64s\n"
				"  Region: %x [%s] / %s\n\n",
			testmode ? "WOULD c" : "C", dhead->id6.id6, oft_info[oft].name, mp->fo.f.fname,
			dhead->disc_title, regnum, GetRegionName (regnum, "?"),
...
enumError cmd_mix ()
{
	...
	if (mp.max_sector > WII_MAX_SECTORS)
		return ERROR0 (ERR_SEMANTIC, "Total size (%u MiB) exceeds maximum size (%u MiB).\n",
			mp.max_sector / WII_SECTORS_PER_MIB, WII_MAX_SECTORS / WII_SECTORS_PER_MIB);
	...
	u64 dest_file_size = mp.is_gc ? 0 : WII_SECTORS_SINGLE_LAYER * (u64)WII_SECTOR_SIZE;
	...
	err = mp.is_gc ? create_gc_mix (&mp, &dhead, &reg) : create_wii_mix (&mp, &dhead, &reg);
	if (err)
		RemoveSF (&mp.fo);
