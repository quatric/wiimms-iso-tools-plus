...
#define TITLE WIT_SHORT ": " WIT_LONG " v" VERSION " r" REVISION " " SYSTEM2 " - " AUTHOR " - " DATE
...
{
	if (current_command && current_command->id >= 0 && current_command->id < InfoUI_wit.n_cmd)
	{
...
static enumError exec_analyze (SuperFile_t *fi, Iterator_t *it)
{
	...
	const uint main_total = af.pstat_main.n_host + af.pstat_main.n_string + af.pstat_main.n_http
		+ af.pstat_main.n_domain + af.pstat_main.n_agent + af.pstat_main.n_p2p
		+ af.pstat_main.n_master;

	const uint rel_total = af.pstat_staticr.n_http + af.pstat_staticr.n_domain;
	...
	PrintScriptVars (ps, 1,
		...
		"patch=%u\n"
		"dol_avail=%u\n",
		fname, fi->f.fatt.mtime.tv_sec, fi->f.fatt.mtime.tv_nsec,
		PrintTimeByFormat ("%F %T %z", fi->f.fatt.mtime.tv_sec), fi->f.fatt.ctime.tv_sec,
		fi->f.fatt.ctime.tv_nsec, PrintTimeByFormat ("%F %T %z", fi->f.fatt.ctime.tv_sec),
		fi->f.fatt.size, fi->file_size, fi->f.split_used, GetNameFT (fi->f.ftype, 0),
		fi->f.ftype & FT_A_ISO ? oft_info[fi->iod.oft].option : "", fi->f.slot,
		(fi->f.ftype & FT_A_GC_ISO) != 0, (fi->f.ftype & FT_A_WII_ISO) != 0, fi->f.id6_dest,
		(main_total ? 1 : 0) | (rel_total ? 2 : 0), af.pstat_main.n_files);
	...
	if (af.dol)
		PrintScriptVars (ps, 1,
			...
			af.dol_file_size, af.dol_is_mkw, af.dol_region, main_total, af.pstat_main.n_host,
			af.pstat_main.n_string, af.pstat_main.n_http, af.pstat_main.n_domain,
			af.pstat_main.n_agent, af.pstat_main.n_p2p, af.pstat_main.n_master);

	PrintScriptVars (ps, 1, "rel_avail=%u\n", af.pstat_staticr.n_files);

	if (af.pstat_staticr.n_files)
		PrintScriptVars (ps, 1, "rel_patch=\"%u= h:%u, d:%u\"\n", rel_total,
			af.pstat_staticr.n_http, af.pstat_staticr.n_domain);
...
static enumError cmd_anaid ()
{
	...
	for (param = first_param; param; param = param->next)
	{
		...
		if (valid)
			printf (" %02x%02x%02x%02x  %-6s  %s\n", (uchar)id6[0], (uchar)id6[1], (uchar)id6[2],
				(uchar)id6[3], id6, GetTitle (id6, "?"));
		else
			printf ("? %s\n", param->arg);
	}
...
static enumError cmd_cert ()
{
	...
	if ( cert_data && item->name && item->cert_size )
	{
		...
		{
			if (pat_fakesign->is_active && MatchFilePattern (pat_fakesign, item->name, '-'))
			{
				cert_fake_sign (item);
			}
...
static enumError cmd_create ()
{
	...
	static const KeywordTab_t tab[] = { // min + max param
		{ SC_TICKET, "TICKET", "TIK", 0 | 0x100 * 2 }, { SC_TMD, "TMD", 0, 0 | 0x100 * 1 },
	...
	switch (cmd->id)
	{
		case SC_TICKET:
		{
			...
			{
				const enumError err
					= SaveFileOpt (path, 0, opt_overwrite, opt_mkdir, &tik, sizeof (tik), false);
				if (!err)
					return err;
			}
		}
		...
		case SC_TMD:
		{
			...
			if (param)
			{
				...
				{
					const enumError err
						= ScanHex (tmd->content[0].hash, sizeof (tmd->content[0].hash), param->arg);
					if (err)
						return err;
				}
				param = param->next;
			...
			{
				const enumError err
					= SaveFileOpt (path, 0, opt_overwrite, opt_mkdir, tmd, sizeof (tmd_buf), false);
				if (!err)
					return err;
			}
		}
...
static void DumpDolRec (dol_patch_t *dol)
{
	...
	for (i = 0; i < dol->n_rec; i++, rec--)
		printf ("%3u.  %-3s %8x  %6x %6x  %8x\n", i + 1, rec->name, rec->addr, rec->size,
			rec->xsize, rec->delta);
...
static char *ScanHexStringWit (u8 *buf, // destination buffer
...
static enumError CreateSection (dol_patch_t *dol, bool is_data, u32 addr, u32 size)
{
	// align addr and size
	...
	if (verbose >= 0)
	{
		...
		printf ("CREATE %s section '%c%u': %x +%x, offset=%x\n", section_name, *section_name,
			sect_idx - base_idx, addr, size, offset);
	}
...
static enumError PatchDol (dol_patch_t *dol, hex_patch_t *hex)
{
	...
	if (!rec)
	{
		...
		if (!rec)
		{
			if (verbose >= -1)
				printf (
					"!Can't patch: Range outside dol: addr %08x+%02x\n", hex->addr, hex->n_patch);
			dol->warn_count++;
			return ERR_WARNING;
		}
	}
	...
	if (offset > dol->size || offset + size > dol->size // 2 steps because of possible overflow
	...
	if (reason)
	{
		if (verbose >= 1)
		{
			int len = printf ("-%-17s[%s%s] addr %08x+%02x, offset %7x:", reason, rec->name, plus,
				hex->addr, hex->n_patch, offset);
	...
	if (verbose >= 0)
	{
		int len = printf ("+Patched:         [%s%s] addr %08x+%02x, offset %7x:", rec->name, plus,
			hex->addr, hex->n_patch, offset);
...
static void LoadPatchWFile (dol_patch_t *dol, hex_patch_t *hex, ccp param, ccp src_path)
{
	DASSERT (dol);
	DASSERT (hex);
	DASSERT (param);
	...
	{
		size_t size;
		if (!LoadFileAlloc (param, 0, 0, &hex->patch_data, &size, 10 * MiB, 2, 0, false))
		{
			noPRINT (" -> |%s|\n", param);
			hex->n_patch = size;
			return;
	...
	if (source_list.used)
	{
		...
		for (i = 0; i < source_list.used; i++)
		{
			...
			size_t size;
			if (fname
				&& !LoadFileAlloc (fname, 0, 0, &hex->patch_data, &size, MAX_SIZE, 2, 0, false))
			{
				if (verbose >= 1)
	...
	if (!hex->patch_data)
	{
		...
		size_t size;
		if (!LoadFileAlloc (fname, 0, 0, &hex->patch_data, &size, MAX_SIZE, 0, 0, false))
		{
			if (verbose >= 1)
				printf ("+WFile loaded [%zu bytes]: %s\n", size, fname);
			hex->n_patch = size;
...
static enumError ScanXML (dol_patch_t *dol, ccp fname)
{
	...
	u8 *xml;
	size_t xml_size;
	enumError err = LoadFileAlloc (fname, 0, 0, &xml, &xml_size, 10 * MiB, 0, 0, false);
	if (err)
		return err;
...
static enumError cmd_dolpatch ()
{
	...
	enumError err
		= LoadFileAlloc (dol_fname, 0, 0, &dol.data, &dol.size, 100 * MiB, 0, &dol.fatt, false);
	if (err)
		return err;
	...
	for (; param; param = param->next)
	{
		...
		else if (have_entry)
		{
			...
			if (addr != dh.entry_addr)
			{
				if (verbose >= 0)
					printf (
						"+Patched:         [ENTRY POINT] addr %08x -> %08x\n", dh.entry_addr, addr);
				dh.entry_addr = addr;
	...
	if (opt_dest && *opt_dest)
	{
		if (IsDirectory (opt_dest, 0))
		{
			...
			dest_fname
				= PathCatPP (path_buf, sizeof (path_buf), opt_dest, slash ? slash + 1 : dol_fname);
		}
		else
			dest_fname = opt_dest;
	}
	...
	FREE (dol.data);
	return err ? err : dol.warn_count ? ERR_WARNING : dol.cond_count ? ERR_DIFFER : ERR_OK;
}
...
enumError exec_filetype (SuperFile_t *sf, Iterator_t *it)
{
	...
	if (it->long_count > 1)
	{
		...
		printf ("%-8s %-6s %s %s %s  %s\n", ftype, sf->f.id6_dest[0] ? sf->f.id6_dest : "-", size,
			region, split, it->long_count > 2 ? it->real_path : sf->f.fname);
	}
	else if (it->long_count)
	{
		...
		if (sf->f.split_used > 1)
			snprintf (split, sizeof (split), "%2d", sf->f.split_used);
		printf ("%-8s %-6s %s  %s\n", ftype, sf->f.id6_dest[0] ? sf->f.id6_dest : "-", split,
			sf->f.fname);
	}
	else
	{
...
static enumError cmd_filetype ()
{
	...
	it.act_non_iso = ignore_count > 1 ? ACT_IGNORE : ACT_ALLOW;
	it.act_gc = ACT_ALLOW;
	it.act_fst = opt_allow_fst < OFFON_AUTO ? ACT_IGNORE : long_count > 1 ? ACT_EXPAND : ACT_ALLOW;
	it.long_count = long_count;
	const enumError err = SourceIterator (&it, 1, true, false);
	...
	if (it->long_count > 1)
	{
		...
		if (!it->done_count++ && print_header)
			printf ("\n"
					...
					"blocks %*s %*s %*s %*s  %s\n"
					"%s\n",
				size_fw, "ISO", size_fw, ".wbfs", size_fw, "500g", size_fw, "  3t", size_fw,
				wd_get_size_unit (opt_unit, "?"), wbfs_fw, "file", size_fw, "WBFS", size_fw, "WBFS",
				it->long_count > 2 ? "real path" : "filename", sep_79);

		if (blocks)
		{
			// wbfs: size=10g => block size = 2 MiB
			const u32 wfile = 1 + wd_count_used_blocks (wdisc_usage_tab, 2 * WII_SECTORS_PER_MIB);
			// wbfs: size=500g => block size = 8 MiB
			const u32 w500g = wd_count_used_blocks (wdisc_usage_tab, 8 * WII_SECTORS_PER_MIB);

			// wbfs: size=3t => block size = 8 MiB
			const u32 w3t = wd_count_used_blocks (wdisc_usage_tab, 64 * WII_SECTORS_PER_MIB);

			printf ("%6llu %*s %*s %*s %*s  %s\n", blocks, size_fw,
				wd_print_size (0, 0, blocks * WII_SECTOR_SIZE, false, opt_unit), wbfs_fw,
				wd_print_size (0, 0, 2ull * MiB * wfile, false, opt_unit), size_fw,
				wd_print_size (0, 0, 8ull * MiB * w500g, false, opt_unit), size_fw,
				wd_print_size (0, 0, 64ull * MiB * w3t, false, opt_unit), sf->f.fname);
		}
		else
			printf (
				"     - %*s %*s %*s  %s\n", size_fw, "-", wbfs_fw, "-", size_fw, "-", sf->f.fname);
	}
	else if (it->long_count)
	{
		if (!it->done_count++ && print_header)
			printf ("\n"
					...
					"blocks %*s  filename\n"
					"%s\n",
				size_fw, "ISO", size_fw, wd_get_size_unit (opt_unit, "?"), sep_79);

		if (blocks)
			printf ("%6llu %*s  %s\n", blocks, size_fw,
				wd_print_size (0, 0, blocks * WII_SECTOR_SIZE, false, opt_unit), sf->f.fname);
		else
			printf ("     - %*s  %s\n", size_fw, "-", sf->f.fname);
	}
	else
...
enumError exec_print_id6 (SuperFile_t *sf, Iterator_t *it)
{
	...
	else
	{
		...
		printf ("%6s %4s %4s %6s", wd_print_id (&disc->dhead, 6, 0),
	...
	printf (" %-6s %s  %s\n", wd_print_id_col (sf->wbfs_id6, 6, &disc->dhead, colout),
		wd_print_part_name (0, 0, main_part->part_type, WD_PNAME_NAME_NUM_9), sf->f.fname);
...
enumError exec_print_id8 (SuperFile_t *sf, Iterator_t *it)
{
	...
	if (it->long_count < 1)
	{
		printf ("%6s %02x.%02x\n", wd_print_id (&disc->dhead, 6, 0), disc->dhead.disc_number,
			disc->dhead.disc_version);
		return ERR_OK;
	}
	...
	if (!main_part || !disc->n_part)
		printf ("%6s %02x.%02x  --   --    --  ", wd_print_id (&disc->dhead, 6, 0),
	...
	else
	{
		...
		printf ("%6s %02x.%02x %4s %4s %6s %02x.%02x", wd_print_id (&disc->dhead, 6, 0),
			disc->dhead.disc_number, disc->dhead.disc_version,
			wd_print_id_col (tick_id, 4, &disc->dhead, colout),
			wd_print_id_col (tmd_id, 4, &disc->dhead, colout),
			wd_print_id_col (&main_part->boot, 6, &disc->dhead, colout),
	...
	for (pi = 0; pi < disc->n_part; pi++)
	{
		...
		printf ("           > %4s %4s %6s %02x.%02x %16s\n",
			tick_id[0] ? wd_print_id (tick_id, 4, 0) : "-- ",
			part->tmd ? wd_print_id (part->tmd->title_id + 4, 4, 0) : "-",
			wd_print_id (&part->boot, 6, 0), part->boot.dhead.disc_number,
			part->boot.dhead.disc_version,
			wd_print_part_name (0, 0, part->part_type, WD_PNAME_NAME_NUM_9));
	}
	...
	if (print_header)
	{
		if (mode == 8)
			printf ("\n"
					...
					"%s%.79s%s\n",
				colout->heading, colout->reset, colout->heading, colout->reset, colout->heading,
				Minus300, colout->reset);
		else
			printf ("\n"
...
static void print_fragments (SuperFile_t *sf, FileMap_t *fm, ccp title, ccp off1, ccp off2)
{
	uint i;
	...
	printf ("\n"
			...
			"   index  %*s%-*s -> %*s%-*s : %*s\n"
			"  %.*s\n",
		title, oft_info[sf->iod.oft].name, sf->f.fname, fw_h1a, "", fw_h1b, off1, fw_h2a, "",
		fw_h2b, off2, fw_s, "chunk", fw_h1a, "", fw_h1b, "offset", fw_h2a, "", fw_h2b, "offset",
		fw_s, "size", fw_sep, wd_sep_200);

	for (i = 0; i < fm->used; i++)
	{
		const FileMapItem_t *mi = fm->field + i;
		printf ("%7d.  %*llx .. %*llx -> %*llx .. %*llx : %*llx\n", i + 1, fw_1a, mi->src_off,
			fw_1b, mi->src_off + mi->size - 1, fw_2a, mi->dest_off, fw_2b,
			mi->dest_off + mi->size - 1, fw_s, mi->size);
	}
	...
	if (long_count)
	{
		...
		if (long_count != 1)
			printf ("   align: %*llx    %*llx    %*llx    %*llx   %*llx\n", fw_1a,
				GetAlign64 (align1a), fw_1b, GetAlign64 (align1b), fw_2a, GetAlign64 (align2a),
				fw_2b, GetAlign64 (align2b), fw_s, GetAlign64 (align_s));

		if (long_count != 2)
			printf ("   align:%*s %*s %*s %*s %*s\n", fw_1a + 1,
				wd_print_size_1024 (0, 0, GetAlign64 (align1a), false), fw_1b + 3,
				wd_print_size_1024 (0, 0, GetAlign64 (align1b), false), fw_2a + 3,
				wd_print_size_1024 (0, 0, GetAlign64 (align2a), false), fw_2b + 3,
				wd_print_size_1024 (0, 0, GetAlign64 (align2b), false), fw_s + 2,
				wd_print_size_1024 (0, 0, GetAlign64 (align_s), false));
	}
}
...
static enumError exec_fragments (SuperFile_t *sf, Iterator_t *it)
{
	...
	if (brief_count)
	{
		...
		if (long_count)
		{
			...
			wd_print_size (buf, sizeof (buf), GetAlign64 (align), true, WD_SIZE_AUTO);
		}
		else
			*buf = 0;
		...
		if (!is_reg)
			...
			printf ("%4u %6u %6u%c %s %-4s %s\n", fm1.used, fm2.used, fm3.used,
				oft_info[sf->iod.oft].attrib & OFT_A_LOADER && fm3.used > LOADER_MAX_FRAGMENTS
					? '!'
					: ' ',
				buf, oft_info[sf->iod.oft].name, sf->f.fname);
		else
			printf ("%4u%c%6s %6s %s %-4s %s\n", fm1.used,
				oft_info[sf->iod.oft].attrib & OFT_A_LOADER && fm1.used > LOADER_MAX_FRAGMENTS
					? '!'
					: ' ',
				"-", "-", buf, oft_info[sf->iod.oft].name, sf->f.fname);
	}
...
static enumError cmd_list (int long_level)
{
	...
	if (print_header)
	{
		...
		{
			const int plen = strlen (witem->title ? witem->title : witem->name64);
			if (max_name_wd < plen)
				max_name_wd = plen;
		...
		footer_len = strcmp (size1, size2)
			? snprintf (footer, sizeof (footer), "Total: %u disc%s, %s ~ %s used.", wlist.used,
				  wlist.used == 1 ? "" : "s", size1, size2)
			: snprintf (footer, sizeof (footer), "Total: %u disc%s, %s used.", wlist.used,
				  wlist.used == 1 ? "" : "s", size1);
	}
	...
	if (long_count)
	{
		...
		if (print_header)
		{
			...
			printf ("ID6   %s %*s Reg.  %n%d discs (%s)%n\n", pt.head, size_fw,
				wd_get_size_unit (column_unit, "?"), &n1, wlist.used,
		...
		for (witem = wlist.first_disc; witem < wend; witem++)
		{
			const u64 size = witem->fatt.size ? (u64)witem->fatt.size : witem->size_mib * (u64)MiB;
			printf ("%s%s %*s %s  %s\n", witem->id6, PrintTime (&pt, &witem->fatt), size_fw,
				wd_print_size (0, 0, size, false, column_unit),
				GetRegionInfo (witem->id6[3])->name4, witem->title ? witem->title : witem->name64);
			if (line2)
				printf ("%6d %-*s %-8s %s\n", witem->n_part, part_info_fw, witem->part_info,
					GetNameFT (witem->ftype, 0), witem->fname ? witem->fname : "");
		}
	}
	else
	{
		if (print_header)
		{
			...
			printf ("ID6    %s %n%d disc%s (%s)%n\n", pt.head, &n1, wlist.used,
				wlist.used == 1 ? "" : "s", wd_print_size (0, 0, total_size, false, total_unit),
				&n2);
			max_name_wd += n1;
			if (max_name_wd < n2)
				max_name_wd = n2;
...
enumError exec_files (SuperFile_t *fi, Iterator_t *it)
{
	...
	if (fi->f.ftype & FT__SPC_MASK || !fi->f.id6_dest[0])
		return OptionUsed[OPT_IGNORE] ? ERR_OK : PrintErrorFT (&fi->f, FT_A_WII_ISO);
	...
	if (it.show_mode & SHOW__DEFAULT)
	{
		switch (long_count)
		{
			case 0:
				...
				break;
			case 1:
				it.show_mode = SHOW_F_HEAD | SHOW_SIZE | SHOW_F_DEC;
				break;
			case 2:
				it.show_mode = SHOW_F_HEAD | SHOW__ALL & ~SHOW_PATH;
				break;
			default:
				it.show_mode = SHOW_F_HEAD | SHOW__ALL;
				break;
		}
	}
	...
	if (testmode || verbose >= 0)
	{
		if (print_sections)
		{
			printf ("[EXTRACT]\n"
					...
					"dest-type=FST\n"
					"\n",
				testmode > 0, it->source_index + 1, it->source_list.used, fi->f.fname,
				it->real_path, oft_info[fi->iod.oft].name, fi->f.split_used, dest_dir);
		}
		else
		{
			char split_buf[10];
			...
			printf ("%s: %sEXTRACT %*u/%u %s%s:%s -> %s\n", ProgInfo.progname,
				testmode ? "WOULD " : "", count_fw, it->source_index + 1, it->source_list.used,
				oft_info[fi->iod.oft].name, split_buf, fi->f.fname, dest_dir);
		}
		fflush (0);
...
static enumError get_two_params (ccp *source, ccp *dest)
{
	...
	if (!param || !param->next || param->next->next)
		return ERROR0 (ERR_SYNTAX, "Exactly 2 parameters expected: source and destination.\n");
	*source = param->arg;
	*dest = param->next->arg;
	return ERR_OK;
}
...
static enumError check_dest (ccp dest)
{
	...
	if (!stat (dest, &st) && !OptionUsed[OPT_OVERWRITE])
		return ERROR0 (
			ERR_ALREADY_EXISTS, "Destination already exists (use --overwrite): %s\n", dest);
	return ERR_OK;
}
...
static enumError cmd_riivolution ()
{
	...
	if (n_params == 0 && !riiv_options.xml_file)
	{
		return ERROR0 (ERR_SYNTAX,
			"wit RIIVOLUTION requires XML file and game image / FST source.\n"
	...
	{
		return ERROR0 (ERR_SYNTAX,
			"Riivolution builder requires both a source image/FST and a Riivolution XML.\n"
			"Usage: wit RIIVOLUTION [options] source xml [dest]\n"
			"       wit RIIVOLUTION [options] xml\n");
	}
...
static enumError cmd_brawlbuilder ()
{
	...
	{
		return ERROR0 (ERR_SYNTAX,
			"wit BRAWLBUILDER requires source Brawl image and mod folder.\n"
...
static enumError cmd_patch ()
{
	...
	if (!patch_options.list_only)
	{
		...
		if (n_params == 0)
			return ERROR0 (ERR_SYNTAX,
				"wit PATCH requires a source game image / FST directory.\n"
...
enumError exec_copy (SuperFile_t *fi, Iterator_t *it)
{
	...
	const bool convert_it = it->convert_it && !it->diff_it
		&& (output_file_type == OFT_UNKNOWN || output_file_type == fi->iod.oft);

	ccp fname = fi->f.path ? fi->f.path : fi->f.fname;
	if (!fi->oft_orig)
		fi->oft_orig = fi->iod.oft == OFT_FST ? OFT__DEFAULT : fi->iod.oft;
	const enumOFT oft
		= convert_it ? fi->oft_orig : CalcOFT (output_file_type, opt_dest, fname, fi->oft_orig);
	...
	ccp job_mode = raw_mode ? "RAW" : it->diff_it && OptionUsed[OPT_FILES] ? "FILES" : "SCRUB";
	...
	if (testmode || verbose >= 0)
	{
		...
		if (print_sections)
		{
			printf ("[%s]\n"
					...
					"dest-type=%s\n"
					"\n",
				job, testmode > 0, job_mode, it->source_index + 1, it->source_list.used,
				fi->f.fname, it->real_path, oft_info[fi->iod.oft].name, fi->f.split_used,
				fo.f.fname, it->diff_it ? "?" : oft_info[oft].name);
		}
		else
		{
			char split_buf[10];
			...
			printf ("* %s%s/%s %*u/%u %s%s:%s %s %s:%s\n", testmode ? "WOULD " : "", job, job_mode,
				count_fw, it->source_index + 1, it->source_list.used, oft_info[fi->iod.oft].name,
				split_buf, fi->f.fname, it->diff_it ? ":" : "->",
	...
	if (it->diff_it)
	{
			...
			if (err == ERR_DIFFER)
			{
				...
				if (verbose >= 0 && print_sections)
					printf ("[differ]\n"
							...
							"dest-type=%s\n"
							"\n",
						job_mode, it->source_index + 1, it->source_list.used, fi->f.fname,
						oft_info[fi->iod.oft].name, fo.f.fname, oft_info[oft].name);
			}
			it->done_count++;
			ResetSF (&fo, 0);
			return err;
		...
		else
		{
			err = !raw_mode && OptionUsed[OPT_FILES]
				? oldDiffFilesSF (fi, &fo, it->long_count, GetDefaultFilePattern (), prefix_mode)
				...
				if (verbose >= 0)
				{
					if (print_sections)
						...
						printf ("! ISOs differ: %s:%s : %s:%s\n", oft_info[fi->iod.oft].name,
							fi->f.fname, oft_info[fo.iod.oft].name, fo.f.fname);
				}
			}
			it->done_count++;
			ResetSF (&fo, 0);
	...
	enumError err
		= CopyImage (fi, &fo, oft, it->overwrite, OptionUsed[OPT_PRESERVE] != 0, it->remove_source);
	...
	if (testmode || verbose >= 0)
	{
		if (print_sections)
		{
			printf ("[EDIT]\n"
					...
					"\n",
				testmode > 0, it->source_index + 1, it->source_list.used, fi->f.fname,
				it->real_path, oft_info[fi->iod.oft].name, fi->f.split_used);
		}
		else
		{
			char split_buf[10];
			...
			printf (" - %sEDIT %*u/%u %s%s:%s\n", testmode ? "WOULD " : "", count_fw,
				it->source_index + 1, it->source_list.used, oft_info[fi->iod.oft].name, split_buf,
				fi->f.fname);
		}
		fflush (0);
...
enumError exec_remove (SuperFile_t *fi, Iterator_t *it)
{
	...
	char split_buf[10];
	const int count_fw = snprintf (split_buf, sizeof (split_buf), "%u", it->source_list.used);
	if (fi->f.split_used > 1)
...
enumError exec_move (SuperFile_t *fi, Iterator_t *it)
{
	...
	ccp oname = fi->iod.oft == OFT_WBFS && fi->f.id6_dest[0] ? fi->f.id6_dest
		: fi->f.outname										 ? fi->f.outname
															 : fi->f.fname;
	...
	if (strcmp (fi->f.fname, fo.f.fname))
	{
		...
		if (!stat_status && fi->f.st.st_dev == fo.f.st.st_dev && fi->f.st.st_ino == fo.f.st.st_ino)
		{
			...
			if (testmode || verbose >= 0)
			{
				if (print_sections)
				{
					printf ("[MOVE]\n"
							...
							"\n",
						testmode > 0, it->source_index + 1, it->source_list.used, fi->f.fname,
						it->real_path, oft_info[fi->iod.oft].name, fi->f.split_used, fo.f.fname);
				}
				else
				{
					char split_buf[10];
					...
					printf (" - %sMOVE %*u/%u %s%s:%s -> %s\n", testmode ? "WOULD " : "", count_fw,
						it->source_index + 1, it->source_list.used, oft_info[fi->iod.oft].name,
						split_buf, fi->f.fname, fo.f.fname);
				}
				fflush (0);
			}
...
enumError exec_rename (SuperFile_t *fi, Iterator_t *it)
{
	...
	if (fi->iod.oft == OFT_WBFS)
	{
		ASSERT (fi->wbfs);
		return RenameWDisc (
			fi->wbfs, param->id6, param->arg, change_wbfs, change_iso, verbose, testmode);
	}
	...
	if (RenameISOHeader (buf, fi->f.fname, param->id6, param->arg, verbose, testmode))
	{
	...
	for (;;)
	{
		...
		switch ((enumGetOpt)opt_stat)
		{
			...
			case GO_FORCE:
				...
				break;
			case GO_DSYNC:
				err += ScanOptDSync (optarg);
				break;

			case GO_TITLES:
				AtFileHelper (optarg, 0, 0, AddTitleFile);
				break;
			case GO_UTF_8:
				use_utf8 = true;
				break;
			...
			case GO_LANG:
				...
				break;
			case GO_CERT:
				AtFileHelper (optarg, 0, 0, AddCertFile);
				break;
			case GO_FAKE_SIGN:
				err += ScanRule (optarg, PAT_FAKE_SIGN);
				break;
			...
			case GO_SOURCE:
				AppendStringFieldExpand (&source_list, optarg, 0, false);
				break;
			case GO_NO_EXPAND:
				opt_no_expand = true;
				break;
			case GO_RECURSE:
				AppendStringFieldExpand (&recurse_list, optarg, 0, false);
				break;
			case GO_RDEPTH:
				err += ScanOptRDepth (optarg);
				break;
			case GO_AUTO:
				opt_source_auto++;
				break;
			...
			case GO_INCLUDE:
				AtFileHelper (optarg, SEL_ID, SEL_FILE, AddIncludeID);
				break;
			case GO_EXCLUDE:
				AtFileHelper (optarg, SEL_ID, SEL_FILE, AddExcludeID);
				break;
			case GO_INCLUDE_PATH:
				AtFileHelper (optarg, 0, 0, AddIncludePath);
				break;
			case GO_EXCLUDE_PATH:
				AtFileHelper (optarg, 0, 0, AddExcludePath);
				break;
			case GO_INCLUDE_FIRST:
				include_first = true;
				break;
			...
			case GO_PSEL:
				err += ScanOptPartSelector (optarg);
				break;
			case GO_RAW:
				part_selector.whole_disc = part_selector.whole_part = true;
				break;
			case GO_FLAT:
				opt_flat++;
				break;
			...
			case GO_HOOK:
				...
				break;
			case GO_ENC:
				err += ScanOptEncoding (optarg);
				break;
			case GO_REGION:
				err += ScanOptRegion (optarg);
				break;
			case GO_COMMON_KEY:
				err += ScanOptCommonKey (optarg);
				break;
			case GO_IOS:
				err += ScanOptIOS (optarg);
				break;
			case GO_MODIFY:
				err += ScanOptModify (optarg);
				break;
			case GO_HTTP:
				err += ScanOptDomain (1, 0, 0);
				break;
			case GO_DOMAIN:
				err += ScanOptDomain (0, 0, optarg);
				break;
			case GO_SECURITY_FIX:
				err += ScanOptDomain (0, 1, 0);
				break;
			case GO_WIIMMFI:
				err += ScanOptDomain (1, 1, "wiimmfi.de");
				break;
			case GO_TWIIMMFI:
				err += ScanOptDomain (1, 1, "test.wiimmfi.de");
				break;
			case GO_NAME:
				err += ScanOptName (optarg);
				break;
			case GO_ID:
				err += ScanOptId (optarg);
				break;
			case GO_DISC_ID:
				err += ScanOptDiscId (optarg);
				break;
			case GO_BOOT_ID:
				err += ScanOptBootId (optarg);
				break;
			case GO_TICKET_ID:
				err += ScanOptTicketId (optarg);
				break;
			case GO_TMD_ID:
				err += ScanOptTmdId (optarg);
				break;
			case GO_TT_ID:
				err += ScanOptTicketId (optarg);
				err += ScanOptTmdId (optarg);
				break;
			case GO_WBFS_ID:
				err += ScanOptWbfsId (optarg);
				break;
			case GO_FILES:
				err += ScanRule (optarg, PAT_FILES);
				break;
			case GO_RM_FILES:
				err += ScanRule (optarg, PAT_RM_FILES);
				break;
			case GO_ZERO_FILES:
				err += ScanRule (optarg, PAT_ZERO_FILES);
				break;
			case GO_IGNORE_FILES:
				err += ScanRule (optarg, PAT_IGNORE_FILES);
				break;
			case GO_REPL_FILE:
				err += ScanOptFile (optarg, false);
				break;
			case GO_ADD_FILE:
				err += ScanOptFile (optarg, true);
				break;
			case GO_TRIM:
				err += ScanOptTrim (optarg);
				break;
			case GO_ALIGN:
				err += ScanOptAlign (optarg);
				break;
			case GO_ALIGN_PART:
				err += ScanOptAlignPart (optarg);
				break;
			case GO_ALIGN_FILES:
				opt_align_files = true;
				break;
			case GO_DISC_SIZE:
				err += ScanOptDiscSize (optarg);
				break;
			case GO_OVERLAY:
				break;
			case GO_PATCH_FILE:
				opt_patch_file = optarg;
				break;
			case GO_RIIV_ROOT:
				riiv_options.root_dir = optarg;
				break;
			case GO_RIIV_CHOICE:
				AppendStringField (&riiv_options.choices, optarg, false);
				break;
			case GO_RIIV_ALL:
				riiv_options.all_choices = true;
				break;
			case GO_RIIV_DEFAULT:
				riiv_options.default_choices = true;
				break;
			case GO_RIIV_INFO:
				riiv_options.info_only = true;
				break;
			case GO_RIIV_IGNORE_REGIONS:
				riiv_options.ignore_regions = true;
				break;
			case GO_RIIV_SAVE_XML:
				riiv_options.save_xml_path = optarg;
				break;
			case GO_KEEP_TEMP:
				riiv_options.keep_temp = true;
				break;
			case GO_RIIV_XML:
				riiv_options.xml_file = optarg;
				break;
			case GO_RIIV_GCT:
				...
				break;
			case GO_RIIV_DOL:
				riiv_options.custom_dol = optarg;
				break;
			case GO_RIIV_INTERACTIVE:
				...
				break;
			case GO_BRAWL_MOD:
				brawl_options.mod_folder = optarg;
				break;
			case GO_BRAWL_REMOVE_SSE:
				brawl_options.remove_sse = true;
				break;
			case GO_BRAWL_BANNER:
				brawl_options.banner_file = optarg;
				break;
			case GO_BRAWL_NO_GCT_PATCH:
				brawl_options.no_gct_patch = true;
				break;
			case GO_BRAWL_NO_ALT_PAD:
				brawl_options.no_alt_pad = true;
				break;
			case GO_BRAWL_OFFSET:
				brawl_options.gct_offset = (u32)strtoul (optarg, NULL, 16);
				break;
			case GO_PATCH_LIST:
				patch_options.list_only = true;
				break;
			case GO_PATCH_SELECT:
				AppendStringField (&patch_options.select, optarg, false);
				break;
			case GO_DEST:
			...
			case GO_AUTO_SPLIT:
				...
				break;
			case GO_NO_SPLIT:
				opt_auto_split = opt_split = 0;
				break;
			case GO_SPLIT:
				opt_split++;
				break;
			case GO_SPLIT_SIZE:
				err += ScanOptSplitSize (optarg);
				break;
			case GO_PREALLOC:
				err += ScanPreallocMode (optarg);
				break;
			case GO_TRUNC:
				opt_truncate++;
				break;
			case GO_CHUNK_MODE:
				err += ScanChunkMode (optarg);
				break;
			case GO_CHUNK_SIZE:
				err += ScanChunkSize (optarg);
				break;
			case GO_MAX_CHUNKS:
				err += ScanMaxChunks (optarg);
				break;
			case GO_COMPRESSION:
				err += ScanOptCompression (false, optarg);
				break;
			case GO_MEM:
				err += ScanOptMem (optarg, true);
				break;
			case GO_PRESERVE:
				break;
			case GO_UPDATE:
			...
			case GO_WDF:
				err += SetModeWDF (0, optarg);
				break;
			case GO_WDF1:
				err += SetModeWDF (1, optarg);
				break;
			case GO_WDF2:
				err += SetModeWDF (2, optarg);
				break;
			case GO_ALIGN_WDF:
				err += ScanOptAlignWDF (optarg, 0);
				break;

			case GO_WIA:
				err += ScanOptCompression (true, optarg);
				break;
			case GO_ISO:
				output_file_type = OFT_PLAIN;
				break;
			case GO_CISO:
				output_file_type = OFT_CISO;
				break;
			case GO_WBFS:
				output_file_type = OFT_WBFS;
				break;
			case GO_GCZ:
				output_file_type = OFT_GCZ;
				break;
			case GO_GCZ_ZIP:
				opt_gcz_zip = true;
				break;
			case GO_GCZ_BLOCK:
				err += ScanOptGCZBlock (optarg);
				break;
			case GO_FST:
				output_file_type = OFT_FST;
				break;
			case GO_ALLOW_FST:
				err += ScanOptAllowFST (optarg);
				break;
			case GO_ALLOW_NKIT:
				err += ScanOptAllowNKIT (optarg);
				break;

			case GO_JSON:
				script_fform = PSFF_JSON;
				break;
			case GO_SH:
				script_fform = PSFF_SH;
				break;
			case GO_BASH:
				script_fform = PSFF_BASH;
				break;
			case GO_PHP:
				script_fform = PSFF_PHP;
				break;
			case GO_MAKEDOC:
				script_fform = PSFF_MAKEDOC;
				break;
			case GO_VAR:
			...
			case GO_ITIME:
				SetTimeOpt (PT_USE_ITIME | PT_F_ITIME);
				break;
			case GO_MTIME:
				SetTimeOpt (PT_USE_MTIME | PT_F_MTIME);
				break;
			case GO_CTIME:
				SetTimeOpt (PT_USE_CTIME | PT_F_CTIME);
				break;
			case GO_ATIME:
				SetTimeOpt (PT_USE_ATIME | PT_F_ATIME);
				break;
			...
			case GO_SECTIONS:
				...
				break;
			case GO_SHOW:
				err += ScanOptShow (optarg);
				break;
			case GO_UNIT:
				err += ScanOptUnit (optarg);
				break;
			case GO_SORT:
				err += ScanOptSort (optarg);
				break;
			case GO_NO_SORT:
				err += ScanOptSort ("none");
				break;
			...
			case GO_BLOCK_SIZE:
			{
				u32 block_size;
				if (ScanSizeOptU32 (
						&block_size, optarg, 1, 0, "block-size", 0, INT_MAX, 0, 0, true))
					err++;
				else
...
enumError CheckCommand (int argc, char **argv)
{
	...
	switch ((enumCommands)cmd_ct->id)
	{
		...
		case CMD_HELP:
			PrintHelp (
				&InfoUI_wit, stdout, 0, "HELP", 0, URI_HOME, first_param ? first_param->arg : 0);
			break;
		case CMD_CONFIG:
			err = cmd_config ();
			break;
		...
		case CMD_XCONVERT:
			err = cmd_xconvert ();
			break;
		case CMD_RIIVOLUTION:
			err = cmd_riivolution ();
			break;
		case CMD_BRAWLBUILDER:
			err = cmd_brawlbuilder ();
			break;
		case CMD_PATCH:
			err = cmd_patch ();
			break;
