	...
	*sp = 0;
	...
	//----- setup common key
	{
		...
		static u8 key_mask[WD_CKEY__N][WII_KEY_SIZE] = {
			{ 0x25, 0x00, 0x94, 0x12, 0x2a, 0x3e, 0x4e, 0x1f, 0x03, 0x2d, 0xc5, 0xc9, 0x30, 0x2a,
				0x58, 0xab },
			{ 0xad, 0x5c, 0x95, 0x84, 0x80, 0xda, 0x93, 0xd5, 0x58, 0x06, 0xfe, 0x77, 0xf9, 0xe7,
				0x69, 0x22 },
#ifdef SUPPORT_CKEY_DEVELOP
			{ 0x6f, 0x84, 0xf4, 0x5a, 0x05, 0x98, 0x68, 0xd2, 0xe5, 0x7f, 0xec, 0xbe, 0x8b, 0xbd,
				0x0e, 0xf6 },
#endif
		};
...
ccp LibGetErrorName (int stat, ccp ret_not_found)
{
	switch (stat)
	{
		...
		case ERR_NO_WDF:
			return "NO WDF";
		case ERR_WDF_VERSION:
			return "WDF VERSION NOT SUPPORTED";
		case ERR_WDF_SPLIT:
			return "SPLITTED WDF NOT SUPPORTED";
		case ERR_WDF_INVALID:
			return "INVALID WDF";

		case ERR_NO_CISO:
			return "NO CISO";
		case ERR_CISO_INVALID:
			return "INVALID CISO";

		case ERR_NO_GCZ:
			return "NO GCZ";
		case ERR_GCZ_INVALID:
			return "INVALID GCZ";

		case ERR_WPART_INVALID:
			return "INVALID WII PARTITION";
		case ERR_WDISC_INVALID:
			return "INVALID WII DISC";
		case ERR_WDISC_NOT_FOUND:
			return "WII DISC NOT FOUND";

		case ERR_NO_WBFS_FOUND:
			return "NO WBFS FOUND";
		case ERR_TO_MUCH_WBFS_FOUND:
			return "TO MUCH WBFS FOUND";
		case ERR_WBFS_INVALID:
			return "INVALID WBFS";

		case ERR_NO_WIA:
			return "NO WIA FOUND";
		case ERR_WIA_INVALID:
			return "INVALID WIA";
		case ERR_BZIP2:
			return "BZIP2 ERROR";
		case ERR_LZMA:
...
ccp LibGetErrorText (int stat, ccp ret_not_found)
{
	switch (stat)
	{
		...
		case ERR_NO_WDF:
			return "File is not a WDF";
		case ERR_WDF_VERSION:
			return "WDF version not supported";
		case ERR_WDF_SPLIT:
			return "Splitted WDF not supported";
		case ERR_WDF_INVALID:
			return "File is an invalid WDF";

		case ERR_NO_CISO:
			return "File is not a CISO";
		case ERR_CISO_INVALID:
			return "File is an invalid CISO";

		case ERR_NO_GCZ:
			return "File is not a GCZ";
		case ERR_GCZ_INVALID:
			return "File is an invalid GCZ";
		...
		case ERR_NO_WIA:
			return "File is not a WIA";
		case ERR_WIA_INVALID:
			return "File is an invalid WIA";
		case ERR_BZIP2:
			return "A bzip2 error occurred";
		case ERR_LZMA:
			return "A lzma error occurred";

		case ERR_WBFS:
			return "A WBFS error occurred";
	}
	return ret_not_found;
}
	...
	)
{
	...
	ccp end = ptr + max_len;
	while (ptr != end
		&& (*ptr >= 'A' && *ptr <= 'Z' || ignore_case && *ptr >= 'a' && *ptr <= 'z'
			|| *ptr >= '0' && *ptr <= '9' || allow_point && *ptr == '.' || *ptr == '_'))
		ptr++;
...
#define AT PT_USE_ATIME | PT_F_ATIME
	...
	static const KeywordTab_t tab[] = { { 0, "RESET", "-", PT__MASK },

		{ PT_DISABLED, "OFF", 0, EM }, { PT_ENABLED, "ON", 0, EM },
		...
		{ E | IT, "I", 0, EM | PT__USE_MASK }, { E | MT, "M", 0, EM | PT__USE_MASK },
		{ E | CT, "C", 0, EM | PT__USE_MASK }, { E | AT, "A", 0, EM | PT__USE_MASK },
...
char *PrintTime (PrintTime_t *pt, const FileAttrib_t *fa)
{
	...
	{
		const time_t *timbuf[]
			= { &fa->itime.tv_sec, &fa->mtime.tv_sec, &fa->ctime.tv_sec, &fa->atime.tv_sec, 0 };
...
time_t SelectTime (const FileAttrib_t *fa, int opt_time)
{
	...
	{
		case PT_USE_ITIME:
			return fa->itime.tv_sec;
		case PT_USE_CTIME:
			return fa->ctime.tv_sec;
		case PT_USE_ATIME:
			return fa->atime.tv_sec;
		default:
			return fa->mtime.tv_sec;
	}
}
...
{
	static ccp tab[] = { "%Y-%m-%d %H:%M:%S", "%Y-%m-%d %H:%M", "%Y-%m-%d %H%M%S", "%Y-%m-%d %H%M",
		"%Y-%m-%d %H", "%Y-%m-%d", "%Y%m%d %H%M%S", "%Y%m%d %H%M", "%Y%m%d %H", "%Y%m%d", "%s", 0 };
...
u64 ScanSizeFactorWii (char ch_factor, int force_base)
{
	switch (ch_factor)
	{
		case 's':
		case 'S':
			return WII_SECTOR_SIZE;
		case 'u':
		case 'U':
			return GC_DISC_SIZE;
		case 'w':
		case 'W':
			return WII_SECTORS_SINGLE_LAYER * (u64)WII_SECTOR_SIZE;
	}
	return 0;
}
...
int ScanOptSplitSize (ccp source)
{
	...
	opt_split++;
	return ERR_OK
		!= ScanSizeOptU64 (&opt_split_size, // u64 * num
			source, // ccp source
			GiB, // default_factor1
			0, // int force_base
...
{
	return ERR_OK
		!= ScanSizeOptU32 (&opt_recurse_depth, // u32 * num
			source, // ccp source
			1, // default_factor1
			0, // int force_base
			"rdepth", // ccp opt_name
...
char *ScanHexHelper (void *buf, // valid pointer to result buf
...
enumError ScanHex (void *buf, // valid pointer to result buf
	int buf_size, // number of byte to read
	ccp arg // source string
)
{
...
enumError ScanHexSilent (void *buf, // valid pointer to result buf
	int buf_size, // number of byte to read
	ccp arg // source string
)
{
...
static wd_compression_t ScanCompression_helper (ccp arg, // argument to scan
...
)
{
	...
	static const KeywordTab_t tab[]
		= { { WD_COMPR_NONE, "NONE", 0, 0 }, { WD_COMPR_PURGE, "PURGE", "WDF", 0 },
			  { WD_COMPR_BZIP2, "BZIP2", "BZ2", 0 }, { WD_COMPR_LZMA, "LZMA", "LZ", 0 },
			  { WD_COMPR_LZMA2, "LZMA2", "LZ2", 0 }, { WD_COMPR_ZSTD, "ZSTD", "ZS", 0 },

			  { WD_COMPR__DEFAULT, "DEFAULT", "D", 0 }, { WD_COMPR__FAST, "FAST", "F", 0x300 + 10 },
			  { WD_COMPR__GOOD, "GOOD", "G", 0 }, { WD_COMPR__BEST, "BEST", "B", 0x900 + 50 },

			  { COMPR_MEM, "MEM", "M", 0 },

			  { WD_COMPR_NONE, "0", 0, 0 }, { WD_COMPR_LZMA, "1", 0, 0x100 + 5 },
			  { WD_COMPR_LZMA, "2", 0, 0x200 + 10 }, { WD_COMPR_LZMA, "3", 0, 0x300 + 30 },
			  { WD_COMPR_LZMA, "4", 0, 0x400 + 30 }, { WD_COMPR_LZMA, "5", 0, 0x400 + 50 },
			  { WD_COMPR_LZMA, "6", 0, 0x500 + 20 }, { WD_COMPR_LZMA, "7", 0, 0x500 + 50 },
			  { WD_COMPR_LZMA, "8", 0, 0x600 + 50 }, { WD_COMPR_LZMA, "9", 0, 0x900 + 50 },
				...
				if (found)
				{
					*found++ = 0;
					...
					if (*end)
					{
						if (!silent)
							ERROR0 (
								ERR_SYNTAX, "Unsigned number expected: --compression @%s\n", found);
						return -1;
					}
...
wd_compression_t ScanCompression (ccp arg, // argument to scan
	bool silent, // don't print error message
	int *level, // not NULL: appendix '.digit' allowed
				// The level will be stored in '*level'
	u32 *chunk_size // not NULL: appendix '@size' allowed
...
int ScanOptCompression (bool set_oft_wia, // true: output_file_type := OFT_WIA
...
int ScanOptMem (ccp arg, // argument to scan
	bool print_err // true: print error messages
)
{
	u64 num;
	enumError err = ScanSizeOptU64 (&num, // u64 * num,
		arg, // ccp source,
...
{
	static const KeywordTab_t tab[] = { { SORT_NONE, "NONE", "-", SORT__MASK },

		{ SORT_ID, "ID", 0, SORT__MODE_MASK }, { SORT_NAME, "NAME", "N", SORT__MODE_MASK },
		{ SORT_TITLE, "TITLE", "T", SORT__MODE_MASK }, { SORT_PATH, "PATH", "P", SORT__MODE_MASK },
		{ SORT_NINTENDO, "NINTENDO", "NIN", SORT__MODE_MASK },
		{ SORT_FILE, "FILE", "F", SORT__MODE_MASK }, { SORT_SIZE, "SIZE", "SZ", SORT__MODE_MASK },
		{ SORT_OFFSET, "OFFSET", "OF", SORT__MODE_MASK },
		{ SORT_REGION, "REGION", "R", SORT__MODE_MASK }, { SORT_WBFS, "WBFS", 0, SORT__MODE_MASK },
		{ SORT_NPART, "NPART", 0, SORT__MODE_MASK }, { SORT_FRAG, "FRAGMENTS", 0, SORT__MODE_MASK },
		...
		{ SORT_TIME, "TIME", "TI", SORT__MODE_MASK }, { SORT_TIME, "DATE", "D", SORT__MODE_MASK },
...
ShowMode ScanShowMode (ccp arg)
{
	...
	static const KeywordTab_t tab[]
		= { { SHOW__NONE, "NONE", "-", SHOW__ALL }, { SHOW__ALL, "ALL", 0, 0 },

			  { SHOW_INTRO, "INTRO", 0, 0 }, { SHOW_FHEADER, "FHEADER", 0, 0 },
			  { SHOW_SLOT, "SLOTS", 0, 0 }, { SHOW_GEOMETRY, "GEOMETRY", 0, 0 },
			  { SHOW_D_ID, "D-ID", "DID", 0 }, { SHOW_P_ID, "P-IDS", "PIDS", 0 },
			  { SHOW_P_TAB, "P-TAB", "PTAB", 0 }, { SHOW_P_INFO, "P-INFO", "PINFO", 0 },
			  { SHOW_P_MAP, "P-MAP", "PMAP", 0 }, { SHOW_D_MAP, "D-MAP", "DMAP", 0 },
			  { SHOW_W_MAP, "W-MAP", "WMAP", 0 }, { SHOW_CERT, "CERTIFICATES", 0, 0 },
			  { SHOW_TICKET, "TICKET", "TIK", 0 }, { SHOW_TMD, "TMD", 0, 0 },
			  { SHOW_USAGE, "USAGE", 0, 0 }, { SHOW_FILES, "FILES", 0, 0 },
			  { SHOW_PATCH, "PATCH", 0, 0 }, { SHOW_RELOCATE, "RELOCATE", 0, 0 },
			  { SHOW_PATH, "PATH", 0, 0 }, { SHOW_CHECK, "CHECK", 0, 0 },

			  { SHOW_UNUSED, "UNUSED", 0, 0 }, { SHOW_OFFSET, "OFFSET", 0, 0 },
			  { SHOW_SIZE, "SIZE", 0, 0 },

			  { SHOW__ID, "IDS", 0, 0 }, { SHOW__PART, "PART", 0, 0 }, { SHOW__DISC, "DISC", 0, 0 },
			  { SHOW__MAP, "MAP", 0, 0 },

			  { DEC_ALL, "DEC", 0, SHOW_F_HEX1 }, { 0, "-DEC", 0, SHOW_F_DEC },
			  { DEC_ALL, "=DEC", 0, HEX_ALL }, { HEX_ALL, "HEX", 0, SHOW_F_DEC1 },
			  { 0, "-HEX", 0, SHOW_F_HEX }, { HEX_ALL, "=HEX", 0, DEC_ALL },

			  { SHOW_F_HEAD, "HEADER", 0, 0 }, { SHOW_F_PRIMARY, "PRIMARY", "1", 0 },
...
int ConvertShow2PFST (ShowMode show_mode, // show mode
...
)
{
	...
	noTRACE (" --> val=%x, off/size=%x, dec/hex=%x\n", show_mode, show_mode & MASK_OFF_SIZE,
		show_mode & MASK_DEC_HEX);
	...
	if (show_mode & SHOW_SIZE)
	{
		switch (show_mode & MASK_DEC_HEX)
		{
			...
			default:
				pfst |= WD_PFST_SIZE_DEC | WD_PFST_SIZE_HEX;
				break;
		}
	}
...
wd_size_mode_t ScanUnit (ccp arg)
{
	...
	static const KeywordTab_t tab[] = { { WD_SIZE_DEFAULT, "DEFAULT", 0,
											WD_SIZE_M_MODE | WD_SIZE_M_BASE },

		{ WD_SIZE_F_1000, "1000", "10", WD_SIZE_M_BASE },
		{ WD_SIZE_F_1024, "1024", "2", WD_SIZE_M_BASE },

		{ WD_SIZE_AUTO, "AUTO", 0, WD_SIZE_M_MODE }, { WD_SIZE_BYTES, "BYTES", 0, WD_SIZE_M_MODE },
		{ WD_SIZE_K, "K", 0, WD_SIZE_M_MODE }, { WD_SIZE_M, "M", 0, WD_SIZE_M_MODE },
		{ WD_SIZE_G, "G", 0, WD_SIZE_M_MODE }, { WD_SIZE_T, "T", 0, WD_SIZE_M_MODE },
		{ WD_SIZE_P, "P", 0, WD_SIZE_M_MODE }, { WD_SIZE_E, "E", 0, WD_SIZE_M_MODE },
		...
		{ WD_SIZE_GC, "GAMECUBE", "GC", WD_SIZE_M_MODE }, { WD_SIZE_WII, "WII", 0, WD_SIZE_M_MODE },
...
{
	static const KeywordTab_t tab[] = { { REPAIR_NONE, "NONE", "-", REPAIR_ALL },

		{ REPAIR_FBT, "FBT", "F", 0 }, { REPAIR_INODES, "INODES", "I", 0 },
		{ REPAIR_DEFAULT, "STANDARD", "STD", 0 },

		{ REPAIR_RM_INVALID, "RM-INVALID", "RI", 0 }, { REPAIR_RM_OVERLAP, "RM-OVERLAP", "RO", 0 },
		{ REPAIR_RM_FREE, "RM-FREE", "RF", 0 }, { REPAIR_RM_EMPTY, "RM-EMPTY", "RE", 0 },
		{ REPAIR_RM_ALL, "RM-ALL", "RA", 0 },

		{ REPAIR_ALL, "ALL", "*", 0 }, { 0, 0, 0, 0 } };
...
static uint FindWiiParamFieldHelper (const WiiParamField_t *pf, bool *p_found, ccp key)
{
	...
	if (pf && key)
	{
		...
		while (beg <= end)
		{
			...
			{
				TRACE (
					"FindWiiParamFieldHelper(%s) FOUND=%d/%d/%d\n", key, idx, pf->used, pf->size);
				if (p_found)
					*p_found = true;
				return idx;
			}
...
WiiParamFieldItem_t *InsertWiiParamField (WiiParamField_t *pf, // valid param field
...
enumError LoadWiiParamField (WiiParamField_t *pf, // param field
...
enumError SaveWiiParamField (WiiParamField_t *pf, // valid param field
...
enumError WriteWiiParamField (FILE *f, // open destination file
...
int AtFileHelper (ccp arg, int mode, int mode_expand, int (*func) (ccp arg, int mode))
{
	if (!arg || !*arg || !func)
		return 0;
...
void InsertDiscMemMap (MemMap_t *mm, // valid memore map pointer
...
char *SubstString (char *buf, size_t bufsize, SubstString_t *tab, ccp source, int *count)
{
	ASSERT (buf);
	ASSERT (bufsize > 1);
	ASSERT (tab);
...
)
{
	...
	if (fm->used)
	{
		...
		if (mi->src_off + mi->size == src_off && mi->dest_off + mi->size == dest_off)
		{
			mi->size += size;
			return mi;
		}
...
uint CombineFileMaps (FileMap_t *fm, // resulting filemap
...
)
{
	...
	while (i1 < e1)
	{
		...
		noPRINT ("%4zu %9llx %11llx %9llx | %4zu %9llx %11llx %9llx | NEXT FM-1\n", i1 - fm1->field,
			fm1_off1, fm1_off2, fm1_size, i2 - fm2->field, fm2_off1, fm2_off2, fm2_size);
		...
		while (fm1_size)
		{
			while (fm1_off2 >= fm2_off1 + fm2_size)
			{
				...
				noPRINT ("%4zu %9llx %11llx %9llx | %4zu %9llx %11llx %9llx | NEXT FM-2\n",
					i1 - fm1->field, fm1_off1, fm1_off2, fm1_size, i2 - fm2->field, fm2_off1,
					fm2_off2, fm2_size);
			}
			DASSERT (fm1_off2 < fm2_off1 + fm2_size);
			noPRINT ("%9llx + %9llx = %9llx < %9llx\n", fm1_off2, fm1_size, fm1_off2 + fm1_size,
				fm2_off1);
			...
			noPRINT ("%4zu %9llx %11llx %9llx | %4zu %9llx %11llx %9llx | ADD %llx\n",
				i1 - fm1->field, fm1_off1, fm1_off2, fm1_size, i2 - fm2->field, fm2_off1, fm2_off2,
				fm2_size, size);
		}
	}
	return fm->used;
...
void SetupDataList (DataList_t *dl, // Object for setup
...
)
{
	...
	const struct assign_t assign_tab[] = { { "base", base_path, &config->base_path },
		...
		{ "titles", "$(share)", &config->titles_path }, { 0, 0, 0 } };
	...
	const enumError err = path && *path ? ScanSetupFile (0, list_def, path, 0, silent) : ERR_OK;
...
void PrintConfig (FILE *f, const config_t *config, bool verbose)
{
	if (f && config)
	{
		...
		fprintf (f,
			"  share path:     %s\n"
			"  titles path:  %s\n",
			config->share_path, config->titles_path);
	}
}
...
static const struct config_offset_t config_offset_tab[]
	= { { 0, "config", offsetof (config_t, config_file) },
		  ...
		  { 1, "titles", offsetof (config_t, titles_path) }, { 0, 0 } };
...
void PrintConfigFile (FILE *f, const config_t *config)
{
	if (f && config)
	{
		...
		if (brief_count > 0)
			...
			fprintf (f, text_config_paths, dest.base_path, dest.install_path, dest.install_config,
				dest.share_path, dest.titles_path);
...
void SetupPrintScriptByOptions (PrintScript_t *ps)
{
	...
	ps->fform = script_fform == PSFF_UNKNOWN && print_sections ? PSFF_CONFIG : script_fform;
}
...
int ScanPreallocMode (ccp arg)
{
	...
	static const KeywordTab_t tab[] = { { PREALLOC_OFF, "OFF", "0", 0 },
		{ PREALLOC_SMART, "SMART", "1", 0 }, { PREALLOC_ALL, "ALL", "2", 0 },
...
{
	ERROR0 (ERR_WARNING, "Option --%s is deprecated! Don't use it any longer!\n", name);
}
...
{
	ERROR0 (ERR_WARNING, "Option --%s is deprecated and ignored! Don't use it any longer!\n", name);
}
