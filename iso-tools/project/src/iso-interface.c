...
static void dump_data (FILE *f, int indent, off_t base, u32 off4, off_t size, ccp text)
{
	...
	fprintf (f,
		...
		indent, "", text, off, end, (u64)base + off, (u64)base + end, (u64)size, (u64)size);
}

//-----------------------------------------------------------------------------

static void dump_size (FILE *f, // output stream
	int indent, // indent
	ccp title, // header
...
{
	fprintf (f, "%*s%-17s%11llx/hex =%11llu", indent, "", title, size, size);
...
static void dump_sys_version (FILE *f, // output stream
	int indent, // indent
...
static void dump_id (FILE *f, // output stream
	int indent, // indent
	...
	if (part)
	{
		if (disc_id6 && *disc_id6)
			fprintf (f, "%*s%-*sdisc=%s, ticket=%s, tmd=%s, boot=%s%s\n", indent, "", title_fw,
				"Disc & part IDs: ", wd_print_id (disc_id6, 6, 0),
			...
			fprintf (f, "%*s%-*sticket=%s, tmd=%s, boot=%s%s\n", indent, "", title_fw,
				"Partition IDs: ", wd_print_id (part->ph.ticket.title_id + 4, 4, 0),
				part->tmd ? wd_print_id (part->tmd->title_id + 4, 4, 0) : "-",
				wd_print_id (&part->boot, 6, 0), wbfs);
	}
	else if (disc_id6 && *disc_id6)
		fprintf (f, "%*s%-*s%s%s\n", indent, "", title_fw,
			"Disc ID: ", wd_print_id (disc_id6, 6, 0), wbfs);
}

//-----------------------------------------------------------------------------

static int dump_header (FILE *f, // output stream
	int indent, // indent
	...
	if (sf->wia)
	{
		...
		if (disc)
			snprintf (ftype_info, sizeof (ftype_info), " (v%s,%s)",
				...
				wd_print_compression (
					0, 0, disc->compression, disc->compr_level, disc->chunk_size, 2));
	}

	if (disc)
	{
		fprintf (f, "%*sFile & disc type:  %s%s  &  %s%s%s%s\n", indent, "",
			GetNameFT (sf->f.ftype, 0), ftype_info, wd_get_disc_type_name (disc->disc_type, "?"),
...
static void dump_sig_type (FILE *f, // output stream
	int indent, // indent
...
{
	const int size
		= is_pubkey ? cert_get_pubkey_size (sig_type) : cert_get_signature_size (sig_type);

	fprintf (f, "%*s%-17s%11x/hex =%11u  [%s, size=%u%s%s]\n", indent, "",
		is_pubkey ? "Public key type:" : "Signature type:", sig_type, sig_type,
		cert_get_signature_name (sig_type | 0x10000, "?"), size, sig_status ? ", " : "",
...
static void dump_hex (FILE *f, // output file
...
static void dump_gc_part (FILE *f, // valid output stream
	...
	if (show_mode & SHOW_P_INFO && disc->n_part)
	{
		...
		for (i = 0; i < np; i++, part++)
		{
			...
			fprintf (f, "\n%*sGameCube partition #%d, offset 0x%llx, size %s:\n", indent, "",
				part->index, (u64)part->part_off4 << 2,
...
static void dump_wii_part (FILE *f, // valid output stream
	int indent, // indent of output
	wd_disc_t *disc, // disc pointer
	ShowMode show_mode // what should be printed
)
{
	...
	if (show_mode & SHOW_P_TAB && wd_disc_has_ptab (disc))
	{
		...
		fprintf (f,
			...
			"%*s%s\n",
			indent, "", nt, nt == 1 ? "" : "s", np, np == 1 ? "" : "s",
			disc->patch_ptab_recommended ? " (patching recommended)" : "", indent, "", indent, "",
			sep);
		...
		for (i = 0; i < WII_MAX_PTAB; i++, pc++)
		{
			...
			if (np)
			{
				...
				fprintf (f,
					...
					indent, "", i, off, off + size, size, size, np, np == 1 ? "" : "s");
			}
		}
		...
		for (i = 0; i < np; i++)
		{
			...
			if (part->is_valid)
			{
				...
				fprintf (f, "%*s%4d.%-2d %s %9llx ..%10llx %10llx =%11llu =%5llu  %s\n", indent, "",
					part->ptab_index, part->ptab_part_index, pname, off, off + size, size, size,
					(size + MiB / 2) / MiB, wd_print_part_status (0, 0, part, true));
			}
			else
				fprintf (f,
					"%*s%4d.%-2d %s %9llx         "
					"** INVALID PARTITION **               invalid\n",
					indent, "", part->ptab_index, part->ptab_part_index, pname, off);
		}
	...
	if (show_mode & SHOW__PART && disc->n_part && wd_disc_has_ptab (disc))
	{
		for (i = 0; i < np; i++)
		{
			...
			fprintf (f, "\n%*sPartition table #%d, partition #%d, type %s:\n", indent, "",
				part->ptab_index, part->ptab_part_index, pname);
			...
			if (show_mode & SHOW_P_INFO)
			{
				...
				u8 *p8 = part->key;
				fprintf (f,
					"%*s  Partition key:    %02x%02x%02x%02x %02x%02x%02x%02x"
					" %02x%02x%02x%02x %02x%02x%02x%02x\n",
					indent, "", p8[0], p8[1], p8[2], p8[3], p8[4], p8[5], p8[6], p8[7], p8[8],
					p8[9], p8[10], p8[11], p8[12], p8[13], p8[14], p8[15]);

				if (!(show_mode & SHOW_F_PRIMARY))
				{
					dump_id (f, indent + 2, 0, 0, part, 18);
					fprintf (f, "%*s  boot.bin, title:  %.64s\n", indent, "",
						part->boot.dhead.disc_title);
...
enumError Dump_ISO (FILE *f, // valid output stream
	int indent, // indent of output
	SuperFile_t *sf, // file to dump
	ccp real_path, // NULL or pointer to real path
	ShowMode show_mode, // what should be printed
...
)
{
	...
	if (show_mode & SHOW_INTRO)
	{
		...
		else
		{
			...
			fprintf (f, "%*sRegion setting:    %x [%s] / %s\n", indent, "", reg,
				GetRegionName (reg, "?"), wd_print_age_rating (0, 0, disc->region.age_rating));
	...
	if (show_mode & SHOW_FILES)
	{
		...
		const u32 used_mib = (wd_count_used_disc_blocks (disc, 1, 0) + WII_SECTORS_PER_MIB / 2)
			/ WII_SECTORS_PER_MIB;

		fprintf (f, "\n\n%*s%u director%s with %u file%s, disk usage %u MiB:\n\n", indent, "",
			disc->fst_dir_count, disc->fst_dir_count == 1 ? "y" : "ies", disc->fst_file_count,
			disc->fst_file_count == 1 ? "" : "s", used_mib);
...
enumError Dump_DOL (FILE *f, // valid output stream
	int indent, // indent of output
	SuperFile_t *sf, // file to dump
	ccp real_path, // NULL or pointer to real path
...
enumError Dump_CERT_BIN (FILE *f, // valid output stream
	int indent, // indent of output
	SuperFile_t *sf, // file to dump
	ccp real_path, // NULL or pointer to real path
	int print_ext // 0: off
...
enumError Dump_CERT_MEM (FILE *f, // valid output stream
	int indent, // indent of output
...
enumError Dump_CERT (FILE *f, // valid output stream
	int indent, // indent of output
...
void Dump_CERT_Item (FILE *f, // valid output stream
...
static int try_load_cert (cert_chain_t *cc, // valid pointer to uninitialized data
...
enumError Dump_TIK_BIN (FILE *f, // valid output stream
	int indent, // indent of output
	SuperFile_t *sf, // file to dump
	ccp real_path // NULL or pointer to real path
)
	...
	if (!err)
	{
		...
		else
		{
			...
			if (sizeof (wd_ticket_t) < load_size)
			{
				cert_initialize (&cc);
				cert_append_data (
					&cc, buf + sizeof (wd_ticket_t), load_size - sizeof (wd_ticket_t), false);
			}
			else
...
enumError Dump_TIK_MEM (FILE *f, // valid output stream
	int indent, // indent of output
...
)
{
	...
	fprintf (f, "%*sTime limit:      %11x/hex =%11u [%sabled]\n", indent, "", val, val,
		ntohl (tik->enable_time_limit) ? "en" : "dis");
...
enumError Dump_TMD_BIN (FILE *f, // valid output stream
	int indent, // indent of output
	SuperFile_t *sf, // file to dump
	ccp real_path // NULL or pointer to real path
)
...
enumError Dump_TMD_MEM (FILE *f, // valid output stream
	int indent, // indent of output
...
enumError Dump_HEAD_BIN (FILE *f, // valid output stream
	int indent, // indent of output
	SuperFile_t *sf, // file to dump
	ccp real_path // NULL or pointer to real path
)
...
enumError Dump_BOOT_BIN (FILE *f, // valid output stream
	int indent, // indent of output
	SuperFile_t *sf, // file to dump
	ccp real_path // NULL or pointer to real path
)
...
enumError Dump_FST_BIN (FILE *f, // valid output stream
	int indent, // indent of output
	SuperFile_t *sf, // file to dump
	ccp real_path, // NULL or pointer to real path
	ShowMode show_mode // what should be printed
...
enumError Dump_FST_MEM (FILE *f, // valid output stream
	int indent, // indent of output
...
enumError Dump_PATCH (FILE *f, // valid output stream
	int indent, // indent of output
	SuperFile_t *sf, // file to dump
	ccp real_path // NULL or pointer to real path
)
...
int RenameISOHeader (void *data, ccp fname, ccp set_id6, ccp set_title, int verbose, int testmode)
{
	ASSERT (data);
	...
	if (set_title)
	{
		...
		SubstString_t subst_tab[] = { { 'j', 'J', 0, old_id6 }, { 'i', 'I', 0, new_id6 },

			{ 'n', 'N', 0, old_name },

			{ 'p', 'P', 0, old_title }, { 't', 'T', 0, new_title },
...
OffOn_t opt_allow_fst = OFFON_AUTO; // disable or enable usage of FST images
OffOn_t opt_allow_nkit = OFFON_AUTO; // disable or enable detection of NKIT images
...
{
	const int stat = ScanKeywordOffAutoOn (arg, OFFON_ON, OFFON_FORCE, "Option --allow-fst");
	if (stat == OFFON_ERROR)
		return 1;
...
{
	const int stat = ScanKeywordOffAutoOn (arg, OFFON_ON, OFFON_FORCE, "Option --allow-nkit");
	if (stat == OFFON_ERROR)
		return 1;
...
static s64 PartSelectorFunc (void *param, // NULL or user defined parameter
...
)
{
	...
	if (strlen (name) == 4 && isalnum ((int)name[0]) && isalnum ((int)name[1])
		&& isalnum ((int)name[2]) && isalnum ((int)name[3]))
	{
		u32 ptype = toupper ((int)name[0]) << 24 | toupper ((int)name[1]) << 16
			| toupper ((int)name[2]) << 8 | toupper ((int)name[3]);
	...
	if (*name == '#')
	{
		...
		if (end > ptr)
		{
			if (!*end)
			{
				...
				{
					mode |= mode2 < 0 ? WD_SM_ALLOW_LT_INDEX
						: mode2 > 0	  ? WD_SM_ALLOW_GT_INDEX
									  : WD_SM_ALLOW_INDEX;
	...
	ERROR0 (
		ERR_SYNTAX, "Illegal partition selector%s: %.20s\n", scan_select->err_text_extend, name);
	return ERR_SYNTAX;
}

//-----------------------------------------------------------------------------

enumError ScanPartSelector (wd_select_t *select, // valid partition selector
	ccp arg, // argument to scan
	ccp err_text_extend // error message extention
)
{
	static const KeywordTab_t tab[] = { { WD_SM_ALLOW_PTYPE, "DATA", "D", WD_PART_DATA },
		...
		{ WD_SM_ALLOW_PTAB, "PTAB0", "T0", 0 }, { WD_SM_ALLOW_PTAB, "PTAB1", "T1", 1 },
		{ WD_SM_ALLOW_PTAB, "PTAB2", "T2", 2 }, { WD_SM_ALLOW_PTAB, "PTAB3", "T3", 3 },

		{ WD_SM_ALLOW_ID, "ID", 0, 0 }, { WD_SM_ALLOW_GC_BOOT, "BOOT", 0, 0 },
		{ WD_SM_ALLOW_ALL, "ALL", 0, 0 },

		{ WD_SM_N_MODE + 1, "1:1", "RAW", 0 }, { WD_SM_N_MODE + 2, "WHOLE", 0, 0 },
	...
	const enumError err = ScanKeywordListFunc (arg, tab, PartSelectorFunc, &scan_select, true);
...
{
	static const KeywordTab_t tab[] = { { WD_PART_DATA, "DATA", "D", 0 },
		{ WD_PART_DATA, "GAME", "G", 0 }, { WD_PART_UPDATE, "UPDATE", "U", 0 },
		{ WD_PART_UPDATE, "INSTALLER", "I", 0 }, { WD_PART_CHANNEL, "CHANNEL", "C", 0 },
	...
	if (strlen (arg) == 4 && isalnum ((int)arg[0]) && isalnum ((int)arg[1]) && isalnum ((int)arg[2])
		&& isalnum ((int)arg[3]))
	{
		return be32 (arg);
	}
...
enumError ScanPartTabAndType (u32 *res_ptab, // NULL or result: partition table
...
)
{
	...
	if (ptab_valid)
	{
		...
		if (end != sep || ptab >= WII_MAX_PTAB)
			return ERROR0 (
				ERR_SYNTAX, "Not a valid partition table%s: %.20s\n", err_text_extend, arg);
		arg = sep + 1;
	}
...
wd_ipm_t ScanPrefixMode (ccp arg)
{
	static const KeywordTab_t tab[]
		= { { WD_IPM_DEFAULT, "DEFAULT", "D", 0 }, { WD_IPM_AUTO, "AUTO", "A", 0 },

			  { WD_IPM_NONE, "NONE", "-", 0 }, { WD_IPM_SLASH, "SLASH", "/", 0 },
			  { WD_IPM_POINT, "POINT", ".", 0 }, { WD_IPM_PART_ID, "ID", "I", 0 },
			  { WD_IPM_PART_NAME, "NAME", "N", 0 }, { WD_IPM_PART_INDEX, "INDEX", "IDX", 0 },
...
IsoMappingItem_t *InsertIM (IsoMapping_t *im, enumIsoMapType imt, u64 offset, u64 size)
{
	ASSERT (im);
...
void PrintIM (IsoMapping_t *im, FILE *f, int indent)
{
	...
	for (imi = im->field; imi <= last; imi++)
	{
		...
		fprintf (f, "%*s%c %9llx .. %9llx %9llx  %s\n", indent, "",
			imi->offset < prev_end ? '!' : ' ', imi->offset, end, imi->size, imi->info);
		prev_end = end;
	}
	putc ('\n', f);
}
...
static uint FindFileIndexHelper (const FileIndex_t *fidx, bool *found, dev_t dev, ino_t ino)
{
	DASSERT (fidx);
	...
	while (beg <= end)
	{
		...
		else if (ino > item->ino)
			beg = idx + 1;
		else if (dev < item->dev)
			end = idx - 1;
		else if (dev > item->dev)
			beg = idx + 1;
		else
...
FileIndexItem_t *InsertFileIndex (FileIndex_t *fidx, bool *found, dev_t dev, ino_t ino)
{
	DASSERT (fidx);
...
ccp SpecialRequiredFilesFST[] = { "/sys/boot.bin",
	//	"/sys/bi2.bin",
	"/sys/apploader.img", "/sys/main.dol", 0 };
...
WiiFstFile_t *AppendFileFST (WiiFstPart_t *part)
{
	...
	if (part->file_used == part->file_size)
	{
		...
		TRACE ("REALLOC(part->file): %u (size=%zu) -> %p\n", part->file_size,
			part->file_size * sizeof (*part->file), part->file);
	}
...
#ifdef DEBUG
	{
		TRACE ("FindFileFST(%x) %d/%d/%d, searched-off=%x\n", offset4, beg, part->file_used,
			part->file_size, offset4 << 2);
...
static int CollectFST_helper (struct wd_iterator_t *it // iterator struct with all infos
)
{
	DASSERT (it);
			...
			case WD_ICM_CLOSE_PART:
				if (part)
				{
					...
					TRACE ("PART CLOSED: %u/%u files, %llu/%llu MiB\n", part->file_used,
						fst->total_file_count, part->total_file_size / MiB,
						fst->total_file_size / MiB);
				}
				break;
...
int CollectFST (WiiFst_t *fst, // valid fst pointer
	wd_disc_t *disc, // valid disc pointer
	FilePattern_t *pat, // NULL or a valid filter
	bool ignore_dir, // true: ignore directories
...
int CollectFST_BIN (WiiFst_t *fst, // valid fst pointer
	const wd_fst_item_t *ftab_data, // the FST data
	FilePattern_t *pat, // NULL or a valid filter
	bool ignore_dir // true: ignore directories
)
...
void DumpFilesFST (FILE *f, // NULL or output file
...
)
{
	...
	for (part = fst->part; part < part_end; part++)
	{
		...
		for (; ptr < end; ptr++)
			wd_print_fst_item (
				&pf, part->part, ptr->icm, ptr->offset4, ptr->size, prefix, ptr->path);
	}
}
...
enumError CreateFST (WiiFstInfo_t *wfi, ccp dest_path)
{
	...
	if (wfi->verbose)
	{
		if (print_sections)
		{
			printf ("[extract:info]\n"
					...
					"total-size=%llu\n"
					"\n",
				fst->part_used, fst->total_file_count, fst->total_file_size);
		}
		else
		{
			char buf[20];
			...
			printf (" - will extract %u file%s of %u partition%s, %llu MiB total\n",
				fst->total_file_count, fst->total_file_count == 1 ? "" : "s", fst->part_used,
				fst->part_used == 1 ? "" : "s", (fst->total_file_size + MiB / 2) / MiB);
		}
	}
	...
	if (wfi->copy_image && wfi->sf && wfi->sf->disc1)
	{
		...
		bool need_copy = true;
		if (wfi->link_image && wfi->sf->iod.oft == OFT_PLAIN // ISO format
		...
		{
			if (print_sections)
				printf ("[extract:%s-image]\n"
						"source-image=%s\n"
						"dest-image=%s\n"
						"\n",
					need_copy ? "copy" : "link", wfi->sf->f.fname, iobuf);
			else
		...
		if (need_copy)
		{
			enumError err = CopyImageName (
				wfi->sf, iobuf, 0, OFT_PLAIN, wfi->overwrite, wfi->set_time != 0, false);
			if (err)
...
enumError CreateFileFST (WiiFstInfo_t *wfi, ccp dest_path, WiiFstFile_t *file)
{
	...
	if (file->icm == WD_ICM_DIRECTORY)
	{
		if (wfi->verbose > 1)
		{
			if (print_sections) // [sections]
				printf ("[extract:create-dir]\n"
						"done-count=%u\n"
						"total-count=%u\n"
						"iso-path=%s%s\n"
						"dest-path=%s\n"
						"\n",
					wfi->done_count, wfi->total_count, part->path, file->path, dest);
			else
				printf (" - %*u/%u create directory %s\n", wfi->fw_done_count, wfi->done_count,
					wfi->total_count, dest);
		}
	...
	if (file->icm == WD_ICM_FILE && !memcmp (file->path, "files/", 6)
				...
				if (wfi->verbose > 1)
				{
					if (print_sections) // [sections]
						printf ("[extract:link-file]\n"
								...
								"file-size=%u\n"
								"\n",
							wfi->done_count, wfi->total_count, part->path, file->path, dest, source,
							file->size);
					else
						printf (" - %*u/%u link %s -> %s\n", wfi->fw_done_count, wfi->done_count,
							wfi->total_count, source, dest);
				}
				return ERR_OK;
			}
		}
	...
	if (wfi->verbose > 1)
	{
		if (print_sections) // [sections]
			printf ("[extract:create-file]\n"
					...
					"file-size=%u\n"
					"\n",
				wfi->done_count, wfi->total_count, part->path, file->path, dest, file->size);
		else
			printf (" - %*u/%u %s %8u bytes to %s\n", wfi->fw_done_count, wfi->done_count,
				wfi->total_count, file->icm == WD_ICM_DATA ? "write  " : "extract", file->size,
				dest);
	}
...
enumError ReadFileFST4 (WiiFstPart_t *part, // valid fst partition pointer
...
)
{
	...
	return ERROR0 (ERR_READ_FAILED, "Reading from FST file failed [%c:%llx+%x>%x]: %s\n", mode,
		(u64)off4 << 2, size, file->size, file->path);
}

///////////////////////////////////////////////////////////////////////////////

enumError ReadFileFST (WiiFstPart_t *part, // valid fst partition pointer
	const WiiFstFile_t *file, // valid fst file pointer
	u64 off, // file offset
	void *buf, // destination buffer with at least 'size' bytes
	u32 size // number of bytes to read
...
SortMode SortPartFST (WiiFstPart_t *part, SortMode sort_mode, SortMode default_sort_mode)
{
	...
	{
		case SORT_SIZE:
			func = sort_fst_by_size;
			break;
		case SORT_OFFSET:
			func = sort_fst_by_offset;
			break;
...
static SetupDef_t part_setup_def[] = { { "disc-type", 0 }, { "image-type", 0 }, { "part-id", 0 },
	{ "part-name", 0 }, { "part-offset", 0x10000 }, { 0, 0 } };

///////////////////////////////////////////////////////////////////////////////

static enumError ScanSetupDef (ccp path, // filename of text file, part 1
...
)
{
	...
	static const KeywordTab_t tab[]
		= { { WD_DT_GAMECUBE, "GAMECUBE", "GC", 0 }, { WD_DT_WII, "WII", 0, 0 }, { 0, 0, 0, 0 } };
	...
	PRINT ("Scan %s: disc-type = %lld [%s] / image-type = %lld [%s]\n", FST_SETUP_FILE,
		dtype->value, wd_get_disc_type_name (dtype->value, "?"), itype->value,
		oft_info[itype->value].name);
	return err;
}
...
int SearchPartitionsFST (
	ccp base_path, char *id6_result, char *data_part, char *update_part, char *channel_part)
{
	if (id6_result)
	...
	*data_part = *update_part = *channel_part = 0;
	...
	return (data_found ? DATA_PART_FOUND : 0) | (update_found ? UPDATE_PART_FOUND : 0)
...
enumFileType IsFSTPart (ccp base_path, char *id6_result)
{
	...
	if (fstat (fd, &st) || !S_ISREG (st.st_mode) || st.st_size != WII_BOOT_SIZE
...
static u32 scan_part (scan_data_t *sd)
{
	...
	TRACE ("scan_part(%.*s|%.*s|%s) depth=%d\n", (int)(sd->path_part - sd->path), sd->path,
		(int)(sd->path_dir - sd->path_part), sd->path_part, sd->path_dir, sd->depth);
		...
		for (;;)
		{
			...
			if (use_utf8)
			{
				...
				while (ScanUTF8AnsiChar (&nameptr))
					namesize++;
				PRINT_IF (namesize - 1 != strlen (name), "NAME-LEN: %u,%zu: %s\n", namesize - 1,
					strlen (name), name);
			}
			else
			...
			else if (S_ISREG (st.st_mode))
			{
				...
				{
					file->data
						= (u8 *)InsertFileIndex (&sd->part->fidx, &found, st.st_dev, st.st_ino);
					PRINT ("SET LINK: %p = %s\n", file->data, name);
				}
				if (!found)
...
u32 ScanPartFST (WiiFstPart_t *part, ccp base_path, u32 cur_offset4, wd_boot_t *boot)
{
	ASSERT (part);
	ASSERT (base_path);
	...
	*sd.path_part = 0;
	...
	part->ftab_size = sizeof (wd_fst_item_t) * part->file_used + sd.name_pool_size + fst_plus_size;
	...
	if (fst_plus_size > 0)
	{
		...
		memcpy (dest, "fst+", 4);
		LoadFile (sd.path, "sys/fst+.bin", 0, dest + 4, fst_plus_fsize, 0, 0, false);
	}
#endif
	...
	ASSERT ((u8 *)namebase + sd.name_pool_size + fst_plus_size == part->ftab + part->ftab_size);
	...
	for (file = part->file + 1; file < file_end; file++)
	{
		...
		noTRACE ("-> %4x: %u %8x %8x %s\n", idx, ftab->is_dir, ntohl (ftab->offset4),
			ntohl (ftab->size), namebase + (ntohl (ftab->name_off) & 0xffffff));
...
#ifdef DEBUG
	if (Dump_FST_MEM (0, 0, (wd_fst_item_t *)part->ftab, part->ftab_size, "scanned", WD_PFST__ALL))
	{
		Dump_FST_MEM (
			TRACE_FILE, 0, (wd_fst_item_t *)part->ftab, part->ftab_size, "scanned", WD_PFST__ALL);
		return ERROR0 (ERR_FATAL, "internal fst.bin is invalid -> see trace file\n");
	}
#endif

	imi = InsertIM (
		&part->im, IMT_PART_FILES, (u64)cur_offset4 << 2, (u64)(offset4 - cur_offset4) << 2);
	imi->part = part;
...
u64 GenPartFST (SuperFile_t *sf, // valid file pointer
...
)
{
	...
	LoadFile (path, "sys/boot.bin", 0, imi->data, WII_BOOT_SIZE, 0, &part->max_fatt, true);
	...
	{
		LoadFile (
			path, "disc/header.bin", 0, &fst->dhead, sizeof (fst->dhead), 2, &part->max_fatt, true);
	...
	if (part->part_type == WD_PART_DATA)
	{
		...
		const RegionInfo_t *rinfo = GetRegionInfo (fst->dhead.id6.region_code);

		if (reg == REGION__AUTO && rinfo->mandatory)
			reg = rinfo->reg;
		else if (reg == REGION__AUTO || reg == REGION__FILE)
		{
			reg = LoadFile (path, "disc/region.bin", 0, &fst->region, sizeof (fst->region), 2,
					  &part->max_fatt, true)
				? REGION__AUTO
				: REGION__FILE;
		}
		...
		if (reg != REGION__FILE)
		{
			...
			fst->region.region = htonl (reg == REGION__AUTO ? rinfo->reg : reg);
		}
	}
	...
	TRACE ("dol4=%x,%x  fst4=%x,%x,  cur_offset4=%x\n", ntohl (boot->dol_off4), dol_fsize4,
		ntohl (boot->fst_off4), ntohl (boot->fst_size4), cur_offset4);
	...
	if (clear_part_control (pc, WII_TMD_GOOD_SIZE, cert_size, (u64)blocks * WII_SECTOR_SIZE))
		return ERROR0 (ERR_INVALID_FILE, "Content of file 'cert.bin' wrong!\n");

	//----- setup partition control content

	if (load_ticket)
		load_ticket = LoadFile (path, "ticket.bin", 0, &pc->head->ticket, WII_TICKET_SIZE, 0,
						  &part->max_fatt, true)
			== ERR_OK;
	if (!load_ticket)
	...
	if (load_tmd)
		load_tmd = LoadFile (path, "tmd.bin", 0, pc->tmd, pc->tmd_size, 0, &part->max_fatt, true)
			== ERR_OK;
	if (!load_tmd)
	...
	if (use_std_cert_chain)
	{
		...
		LoadFile (path, "cert.bin", 0, pc->cert, pc->cert_size, 0, &part->max_fatt, true);
...
enumError SetupReadFST (SuperFile_t *sf)
{
	ASSERT (sf);
	ASSERT (!sf->fst);
	PRINT ("SetupReadFST() -> %llx %s\n", (u64)sf->f.ftype, sf->f.fname);
	...
	fst->encoding = SetEncoding (enc, 0,
		enc & ENCODE_F_FAST ? ENCODE_NO_SIGN | ENCODE_DECRYPT | ENCODE_CLEAR_HASH
	...
	if (sf->f.ftype & FT_A_PART_DIR)
	{
		...
		min_offset = data_off + WII_PARTITION_BIN_SIZE + data_size;
	...
	else
	{
		...
		if (stat & UPDATE_PART_FOUND)
		{
			...
			min_offset = update_off + WII_PARTITION_BIN_SIZE + update_size;
		...
		if (stat & CHANNEL_PART_FOUND)
		{
			...
			min_offset = channel_off + WII_PARTITION_BIN_SIZE + channel_size;
	...
	const u64 part_tab_size = cnt_tab_size + n_part * sizeof (wd_ptab_entry_t);
	...
	snprintf (
		imi->info, sizeof (imi->info), "region settings, region=%u", ntohl (fst->region.region));
	imi->data = &fst->region;
...
{
	TRACE ("---\n");
	TRACE (TRACE_RDWR_FORMAT, "#FST# ReadFST()", GetFD (&sf->f), GetFP (&sf->f), (u64)off,
		(u64)off + count, count, "");
	...
	while (count > 0)
	{
		...
		switch (imi->imt)
		{
			...
			case IMT_PART:
				ASSERT (imi->part);
				{
					const enumError err = ReadPartFST (sf, imi->part, delta, dest, copy_count);
					if (err)
						return err;
				}
				break;
...
enumError ReadPartFST (SuperFile_t *sf, WiiFstPart_t *part, off_t off, void *buf, size_t count)
{
	TRACE (TRACE_RDWR_FORMAT, "#PART# ReadPartFST()", GetFD (&sf->f), GetFP (&sf->f), (u64)off,
		(u64)off + count, count, "");
...
enumError ReadPartGroupFST (
	SuperFile_t *sf, WiiFstPart_t *part, u32 group_no, void *buf, u32 n_groups)
{
	TRACELINE;
	TRACE (TRACE_RDWR_FORMAT, "#PART# read GROUP", GetFD (&sf->f), GetFP (&sf->f), (u64)group_no,
		(u64)group_no + n_groups, (size_t)n_groups, "");
	...
	if (sf->fst->encoding & ENCODE_CALC_HASH)
	{
		...
		{
			EncryptSectorGroup (sf->fst->encoding & ENCODE_ENCRYPT ? &part->akey : 0,
				(wd_part_sector_t *)dest, WII_GROUP_SECTORS,
...
void EncryptSectorGroup (const aes_key_t *akey, wd_part_sector_t *sect0, size_t n_sectors, void *h3)
{
...
static enumError PrintVerifyMessage (Verify_t *ver, ccp msg)
{
	...
	if (ver->verbose >= -1)
	{
		...
		{
			// no index supported
			snprintf (count_buf, sizeof (count_buf), ".%u%c", part->index,
				part->is_encrypted ? ' ' : 'd');
		}
		else if (ver->disc_total < 2)
		{
			// a single check
			snprintf (count_buf, sizeof (count_buf), "%u.%u%c", ver->disc_index, part->index,
				part->is_encrypted ? ' ' : 'd');
		}
		else
		{
			...
			snprintf (count_buf, sizeof (count_buf), "%*u.%u/%u%c", fw, ver->disc_index,
				part->index, ver->disc_total, part->is_encrypted ? ' ' : 'd');
		}

		printf ("%*s%-7.7s %s %n%-7s %6.6s %s\n", ver->indent, "", msg, count_buf,
			&ver->info_indent, pname_buf, (ccp)&ver->sf->disc2->dhead,
			ver->fname ? ver->fname : ver->sf->f.fname);
		fflush (stdout);

		TRACE ("%*s%-7.7s %s %n%-7s %6.6s %s\n", ver->indent, "", msg, count_buf, &ver->info_indent,
			pname_buf, (ccp)&ver->sf->disc2->dhead, ver->fname ? ver->fname : ver->sf->f.fname);
	}
	return ERR_DIFFER;
}
...
static enumError VerifyHash (Verify_t *ver, ccp msg, u8 *data, size_t data_len, u8 *ref)
{
	DASSERT (ver);
	DASSERT (ver->part);
...
#if WATCH_BLOCK && 0
	{
		...
		const u64 offset = ver->part->part_offset + ver->part->pc->data_off
			+ ver->group * (u64)WII_GROUP_SIZE + delta_off;
		const u32 block = offset / WII_SECTOR_SIZE;

		if (block == WATCH_BLOCK)
			printf (
				"WB: %s: delta=%x->%x, cmp(%02x,%02x)\n", msg, delta_off, delta_blk, *hash, *ref);
	}
#endif
	...
	if (ver->verbose >= -1 && ver->long_count > 0)
	{
		...
		const u32 delta_blk = delta_off / WII_SECTOR_SIZE;
		const u64 offset
			= ((u64)ver->part->data_off4 << 2) + ver->group * (u64)WII_GROUP_SIZE + delta_off;
		...
		if ((ccp)ref >= iobuf && (ccp)ref <= iobuf + sizeof (iobuf))
		{
			...
			printf ("%*sgroup=%x.%x, block=%x.%x, data-off=%llx=B+%x, hash-off=%llx=B+%x\n",
				ver->info_indent, "", ver->group, delta_blk, block, sub_block, offset, block_off,
				offset + ref_delta, block_off + ref_delta);
			flush = true;
		}
...
enumError VerifyPartition (Verify_t *ver)
{
	...
	if (ver->verbose > 0
		|| ver->verbose >= 0
			&& (!ver->part->is_encrypted
				|| !(CERT_F_HASH_OK & wd_get_cert_ticket_stat (ver->part, true)
					& wd_get_cert_tmd_stat (ver->part, true))))
	{
		PrintVerifyMessage (ver, ">scan");
		printf ("%*s%-*s%s\n", ver->indent, "", ver->info_indent + 8 - ver->indent, ">info",
	...
	TRACE (" - Partition %u [%s], valid=%d, blocks=%x..%x [%llx..%llx]\n", part->part_type,
		wd_get_part_name (part->part_type, "?"), part->is_valid, block, block_end,
	...
	const int max_differ_count = ver->verbose <= 0 ? 1
		: ver->max_err_msg > 0					   ? ver->max_err_msg
												   : INT_MAX;
	int differ_count = 0;
	...
	for (ver->group = 0; block < block_end; ver->group++, block += WII_GROUP_SECTORS)
	{
		...
		for (index = 0; blk < block_end && index < WII_GROUP_SECTORS; blk++, index++)
		{
			...
			if (blk == WATCH_BLOCK)
			{
				...
				printf ("WB: ver->usage_tab[%x] = %x, usage_tab_marker=%x\n", blk,
					ver->usage_tab[blk], usage_tab_marker);
			}
#endif
		...
		for (i2 = 0; i2 < WII_N_ELEMENTS_H2 && blk < block_end; i2++)
		{
			...
			for (i1 = 0; i1 < WII_N_ELEMENTS_H1 && blk < block_end; i1++, blk++)
			{
				...
				wd_part_sector_t *sect = (wd_part_sector_t *)iobuf + i2 * WII_N_ELEMENTS_H1 + i1;
...
enumError VerifyDisc (Verify_t *ver)
{
	...
	DASSERT (ver->sf);
	TRACE (
		"#VERIFY# VerifyDisc(%p) sf=%p utab=%p part=%p\n", ver, ver->sf, ver->usage_tab, ver->part);
	TRACE (" - psel=%p, v=%d, lc=%d, maxerr=%d\n", ver->psel, ver->verbose, ver->long_count,
		ver->max_err_msg);
...
static void skel_copy_phead (wd_memmap_t *mm, // valid memory map
...
)
{
	...
	mi->data = data;
	if (id4_off)
		snprintf (mi->info, sizeof (mi->info), "%s, id=%s", info,
			wd_print_id ((ccp)data + id4_off, 4, 0));
	else
...
static enumError skel_load_part (wd_memmap_t *mm, // valid memory map
...
)
{
	...
	PRINT ("P: %llx+%x -> %llx..%llx: %s\n", data_offset4, data_size, disc_off_begin, disc_off_end,
		info);
	...
	while (disc_off_begin < disc_off_end)
	{
		...
		wd_memmap_item_t *mi = wd_insert_memmap (mm, 0, disc_off_begin, chunk_size);
		DASSERT (mi);
		mi->data = data;
		if (!idx)
		...
		if (idx++ || end < disc_off_end)
			...
			snprintf (mi->info, sizeof (mi->info), "%s%s%s", info, have_id ? ", id=" : "",
...
static enumError SkeletonizeWii (SuperFile_t *fi, // valid input file
	wd_disc_t *disc, // valid disc pointer
...
)
{
	...
	for (ip = 0; ip < disc->n_part; ip++)
	{
		...
		skel_copy_phead (mm, &part->ph, (u64)part->part_off4 << 2, sizeof (part->ph), "ticket",
			WII_TICKET_ID4_OFF);

		skel_copy_phead (mm, part->tmd, (u64)(part->part_off4 + part->ph.tmd_off4) << 2,
			part->ph.tmd_size, "tmd", WII_TMD_ID4_OFF);

		skel_copy_phead (mm, part->cert, (u64)(part->part_off4 + part->ph.cert_off4) << 2,
			part->ph.cert_size, "cert", 0);

		const size_t ssm = sizeof (skeleton_marker);
		mi = wd_insert_memmap_alloc (mm, 0, wd_calc_disc_offset (part, 0) - ssm, ssm);
		DASSERT (mi);
		memcpy (mi->data, skeleton_marker, ssm);
		snprintf (mi->info, sizeof (mi->info), "Marker '%s'", skeleton_marker);

		err = skel_load_part (mm, part, WII_BOOT_OFF >> 2, WII_BOOT_SIZE, "boot.bin", false);
		if (err)
			return err;

		err = skel_load_part (mm, part, WII_BI2_OFF >> 2, WII_BI2_SIZE, "bi2.bin", false);
		if (err)
			return err;

		err = skel_load_part (
			mm, part, part->boot.fst_off4, part->boot.fst_size4 << 2, "fst.bin", false);
		if (err)
			return err;

		err = skel_load_part (
			mm, part, part->boot.dol_off4, DOL_HEADER_SIZE, "main.dol/header", false);
		if (err)
			return err;

		err = skel_load_part (mm, part, WII_APL_OFF >> 2, 0x20, "apploader/header", false);
		if (err)
			return err;
...
static enumError SkeletonizeGC (SuperFile_t *fi, // valid input file
	wd_disc_t *disc, // valid disc pointer
	wd_memmap_t *mm // valid memory map
)
{
	...
	for (ip = 0; ip < disc->n_part; ip++)
	{
		...
		enumError err
			= skel_load_part (mm, part, WII_BOOT_OFF >> 2, WII_BOOT_SIZE, "boot.bin", true);
		if (err)
			return err;

		const size_t ssm = sizeof (skeleton_marker);
		err = skel_load_part (mm, part, WII_BI2_OFF, WII_BI2_SIZE - ssm, "bi2.bin", true);
		if (err)
			return err;
		...
		err = skel_load_part (mm, part, WII_APL_OFF, 0x20, "apploader/header", false);
		if (err)
			return err;
...
enumError Skeletonize (SuperFile_t *fi, // valid input file
...
)
{
	...
	if (testmode || verbose >= 0)
		printf ("SKELETONIZE %u/%u [%s] %s\n", disc_index, disc_total, fi->f.id6_dest,
			path ? path : fi->f.fname);
	...
	switch (disc->disc_type)
	{
		...
		default:
			err = ERROR0 (
				ERR_WRONG_FILE_TYPE, "SKELETON: Disk type not supported: %s\n", fi->f.fname);
	}
	if (err)
		goto abort;
...

	{
		...
		snprintf (fname, sizeof (fname),
			...
			local_dest, fi->f.id6_dest, h[0], h[1], h[2], h[3], h[4], h[5], h[6], h[7], h[8], h[9],
