...
{
	return compr_level < 1 ? 5 : compr_level < 9 ? compr_level : 9;
}
...
)
{
	...
	if (!bz || bzerror != BZ_OK)
	{
		...
		return ERROR0 (ERR_BZIP2, "Error while opening bzip2 stream: %s\n-> bzip2 error: %s\n",
...
)
{
	...
	if (data_size)
	{
		...
		if (bzerror != BZ_OK)
		{
			...
			return ERROR0 (ERR_BZIP2, "Error while writing bzip2 stream: %s\n-> bzip2 error: %s\n",
				bz->file->fname, GetMessageBZIP2 (bzerror, "?"));
		}
...
)
{
	...
	if (bz && bz->handle)
	{
		...
		if (bzerror != BZ_OK)
			return ERROR0 (ERR_BZIP2, "Error while closing bzip2 stream: %s\n-> bzip2 error: %s\n",
				bz->file->fname, GetMessageBZIP2 (bzerror, "?"));
	}
...
)
{
	...
	if (bzerror != BZ_STREAM_END)
	{
		...
		return ERROR0 (ERR_BZIP2, "Error while reading bzip2 stream: %s\n-> bzip2 error: %s\n",
			bz->file->fname, GetMessageBZIP2 (bzerror, "?"));
	}
...
)
{
	...
	*dest_written = dest_size - sizeof (u32);
	int bzerror = BZ2_bzBuffToBuffCompress (
		dest + sizeof (u32), dest_written, (char *)src, src_size, compr_level, 0, 0);
	*dest_written += 4;
...
)
{
	...
	*dest_written = dest_need;
	...
	int bzerror = BZ2_bzBuffToBuffDecompress (
		(char *)dest, dest_written, (char *)src + 4, src_size - 4, 0, 0);
