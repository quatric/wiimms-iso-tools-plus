...
ccp GetTitle (ccp id6, ccp default_if_failed)
{
	...
	TRACE ("#T# GetTitle(%s) tm=%d  idx=%d/%d/%d  stat=%d -> %s %s\n", id6, title_mode, idx,
		title_db.used, title_db.size, stat, idx < title_db.used ? title_db.list[idx]->id : "",
		idx < title_db.used ? title_db.list[idx]->title : "");
	ASSERT (stat == IDB_NOT_FOUND || idx < title_db.used);
	return stat == IDB_NOT_FOUND ? default_if_failed : title_db.list[idx]->title;
}
...
static ccp ScanArgID (char buf[7], // result buffer for ID6: 6 chars + NULL
...
static enumError AddId (StringField_t *sf_id6, StringField_t *sf_pat, ccp arg, int select_mode)
{
	DASSERT (sf_id6);
	DASSERT (sf_pat);
	...
	else
	{
		...
		ccp res = ScanPatID (sf_id6, sf_pat, arg, false, (select_mode & SEL_F_PARAM) != 0);
		if (res)
...
static IdItem_t *FindPatID (StringField_t *sf_id6, // valid pointer: search real ID6
...
static void CheckExcludePath (ccp path, StringField_t *sf, StringField_t *sf_id6, int max_dir_depth)
{
...
bool IsExcludeActive ()
{
	...
	return include_id6.used || include_pat.used || exclude_id6.used || exclude_pat.used;
}
...
bool IsExcluded (ccp id6)
{
	...
	if (include_first && include_db_enabled && FindPatID (&include_id6, &include_pat, id6, false))
		return false;

	if (FindPatID (&exclude_id6, &exclude_pat, id6, false))
		return true;

	return !include_first && include_db_enabled
	...
	if (warn && count_id6 + count_pat > 1)
	{
		if (count_id6 && count_pat)
			ERROR0 (ERR_WARNING, "==> %u disc ID%s and %u pattern not found!\n", count_id6,
				count_id6 == 1 ? "" : "s", count_pat);
		else if (count_id6)
			ERROR0 (
				ERR_WARNING, "==> %u disc ID%s not found!\n", count_id6, count_id6 == 1 ? "" : "s");
		else if (count_pat)
...
int FindID (ID_DB_t *db, ccp id, TDBfind_t *p_stat, int *p_num)
{
	...
	if (beg < db->used)
	{
		...
		xTRACE ("cmp[%s,%s] %d %d -> %d\n", id, elem->id, memcmp (id, elem->id, id_len),
			memcmp (id, elem->id, strlen (elem->id)), stat);
	}
	xTRACE ("#T# FindID(%p,%s,%p,%p) idx=%d/%d/%d, stat=%d, found=%s\n", db, id, p_stat, p_num, beg,
		db->used, db->size, stat, beg < db->used && elem ? elem->id : "-");
...
ccp GetSystemMenu (u32 version, // system version number
