...
void InitializeWDF (wdf_controller_t *wdf)
{
	...
	wdf->min_hole_size = GetChunkSizeWDF (wdf->head.wdf_version) + sizeof (WDF_Hole_t);
	...
	PRINT ("InitializeWDF(), align=0x%x, min-hole=0x%x, data-off=0x%llx\n", wdf->align,
		wdf->min_hole_size, wdf->max_data_off);
}
...
void ConvertToNetworkWC1n (wdf1_chunk_t *dest, // dest data, must not overlay 'src'
...
void ConvertToNetworkWC2n (wdf2_chunk_t *dest, // dest data, may overlay 'src'
	const wdf2_chunk_t *src, // source data, network byte order
	uint n_elem // number of elements to convert
)
{
...
void ConvertToHostWC (wdf2_chunk_t *dest, // dest data, may overlay 'src_data'
...
bool UpdateVersionWDF (const wdf_controller_t *wdf_in, // NULL or input file
	wdf_controller_t *wdf_out // NULL or output file
)
{
	noPRINT ("opt=%d, wdf=%d,%d, vers=%d,%d\n", opt_wdf_version, wdf_in != 0, wdf_out != 0,
		wdf_in ? wdf_in->head.wdf_version : -1, wdf_out ? wdf_out->head.wdf_version : -1);

	if (!opt_wdf_version && wdf_in && wdf_out
...
static wdf2_chunk_t *FindChunkWDF (wdf_controller_t *wdf, // valid controller object
...
)
{
	...
	noPRINT ("CHUNK FOUND, off %llx: %u/%u/%u: %llx %llx %llx\n", offset, (int)(wc - wdf->chunk),
		wdf->chunk_used, wdf->chunk_size, wc->file_pos, wc->data_off, wc->data_size);
	return wc;
}

///////////////////////////////////////////////////////////////////////////////

static wdf2_chunk_t *NewChunkWDF (wdf_controller_t *wdf, // valid controller object
...
)
{
	...
	if (wdf->chunk_used)
	{
		...
		if (wc->data_off + wc->data_size == wdf->max_data_off && ch_file_pos >= wc->file_pos
			&& ch_file_pos < wc->file_pos + wc->data_size + wdf->min_hole_size)
		{
			...
			noPRINT ("#W# GROW CHUNK [idx=%zd/%d/%d] %llx+%llx -> %llx, d=%llx\n", wc - wdf->chunk,
				wdf->chunk_used, wdf->chunk_size, wc->data_off, wc->data_size, wc->file_pos, delta);
	...
	if (wdf->chunk_used == wdf->chunk_size)
	{
		...
		noPRINT ("#W# NewChunkWDF() GROW LIST %d ->%d [used=%d]\n", wdf->chunk_size,
			wdf->chunk_size + WC_GROW_SIZE, wdf->chunk_used);
...
enumError SetupReadWDF (SuperFile_t *sf)
{
	...
	if (chunk_end > min_data_off && wh.chunk_off < max_data_off)
		return ERROR0 (ERR_WDF_INVALID, "Chunk table overlaps with data: %s\n", sf->f.fname);

	wdf->image_size = wdf->head.file_size;
	wdf->max_virt_off = max_virt_off;
	wdf->max_data_off = wdf->align ? ALIGN64 (max_data_off, wdf->align) : max_data_off;
	...
	PRINT ("#W#  - chunk loop exits [n=%u/%u], max_off=%llx, align=%x\n", wdf->chunk_used,
		wdf->chunk_size, wdf->max_data_off, wdf->align);
...
enumError ReadWDF (SuperFile_t *sf, off_t off, void *buf, size_t count)
{
	...
	TRACE ("#W# -----\n");
	TRACE (TRACE_RDWR_FORMAT, "#W# ReadWDF()", GetFD (&sf->f), GetFP (&sf->f), (u64)off,
		(u64)off + count, count, "");
	...
	if (off + count > wdf->head.file_size)
	{
		...
		const off_t max_read = wdf->head.file_size > off ? wdf->head.file_size - off : 0;
	...
	while (count > 0)
	{
		...
		if (off >= wc->file_pos && off < wc->file_pos + wc->data_size)
		{
			...
			TRACE ("#W# >READ %p +%8zx = %p .. %x <- %10llx\n", buf, dest - (ccp)buf, dest,
				read_size, wc->data_off + delta);
...
off_t DataBlockWDF (SuperFile_t *sf, off_t off, size_t hint_align, off_t *block_size)
{
	DASSERT (sf);
	wdf_controller_t *wdf = sf->wdf;
	if (!wdf)
...
enumError TermWriteWDF (SuperFile_t *sf)
{
	...
	if (!wdf)
		ERROR0 (ERR_INTERNAL, 0);
	DASSERT (wdf->chunk);
	PRINT ("#W# TermWriteWDF(%p) v%u [%u,%u]\n", sf, wdf->head.wdf_version, opt_wdf_version,
		use_wdf_version);
...
enumError WriteWDF (SuperFile_t *sf, off_t off, const void *buf, size_t count)
{
	...
	TRACE ("#W# -----\n");
	TRACE (TRACE_RDWR_FORMAT, "#W# WriteWDF()", GetFD (&sf->f), GetFP (&sf->f), (u64)off,
	...
	TRACE ("#W#  - FOUND #%03d: off=%09llx ds=%llx, off=%09llx\n", (int)(wc - wdf->chunk),
		wc->file_pos, wc->data_size, (u64)off);
	...
	while (count > 0)
	{
		TRACE ("#W# %d/%d count=%zd off=%llx,%llx \n", (int)(wc - wdf->chunk), wdf->chunk_used,
			count, (u64)off, wc->file_pos);
		...
		if (off < wc->file_pos)
		{
			...
			if (wr_size)
			{
				TRACE ("#W# >CREATE#%02d    %p +%8zx = %10llx .. +%4x\n", (int)(wc - wdf->chunk),
					buf, src - (ccp)buf, (u64)off, wr_size);
...
enumError WriteZeroWDF (SuperFile_t *sf, off_t off, size_t count)
{
	...
	TRACE ("#W# -----\n");
	TRACE (TRACE_RDWR_FORMAT, "#W# WriteZeroWDF()", GetFD (&sf->f), GetFP (&sf->f), (u64)off,
		(u64)off + count, count, off < sf->max_virt_off ? " <" : "");
	...
	for (; wc < last_wc || wc->file_pos < data_end; wc++)
	{
		...
		TRACE ("loop: wc=%llx,%llx,%llx off=%llx, end=%llx\n", wc->file_pos, wc->data_off,
			wc->data_size, (u64)off, (u64)end);
		if (off >= end)
			continue;
...
void SetupOptionsWDF ()
{
	...
	use_wdf_version
		= opt_wdf_version && opt_wdf_version <= WDF_MAX_VERSION ? opt_wdf_version : WDF_MAX_VERSION;
#else
	use_wdf_version = opt_wdf_version ? opt_wdf_version
		: use_wdf_align >= 0x80		  ? 2
									  : WDF_DEF_VERSION;
...
int ScanOptAlignWDF (ccp arg, ccp opt_name)
{
	...
	u32 align = 0;
	enumError stat = !*arg ? 0
						   : ScanSizeOptU32 (&align, // u32 * num
								 arg, // ccp source
								 1, // default_factor1
								 0, // int force_base
	...
	if (!stat)
	{
		...
		{
			u32 value;
			stat = ScanSizeOptU32 (&value, // u32 * num
					   minhole, // ccp source
					   1, // default_factor1
					   0, // int force_base
					   opt_name, // ccp opt_name
