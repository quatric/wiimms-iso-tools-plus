...
static s64 dol_addr_to_file_offset (const dol_mem_ctx_t *ctx, u32 addr)
{
	for (int i = 0; i < DOL_MAX_SECT; i++)
	{
		if (ctx->sect_size[i] > 0 && addr >= ctx->sect_addr[i]
			&& addr < ctx->sect_addr[i] + ctx->sect_size[i])
...
static const BrawlStageMapping_t brawl_stage_mappings[] = { { "stgdonkey", "st_donkey" }, /* 75m */
...
static int search_bytes (
	const u8 *haystack, size_t haystack_len, const u8 *needle, size_t needle_len)
{
...
static int replace_bytes (
	u8 **buf_ptr, size_t *len_ptr, const u8 *find, size_t find_len, const u8 *repl, size_t repl_len)
{
	int count = 0;
	int idx;
	while ((idx = search_bytes (*buf_ptr, *len_ptr, find, find_len)) >= 0)
	{
		...
		else if (repl_len < find_len)
		{
			memcpy (*buf_ptr + idx, repl, repl_len);
			memmove (
				*buf_ptr + idx + repl_len, *buf_ptr + idx + find_len, *len_ptr - (idx + find_len));
...
static const u8 pat_stock_remove[40] = { 0x04, 0x42, 0x18, 0xEC, 0x00, 0x09, 0x5F, 0x00, 0x04, 0x42,
	0x19, 0x0C, 0x00, 0x18, 0x00, 0x00, 0x04, 0x49, 0x49, 0x90, 0x00, 0x09, 0x5F, 0x00, 0x04, 0x49,
	0x49, 0xEC, 0x80, 0xC2, 0x3A, 0x60, 0x04, 0x49, 0x49, 0xF0, 0x00, 0x18, 0x00, 0x00 };

static const u8 pat_stock_patch[16] = { 0x7F, 0xFC, 0xFB, 0x78, 0x3D, 0x80, 0x80, 0xC2, 0xA1, 0x8C,
	0x43, 0x14, 0x3B, 0xA0, 0x5F, 0x65 };

static const u8 pat_stock_to[16] = { 0x7F, 0xFC, 0xFB, 0x78, 0x3D, 0x80, 0x80, 0xC3, 0xA1, 0x8C,
	0xD1, 0x14, 0x3B, 0xA0, 0x5F, 0x65 };

static const u8 pat_altstage_if[32] = { 0x91, 0x62, 0x00, 0x00, 0x90, 0xE2, 0x00, 0x04, 0x91, 0x82,
	0x00, 0x08, 0x90, 0x62, 0x00, 0x10, 0x91, 0x22, 0x00, 0x14, 0x3C, 0x60, 0x70, 0x72, 0x60, 0x63,
	0x6F, 0x6A, 0x80, 0xE1, 0x00, 0x20 };
...
static const u8 pat_altstage_patch2[56] = { 0x38, 0x61, 0x00, 0x88, 0x4B, 0xA7, 0x4D, 0xB9, 0x7C,
	0x7C, 0x1B, 0x78, 0x2C, 0x03, 0x00, 0x00, 0x40, 0x82, 0x00, 0x0C, 0x38, 0x21, 0x00, 0x80, 0x48,
	0x00, 0x00, 0x1C, 0xB8, 0x41, 0x00, 0x08, 0x38, 0x21, 0x00, 0x80, 0x4B, 0xE5, 0x24, 0xE5, 0x38,
	0x61, 0x00, 0x08, 0x4B, 0xA7, 0x42, 0xE1, 0x7C, 0x7C, 0x1B, 0x78, 0x4B, 0xA7, 0x41, 0xE8 };

static const u8 pat_altstage_to2[80] = { 0x38, 0x61, 0x00, 0x88, 0x80, 0x83, 0x00, 0x00, 0x38, 0x84,
	0x00, 0x0C, 0x90, 0x83, 0x00, 0x00, 0x4B, 0xA7, 0x42, 0xFD, 0x7C, 0x7C, 0x1B, 0x78, 0x2C, 0x03,
	0x00, 0x00, 0x40, 0x82, 0x00, 0x0C, 0x38, 0x21, 0x00, 0x80, 0x48, 0x00, 0x00, 0x28, 0xB8, 0x41,
	0x00, 0x08, 0x38, 0x21, 0x00, 0x80, 0x4B, 0xE5, 0x24, 0xD9, 0x38, 0x61, 0x00, 0x08, 0x80, 0x83,
	0x00, 0x00, 0x38, 0x84, 0xFF, 0xF4, 0x90, 0x83, 0x00, 0x00, 0x4B, 0xA7, 0x42, 0xC9, 0x7C, 0x7C,
	0x1B, 0x78, 0x4B, 0xA7, 0x41, 0xD0 };

static const u8 pat_sfx_pm_if[16] = { 0x2F, 0x70, 0x72, 0x6F, 0x6A, 0x65, 0x63, 0x74, 0x6D, 0x2F,
	0x70, 0x66, 0x2F, 0x73, 0x66, 0x78 };

static const u8 pat_sfx_rsbe_if[12]
	= { 0x2F, 0x52, 0x53, 0x42, 0x45, 0x2F, 0x70, 0x66, 0x2F, 0x73, 0x66, 0x78 };

static const u8 pat_sfx_patch1[40] = { 0x90, 0x81, 0x00, 0x18, 0x38, 0x80, 0xFF, 0xFF, 0x90, 0x81,
	0x00, 0x1C, 0x38, 0x61, 0x00, 0x20, 0x90, 0x61, 0x00, 0x08, 0x7C, 0xE4, 0x3B, 0x78, 0x38, 0xA0,
	0x00, 0x80, 0x4B, 0xE5, 0x29, 0xF5, 0x38, 0x61, 0x00, 0x08, 0x4B, 0xA7, 0x52, 0xA1 };

static const u8 pat_sfx_to1[40] = { 0x90, 0x81, 0x00, 0x18, 0x38, 0x80, 0x00, 0x00, 0x90, 0x81,
	0x00, 0x1C, 0x38, 0x61, 0x00, 0x20, 0x90, 0x61, 0x00, 0x08, 0x7C, 0xE4, 0x3B, 0x78, 0x38, 0xA0,
	0x00, 0x80, 0x4B, 0xE5, 0x29, 0xF5, 0x38, 0x61, 0x00, 0x08, 0x4B, 0xA7, 0x47, 0xF1 };
...
static void patch_gct_for_brawl (u8 **gct_buf_ptr, size_t *gct_len_ptr, bool *remove_en_ptr)
{
	*remove_en_ptr = false;
	...
	replace_bytes (gct_buf_ptr, gct_len_ptr, pat_stock_patch, sizeof (pat_stock_patch),
		pat_stock_to, sizeof (pat_stock_to));

	// 2. Alt stage & disc loading
	if (search_bytes (*gct_buf_ptr, *gct_len_ptr, pat_altstage_if, sizeof (pat_altstage_if)) >= 0)
	{
		*remove_en_ptr = true;
		replace_bytes (gct_buf_ptr, gct_len_ptr, pat_altstage_patch1, sizeof (pat_altstage_patch1),
			pat_altstage_to1, sizeof (pat_altstage_to1));
		replace_bytes (gct_buf_ptr, gct_len_ptr, pat_altstage_patch2, sizeof (pat_altstage_patch2),
			pat_altstage_to2, sizeof (pat_altstage_to2));
	}

	// 3. SFX DVD loading
	if (search_bytes (*gct_buf_ptr, *gct_len_ptr, pat_sfx_pm_if, sizeof (pat_sfx_pm_if)) >= 0)
	{
		replace_bytes (gct_buf_ptr, gct_len_ptr, pat_sfx_patch1, sizeof (pat_sfx_patch1),
			pat_sfx_to1, sizeof (pat_sfx_to1));
		replace_bytes (gct_buf_ptr, gct_len_ptr, pat_sfx_patch2, sizeof (pat_sfx_patch2),
			pat_sfx_pm_to2, sizeof (pat_sfx_pm_to2));
	}
	else if (search_bytes (*gct_buf_ptr, *gct_len_ptr, pat_sfx_rsbe_if, sizeof (pat_sfx_rsbe_if))
		>= 0)
	{
		replace_bytes (gct_buf_ptr, gct_len_ptr, pat_sfx_patch1, sizeof (pat_sfx_patch1),
			pat_sfx_to1, sizeof (pat_sfx_to1));
		replace_bytes (gct_buf_ptr, gct_len_ptr, pat_sfx_patch2, sizeof (pat_sfx_patch2),
			pat_sfx_rsbe_to2, sizeof (pat_sfx_rsbe_to2));
	}
}
		...
		if (len > 6 && !strcasecmp (ent->d_name + len - 4, ".pac"))
		{
			...
			if (ent->d_name[len - 6] == '_' && isalpha ((int)ent->d_name[len - 5]))
			{
				...
				if (rel_name)
				{
					...
					snprintf (
						base_rel, sizeof (base_rel), "%s/module/%s.rel", dest_files_dir, rel_name);
					snprintf (alt_rel, sizeof (alt_rel), "%s/module/%s%s.rel", dest_files_dir,
						rel_name, alt_code);
					...
					while ((ent2 = readdir (d2)) != NULL)
					{
						...
						if (l2 > 6 && !strcasecmp (ent2->d_name + l2 - 4, ".pac"))
						{
							if (!strncasecmp (ent2->d_name, base_name, strlen (base_name))
								&& ent2->d_name[l2 - 6] == '_'
								&& isalpha ((int)ent2->d_name[l2 - 5]))
							{
								char alt_path[PATH_MAX];
								snprintf (alt_path, sizeof (alt_path), "%s/%s", dest_stage_dir,
									ent2->d_name);
								struct stat st_alt;
				...
				if (largest_size > st_base.st_size)
				{
					if (verbose >= 1)
						printf ("  + Padded %s from %lld to %ld bytes\n", ent->d_name,
							(long long)st_base.st_size, largest_size);
...
static int copy_mod_dir_recursive (
	const char *src_base, const char *rel_sub, const char *dest_files, bool remove_en)
{
		...
		if (stat (item_full, &st) == 0)
		{
			...
			else if (S_ISREG (st.st_mode))
			{
				...
				if (dot)
				{
					...
					snprintf (
						base_without_ext, sizeof (base_without_ext), "%.*s", (int)blen, item_rel);
					snprintf (target_en, sizeof (target_en), "%s/%s_en%s", dest_files,
						base_without_ext, dot);
				}
				else
				{
...
static enumError patch_brawl_dol (
	const char *dol_path, const u8 *gct_data, size_t gct_len, u32 gct_offset, const char *custom_id)
{
	...
	if (verbose >= 1)
		printf (
			"  + Injected GCT section #%d at 0x%08X (%zu bytes)\n", gct_sec, gct_offset, gct_len);
...
static void prompt_brawl_interactive (BrawlOptions_t *opt, char *custom_id_buf, size_t id_buf_size,
	char *custom_title_buf, size_t title_buf_size)
{
	printf ("\n===================================================\n");
	...
	if (fgets (input_line, sizeof (input_line), stdin))
	{
		...
		if (nl)
			*nl = '\0';
		while (*input_line && isspace ((int)*input_line))
			memmove (input_line, input_line + 1, strlen (input_line));
		char *end = input_line + strlen (input_line) - 1;
		while (end >= input_line && isspace ((int)*end))
		{
			*end = '\0';
			end--;
		}
...
enumError BrawlCommand (BrawlOptions_t *opt)
{
	if (!opt || !opt->source_image)
		return ERROR0 (ERR_SYNTAX,
			"wit BRAWLBUILDER requires a source Brawl disc image / extracted FST.\n"
	...
	else if (opt->mod_folder)
	{
		const char *candidates[]
			= { "%s/codes/RSBE01.gct", "%s/RSBE01.gct", "%s/projectm/codes/RSBE01.gct",
				  "%s/projectm/RSBE01.gct", "%s/codes/RSBE.gct", "%s/RSBE.gct", NULL };
	...
	if (opt->interactive)
		prompt_brawl_interactive (opt, custom_id_buf, sizeof (custom_id_buf), custom_title_buf,
			sizeof (custom_title_buf));
	...
	else
	{
		...
		snprintf (extract_cmd, sizeof (extract_cmd),
			"./bin/wit EXTRACT \"%s\" --psel=DATA --dest \"%s\" %s", opt->source_image, work_dir,
			verbose >= 1 ? "-vv" : "-q");
	...
	if (stat (main_dol_path, &st_md) == 0 && gct_buf && gct_len > 0)
	{
		...
		enumError err
			= patch_brawl_dol (main_dol_path, gct_buf, gct_len, opt->gct_offset, opt->custom_id);
		if (err != ERR_OK)
		{
			FREE (gct_buf);
			if (temp_workdir && !opt->keep_temp)
			{
				char r[PATH_MAX + 16];
				snprintf (r, sizeof (r), "rm -rf \"%s\"", work_dir);
				system (r);
	...
	if (opt->dest_path)
	{
		...
		if (opt->custom_id)
			snprintf (extra_args + strlen (extra_args), sizeof (extra_args) - strlen (extra_args),
				" --id=\"%.6s\"", opt->custom_id);
		if (opt->custom_name)
			snprintf (extra_args + strlen (extra_args), sizeof (extra_args) - strlen (extra_args),
				" --name=\"%s\"", opt->custom_name);
		if (opt->overwrite)
			strcat (extra_args, " --overwrite");

		snprintf (pack_cmd, sizeof (pack_cmd), "./bin/wit COPY \"%s\" \"%s\" %s %s", work_dir,
			opt->dest_path, extra_args, verbose >= 1 ? "-vv" : "-q");
		...
		if (ret != 0)
		{
			...
			return ERROR0 (
				ERR_CANT_CREATE, "Failed to pack final disc image: %s\n", opt->dest_path);
		}
