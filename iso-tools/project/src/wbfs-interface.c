...
enumError AnalyzePartitions (FILE *outfile, bool non_found_is_ok, bool scan_wbfs)
{
	TRACE ("AnalyzePartitions(,%d,%d) PART1=%d, AUTO=%d, all=%d, first=%p\n", non_found_is_ok,
		scan_wbfs, opt_part, opt_auto, opt_all, first_partition_info);
	...
	for (info = first_partition_info; info; info = info->next)
	{
		...
		if (info->part_mode == PM_UNKNOWN)
		{
			...
			TRACE (" - st_mode=%x reg=%d dir=%d chr=%d blk=%d fifo=%d link=%d sock=%d\n",
				F.st.st_mode, S_ISREG (F.st.st_mode), S_ISDIR (F.st.st_mode),
				S_ISCHR (F.st.st_mode), S_ISBLK (F.st.st_mode), S_ISFIFO (F.st.st_mode),
				S_ISLNK (F.st.st_mode), S_ISSOCK (F.st.st_mode));
			...
			wbfs_head_t whead;
			stat = F.st.st_size < sizeof (whead) ? ERR_WARNING : ReadF (&F, &whead, sizeof (whead));
			if (stat)
			{
	...
	else if (return_stat)
	{
		...
		if (!print_sections)
			fprintf (
				outfile, "%d WBFS partition%s found\n", wbfs_count, wbfs_count == 1 ? "" : "s");
...
void ScanPartitionGames ()
{
	...
	PartitionInfo_t *info;
	enumError stat;
	for (stat = GetFirstWBFS (&wbfs, &info, false); !stat; stat = GetNextWBFS (&wbfs, &info, false))
	{
...
enumError ScanParamID6 (StringField_t *select_list, // append all results to this list
	...
	for (; param; param = param->next)
	{
		...
		for (;;)
		{
			...
			if (err || wildcards > 1 || arglen > 6)
				return ERROR0 (
					ERR_SEMANTIC, "Illegal ID selector: %.*s\n", (int)(arg - start), start);
...
bool MatchRulesetID (const StringField_t *select_list, // selector list
...
bool MatchPatternID (ccp pattern, // pattern, '.' is a wildcard
	ccp id // id to compare
)
{
	DASSERT (pattern);
...
enumError CheckParamRename (bool rename_id, bool allow_plus, bool allow_index)
{
	...
	for (param = first_param; param; param = param->next)
	{
		...
		else if (allow_index)
		{
			...
			if (arg == start)
			{
				ERROR0 (ERR_SEMANTIC, "ID6 or INDEX or #SLOT expected: %s\n", param->arg);
				syntax_count++;
				continue;
			}
			snprintf (param->selector, sizeof (param->selector), "$%lu", index);
		...
		{
			ERROR0 (ERR_SEMANTIC, "ID6 expected: %s\n", param->arg);
			syntax_count++;
			continue;
		}
		...
		if (index > 99999)
		{
			ERROR0 (ERR_SEMANTIC, "Slot or disc index too large: %s\n", param->arg);
			semantic_count++;
			continue;
		}
...
enumError ResetWBFS (WBFS_t *w)
{
	...
	enumError err = ERR_OK;
	if (wbfs_cache_enabled && w->cache_candidate && w->sf_alloced && w->sf && IsOpenSF (w->sf))
	{
		CloseWBFSCache ();
...
enumError SetupWBFS (WBFS_t *w, SuperFile_t *sf, bool print_err, int sector_size, bool recover)
{
	ASSERT (w);
	ASSERT (sf);
	...
	if (S_ISREG (sf->f.st.st_mode) && sf->src && prealloc_mode > PREALLOC_OFF)
	{
		...
		else
		{
			PreallocateF (&sf->f, 0,
				sector_size
					+ ALIGN64 (
						sizeof (wbfs_disc_info_t) + 2 * WII_MAX_DISC_SIZE / bl_size, sector_size));
...
static enumError OpenWBFSHelper (WBFS_t *w, ccp filename,
	bool open_modify, // true: open read+write
	bool print_err, wbfs_param_t *par, int sector_size, bool recover)
{
	ASSERT (w);
	PRINT ("OpenWBFSHelper(%s,rw=%d,pe=%d,ss=%d,recover=%d)\n", filename, open_modify, print_err,
		sector_size, recover);

	if (wbfs_cache_valid && IsOpenSF (wbfs_cache.sf)
	...
	enumError err = open_modify ? OpenWFileModify (&sf->f, filename, IOM_IS_WBFS_PART)
...
enumError OpenWBFS (WBFS_t *w, // valid data structure
...
enumError FormatWBFS (
	WBFS_t *w, ccp filename, bool print_err, wbfs_param_t *par, int sector_size, bool recover)
{
...
enumError RecoverWBFS (WBFS_t *wbfs, ccp fname, bool testmode)
{
	...
	if (!err)
	{
		...
		if (inodes_dirty)
			WriteSF (
				wbfs->sf, w->hd_sec_sz, first_sector + w->hd_sec_sz, w->max_disc * w->disc_info_sz);
		...
		if (CheckWBFS (&ck, wbfs, 0, -1, 0, 0))
		{
			...
			for (slot = 0; slot < w->max_disc; slot++)
				if (w->head->disc_table[slot] & WBFS_SLOT__USER)
				{
					...
					if (cd->no_blocks || cd->bl_overlap || cd->bl_invalid)
					{
				...
				for (slot = 0; slot < w->max_disc; slot++)
					if (w->head->disc_table[slot] & WBFS_SLOT__USER)
					{
						...
						printf ("   - slot #%03u [%.6s] %s\n", slot, id6,
							GetTitle (id6, (ccp)inode->disc_title));
					}
			}
...
enumError TruncateWBFS (WBFS_t *w)
{
	...
	if (w->wbfs && w->sf)
	{
		...
		TRACE (" - cut = %u * %u = %llu = %llx/hex\n", w->wbfs->n_hd_sec, w->wbfs->hd_sec_sz, cut,
			cut);
		SetSizeF (&w->sf->f, cut);
	}
	return err;
...
enumError OpenPartWBFS (WBFS_t *w, PartitionInfo_t *info, bool open_modify)
{
	...
	const enumError err
		= OpenWBFSHelper (w, info->real_path, open_modify, info->source == PS_PARAM, 0, 0, 0);
	if (err)
	{
...
static enumError GetWBFSHelper (WBFS_t *w, PartitionInfo_t **p_info, bool open_modify)
{
	ResetWBFS (w);
...
static void print_wbfs_section (WBFS_t *w, // valid and opened WBFS
...
)
{
	...
	printf ("path=%s\n"
		...
		path		? path
			: w->sf ? w->sf->f.fname
					: "",
		w->wbfs->head->wbfs_version, wbfs_count_discs (w->wbfs), w->wbfs->max_disc, w->free_mib,
		w->used_mib, w->total_mib, term_string);
}

//-----------------------------------------------------------------------------

void LogOpenedWBFS (WBFS_t *w, // valid and opened WBFS
	int count, // wbfs counter, 1 based
			   // if NULL: neither 'count' nor 'total' are printed
	int total, // total wbfs count to handle
			   // if NULL: don't print info
...
void LogCloseWBFS (WBFS_t *w, // valid and opened WBFS
	int count, // wbfs counter, 1 based
			   // if NULL: neither 'count' nor 'total' are printed
	int total, // total wbfs count to handle
			   // if NULL: don't print info
...
uint GetIdWBFS (WBFS_t *wbfs, IdField_t *idf)
{
	...
	if (w)
	{
		...
		for (slot = 0; slot < w->max_disc; slot++)
			...
			{
				const time_t mtime
					= wbfs_is_inode_info_valid (w, &head.iinfo) ? ntoh64 (head.iinfo.mtime) : 0;
...
enumError DumpWBFS (WBFS_t *wbfs, // valid WBFS
...
)
{
	...
	if (head && show_mode & (SHOW_FHEADER | SHOW_SLOT))
	{
		...
		if (show_mode & SHOW_FHEADER)
		{
			...
			fprintf (f, "%*s  WBFS MAGIC: %10x %02x %02x %02x =         '%s'\n", indent, "",
				magic[0], magic[1], magic[2], magic[3], wd_print_id (magic, 4, 0));

			fprintf (f, "%*s  WBFS VERSION:     %#13x =%15u\n", indent, "", head->wbfs_version,
				head->wbfs_version);
		...
		for (slot = 0; slot < w->max_disc; slot++)
		{
			...
			if (show_mode & (SHOW_FILES | SHOW_D_MAP))
			{
				fprintf (f, "\n%*sDump of %sWii disc at slot #%d of %d:\n", indent, "",
					d->is_used ? "" : "*DELETED* ", slot, w->max_disc);
			...
			if (show_mode & (SHOW_D_MAP | SHOW_W_MAP))
			{
				...
				for (;;)
				{
					...
					if (start == idx - 1)
						fprintf (f, "%*s%11u      : %9u      :%10llx ..%10llx :%9llx\n", ind, "",
							start, start + delta, (u64)off, (u64)(off + size), (u64)size);
					else
						fprintf (f, "%*s%7u ..%6u : %5u .. %5u :%10llx ..%10llx :%9llx\n", ind, "",
							start, idx - 1, start + delta, idx - 1 + delta, (u64)off,
							(u64)(off + size), (u64)size);
				}
				...
				if (show_mode & SHOW_W_MAP && d->is_used)
				{
					...
					snprintf (
						mi->info, sizeof (mi->info), "Inode of slot #%03u [%s]", slot, dinfo.id6);
					...
					for (;; segment++)
					{
						...
						if (segment)
							snprintf (mi->info, sizeof (mi->info),
								"%4u block%s -> disc #%03u.%u [%s]", n, n == 1 ? " " : "s", slot,
								segment, dinfo.id6);
						else
							snprintf (mi->info, sizeof (mi->info),
								"%4u block%s -> disc #%03u   [%s]", n, n == 1 ? " " : "s", slot,
								dinfo.id6);
					}
				}
			}
	...
	if (show_mode & SHOW_W_MAP)
	{
		...
		for (;;)
		{
			...
			{
				mi = InsertMemMap (
					&mm, w->hd_sec_sz + start * w->disc_info_sz, count * w->disc_info_sz);
				if (count > 1)
					snprintf (mi->info, sizeof (mi->info), "%d unused inodes, slots #%03u..#%03u",
						count, start, slot - 1);
				else
					snprintf (mi->info, sizeof (mi->info), "Unused inode, slot #%03u", start);
			}
			if (slot == w->max_disc)
				break;
			slot++;
...
int ScanOptWbfsAlloc (ccp arg)
{
	static const KeywordTab_t tab[]
		= { { WBFS_BA_AUTO, "AUTO", "DEFAULT", 0 }, { WBFS_BA_FIRST, "FIRST", "FRAG", 0 },
			  { WBFS_BA_AVOID_FRAG, "NO-FRAG", "NOFRAG", 0 }, { 0, 0, 0, 0 } };
...
AWRecord_t *AW_get_record (AWData_t *awd)
{
	...
	AWRecord_t *r = awd && awd->n_record < AW_MAX_RECORD ? awd->rec + awd->n_record++ : &dummy;
	memset (r, 0, sizeof (*r));
	return r;
}
...
static void AW_header (AWData_t *awd, WFile_t *f)
{
	...
	wbfs_head_t wh;
	if (ReadAtF (f, 0, &wh, sizeof (wh)) == ERR_OK && !memcmp ((ccp)&wh.magic, "WBFS", 4))
	{
...
static AWRecord_t *AW_insert_inode (AWData_t *awd, aw_inode_t *inode, ccp head)
{
	ASSERT (awd);
	ASSERT (inode);
	ASSERT (head);
...
static void AW_inodes (AWData_t *awd, WFile_t *f, ccp data)
{
	...
	for (; ptr < end; ptr += 512)
	{
		...
		if (!memcmp ((ccp)&iinfo->magic, "WBFS", 4))
		{
			...
			if (hd_sec_size >= HD_SECTOR_SIZE && hd_sec_size < wbfs_sec_size
...
static void AW_discs (AWData_t *awd, WFile_t *f, ccp data)
{
	...
	for (sec = 1; sec <= SEC_MAX; sec++)
	{
		...
		if ((magic == WII_MAGIC || magic == WII_MAGIC_DELETED) && CheckID6 (wd, false, false))
		{
...
static void AW_calc (AWData_t *awd, WFile_t *f, u32 sec_size, bool old)
{
	...
	const u32 n_wii_sec
		= old ? (num_sec / WII_SECTOR_SIZE) * sec_size : num_sec / (WII_SECTOR_SIZE / sec_size);
	...
	if (old)
	{
		...
		{
			r = awd->rec + i;
			if (r->status == AW_CALC && r->hd_sectors == num_sec
...
int AnalyzeWBFS (AWData_t *awd, WFile_t *f)
{
	...
	for (i = 0; i < awd->n_record; i++)
	{
		...
		wbfs_calc_geometry (&wbfs, r->hd_sectors, r->hd_sector_size, r->wbfs_sector_size);
...
int PrintAnalyzeWBFS (FILE *out, // valid output stream
...
)
{
	...
	for (i = 0; i < awd->n_record; i++)
	{
		...
		{
			snprintf (buf, sizeof (buf), " \" but sector-size=%u", ar->hd_sector_size);
			info = buf;
		}

		fprintf (out, "%*s%-10s %s %2x %8x %4x %5x %7x %4x %4x  %s\n", indent, "", ar->title,
			ar->magic_found ? "ok" : " -", ar->wbfs_version, ar->hd_sectors, ar->hd_sector_size,
			ar->wbfs_sectors, ar->wbfs_sector_size, ar->max_disc, ar->disc_info_size, info);
	}
...
static void check_func (wbfs_t *p, // valid WBFS descriptor
...
enumError CheckWBFS (
	CheckWBFS_t *ck, WBFS_t *wbfs, int prt_sections, int verbose, FILE *f, int indent)
{
	ASSERT (ck);
	ASSERT (wbfs);
	...
	if (prt_sections)
		fprintf (f, "column-header=           slot _id6__ index wbfs-block count\n");
				...
				if (blc[wlba] > 1)
				{
					...
					if (prt_sections)
						fprintf (f, "err-block-multi-usage=%7u %s %5u %7u %7u\n", slot,
							wd_print_id (g->id6, 6, 0), bl, wlba, blc[wlba]);
					else if (verbose >= SHOW_DETAILS)
						fprintf (f, "%*s  - #%d=%s/%u: WBFS block #%u is used %u times!\n", indent,
							"", slot, g->id6, bl, wlba, blc[wlba]);
				}
			}
		}
		...
		else if (!blc[bl] && ubl[bl])
		{
			...
			if (verbose >= SHOW_DETAILS && !prt_sections)
			{
				...
				if (wbfs0_count && !total_err_fbt_free_wbfs0)
					fprintf (f,
						"%*sNote: Free sectors >= #%u are marked 'used'"
	...
	ck->err_total = total_err_fbt_used + total_err_fbt_free + total_err_no_blocks
		+ total_err_overlap + total_err_invalid;
	...
	if (ck->err_total && verbose >= PRINT_DUMP)
	{
		...
		DumpWBFS (wbfs, f, indent, show_mode, 0, verbose >= PRINT_FULL_DUMP,
...
enumError PrintCheckedWBFS (CheckWBFS_t *ck, FILE *f, int indent, int prt_sections)
{
	...
	if (prt_sections)
	{
		fprintf (f,
			...
			ck->err_fbt_used, ck->err_fbt_free, ck->err_bl_overlap, ck->err_bl_invalid,
			ck->err_no_blocks, ck->err_total, ck->invalid_disc_count);
	}
					...
					|| rm & REPAIR_RM_EMPTY && disc->no_blocks))
			{
				if (verbose >= 0)
				{
					...
					fprintf (f, "%*s* %sDrop disc at slot #%u, id=%s%s%s\n", indent, "",
						testmode ? "WOULD " : "", slot, disc->id6, title ? ", " : "",
						title ? title : "");
				}
		...
		if (CalcFBT (ck))
		{
			...
			if (verbose >= 0)
				fprintf (f, "%*s* %sStore fixed 'free blocks table' (%zd bytes).\n", indent, "",
					testmode ? "WOULD " : "", ck->fbt_size);

			if (!testmode)
			{
				TRACELINE;
				enumError err
					= WriteAtF (&ck->wbfs->sf->f, ck->fbt_off, ck->good_fbt, ck->fbt_size);
				if (err)
					return err;
		...
		if (w->head->wbfs_version < WBFS_VERSION)
		{
			if (verbose >= 0)
				fprintf (f, "%*s* %sSet WBFS version from %u to %u.\n", indent, "",
					testmode ? "WOULD " : "", w->head->wbfs_version, WBFS_VERSION);
			if (!testmode)
			{
...
enumError CheckRepairWBFS (
	WBFS_t *wbfs, int testmode, RepairMode rm, int verbose, FILE *f, int indent)
{
	ASSERT (wbfs);
...
enumError GetWDiscInfo (WBFS_t *w, WDiscInfo_t *dinfo, int disc_index)
{
	...
	u32 slot, size4;
	const enumError err
		= wbfs_get_disc_info (w->wbfs, disc_index, (u8 *)&dinfo->dhead, sizeof (dinfo->dhead),
			&slot, &dinfo->disc_type, &dinfo->disc_attrib, &size4, &dinfo->wbfs_fragments);
...
enumError GetWDiscInfoBySlot (WBFS_t *w, WDiscInfo_t *dinfo, u32 disc_slot)
{
	...
	u32 size4;
	const enumError err = wbfs_get_disc_info_by_slot (w->wbfs, disc_slot, (u8 *)&dinfo->dhead,
		sizeof (dinfo->dhead), &dinfo->disc_type, &dinfo->disc_attrib, &size4,
		&dinfo->wbfs_fragments);
...
enumError FindWDiscInfo (WBFS_t *w, WDiscInfo_t *dinfo, ccp id6)
{
	...
	{
		if (GetWDiscInfo (w, dinfo, i) == ERR_OK && !memcmp (id6, dinfo->dhead.id6.id6, 6))
			return ERR_OK;
	}
...
enumError LoadIsoHeader (WBFS_t *w, wd_header_t *iso_header, wbfs_disc_t *disc)
{
	...
	enumError err = ReadSF (w->sf, wlba * (off_t)w->wbfs->wbfs_sec_sz, buf, sizeof (buf));
	if (!err)
...
enumError DumpWDiscInfo (WDiscInfo_t *di, wd_header_t *ih, FILE *f, int indent)
{
	if (!di || !f)
		return ERROR0 (ERR_INTERNAL, 0);
...
WDiscListItem_t *AppendWDiscList (WDiscList_t *wlist, WDiscInfo_t *dinfo)
{
	...
	{
		wlist->size += 200;
		wlist->first_disc = REALLOC (wlist->first_disc, wlist->size * sizeof (*wlist->first_disc));
	}
...
static int sort_by_title (const void *va, const void *vb)
{
	...
	int stat = strcasecmp (a->title ? a->title : a->name64, b->title ? b->title : b->name64);
	if (!stat)
	{
		...
		if (!stat)
		{
			...
			if (!stat)
			{
				stat = strcmp (a->fname ? a->fname : "", b->fname ? b->fname : "");
				if (!stat)
...
static int sort_by_file (const void *va, const void *vb)
{
	...
	const int stat = strcmp (a->fname ? a->fname : "", b->fname ? b->fname : "");
	return stat ? stat : sort_by_title (va, vb);
}
...
void SortWDiscList (WDiscList_t *wlist, SortMode sort_mode, SortMode default_sort_mode, int unique)
{
	if (!wlist)
		return;

	TRACE ("SortWDiscList(%p, s=%d,%d, u=%d) prev=%d\n", wlist, sort_mode, default_sort_mode,
		unique, wlist->sort_mode);
	...
	if (unique)
	{
		...
		for (; src < end; src++)
		{
			if (!prev || memcmp (src->id6, prev->id6, 6)
				|| unique < 2
					&& (memcmp (src->name64, prev->name64, 64) || src->size_mib != prev->size_mib))
			{
				if (dest != src)
...
void PrintSectWDiscListItem (FILE *f, WDiscListItem_t *witem, ccp def_fname)
{
	...
	fprintf (f, "size=%llu\n",
		(u64)witem->fatt.size ? (u64)witem->fatt.size : (u64)witem->size_mib * MiB);
...
enumError CloseWDisc (WBFS_t *w)
{
	...
	if (w->disc)
	{
		...
		if (sf && sf->f.is_writing && w->disc->header && w->is_wbfs_file
			&& CopyPatchWbfsId ((char *)w->disc->header->dhead,
				*sf->wbfs_id6 ? sf->wbfs_id6 : (ccp)w->disc->header->dhead)) // [[id+]]
		{
...
enumError ExistsWDisc (WBFS_t *w, ccp id6)
{
	...
	return wbfs_find_slot (w->wbfs, (u8 *)patched_id6) < 0 ? ERR_WDISC_NOT_FOUND : ERR_OK;
}
...
{
	return w && w->disc && w->disc->header ? (wd_header_t *)w->disc->header : 0;
}
...
enumError RemoveWDisc (WBFS_t *w, // valid WBFS descriptor
...
enumError RenameWDisc (WBFS_t *wbfs, ccp set_id6, ccp set_title, bool change_wbfs_head,
	bool change_iso_head, int verbose, int testmode)
{
	ASSERT (wbfs);
	ASSERT (wbfs->wbfs);
	ASSERT (wbfs->disc);
	...
	TRACE ("RenameWDisc(%p,%.6s,%s,%d,%d,v=%d,tm=%d)\n", wbfs, set_id6 ? set_id6 : "-",
		set_title ? set_title : "-", change_wbfs_head, change_iso_head, verbose, testmode);
	...
	{
		ccp mode = !change_wbfs_head ? "(ISO header only)"
			: !change_iso_head		 ? "(WBFS header only)"
									 : "(WBFS+ISO header)";
		printf (
	...
	if (set_title)
	{
		...
		SubstString_t subst_tab[]
			= { { 'j', 0, 0, w_id6 }, { 'J', 0, 0, i_id6 }, { 'i', 'I', 0, n_id6 },

				  { 'n', 0, 0, w_name }, { 'N', 0, 0, i_name },

				  { 'p', 0, 0, w_title }, { 'P', 0, 0, i_title }, { 't', 'T', 0, n_title },
	...
	if (!testmode
		&& wbfs_rename_disc (wbfs->disc, set_id6, set_title, change_wbfs_head, change_iso_head))
