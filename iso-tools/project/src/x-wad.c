...
static enumError wad_split (const u8 *wad, // the whole file
...
)
{
	...
	if (head->header_size != WAD_HEAD_SIZE)
		return ERROR0 (
			ERR_INVALID_FILE, "Unexpected WAD header size 0x%x: %s\n", head->header_size, fname);
	if (head->type != WAD_TYPE_IS && head->type != WAD_TYPE_IB)
		return ERROR0 (ERR_INVALID_FILE, "Unknown WAD type 0x%08x: %s\n", head->type, fname);

	// Sections follow each other, each starting on a 64 byte boundary.
	u64 off = wad_align (head->header_size);
	const u32 size[6] = { head->cert_size, head->crl_size, head->tik_size, head->tmd_size,
		head->data_size, head->footer_size };
	for (int i = 0; i < 6; i++)
	{
		offset[i] = (u32)off;
...
enumError XInfoWAD (ccp source)
{
...

	{
		...
		printf ("      sections: cert 0x%x, crl 0x%x, tik 0x%x,"
			...
			head.cert_size, head.crl_size, head.tik_size, head.tmd_size, head.data_size,
			head.footer_size);
	}
	...
	if (head.tmd_size >= sizeof (wd_tmd_t))
	{
		...
		printf ("      version:  %u, ios %u, %u content%s\n", ntohs (tmd->title_version),
			(uint)(ntoh64 (tmd->sys_version) & 0xffffffff), n, n == 1 ? "" : "s");
		...
		if (head.tmd_size >= sizeof (wd_tmd_t) + n * sizeof (wd_tmd_content_t))
			for (uint i = 0; i < n; i++)
			{
				...
				printf ("        %2u: id %08x  type 0x%04x  %llu bytes\n", ntohs (c->index),
					ntohl (c->content_id), ntohs (c->type), ntoh64 (c->size));
			}
	}
...
enumError XExtractWAD (ccp source, ccp dest)
{
...

	{
		...
		if (head.tmd_size < sizeof (wd_tmd_t) + n * sizeof (wd_tmd_content_t))
		{
			err = ERROR0 (ERR_INVALID_FILE, "TMD is too short for %u contents: %s\n", n, source);
			goto abort;
		}
		...
		for (uint i = 0; i < n && !err; i++)
		{
			...
			const u64 esize = (csize + WII_KEY_SIZE - 1) / WII_KEY_SIZE * WII_KEY_SIZE;

			if (pos + esize > size)
			{
				err = ERROR0 (ERR_INVALID_FILE, "Content %u ends beyond the file: %s\n", i, source);
				break;
			}
...
enumError XCreateWAD (ccp source, ccp dest)
{
	...
	if (memcmp (tmd_orig, tmd_data, tmd_size))
	{
		...
		fseeko (out,
			wad_align (WAD_HEAD_SIZE) + wad_align (cert_size) + wad_align (crl_size)
				+ wad_align (tik_size),
			SEEK_SET);
		wad_put (out, tmd_data, tmd_size, 0);
	}
