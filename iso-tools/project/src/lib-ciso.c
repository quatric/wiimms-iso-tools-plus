...
int ScanChunkMode (ccp source)
{
	static const KeywordTab_t tab[]
		= { { CHUNK_MODE_ISO, "ISO", 0, 0 }, { CHUNK_MODE_POW2, "POW2", "2", 0 },
			  { CHUNK_MODE_32KIB, "32KIB", 0, 0 }, { CHUNK_MODE_ANY, "ANY", 0, 0 },
...
int ScanChunkSize (ccp source)
{
	...
	return ERR_OK
		!= ScanSizeOptU32 (&opt_chunk_size, // u32 * num
			source, // ccp source
			1, // default_factor1
			0, // int force_base
...
{
	return ERR_OK
		!= ScanSizeOptU32 (&opt_max_chunks, // u32 * num
			source, // ccp source
			1, // default_factor1
			0, // int force_base
...
u32 CalcBlockSizeCISO (u32 *result_n_blocks, off_t file_size)
{
	TRACE ("CalcBlockSizeCISO(,%llx) mode=%u, size=%x%s, min=%x\n", (u64)file_size, opt_chunk_mode,
		opt_chunk_size, force_chunk_size ? "!" : "", opt_max_chunks);
	...
	if (!block_size || !force_chunk_size)
	{
		...
		const u32 max_blocks = opt_max_chunks > 0 ? opt_max_chunks
			: opt_chunk_mode >= CHUNK_MODE_ISO	  ? CISO_WR_MAX_BLOCK
												  : CISO_MAP_SIZE;
		...
		const u32 factor = opt_chunk_mode >= CHUNK_MODE_ISO			 ? CISO_WR_MIN_BLOCK_SIZE
			: opt_chunk_size && opt_chunk_size < CISO_MIN_BLOCK_SIZE ? opt_chunk_size
																	 : CISO_MIN_BLOCK_SIZE;
		TRACE (" - factor := %x\n", factor);
...
enumError ReadCISO (SuperFile_t *sf, off_t off, void *buf, size_t count)
{
	...
	TRACE ("#C# -----\n");
	TRACE (TRACE_RDWR_FORMAT, "#C# ReadCISO()", GetFD (&sf->f), GetFP (&sf->f), (u64)off,
		(u64)off + count, count, "");
...
off_t DataBlockCISO (SuperFile_t *sf, off_t off, size_t hint_align, off_t *block_size)
{
	DASSERT (sf);
	CISO_Info_t *ci = &sf->ciso;
	DASSERT (ci->map);
...
enumError SetupWriteCISO (SuperFile_t *sf)
{
	...
	if (sf->src && prealloc_mode > PREALLOC_OFF)
	{
		...
		{
			PreallocateSF (sf, 0, sizeof (CISO_Head_t), ci->block_size / WII_SECTOR_SIZE, 1);
		}
	}
...
enumError WriteCISO (SuperFile_t *sf, off_t off, const void *buf, size_t count)
{
	...
	TRACE ("#C# -----\n");
	TRACE (TRACE_RDWR_FORMAT, "#C# WriteCISO()", GetFD (&sf->f), GetFP (&sf->f), (u64)off,
	...
	while (count > 0)
	{
		if (block >= ci->map_size)
		{
			if (!sf->f.disable_errors && sf->f.last_error != ERR_WRITE_FAILED)
				ERROR1 (ERR_WRITE_FAILED, "File too large [%c=%d, block=%u/%u]: %s\n",
					GetFT (&sf->f), GetFD (&sf->f), block, ci->map_size, sf->f.fname);
