...
#define TITLE                                                                                      \
	WFUSE_SHORT ": " WFUSE_LONG " v" VERSION " r" REVISION " " SYSTEM2 " - " AUTHOR " - " DATE
...
{
	fprintf (stderr, "-> Type '%s -h' (pipe it to a pager like 'less') for more help.\n\n",
		ProgInfo.progname);
	exit (stat);
}
...
static DiscFile_t *get_disc_file (uint slot)
{
	...
	for (df = dfile; df < end_dfile; df++)
		if (df->used && df->slot == slot)
		{
			...
			TRACE (">>D<< ALLOC DISC #%zu, slot=%d, lock=%d, n=%d/%d\n", df - dfile, df->slot,
				df->lock_count, n_dfile, MAX_DISC_FILES);
			unlock_mutex ();
			return df;
		}
	...
	time_t ref_time
		= time (0) - (n_dfile < GOOD_DISC_FILES ? DISC_FILES_TIMEOUT1 : DISC_FILES_TIMEOUT2);

	DiscFile_t *found_df = 0;
	for (df = dfile; df < end_dfile; df++)
	{
		if (df->used && !df->lock_count && (n_dfile == MAX_DISC_FILES || df->timestamp < ref_time))
		{
	...
	if (found_df)
	{
		...
		else
		{
			...
			found_df->stat_dir.st_atime = found_df->stat_file.st_atime
				= found_df->stat_link.st_atime = si->atime;
			found_df->stat_dir.st_mtime = found_df->stat_file.st_mtime
				= found_df->stat_link.st_mtime = si->mtime;
			found_df->stat_dir.st_ctime = found_df->stat_file.st_ctime
				= found_df->stat_link.st_ctime = si->ctime;

			n_dfile++;

			TRACE (">>D<< OPEN DISC #%zu, slot=%d, lock=%d, n=%d/%d\n", found_df - dfile,
				found_df->slot, found_df->lock_count, n_dfile, MAX_DISC_FILES);
		}
	}
...
static size_t print_root_info (char *pbuf, size_t bufsize)
{
	DASSERT (pbuf);
	...
	return snprintf (pbuf, bufsize,
		...
		"file-size=%llu\n"
		"split-count=%u\n",
		is_iso, is_wbfs, oft_info[main_sf.iod.oft].name, (u64)main_sf.f.st.st_size,
		main_sf.f.split_used > 1 ? main_sf.f.split_used : 0);
}

///////////////////////////////////////////////////////////////////////////////

static size_t print_wbfs_info (char *pbuf, size_t bufsize)
{
	DASSERT (pbuf);
	wbfs_t *w = wbfs.wbfs;
	return !is_wbfs || !w ? 0
						  : snprintf (pbuf, bufsize,
								...
								,
								w->hd_sec_sz, w->n_hd_sec

								,
								w->wii_sec_sz, w->n_wii_sec, w->n_wii_sec_per_disc

								,
								w->wbfs_sec_sz, w->n_wbfs_sec, w->n_wbfs_sec_per_disc

								,
								w->part_lba

								,
								wbfs.used_discs, wbfs.free_discs, wbfs.total_discs

								,
								wbfs.used_mib, wbfs.free_mib, wbfs.total_mib);
}

///////////////////////////////////////////////////////////////////////////////

static size_t print_iso_info (DiscFile_t *df, char *pbuf, size_t bufsize)
{
	DASSERT (df);
	DASSERT (pbuf);
	char *dest = pbuf;
	...
	if (disc)
	{
		...
		dest += snprintf (pbuf, bufsize,
			...
			"db-title=%s\n"
			"n-part=%u\n",
			id, (ccp)disc->dhead.disc_title, GetTitle (id, ""), disc->n_part);
	}
...
{
	if (stat & CERT_F_HASH_FAILED)
		return "not signed";
	if (stat & CERT_F_HASH_FAKED)
		return "fake signed";
	if (stat & CERT_F_HASH_OK)
		return "well signed";
	return "";
}
...
static size_t print_part_info (wd_part_t *part, char *pbuf, size_t bufsize)
{
	DASSERT (part);
	DASSERT (pbuf);
	...
	return snprintf (pbuf, bufsize,
		...
		,
		part->index, part->ptab_index, part->ptab_part_index, part->part_type, part->part_size

		,
		part->is_overlay, part->is_gc, 0 != (part->sector_stat & WD_SS_ENCRYPTED),
		0 != (part->sector_stat & WD_SS_DECRYPTED), 0 != (part->sector_stat & WD_SS_F_SKELETON),
		0 != (part->sector_stat & WD_SS_F_SCRUBBED)

			,
		get_sign_info (part->cert_tik_stat), get_sign_info (part->cert_tmd_stat)

												 ,
		part->data_sector, part->end_sector

		,
		part->fst_n, part->fst_dir_count, part->fst_file_count);
}
...
static AnaPath_t ana_path_tab_root[] = { DEF_AP (AP_ISO, "/iso"),
	DEF_AP (AP_WBFS_SLOT, "/wbfs/slot"), DEF_AP (AP_WBFS_ID, "/wbfs/id"),
	DEF_AP (AP_WBFS_TITLE, "/wbfs/title"), DEF_AP (AP_WBFS_INFO, "/wbfs/info.txt"),
	DEF_AP (AP_WBFS, "/wbfs"), DEF_AP (AP_ROOT_INFO, "/info.txt"), { 0, 0, 0 } };

static AnaPath_t ana_path_tab_iso[] = { DEF_AP (AP_ISO_DISC, "/disc.iso"),
	DEF_AP (AP_ISO_INFO, "/info.txt"), DEF_AP (AP_ISO_PART_DATA, "/part/data"),
	DEF_AP (AP_ISO_PART_UPDATE, "/part/update"), DEF_AP (AP_ISO_PART_CHANNEL, "/part/channel"),
	DEF_AP (AP_ISO_PART_MAIN, "/part/main"), DEF_AP (AP_ISO_PART, "/part"), { 0, 0, 0 } };
...
static wd_part_t *analyze_part (ccp *subpath, // result
...
)
{
	...
	if (disc && path[0] == '/' && path[1] >= '0' && path[1] < '0' + WII_MAX_PTAB && path[2] == '.')
	{
		// TRACELINE;
		char *end;
...
static int analyze_slot (ccp *subpath, // result
	ccp path // source path
)
{
	...
	if (slot_info && path[0] == '/')
	{
		...
		if (end > path + 1 && (!*end || *end == '/') && slot < n_slots && w->head->disc_table[slot])
		{
			found_slot = slot;
			path = end;
		}
...
static int analyze_wbfs_id (ccp path, // source path
...
)
{
	...
	if (!ok && extend && slen >= 8 && path[slen - 8] == '[' && path[slen - 1] == ']')
	{
		ok = true;
		path += slen - 8;
	}
...
static u64 get_iso_size (DiscFile_t *df)
{
	DASSERT (df);
	...
	if (!fsize)
		fsize = (u64)WII_SECTOR_SIZE
			* (df->disc->usage_max > WII_SECTORS_SINGLE_LAYER ? df->disc->usage_max
...
static int wfuse_getattr_iso (const char *path, struct stat *st, DiscFile_t *df)
{
	...
	switch (ap)
	{
		...
		case AP_ISO_PART:
			...
			if (strlen (subpath) == 5 && *subpath == '/' && isalnum (subpath[1])
				&& isalnum (subpath[2]) && isalnum (subpath[3]) && isalnum (subpath[4]))
			{
...
static int wfuse_getattr (const char *path, struct stat *st)
{
...
static int copy_helper (char *buf, // read buffer
...
static int wfuse_read_iso (const char *path, // path to the file
...
)
{
	...
	switch (ap)
	{
		case AP_ISO_INFO:
			if (!*subpath)
				return copy_helper (buf, size, offset, pbuf, print_iso_info (df, pbuf, pbuf_size));
			break;
		...
		case AP_ISO_PART:
			...
			if (part && *subpath)
			{
				if (!strcmp (subpath, "/info.txt"))
					return copy_helper (
						buf, size, offset, pbuf, print_part_info (part, pbuf, pbuf_size));
...
static int wfuse_read (const char *path, // path to the file
	char *buf, // read buffer
	size_t size, // size of buf and size to read
	off_t offset, // read offset
	fuse_file_info *info // fuse info
)
{
	...
	switch (ap)
	{
		case AP_ROOT_INFO:
			if (!*subpath)
				return copy_helper (buf, size, offset, pbuf, print_root_info (pbuf, sizeof (pbuf)));
			break;

		case AP_ISO:
			return wfuse_read_iso (subpath, buf, size, offset, info, dfile, pbuf, sizeof (pbuf));
		...
		case AP_WBFS_SLOT:
			if (is_wbfs && *subpath)
			{
				...
				if (slot >= 0)
				{
					...
					if (df)
					{
						const int stat = wfuse_read_iso (
							subpath, buf, size, offset, info, df, pbuf, sizeof (pbuf));
						free_disc_file (df);
						return stat;
					}
				}
		...
		case AP_WBFS_INFO:
			if (!*subpath)
				return copy_helper (buf, size, offset, pbuf, print_wbfs_info (pbuf, sizeof (pbuf)));
			break;
...
static int wfuse_readlink_iso (const char *path, char *fuse_buf, size_t bufsize, DiscFile_t *df)
{
	...
	switch (ap)
	{
		...
		case AP_ISO_PART_DATA:
			if (!*subpath)
			{
				...
				if (disc && disc->data_part)
					snprintf (fuse_buf, bufsize, "%u.%u/", disc->data_part->ptab_index,
		...
		case AP_ISO_PART_UPDATE:
			if (!*subpath)
			{
				...
				if (disc && disc->update_part)
					snprintf (fuse_buf, bufsize, "%u.%u/", disc->update_part->ptab_index,
		...
		case AP_ISO_PART_CHANNEL:
			if (!*subpath)
			{
				...
				if (disc && disc->channel_part)
					snprintf (fuse_buf, bufsize, "%u.%u/", disc->channel_part->ptab_index,
		...
		case AP_ISO_PART_MAIN:
			if (!*subpath)
			{
				...
				if (disc && disc->main_part)
					snprintf (fuse_buf, bufsize, "%u.%u/", disc->main_part->ptab_index,
		...
		case AP_ISO_PART:
				...
				&& isalnum (subpath[2]) && isalnum (subpath[3]) && isalnum (subpath[4]))
			{
				...
				{
					u32 ptype
						= ((subpath[1] << 8 | subpath[2]) << 8 | subpath[3]) << 8 | subpath[4];
					int ip;
					for (ip = 0; ip < disc->n_part; ip++)
					{
						wd_part_t *part = disc->part + ip;
						...
						{
							snprintf (fuse_buf, bufsize, "%u.%u/", part->ptab_index,
...
static int wfuse_readlink (const char *path, char *fuse_buf, size_t bufsize)
{
	TRACE ("##### wfuse_readlink(%s)\n", path);
	DASSERT (fuse_buf);
		...
		case AP_WBFS_SLOT:
			...
			if (*subpath)
			{
				...
				if (slot >= 0)
				{
					...
					if (df)
					{
						const int stat = wfuse_readlink_iso (subpath, fuse_buf, bufsize, df);
						free_disc_file (df);
						return stat;
					}
				}
...
static int wfuse_readdir_iso (const char *path, void *fuse_buf, fuse_fill_dir_t filler,
	off_t offset, fuse_file_info *info, DiscFile_t *df)
{
			...
			if (!*subpath)
			{
				...
				if (disc)
				{
					...
					for (ip = 0; ip < disc->n_part; ip++)
					{
						wd_part_t *part = disc->part + ip;
						snprintf (
							pbuf, sizeof (pbuf), "%u.%u", part->ptab_index, part->ptab_part_index);
						filler (fuse_buf, pbuf, 0, 0);

						u32 ptype = htonl (disc->part[ip].part_type);
						ccp id4 = (ccp)&ptype;
						if (isalnum (id4[0]) && isalnum (id4[1]) && isalnum (id4[2])
							&& isalnum (id4[3]))
						{
							memcpy (pbuf, id4, 4);
							pbuf[4] = 0;
...
static int wfuse_readdir (
	const char *path, void *fuse_buf, fuse_fill_dir_t filler, off_t offset, fuse_file_info *info)
{
			...
			if (is_wbfs && slot_info && !*subpath)
			{
				...
				for (slot = 0; slot < n_slots; slot++)
				{
					...
					if (si->id6[0])
					{
						if (ap == AP_WBFS_TITLE)
						{
							snprintf (pbuf, sizeof (pbuf), "%.80s [%s]", si->fname, si->id6);
							filler (fuse_buf, pbuf, 0, 0);
						}
						else
...
static enumError umount_path (ccp mpt, bool silent)
{
	...
	if (stat == -1)
		return ERROR0 (ERR_FATAL, "Can't find neither 'fusermount' nor 'umount' -> abort\n");
...
int main (int argc, char **argv)
{
	...
	{
		return ERROR0 (ERR_INVALID_FILE, "File type '%s' not supported: %s\n", GetNameFT (ftype, 0),
			source_file);
	}
