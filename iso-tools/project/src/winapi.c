...
)
{
	...
	DWORD written;
	int stat = DeviceIoControl ((HANDLE)handle, FSCTL_GET_RETRIEVAL_POINTERS, &start,
		sizeof (start), &rawbuf, sizeof (rawbuf), &written, 0);
	int last_err = GetLastError ();
	PRINT ("stat=%d,%d [moredata=%ld], wr=%ld, size=%zu\n", stat, last_err, ERROR_MORE_DATA,
		written, sizeof (rawbuf));
	...
	if (!stat && last_err == ERROR_MORE_DATA)
	{
		...
		{
			buf = MALLOC (bufsize);
			stat = DeviceIoControl ((HANDLE)handle, FSCTL_GET_RETRIEVAL_POINTERS, &start,
				sizeof (start), buf, bufsize, &written, 0);
	...
	if (stat && buf->ExtentCount)
	{
		...
		PRINT ("file size=%lld, last_cluster = %lld, cluster_size = %d\n", file_size,
			buf->Extents[buf->ExtentCount - 1].NextVcn.QuadPart, blocksize);
		...
		{
			noPRINT (
				"%15lld %11lld\n", buf->Extents[i].NextVcn.QuadPart, buf->Extents[i].Lcn.QuadPart);

			const u64 next_block = buf->Extents[i].NextVcn.QuadPart;
			AppendFileMap (fm, block * blocksize + split_off,
				buf->Extents[i].Lcn.QuadPart * blocksize, (next_block - block) * blocksize);
			block = next_block;
		}
	}
