...
FILE *OpenPipeToPager ()
{
	...
	if (!done)
	{
		...
		static ccp search_tab[] = { ">DC_PAGER", ">DC_PAGER1", ">DC_PAGER2", ">PAGER",
#ifdef __CYGWINxxx__
			"/usr/bin/less", "/cygdrive/c/Program Files/Wiimm/SZS/less", "/usr/bin/more",
...
)
{
	...
	if (limit && f->st.st_size > limit)
	{
		...
		{
			if (!(f->fmode & FM_SILENT))
				ERROR0 (ERR_CANT_OPEN, "File too large (size=%s, limit=%s): %s\n%s", sbuf, lbuf,
...
static void search_paths_dir (search_paths_t *sp, ParamField_t *collect, int min, int max)
{
	...
	if (fdir)
	{
		for (;;)
		{
			...
			if (dent->d_name[0] == '.'
				&& (!local.allow_hidden || !dent->d_name[1]
...
ccp PrintCpuStatus (const CpuStatus_t *cpuinfo)
{
	...
	return PrintCircBuf (
		...
		cpuinfo->idle, cpuinfo->user + cpuinfo->nice + cpuinfo->system + cpuinfo->idle,
