...
)
{
	...
	for (; dptr < dend; dptr++)
	{
		...
		if (sptr->mid == mid && !IsItemEqualBMG (dptr, sptr)
...
bool PatchFillBMG (bmg_t *bmg, ct_mode_t ct_mode, uint rcup_limit, uint track_limit)
{
	...
	for (mid = ctb.arena_name1.beg; mid < ctb.arena_name1.end; mid++)
	{
		...
		if (!old_item || !dptr->len)
		{
			const uint len = snprintf (buf, sizeof (buf), "_A%u_", mid - ctb.arena_name1.beg);
...
const sizeof_info_t sizeof_info_bmg[] = { SIZEOF_INFO_TITLE ("BMG") SIZEOF_INFO_ENTRY (bmg_range_t)
		SIZEOF_INFO_ENTRY (bmg_section_t) SIZEOF_INFO_ENTRY (bmg_header_t)
			SIZEOF_INFO_ENTRY (bmg_inf_item_t) SIZEOF_INFO_ENTRY (bmg_inf_t)
				SIZEOF_INFO_ENTRY (bmg_dat_t) SIZEOF_INFO_ENTRY (bmg_mid_t)
					SIZEOF_INFO_ENTRY (bmg_flw_t) SIZEOF_INFO_ENTRY (bmg_fli_t)
						SIZEOF_INFO_ENTRY (bmg_sect_info_t) SIZEOF_INFO_ENTRY (bmg_sect_list_t)
							SIZEOF_INFO_ENTRY (bmg_raw_section_t) SIZEOF_INFO_ENTRY (bmg_item_t)
