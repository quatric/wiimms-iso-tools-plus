...
)
{
	...
	char connect_usec[7];
	if (ts->connect_usec)
		PrintTimerUSec6 (connect_usec, sizeof (connect_usec), now_usec - ts->connect_usec, true);
	else
		connect_usec[0] = '-', connect_usec[1] = 0;

	char receive_usec[7];
	if (ts->receive_usec)
		PrintTimerUSec6 (receive_usec, sizeof (receive_usec), now_usec - ts->receive_usec, true);
	else
		receive_usec[0] = '-', receive_usec[1] = 0;

	char send_usec[7];
	if (ts->send_usec)
		PrintTimerUSec6 (send_usec, sizeof (send_usec), now_usec - ts->send_usec, true);
	else
