...
| **BNFM** | `.bnfm` | **GLB** | ✅ | ✅ | ✅ | ❌ | Nd Cube Wii U 3D model format (*Mario Party 10*). The *Animal Crossing: Amiibo Festival* attribution could not be confirmed against the retail disc: the game ships zero bare `.bnfm` files anywhere on the WUX -- every model (`content/common/bin/ch_base/chara/**/*.bin`, etc.) is packed inside an undocumented `PAC\0`-magic container that `wszst` doesn't recognise at all (no file-type entry, no cutter, no decoder). Manually walking that container's own file table (header offsets at 0x38/0x3c/0x40 point at a 0x30-byte-stride entry table, a string table, and a data area) does locate a member literally named `cat00.bnfm` inside `cat00.bin` (and sibling `.gtx` textures next to it), but the payload bytes at its recorded offset/size are high-entropy and neither start with the `BNFM` magic nor decompress under zlib or LZMA -- confirming the whole data region of this container is encrypted, for textures as well as models, and out of reach without a new PAC decoder |
