...
| **MPR TXTR** | `.txtr` | ✅ | — | — | ✅ | Retro Studios texture, Remastered revision (*Metroid Prime Remastered*, Switch): LE RFRM form (`TXTR` v47/51) + HEAD + GPU buffers assembled from the FOOT META table, detiled with the Tegra block-linear path and pixel-decoded incl. BC1-7/ASTC (base mip, layer 0; smaller mips validated, not deswizzled). Verified against the retail `MiscData.pak` TXTR (232x232 R8Unorm). `wimgt DECODE` + `wszst xx` (PACK extract leaves a cascade `.TXTR.png`). An `RFRM` form nothing in the family claims reports UNKNOWN rather than falling through to the LZ10/LZ11 single-byte guesses (real TXTRs used to misreport as LZ streams, aborting image decode) |
