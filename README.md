...
| **MSR** | `.bin` | 🟡 | — | — | 🟡 | *Metroid: Samus Returns* (3DS) flat archive container. Verified against the retail romfs: the cart's romfs ships as loose named files under a `files.toc` index (no single packed blob), so no top-level flat-archive decode was exercised. The `.bctex` texture files carry `MTXT` magic and are correctly typed by `FILETYPE`, but the MTXT **decoder is Switch-only** (gzip-wrapped XTX from offset 8); the 3DS `.bctex` stores raw pixel data from offset 8 (not gzip-compressed), so extraction returns error 14 with no output — decode is **not** supported for this variant |
...
See detailed ASH0 compression benchmarks in [COMPRESSION_BENCHMARKS.md](docs/COMPRESSION_BENCHMARKS.md).
