...
#define CACHE(parser, length)                                                                      \
	(parser->unread >= (length) ? 1 : yaml_parser_update_buffer (parser, (length)))
...
#define SKIP_LINE(parser)                                                                          \
	(IS_CRLF (parser->buffer)                                                                      \
			? (parser->mark.index += 2, parser->mark.column = 0, parser->mark.line++,              \
				  parser->unread -= 2, parser->buffer.pointer += 2)                                \
			: IS_BREAK (parser->buffer)                                                            \
			? (parser->mark.index++, parser->mark.column = 0, parser->mark.line++,                 \
				  parser->unread--, parser->buffer.pointer += WIDTH (parser->buffer))              \
			: 0)
...
#define READ(parser, string)                                                                       \
	(STRING_EXTEND (parser, string) ? (COPY (string, parser->buffer), parser->mark.index++,        \
...
#define READ_LINE(parser, string)                                                                  \
	(STRING_EXTEND (parser, string)                                                                \
			? (((CHECK_AT (parser->buffer, '\r', 0) && CHECK_AT (parser->buffer, '\n', 1))         \
					   ? /* CR LF -> LF */                                                         \
					   ...
					   : (CHECK_AT (parser->buffer, '\r', 0)                                       \
						   ...
						   : (CHECK_AT (parser->buffer, '\xC2', 0)                                 \
							   ...
							   ? /* NEL -> LF */                                                   \
							   (*((string).pointer++) = (yaml_char_t)'\n',                         \
								   parser->buffer.pointer += 2, parser->mark.index++,              \
								   parser->mark.column = 0, parser->mark.line++, parser->unread--) \
...
static int yaml_parser_set_scanner_error (
	yaml_parser_t *parser, const char *context, yaml_mark_t context_mark, const char *problem);
...
static int yaml_parser_roll_indent (yaml_parser_t *parser, ptrdiff_t column, ptrdiff_t number,
	yaml_token_type_t type, yaml_mark_t mark);
...
static int yaml_parser_scan_directive_name (
	yaml_parser_t *parser, yaml_mark_t start_mark, yaml_char_t **name);

static int yaml_parser_scan_version_directive_value (
	yaml_parser_t *parser, yaml_mark_t start_mark, int *major, int *minor);

static int yaml_parser_scan_version_directive_number (
	yaml_parser_t *parser, yaml_mark_t start_mark, int *number);

static int yaml_parser_scan_tag_directive_value (
	yaml_parser_t *parser, yaml_mark_t mark, yaml_char_t **handle, yaml_char_t **prefix);

static int yaml_parser_scan_anchor (
	yaml_parser_t *parser, yaml_token_t *token, yaml_token_type_t type);

static int yaml_parser_scan_tag (yaml_parser_t *parser, yaml_token_t *token);

static int yaml_parser_scan_tag_handle (
	yaml_parser_t *parser, int directive, yaml_mark_t start_mark, yaml_char_t **handle);

static int yaml_parser_scan_tag_uri (yaml_parser_t *parser, int uri_char, int directive,
	yaml_char_t *head, yaml_mark_t start_mark, yaml_char_t **uri);

static int yaml_parser_scan_uri_escapes (
...
static int yaml_parser_scan_block_scalar_breaks (yaml_parser_t *parser, int *indent,
	yaml_string_t *breaks, yaml_mark_t start_mark, yaml_mark_t *end_mark);
...
yaml_parser_fetch_more_tokens (yaml_parser_t *parser)
{
	...
	while (1)
	{
		...
		else
		{
			...
			for (simple_key = parser->simple_keys.start; simple_key != parser->simple_keys.top;
				simple_key++)
			{
...
static int yaml_parser_fetch_next_token (yaml_parser_t *parser)
{
	...
	if (CHECK (parser->buffer, '?') && (parser->flow_level || IS_BLANKZ_AT (parser->buffer, 1)))
	...
	if (!(IS_BLANKZ (parser->buffer) || CHECK (parser->buffer, '-') || CHECK (parser->buffer, '?')
		...
		|| (!parser->flow_level && (CHECK (parser->buffer, '?') || CHECK (parser->buffer, ':'))
				...
				|| simple_key->mark.index + 1024 < parser->mark.index))
		{
			...
			if (simple_key->required)
			{
				return yaml_parser_set_scanner_error (parser, "while scanning a simple key",
					simple_key->mark, "could not find expected ':'");
			}
	...
	if (!parser->flow_level)
	{
		...
		if (!parser->simple_key_allowed)
		{
			return yaml_parser_set_scanner_error (
				parser, NULL, parser->mark, "mapping keys are not allowed in this context");
		}
...
static int yaml_parser_fetch_value (yaml_parser_t *parser)
{
	...
	if (simple_key->possible)
	{
		...
		if (!QUEUE_INSERT (
				parser, parser->tokens, simple_key->token_number - parser->tokens_parsed, token))
			return 0;

		/* In the block context, we may need to add the BLOCK-MAPPING-START token. */

		if (!yaml_parser_roll_indent (parser, simple_key->mark.column, simple_key->token_number,
	...
	else
	{
		...
		if (!parser->flow_level)
		{
			...
			{
				return yaml_parser_set_scanner_error (
					parser, NULL, parser->mark, "mapping values are not allowed in this context");
			}
...
int yaml_parser_scan_directive (yaml_parser_t *parser, yaml_token_t *token)
{
	...
	else
	{
		yaml_parser_set_scanner_error (
			parser, "while scanning a directive", start_mark, "found unknown directive name");
		goto error;
	}
	...
	{
		yaml_parser_set_scanner_error (parser, "while scanning a directive", start_mark,
...
static int yaml_parser_scan_tag_directive_value (
	yaml_parser_t *parser, yaml_mark_t start_mark, yaml_char_t **handle, yaml_char_t **prefix)
{
	...
	{
		yaml_parser_set_scanner_error (parser, "while scanning a %TAG directive", start_mark,
	...
	yaml_parser_t *parser, yaml_token_t *token, yaml_token_type_t type)
{
	...
	if (!length
		|| !(IS_BLANKZ (parser->buffer) || CHECK (parser->buffer, '?')
			|| CHECK (parser->buffer, ':') || CHECK (parser->buffer, ',')
			|| CHECK (parser->buffer, ']') || CHECK (parser->buffer, '}')
			|| CHECK (parser->buffer, '%') || CHECK (parser->buffer, '@')
			|| CHECK (parser->buffer, '`')))
	{
		yaml_parser_set_scanner_error (parser,
			type == YAML_ANCHOR_TOKEN ? "while scanning an anchor" : "while scanning an alias",
...
static int yaml_parser_scan_tag (yaml_parser_t *parser, yaml_token_t *token)
{
	...
	if (CHECK_AT (parser->buffer, '<', 1))
	{
		...
		{
			yaml_parser_set_scanner_error (
				parser, "while scanning a tag", start_mark, "did not find the expected '>'");
			goto error;
		}
	...
	yaml_parser_t *parser, int directive, yaml_mark_t start_mark, yaml_char_t **handle)
{
	...
	{
		yaml_parser_set_scanner_error (parser,
			directive ? "while scanning a tag directive" : "while scanning a tag", start_mark,
	...
	yaml_char_t *head, yaml_mark_t start_mark, yaml_char_t **uri)
{
	...
	while (IS_ALPHA (parser->buffer) || CHECK (parser->buffer, ';') || CHECK (parser->buffer, '/')
		|| CHECK (parser->buffer, '?') || CHECK (parser->buffer, ':') || CHECK (parser->buffer, '@')
		|| CHECK (parser->buffer, '&') || CHECK (parser->buffer, '=') || CHECK (parser->buffer, '+')
		|| CHECK (parser->buffer, '$') || CHECK (parser->buffer, '.') || CHECK (parser->buffer, '%')
		|| CHECK (parser->buffer, '!') || CHECK (parser->buffer, '~') || CHECK (parser->buffer, '*')
		|| CHECK (parser->buffer, '\'') || CHECK (parser->buffer, '(')
	...
	if (!length)
	{
		...
		yaml_parser_set_scanner_error (parser,
			directive ? "while parsing a %TAG directive" : "while parsing a tag", start_mark,
...
static int yaml_parser_scan_block_scalar (yaml_parser_t *parser, yaml_token_t *token, int literal)
{
	...
	if (CHECK (parser->buffer, '+') || CHECK (parser->buffer, '-'))
	{
		...
		if (IS_DIGIT (parser->buffer))
		{
			...
			{
				yaml_parser_set_scanner_error (parser, "while scanning a block scalar", start_mark,
	...
	SCALAR_TOKEN_INIT (*token, string.start, string.pointer - string.start,
		literal ? YAML_LITERAL_SCALAR_STYLE : YAML_FOLDED_SCALAR_STYLE, start_mark, end_mark);
	...
	yaml_string_t *breaks, yaml_mark_t start_mark, yaml_mark_t *end_mark)
{
	...
	*end_mark = parser->mark;
	...
	while (1)
	{
		...
		if ((!*indent || (int)parser->mark.column < *indent) && IS_TAB (parser->buffer))
		{
...
static int yaml_parser_scan_flow_scalar (yaml_parser_t *parser, yaml_token_t *token, int single)
{
	...
	while (1)
	{
		...
		if (parser->mark.column == 0
			&& ((CHECK_AT (parser->buffer, '-', 0) && CHECK_AT (parser->buffer, '-', 1)
					&& CHECK_AT (parser->buffer, '-', 2))
				|| (CHECK_AT (parser->buffer, '.', 0) && CHECK_AT (parser->buffer, '.', 1)
					&& CHECK_AT (parser->buffer, '.', 2)))
			&& IS_BLANKZ_AT (parser->buffer, 3))
		{
			yaml_parser_set_scanner_error (parser, "while scanning a quoted scalar", start_mark,
	...
	SCALAR_TOKEN_INIT (*token, string.start, string.pointer - string.start,
		single ? YAML_SINGLE_QUOTED_SCALAR_STYLE : YAML_DOUBLE_QUOTED_SCALAR_STYLE, start_mark,
		end_mark);
...
static int yaml_parser_scan_plain_scalar (yaml_parser_t *parser, yaml_token_t *token)
{
	...
	while (1)
	{
		...
		while (!IS_BLANKZ (parser->buffer))
		{
			/* Check for "x:" + one of ',?[]{}' in the flow context. TODO: Fix the test
			 * "spec-08-13". This is not completely according to the spec See
			...
			{
				yaml_parser_set_scanner_error (
					parser, "while scanning a plain scalar", start_mark, "found unexpected ':'");
				goto error;
			}
	...
	SCALAR_TOKEN_INIT (*token, string.start, string.pointer - string.start, YAML_PLAIN_SCALAR_STYLE,
		start_mark, end_mark);
