...
YAML_DECLARE (int)
yaml_string_join (yaml_char_t **a_start, yaml_char_t **a_pointer, yaml_char_t **a_end,
...
#define STRING_INIT(context, string, size)                                                         \
	(((string).start = YAML_MALLOC (size))                                                         \
			? ((string).pointer = (string).start, (string).end = (string).start + (size),          \
...
#define JOIN(context, string_a, string_b)                                                          \
	((yaml_string_join (&(string_a).start, &(string_a).pointer, &(string_a).end,                   \
		 &(string_b).start, &(string_b).pointer, &(string_b).end))                                 \
...
#define IS_PRINTABLE_AT(string, offset)                                                            \
	(((string).pointer[offset] == 0x0A) /* . == #x0A */                                            \
		...
		|| ((string).pointer[offset] == 0xED && (string).pointer[offset + 1] < 0xA0)               \
...
#define IS_BOM_AT(string, offset)                                                                  \
	(CHECK_AT ((string), '\xEF', (offset)) && CHECK_AT ((string), '\xBB', (offset) + 1)            \
...
#define IS_BREAK_AT(string, offset)                                                                \
	(CHECK_AT ((string), '\r', (offset)) /* CR (#xD)*/                                             \
		...
		|| (CHECK_AT ((string), '\xE2', (offset)) && CHECK_AT ((string), '\x80', (offset) + 1)     \
...
#define COPY(string_a, string_b)                                                                   \
	((*(string_b).pointer & 0x80) == 0x00 ? (*((string_a).pointer++) = *((string_b).pointer++))    \
...
#define STACK_INIT(context, stack, type)                                                           \
	(((stack).start = (type)yaml_malloc (INITIAL_STACK_SIZE * sizeof (*(stack).start)))            \
			? ((stack).top = (stack).start, (stack).end = (stack).start + INITIAL_STACK_SIZE, 1)   \
			: ((context)->error = YAML_MEMORY_ERROR, 0))
...
#define STACK_LIMIT(context, stack, size)                                                          \
	((stack).top - (stack).start < (size) ? 1 : ((context)->error = YAML_MEMORY_ERROR, 0))

#define PUSH(context, stack, value)                                                                \
	(((stack).top != (stack).end                                                                   \
		 || yaml_stack_extend (                                                                    \
			 (void **)&(stack).start, (void **)&(stack).top, (void **)&(stack).end))               \
			? (*((stack).top++) = value, 1)                                                        \
			: ((context)->error = YAML_MEMORY_ERROR, 0))
...
#define QUEUE_INIT(context, queue, size, type)                                                     \
	(((queue).start = (type)yaml_malloc ((size) * sizeof (*(queue).start)))                        \
			? ((queue).head = (queue).tail = (queue).start, (queue).end = (queue).start + (size),  \
				  1)                                                                               \
			: ((context)->error = YAML_MEMORY_ERROR, 0))

#define QUEUE_DEL(context, queue)                                                                  \
	(yaml_free ((queue).start), (queue).start = (queue).head = (queue).tail = (queue).end = 0)
...
#define TOKEN_INIT(token, token_type, token_start_mark, token_end_mark)                            \
	(memset (&(token), 0, sizeof (yaml_token_t)), (token).type = (token_type),                     \
		(token).start_mark = (token_start_mark), (token).end_mark = (token_end_mark))
...
#define TAG_TOKEN_INIT(token, token_handle, token_suffix, start_mark, end_mark)                    \
	(TOKEN_INIT ((token), YAML_TAG_TOKEN, (start_mark), (end_mark)),                               \
		(token).data.tag.handle = (token_handle), (token).data.tag.suffix = (token_suffix))

#define SCALAR_TOKEN_INIT(token, token_value, token_length, token_style, start_mark, end_mark)     \
	(TOKEN_INIT ((token), YAML_SCALAR_TOKEN, (start_mark), (end_mark)),                            \
		(token).data.scalar.value = (token_value), (token).data.scalar.length = (token_length),    \
...
#define EVENT_INIT(event, event_type, event_start_mark, event_end_mark)                            \
	(memset (&(event), 0, sizeof (yaml_event_t)), (event).type = (event_type),                     \
		(event).start_mark = (event_start_mark), (event).end_mark = (event_end_mark))
...
#define DOCUMENT_START_EVENT_INIT(event, event_version_directive, event_tag_directives_start,      \
...
#define SCALAR_EVENT_INIT(event, event_anchor, event_tag, event_value, event_length,               \
	...
	(EVENT_INIT ((event), YAML_SCALAR_EVENT, (start_mark), (end_mark)),                            \
		(event).data.scalar.anchor = (event_anchor), (event).data.scalar.tag = (event_tag),        \
		(event).data.scalar.value = (event_value), (event).data.scalar.length = (event_length),    \
		(event).data.scalar.plain_implicit = (event_plain_implicit),                               \
		(event).data.scalar.quoted_implicit = (event_quoted_implicit),                             \
		(event).data.scalar.style = (event_style))

#define SEQUENCE_START_EVENT_INIT(                                                                 \
	event, event_anchor, event_tag, event_implicit, event_style, start_mark, end_mark)             \
...
#define MAPPING_START_EVENT_INIT(                                                                  \
	event, event_anchor, event_tag, event_implicit, event_style, start_mark, end_mark)             \
...
#define DOCUMENT_INIT(document, document_nodes_start, document_nodes_end,                          \
	document_version_directive, document_tag_directives_start, document_tag_directives_end,        \
	document_start_implicit, document_end_implicit, document_start_mark, document_end_mark)        \
	(memset (&(document), 0, sizeof (yaml_document_t)),                                            \
		...
		(document).start_mark = (document_start_mark), (document).end_mark = (document_end_mark))
...
#define NODE_INIT(node, node_type, node_tag, node_start_mark, node_end_mark)                       \
	(memset (&(node), 0, sizeof (yaml_node_t)), (node).type = (node_type),                         \
		(node).tag = (node_tag), (node).start_mark = (node_start_mark),                            \
		(node).end_mark = (node_end_mark))

#define SCALAR_NODE_INIT(                                                                          \
	node, node_tag, node_value, node_length, node_style, start_mark, end_mark)                     \
	(NODE_INIT ((node), YAML_SCALAR_NODE, (node_tag), (start_mark), (end_mark)),                   \
		(node).data.scalar.value = (node_value), (node).data.scalar.length = (node_length),        \
		(node).data.scalar.style = (node_style))

#define SEQUENCE_NODE_INIT(                                                                        \
	node, node_tag, node_items_start, node_items_end, node_style, start_mark, end_mark)            \
...
#define MAPPING_NODE_INIT(                                                                         \
	node, node_tag, node_pairs_start, node_pairs_end, node_style, start_mark, end_mark)            \
