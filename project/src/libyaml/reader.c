...
static int yaml_parser_set_reader_error (
	yaml_parser_t *parser, const char *problem, size_t offset, int value);
	...
	while (parser->unread < length)
	{
		...
		while (parser->raw_buffer.pointer != parser->raw_buffer.last)
		{
			...
			switch (parser->encoding)
			{
				case YAML_UTF8_ENCODING:
					...
					for (k = 1; k < width; k++)
					{
						...
						if ((octet & 0xC0) != 0x80)
							return yaml_parser_set_reader_error (
								parser, "invalid trailing UTF-8 octet", parser->offset + k, octet);
				...
				case YAML_UTF16BE_ENCODING:
					...
					value
						= parser->raw_buffer.pointer[low] + (parser->raw_buffer.pointer[high] << 8);
					...
					if ((value & 0xFC00) == 0xD800)
					{
						...
						if ((value2 & 0xFC00) != 0xDC00)
							return yaml_parser_set_reader_error (
								parser, "expected low surrogate area", parser->offset + 2, value2);
			...
			if (!(value == 0x09 || value == 0x0A || value == 0x0D
					...
					|| (value >= 0xA0 && value <= 0xD7FF) || (value >= 0xE000 && value <= 0xFFFD)
					|| (value >= 0x10000 && value <= 0x10FFFF)))
				return yaml_parser_set_reader_error (
					parser, "control characters are not allowed", parser->offset, value);
