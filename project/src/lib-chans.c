	...
	if (b < 0x40)
	{
		switch (b)
		{
			...
			case 0x02:
				...
				snprintf (
					dop->desc, sizeof (dop->desc), "NEW [0x%02x args]", (rem >= 2) ? data[1] : 0);
				break;
			case 0x03:
				dop->length = (rem >= 3) ? 3 : 1;
				snprintf (dop->desc, sizeof (dop->desc), "CLOSURE [id=0x%04x]",
					(rem >= 3) ? be16 ((data + 1)) : 0);
				break;
			case 0x04:
				dop->length = 1;
			...
			case 0x18:
				dop->length = (rem >= 2) ? 2 : 1;
				snprintf (dop->desc, sizeof (dop->desc), "ACC = %d (0x%02x)",
					(rem >= 2) ? (s8)data[1] : 0, (rem >= 2) ? data[1] : 0);
				break;
			case 0x19:
				dop->length = (rem >= 2) ? 2 : 1;
				snprintf (
					dop->desc, sizeof (dop->desc), "ACC = ACC + %u", (rem >= 2) ? data[1] : 0);
				break;
			case 0x1a:
				dop->length = (rem >= 2) ? 2 : 1;
				snprintf (
			...
			case 0x1e:
				dop->length = (rem >= 2) ? 2 : 1;
				snprintf (
					dop->desc, sizeof (dop->desc), "ACC = ACC & 0x%02x", (rem >= 2) ? data[1] : 0);
				break;
			case 0x1f:
				dop->length = (rem >= 2) ? 2 : 1;
				snprintf (
			...
			case 0x27:
				...
				snprintf (dop->desc, sizeof (dop->desc), "ACC = 0x%04x (%u)",
					(rem >= 3) ? be16 ((data + 1)) : 0, (rem >= 3) ? be16 ((data + 1)) : 0);
				break;
			case 0x28:
				dop->length = (rem >= 5) ? 5 : 1;
				snprintf (dop->desc, sizeof (dop->desc), "ACC = 0x%08x (%u)",
					(rem >= 5) ? be32 ((data + 1)) : 0, (rem >= 5) ? be32 ((data + 1)) : 0);
				break;
			case 0x29:
			case 0x2a:
			{
			...
			case 0x2c:
			{
				...
				const char *str = (sidx < cs->string_count && cs->strings[sidx].utf8)
			...
			case 0x30:
			{
				...
				const char *name = (sym_id < cs->imported_count && cs->imported[sym_id].name)
					...
					: "unknown";
				snprintf (dop->desc, sizeof (dop->desc),
					"CALL ACC.%s WITH 0x%02x PARAMS FROM STACK", name, args);
				break;
			}
			case 0x31:
				dop->length = (rem >= 2) ? 2 : 1;
				snprintf (dop->desc, sizeof (dop->desc), "CALL ACC WITH 0x%02x PARAMS FROM STACK",
					(rem >= 2) ? data[1] : 0);
				break;
			case 0x32:
			{
			...
			case 0x34:
			{
				...
				snprintf (dop->desc, sizeof (dop->desc), "FOR_IN %s in ACC [break -> 0x%04x]",
					vname, dst);
				break;
			}
			case 0x35:
...
enumError DumpChannelScriptDisasm (const chans_script_t *cs, char **out_buf, size_t *out_len)
{
	...
	for (u32 i = 0; i < cs->method_count; i++)
	{
		...
		csb_printf (&sb,
			"--- Method [%02x]: %s (start: 0x%04x, end: 0x%04x, params: %u, temps: %u) ---\n", i,
			m_name, m_start, m_end, cs->methods[i].param_count, cs->methods[i].temp_count);
...
static void DecompileMethodBody (
	const chans_script_t *cs, u32 start_off, u32 end_off, chans_sb_t *sb)
{
	u32 pc = start_off;
		...
		if (b < 0x40)
		{
			switch (b)
			{
				...
				case 0x02: // NEW
				{
					...
					char new_expr[1024];
					snprintf (
						new_expr, sizeof (new_expr), "new %s(%s)", acc ? acc : "Object", arg_buf);
					if (acc)
						FREE (acc);
					acc = STRDUP (new_expr);
					break;
				...
				case 0x15:
				{
					...
					char bin_expr[1024];
					snprintf (bin_expr, sizeof (bin_expr), "(%s %s %s)", left, op_str,
						acc ? acc : "undefined");
					FREE (left);
					if (acc)
						FREE (acc);
				...
				case 0x26:
				{
					...
					char imm_expr[1024];
					snprintf (imm_expr, sizeof (imm_expr), "(%s %s %u)", acc ? acc : "undefined",
						op_str, imm);
					if (acc)
						FREE (acc);
					acc = STRDUP (imm_expr);
				...
				case 0x30: // CALL_EXT_SYMBOL_ARGS
				{
					...
					char call_expr[1024];
					snprintf (call_expr, sizeof (call_expr), "%s.%s(%s)", acc ? acc : "this",
						m_name, arg_buf);
					if (acc)
						FREE (acc);
				...
				case 0x31: // CALL_ACC_ARGS
				{
					...
					char call_expr[1024];
					snprintf (
						call_expr, sizeof (call_expr), "%s(%s)", acc ? acc : "callee", arg_buf);
					if (acc)
						FREE (acc);
					acc = STRDUP (call_expr);
					break;
				...
				case 0x37: // DELETE_MEMBER
				{
					char *left = PopExpr (&stack);
					char del_expr[1024];
					snprintf (del_expr, sizeof (del_expr), "delete %s[%s]", left,
						acc ? acc : "undefined");
					FREE (left);
					if (acc)
						FREE (acc);
