...
static const patch_def_t patch_db[] =
{
    ...
    {
        "SUKE01", "metafortress",
        "Kirby's Return to Dream Land (USA) - Bypass Metafortress",
        ...
        metafortress_suke01, sizeof(metafortress_suke01)/sizeof(metafortress_suke01[0]),
        true, NULL
    },
    {
        "SUKP01", "metafortress",
        "Kirby's Return to Dream Land (Europe) - Bypass Metafortress",
        "Neutralizes the MetaFortress anti-piracy decoy-level trap.",
        ...
        metafortress_sukp01, sizeof(metafortress_sukp01)/sizeof(metafortress_sukp01[0]),
        true, NULL
    },
    {
        "SUKJ01", "metafortress",
        "Kirby's Adventure Wii (Japan) - Bypass Metafortress",
        "Neutralizes the MetaFortress anti-piracy decoy-level trap.",
        ...
        metafortress_sukj01, sizeof(metafortress_sukj01)/sizeof(metafortress_sukj01[0]),
        true, NULL
    },
    ...
    {
        NULL, "nsmbw-antipiracy",
        "New Super Mario Bros. Wii - anti-piracy message screen",
        "TODO: not implemented, see todo_reason.",
        "TCRF \"New Super Mario Bros. Wii\" (BCA check writeup) -- https://tcrf.net/New_Super_Mario_Bros._Wii ; "
        ...
        "Kirby MetaFortress trap). Needs reverse engineering per game revision before it can be added here."
    },
    ...
    {
        NULL, "error-001",
        "Wii \"Error #001\" (unauthorized device detected)",
        "TODO: not implemented, see todo_reason.",
        "ConsoleMods Wiki: Wii:Error_Codes -- https://consolemods.org/wiki/Wii:Error_Codes",
        NULL, 0, false,
    ...
    {
        NULL, "error-002",
        "Wii \"Error #002\" (wrong IOS / disc read anti-piracy check)",
        "TODO: not implemented, see todo_reason.",
...
static enumError detect_id6 ( ccp source, char id6[7] )
{
    ...
    if (IsDirectory(source,false))
    {
        static const char *rel[] = {
            "disc/header.bin", "DATA/sys/boot.bin", "sys/boot.bin", 0
        };
        for (int i = 0; rel[i]; i++)
        {
            char path[PATH_MAX];
        ...
        for (uint i = 0; i < n_matched; i++)
        {
            ...
            for (uint j = 0; j < p->n_pairs; j++)
                fprintf(f,"  <memory offset=\"0x%08X\" value=\"%08X\"/>\n",
                        p->pairs[j].addr,p->pairs[j].value);
            fprintf(f," </patch>\n");
        }
        fprintf(f,"</wiidisc>\n");
        fclose(f);
