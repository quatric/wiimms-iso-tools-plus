...
struct midi_reader
{
	...
	void (*handle_note_off) (
		struct midi_reader *h, uint8_t channel, int duration, int note, int vel);
	void (*handle_key_after_touch) (
		struct midi_reader *h, uint8_t channel, int duration, int note, int vel);
	void (*handle_control_change) (
	...
	void (*handle_channel_after_touch) (
		struct midi_reader *h, uint8_t channel, int duration, int value);
	void (*handle_pitch_wheel_change) (
		struct midi_reader *h, uint8_t channel, uint32_t duration, uint16_t value);
	void (*handle_meta_event) (
	...
	void (*handle_time_signature) (struct midi_reader *h, int duration, uint8_t numerator,
		uint8_t denominator, uint8_t ticks_per_click, uint8_t quarter_note32nd_notes);
	void (*handle_track_end) (struct midi_reader *h, int duration);
	void (*handle_rpn) (
		struct midi_reader *h, uint8_t channel, int duration, uint16_t number, uint8_t msb);
	void (*handle_rpn_lsb) (
		struct midi_reader *h, uint8_t channel, int duration, uint16_t number, uint8_t lsb);
	void (*handle_nrpn) (
		struct midi_reader *h, uint8_t channel, int duration, uint16_t number, uint8_t msb);
	void (*handle_nrpn_lsb) (
		struct midi_reader *h, uint8_t channel, int duration, uint16_t number, uint8_t lsb);
