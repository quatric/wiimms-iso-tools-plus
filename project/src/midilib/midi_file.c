...
int midi_file_init (
	struct midi_file *f, int file_format, int num_tracks, int ticks_per_quarter_note)
{
...
int midi_file_write (
	struct midi_file *f, int (*write_fn) (void *buf, size_t len, void *data_ptr), void *data_ptr)
{
	uint8_t buf[4];
#define WRITE4(a, b, c, d)                                                                         \
	{                                                                                              \
		buf[0] = a;                                                                                \
		buf[1] = b;                                                                                \
		buf[2] = c;                                                                                \
		buf[3] = d;                                                                                \
		if (write_fn (buf, 4, data_ptr) != 4)                                                      \
			return -1;                                                                             \
	}
#define WRITE2(a, b)                                                                               \
	{                                                                                              \
...
#define WRITEU16(u) WRITE2 (u >> 8, u)
	...
	for (int i = 0; i < f->num_tracks; i++)
	{
		...
		if (write_fn (t->buffer.data, t->buffer.data_len, data_ptr) != t->buffer.data_len)
			return -1;
	}
