...
void midi_reader_eat (struct midi_reader *r, uint8_t b)
{
	switch (r->state)
	{
				...
				switch (r->xx)
				{
					case 0x00: // Track sequence number
						if (r->meta_len >= 2)
						{
							if (r->handle_meta_sequence_number)
								r->handle_meta_sequence_number (
									r, r->duration, (r->meta_buf[0] << 8) | r->meta_buf[1]);
						}
						break;
					...
					case 0x02: // Copyright info text
						if (r->handle_copyright_info_event)
							r->handle_copyright_info_event (
								r, r->duration, (const char *)r->meta_buf);
						break;
					...
					case 0x04: // Instrument name text
						if (r->handle_instrument_name_event)
							r->handle_instrument_name_event (
								r, r->duration, (const char *)r->meta_buf);
						break;
					...
					case 0x51: // Set tempo
						if (r->meta_len >= 3)
							if (r->handle_tempo)
								r->handle_tempo (r, r->duration,
									(r->meta_buf[0] << 16) | (r->meta_buf[1] << 8)
										| r->meta_buf[2]);
						break;
					case 0x58:
					{
						...
						if (r->meta_len >= 1)
							numerator = r->meta_buf[0];
						if (r->meta_len >= 2)
							denominator = r->meta_buf[1];
						if (r->meta_len >= 3)
							ticksPerClick = r->meta_buf[2];
						if (r->meta_len >= 4)
							quarterNote32ndNotes = r->meta_buf[3];
						if (r->handle_time_signature)
							r->handle_time_signature (r, r->duration, numerator, denominator,
			...
			switch (r->nn)
			{
				...
				case 6:
					if (r->rpn_lsb < 127 && r->rpn_msb < 127)
					{
						...
						{
							if (r->handle_nrpn)
								r->handle_nrpn (r, r->duration, r->channel,
									((r->rpn_msb & 0x7f) << 7) | (r->rpn_lsb & 0x7f), b);
						}
						else
						{
							if (r->handle_rpn)
								r->handle_rpn (r, r->duration, r->channel,
									((r->rpn_msb & 0x7f) << 7) | (r->rpn_lsb & 0x7f), b);
						}
					}
					break;
					...
					if (r->rpn_lsb < 127 && r->rpn_msb < 127)
					{
						if (r->nrpn)
						{
							if (r->handle_nrpn_lsb)
								r->handle_nrpn_lsb (r, r->duration, r->channel,
									((r->rpn_msb & 0x7f) << 7) | (r->rpn_lsb & 0x7f), b);
						}
						else
						{
							if (r->handle_rpn_lsb)
								r->handle_rpn_lsb (r, r->duration, r->channel,
									((r->rpn_msb & 0x7f) << 7) | (r->rpn_lsb & 0x7f), b);
						}
					}
					break;
		...
		case SysExLen:
			...
			break;
		case SysExD:
			if (r->handle_sysex_byte)
				r->handle_sysex_byte (r, b);
			r->sysex_count++;
